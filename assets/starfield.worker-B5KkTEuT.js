(function() {
	...
	function lt(t, n = globalThis, e = ["*"]) {
		n.addEventListener("message", function r(o) {
			...
			const { id: s, type: i, path: c } = Object.assign({ path: [] }, o.data), a = (o.data.argumentList || []).map(P0);
			let u;
			try {
				...
				switch (i) {
					case "GET":
						...
						break;
					case "SET":
						h[c.slice(-1)[0]] = P0(o.data.value), u = !0;
						break;
					case "APPLY":
						u = l.apply(h, a);
						break;
	...
	function ht(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, c, a) {
				...
				if (u === jn) return b0(t, n, { type: "ENDPOINT" }).then(P0);
	...
	25 * v1;
	function d2(t) {
		return !Number.isFinite(t) || t < 0 ? 0 : t * v1;
	}
	const Me = 5e4, Se = 2e3, _e = Me / f2, Ae = Se / f2;
	d2(_e);
	d2(Ae);
	const Ee = 25 * v1, W = 299792.458, D1 = .001, P1 = 1e3;
	1 * D1, 10 * D1, 100 * D1, 1 * P1, 10 * P1, 100 * P1, .01 * W, .1 * W, .5 * W, 1 * W, 10 * W, 100 * W, 1e3 * W, 1e4 * W, 1e5 * W, 1e6 * W, 1e7 * W, 1e8 * W, 1e9 * W;
	1 * D1;
	1e9 * W;
	1 * P1;
	function pe(t) {
	...
	const S2 = M2[I.EARTH]?.radiusKm ?? 6371, _2 = M2[I.SUN]?.radiusKm ?? 696340;
	S2 / _2;
	function A2(t) {
	...
	var z1 = Wn({
		...
		JUPITER_MEAN_RADIUS_KM: () => Re,
		JUPITER_POLAR_RADIUS_KM: () => me,
		JupiterMoons: () => f3,
		JupiterMoonsInfo: () => q2,
		KM_PER_AU: () => b,
		LagrangePoint: () => h4,
		...
		TransitInfo: () => kn,
		Vector: () => C,
		VectorFromHorizon: () => b3,
		VectorFromSphere: () => X1,
		VectorObserver: () => r3,
		e_tilt: () => m0
	});
	const N1 = 173.1446326846693, b = 149597870.69098932, p2 = 63241.07708807546, E = .017453292519943295, St = .26179938779914946, V = 57.29577951308232, _t = 3.819718634205488, ge = 71492, me = 66854, Re = 69911, xe = 1821.6, Ie = 1560.8, ve = 2631.2, De = 2410.3, w2 = 365.24217, y2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), n0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), k0 = 484813681109536e-20, g2 = 10800 * 60, Pe = 2 * g2, m2 = 7292115e-11, ze = g2 / Math.PI, Ne = -.17 - 5 * Math.log10(ze), T1 = 29.530588, C1 = 24 * 3600, Te = C1 * 1e3, R2 = .9972695717592592, i1 = 695700, x2 = i1 / b, s0 = .996647180302104, F0 = s0 * s0, M0 = 6378.1366, Ce = M0 / b, Le = M0 * s0, I2 = 6371, Ue = I2 + 88, Oe = 1738.1 / b, j = 1737.4, v2 = 1736, be = v2 / b, ke = 34 / 60, At = 81.30056, a1 = .0002959122082855911, Et = 4912547451450812e-26, pt = 7243452486162703e-25, c1 = 8887692390113509e-25, wt = 9549535105779258e-26, V0 = 2.825345909524226e-7, H0 = 8.459715185680659e-8, K0 = 1.292024916781969e-8, $0 = 1.524358900784276e-8, Fe = 218869976542597e-26, L1 = c1 / At;
	function yt(t) {
		switch (t) {
	...
	function R0(t) {
		...
		function r(L, k, H, $) {
			const Q = [];
			for (let D0 = 0; D0 <= k - L; ++D0) Q.push(e(H, $));
			return {
				min: L,
				array: Q
			};
		...
		let i, c, a, u, h, l, d, _, M, A, g, p, y, w, x, R, P, v, z, D, T, U, B, u0 = r(-6, 6, 1, 4), o0 = r(-6, 6, 1, 4);
		function E0(L, k) {
			return o(u0, L, k);
		}
		function v0(L, k) {
		...
		for (v = .84 * f0 + .31 * jt + 14.27 * Xt + 7.26 * Zt + .28 * t2 + .24 * at, z = 2.94 * f0 + .31 * jt + 14.27 * Xt + 9.34 * Zt + 1.12 * t2 + .83 * at, D = -6.4 * f0 - 1.89 * at, T = .21 * f0 + .31 * jt + 14.27 * Xt - 88.7 * Zt - 15.3 * t2 + .24 * at - 1.86 * N4, U = v - D, _ = -3332e-9 * O(.59734 - 5.37261 * n) - 539e-9 * O(.35498 - 5.37899 * n) - 64e-9 * O(.39943 - 5.37511 * n), y = n0 * G0(.60643382 + 1336.85522467 * n - 313e-8 * d) + v / y0, w = n0 * G0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + z / y0, x = n0 * G0(.99312619 + 99.99735956 * n - 44e-8 * d) + D / y0, R = n0 * G0(.25909118 + 1342.2278298 * n - 892e-8 * d) + T / y0, P = n0 * G0(.82736186 + 1236.85308708 * n - 397e-8 * d) + U / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (l0(0, h, 1), l0(1, h, Math.cos(a) * u), h0(0, h, 0), h0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) it(E0(l - 1, h), v0(l - 1, h), E0(1, h), v0(1, h), (L, k) => (l0(l, h, L), h0(l, h, k)));
			for (l = 1; l <= c; ++l) l0(-l, h, E0(l, h)), h0(-l, h, -v0(l, h));
		}
		function qn(L, k, H, $) {
			for (var Q = {
				x: 1,
				...
				H,
				$
			], p0 = 1; p0 <= 4; ++p0) D0[p0] !== 0 && it(Q.x, Q.y, E0(D0[p0], p0), v0(D0[p0], p0), (n2, s1) => (Q.x = n2, Q.y = s1));
			return Q;
		}
		function S(L, k, H, $, Q, D0, p0, n2) {
			var s1 = qn(Q, D0, p0, n2);
			M += L * s1.y, B += k * s1.y, g += H * s1.x, p += $ * s1.x;
		}
		...
		let T4 = (1.000002708 + 139.978 * _) * (18519.699999999997 + g) * Math.sin(i) - 6.24 * Math.sin(3 * i) + A;
		return {
			geo_eclip_lon: n0 * G0((y + M / y0) / n0),
			geo_eclip_lat: Math.PI / (180 * 3600) * T4,
	...
	function F1(t, n) {
		...
		r *= k0, o *= k0, s *= k0, i *= k0;
		const c = Math.sin(r), a = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), _ = Math.sin(i), M = Math.cos(i), A = M * h - u * _ * d, g = M * u * a + _ * d * h * a - c * _ * l, p = M * u * c + _ * d * h * c + a * _ * l, y = -_ * h - u * M * d, w = -_ * u * a + M * d * h * a - c * M * l, x = -_ * u * c + M * d * h * c + a * M * l, R = u * l, P = -l * h * a - c * d, v = -l * h * c + d * a;
	...
	function je(t, n) {
		...
		let i, c, a;
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - Le;
		else {
	...
	function G1(t, n, e, r, o) {
		...
		const P = R[0] * y[0] + R[1] * y[1] + R[2] * y[2], v = R[0] * w[0] + R[1] * w[1] + R[2] * w[2], z = R[0] * x[0] + R[1] * x[1] + R[2] * x[2];
		let D = Math.hypot(v, z), T;
		D > 0 ? (T = -V * Math.atan2(z, v), T < 0 && (T += 360)) : T = 0;
		let U = V * Math.atan2(D, P), B = e, u0 = r;
		if (o) {
			let o0 = U, E0 = E1(o, 90 - U);
			if (U -= E0, E0 > 0 && U > 3e-4) {
				const v0 = Math.sin(U * E), l0 = Math.cos(U * E), h0 = Math.sin(o0 * E), it = Math.cos(o0 * E), O = [];
				for (let f0 = 0; f0 < 3; ++f0) O.push((R[f0] - it * y[f0]) / h0 * v0 + y[f0] * l0);
				D = Math.hypot(O[0], O[1]), D > 0 ? (B = _t * Math.atan2(O[1], O[0]), B < 0 && (B += 24)) : B = 0, u0 = V * Math.atan2(O[2], D);
			}
		}
		return new V2(T, 90 - U, B, u0);
	}
	function Y0(t) {
	...
	function $2(t) {
		const n = m(t).AddDays(-1 / N1), e = j0(q.Earth, n), [r, o, s] = K1([
			-e.x,
			-e.y,
			-e.z
		], n, F.From2000), i = E * m0(n).tobl, c = Math.cos(i), a = Math.sin(i);
		return It(new C(r, o, s, n), c, a);
	}
	...
	function r3(t, n) {
		...
		return n || (r = J0(r, t.t, F.From2000), r = B0(r, t.t, F.From2000)), je(r, e);
	}
	function o3(t, n) {
	...
	function It(t, n, e) {
		...
		return new H2(new C(r, o, s, t.t), a, c);
	}
	function l1(t) {
		const n = m0(t.t), [e, r, o] = B0(J0([
			...
			t.y,
			t.z
		], t.t, F.From2000), t.t, F.From2000), s = new C(e, r, o, t.t), i = n.tobl * E;
		return It(s, Math.cos(i), Math.sin(i));
	}
	function a0(t) {
		const n = m(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = J0(qe(n, [
	...
	function J1(t) {
		const n = m(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = m0(n), i = Rt(B0(T2(s.mobl, o), n, F.From2000), n), c = s.tobl * E, a = It(i, Math.cos(c), Math.sin(c));
	...
	const W0 = 365250, Pt = 0, zt = 1, B1 = 2;
	function Nt(t) {
	...
	function j0(t, n) {
		const e = n.tt / W0;
		return Nt(G2(z0(t[Pt], e, !0), z0(t[zt], e, !1), z0(t[B1], e, !1))).ToAstroVector(n);
	}
	function f1(t, n) {
		const e = n / W0, r = z0(t[Pt], e, !0), o = z0(t[zt], e, !1), s = z0(t[B1], e, !1), i = Dt(t[Pt], e), c = Dt(t[zt], e), a = Dt(t[B1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), _ = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, A = +(a * d) + s * l * c, g = G2(r, o, s), p = [
			_ / W0,
			M / W0,
			A / W0
		];
		return new T0(n, Nt(g), Nt(p));
	}
	function Q1(t, n, e, r) {
	...
	function s3(t) {
		...
		return Q1(n, t, f.Jupiter, V0), Q1(n, t, f.Saturn, H0), Q1(n, t, f.Uranus, K0), Q1(n, t, f.Neptune, $0), n;
	}
	const Tt = 51, i3 = 29200, X0 = 146, _0 = 201, N0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function a3(t) {
		let [n, [e, r, o], [s, i, c]] = t;
		return new T0(n, new Y(e, r, o), new Y(s, i, c));
	}
	function X(t, n, e, r) {
	...
	var M1 = class {
		constructor(t) {
			...
			this.Jupiter = X(n, t, f.Jupiter, V0), this.Saturn = X(n, t, f.Saturn, H0), this.Uranus = X(n, t, f.Uranus, K0), this.Neptune = X(n, t, f.Neptune, $0), this.Jupiter.r.decr(n.r), this.Jupiter.v.decr(n.v), this.Saturn.r.decr(n.r), this.Saturn.v.decr(n.v), this.Uranus.r.decr(n.r), this.Uranus.v.decr(n.v), this.Neptune.r.decr(n.r), this.Neptune.v.decr(n.v), this.Sun = new T0(t, n.r.mul(-1), n.v.mul(-1));
		}
		Acceleration(t) {
			let n = d1(t, a1, this.Sun.r);
			return n.incr(d1(t, V0, this.Jupiter.r)), n.incr(d1(t, H0, this.Saturn.r)), n.incr(d1(t, K0, this.Uranus.r)), n.incr(d1(t, $0, this.Neptune.r)), n;
		}
	}, Ct = class Yn {
		constructor(n, e, r, o) {
	...
	function Ut(t, n) {
		...
		return new J2(r, new Ct(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const c3 = [];
	function B2(t, n) {
	...
	function Ot(t) {
		const n = a3(t), e = new M1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new J2(e, new Ct(n.tt, r, o, s));
	}
	function u3(t, n) {
		const e = N0[0][0];
		if (n < e || n > N0[Tt - 1][0]) return null;
		const r = B2((n - e) / i3, Tt - 1);
		if (!t[r]) {
			const s = t[r] = [];
			s[0] = Ot(N0[r]).grav, s[_0 - 1] = Ot(N0[r + 1]).grav;
			let i, c = s[0].tt;
	...
	function bt(t, n) {
		...
		const s = u3(c3, t.tt);
		if (s) {
			const i = B2((t.tt - s[0].tt) / X0, _0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = Z0(t.tt - c.tt, c.r, c.v, u), l = Lt(t.tt - c.tt, c.v, u), d = Z0(t.tt - a.tt, a.r, a.v, u), _ = Lt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / X0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(_.mul(M));
		} else {
			let i;
			t.tt < N0[0][0] ? i = Q2(N0[0], t.tt, -X0) : i = Q2(N0[Tt - 1], t.tt, +X0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function h3(t, n, e) {
		...
		const M = i * h - s * l, A = -s * h - i * l, g = 1 / (1 + A), p = 1 / (1 + Math.sqrt(1 - s * s - i * i)), y = r * (h - s - p * i * M), w = r * (l - i + p * s * M), x = u * g * r * (-l - p * i * A), R = u * g * r * (+h + p * s * A), P = 2 * Math.sqrt(1 - c * c - a * a), v = 1 - 2 * a * a, z = 1 - 2 * c * c, D = 2 * a * c;
		return new K(y * v + w * D, y * D + w * z, (c * w - y * a) * P, x * v + R * D, x * D + R * z, (c * R - x * a) * P, t);
	}
	function Y1(t, n) {
	...
	function W2(t, n, e, r) {
		...
		if (b1(e)) {
			const c = c0(e, o);
			if (r) {
				const u = W1(n, o), h = new C(c.x - u.x, c.y - u.y, c.z - u.z, o), l = N1 / h.Length();
	...
	function M3(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
			case f.EMB:
				const o = f1(q[f.Earth], e.tt), s = t === f.Moon ? h1(e) : vt(e);
	...
	function W1(t, n) {
		...
		switch (t) {
			...
			case f.Saturn:
			case f.Uranus:
			case f.Neptune: return L0(f1(q[t], e.tt), e);
			case f.Pluto: return bt(e, !0);
			case f.Moon:
			case f.EMB:
				const o = f1(q.Earth, e.tt), s = t == f.Moon ? h1(e) : vt(e);
	...
	function j1(t, n) {
		...
		let o, s, i, c;
		t === f.Sun ? (i = new C(-r.x, -r.y, -r.z, e), s = new C(0, 0, 0, e), o = 0) : (t === f.Moon ? (i = a0(e), s = new C(r.x + i.x, r.y + i.y, r.z + i.z, e)) : (s = c0(t, n), i = new C(s.x - r.x, s.y - r.y, s.z - r.z, e)), o = O1(i, s));
		let a = i.Length(), u = s.Length(), h;
		if (t === f.Sun) c = Ne + 5 * Math.log10(a);
		else if (t === f.Moon) c = E3(o, u, a);
		else if (t === f.Saturn) {
			const l = A3(o, u, a, i, e);
	...
	function r1(t, n, e) {
		...
		for (let u = 0; u < 100; ++u) {
			let h = -a / 360 * i;
			if (c = c.AddDays(h), Math.abs(h) * C1 < 1) return c;
			let l = a;
	...
	function p3(t) {
		return tn(new Date(t.time.date.getTime() + 6 * Te));
	}
	var nn = class {
		constructor(t, n, e) {
	...
	function g3(t, n, e, r, o, s = 0) {
		...
		const i = y3(t), c = en(n.height - s);
		return rn(t, n, e, r, o, i, w3(n, s) - ke * c.density);
	}
	function m3(t, n, e, r, o, s) {
	...
	function Vt(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new R3(r, o, s, i);
		if (s >= 0 && i < 0) return null;
		if (t > 17) throw "Excessive recursion in rise/set ascent search.";
		const c = o.ut - r.ut;
		if (c * C1 < 1 || Math.min(Math.abs(s), Math.abs(i)) > e * (c / 2)) return null;
	...
	function rn(t, n, e, r, o, s, i) {
		if (Y0(n), N(o), N(s), N(i), i < -90 || i > 90) throw `Invalid target altitude angle: ${i}`;
		const c = .42, a = x3(t, n.latitude);
		function u(A) {
			const g = u1(t, A, n, !0, !0);
			return e * (G1(A, n, g.ra, g.dec).altitude + V * Math.asin(s / g.dist) - i);
		}
		const h = m(r);
	...
	function I3(t, n, e, r, o = 1) {
		...
		for (;;) {
			...
			if (i === 1 ? o > 0 ? u < 0 && (u += 24) : u > 0 && (u -= 24) : u < -12 ? u += 24 : u > 12 && (u -= 24), Math.abs(u) * 3600 < .1) {
				const l = G1(s, n, a.ra, a.dec, "normal");
				return new on(s, l);
			}
			let h = u / 24 * R2;
			s = s.AddDays(h);
	...
	function T3(t, n) {
		...
		for (let d = 0; d < 100; ++d) {
			const _ = r.AddDays(d * u), M = C0(t, _);
	...
	function hn(t, n) {
		if (n = m(n), t === f.Neptune || t === f.Pluto) return T3(t, n);
		function e(a) {
	...
	function U3(t, n, e) {
		...
		const r = N(e) * E, o = Math.cos(r), s = Math.sin(r), i = (n + 1) % 3, c = (n + 2) % 3, a = n;
		let u = [
			[
				0,
				0,
	...
	function Q3(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Gt || (Gt = tt(new g0(-45655.74141261017)), gn = new g0(0));
		const e = X1(new Q0(n, 15 * t, 1), gn), r = Ht(p1(Gt, e)), o = 10 / 240, s = o / 15;
		for (let i of B3) {
	...
	function w1(t, n, e, r) {
		...
		return new q3(n, o, b * Math.hypot(s, i, c), +i1 - (1 + o) * (i1 - t), -i1 + (1 + o) * (i1 + t), e, r);
	}
	function et(t) {
		const n = e0(f.Sun, t, !0), e = new C(-n.x, -n.y, -n.z, n.t);
		return w1(Ue, t, a0(t), e);
	}
	function xn(t) {
		const n = e0(f.Sun, t, !0), e = a0(t), r = new C(-e.x, -e.y, -e.z, e.t);
		return e.x -= n.x, e.y -= n.y, e.z -= n.z, w1(j, t, r, e);
	}
	function Jt(t, n) {
		const e = F2(t, n), r = e0(f.Sun, t, !0), o = a0(t), s = new C(e[0] - o.x, e[1] - o.y, e[2] - o.z, t);
		return o.x -= r.x, o.y -= r.y, o.z -= r.z, w1(j, t, s, o);
	}
	function rt(t, n, e) {
		const r = e0(t, e, !0), o = e0(f.Sun, e, !0), s = new C(r.x - o.x, r.y - o.y, r.z - o.z, e);
	...
	function X3(t, n, e) {
		const o = J((s) => Y3(t, n, s), e.AddDays(-1), e.AddDays(1));
	...
	function vn(t, n) {
		const e = new C(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = In(Math.asin(x2 / e.Length()), Math.asin(be / n.Length()), O1(n, e) * E);
	...
	function t4(t) {
		...
		if (_ > 0) {
			...
			const R = o1(i);
			let P = new C(g / b, p / b, y / b, t.time);
			P = p1(R, P), P.x += t.target.x, P.y += t.target.y, P.z += t.target.z;
			const v = w1(v2, t.time, P, t.dir);
			if (v.r > 1e-9 || v.r < 0) throw `Unexpected shadow distance from geoid intersection = ${v.r}`;
			n = zn(v.k), M = n === Z.Total ? 1 : vn(t.dir, P);
		} else M = void 0;
		return new Pn(n, M, e, r, o, s);
	}
	function n4(t) {
		return t = m(t), Dn(t.AddDays(10));
	}
	function Nn(t) {
		t = m(t);
		const n = 1.8;
	...
	function On(t, n) {
		return new Tn(n, o4(n, t));
	}
	function o4(t, n) {
		const e = u1(f.Sun, t, n, !0, !0);
		return G1(t, n, e.ra, e.dec, "normal").altitude;
	}
	function bn(t, n) {
		t = m(t), Y0(n);
		const e = 1.8;
	...
	function u4(t) {
		const n = J0(B0([
			...
			0,
			1
		], t, F.Into2000), t, F.Into2000), e = new C(n[0], n[1], n[2], t), r = Ht(e), o = 190.41375788700253 + 360.9856122880876 * t.ut;
	...
	function l4(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
				const d = E * (125.045 - .0529921 * r), _ = E * (250.089 - .1059842 * r), M = E * (260.008 + 13.0120009 * r), A = E * (176.625 + 13.3407154 * r), g = E * (357.529 + .9856003 * r), p = E * (311.589 + 26.4057084 * r), y = E * (134.963 + 13.064993 * r), w = E * (276.617 + .3287146 * r), x = E * (34.226 + 1.7484877 * r), R = E * (15.134 - .1589763 * r), P = E * (119.743 + .0036096 * r), v = E * (239.961 + .1643573 * r), z = E * (25.053 + 12.9590088 * r);
			...
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const D = E * (99.360714 + 4850.4046 * o), T = E * (175.895369 + 1191.9605 * o), U = E * (300.323162 + 262.5475 * o), B = E * (114.012305 + 6070.2476 * o), u0 = E * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(D) + 938e-6 * Math.sin(T) + .001432 * Math.sin(U) + 3e-5 * Math.sin(B) + .00215 * Math.sin(u0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(D) + 404e-6 * Math.cos(T) + 617e-6 * Math.cos(U) - 13e-6 * Math.cos(B) + 926e-6 * Math.cos(u0), c = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
		...
		const a = i * E, u = s * E, h = Math.cos(a), l = new C(h * Math.cos(u), h * Math.sin(u), Math.sin(a), e);
		return new Yt(s / 15, i, c, l);
	}
	function h4(t, n, e, r) {
		const o = m(n), s = yt(e), i = yt(r);
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new K(0, 0, 0, 0, 0, 0, o), a = h1(o)) : (c = W1(e, o), a = W1(r, o)), Gn(t, c, s, a, i);
	}
	function Gn(t, n, e, r, o) {
		...
		if (t === 4 || t === 5) {
			...
			const P = Math.sqrt(w * w + x * x + R * R);
			w /= P, x /= P, R /= P, c /= l, a /= l, u /= l;
			const v = t == 4 ? +i : -i, z = .5 * c + v * w, D = .5 * a + v * x, T = .5 * u + v * R, U = .5 * w - v * c, B = .5 * x - v * a, u0 = .5 * R - v * u, o0 = l * z, E0 = l * D, v0 = l * T, l0 = d * c + _ * a + M * u, h0 = d * w + _ * x + M * R;
			A = new K(o0, E0, v0, l0 * z + h0 * U, l0 * D + h0 * B, l0 * T + h0 * u0, n.t);
		} else {
			...
			do {
				const z = P - g, D = P - p;
				v = (y * P + x / (z * z) + R / (D * D)) / (y - 2 * x / (z * z * z) - 2 * R / (D * D * D)), P -= v;
			} while (Math.abs(v / l) > 1e-14);
			w = (P - g) / l, A = new K(w * c, w * a, w * u, w * d, w * _, w * M, n.t);
		}
		return A;
	}
	var f4 = class t0 {
		...
		Update(n) {
			...
			if (r === 0) this.prev = this.Duplicate();
				...
				for (let i = 0; i < this.curr.bodies.length; ++i) {
					...
					a.tt = e.tt, a.r = Z0(r, c.r, c.v, u), a.v = Lt(r, c.v, u);
				}
				this.CalcBodyAccelerations();
			}
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = X(r, n.tt, f.Mercury, Et), e[f.Venus] = X(r, n.tt, f.Venus, pt), e[f.Earth] = X(r, n.tt, f.Earth, c1 + L1), e[f.Mars] = X(r, n.tt, f.Mars, wt), e[f.Jupiter] = X(r, n.tt, f.Jupiter, V0), e[f.Saturn] = X(r, n.tt, f.Saturn, H0), e[f.Uranus] = X(r, n.tt, f.Uranus, K0), e[f.Neptune] = X(r, n.tt, f.Neptune, $0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new T0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = Y.zero(), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, a1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, Et), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, c1 + L1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, wt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, V0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, H0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, K0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, $0);
		}
	...
	const { MakeTime: n6, SearchRiseSet: e6, Body: r6, Observer: d4 } = st(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: a6, CombineRotation: c6, MakeRotation: u6 } = st(z1);
	Math.PI / (180 * 3600);
	function M4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: f6 } = st(z1);
	function S4(t, n) {
	...
	function E4(t) {
		...
		const n = we(A4(t)), { x: e, y: r, z: o, ra: s, dec: i } = t, c = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let a = null;
		return c && (a = he({
			x: e,
			y: r,
			z: o
		})), a == null && (a = E2(s, i)), a == null ? {
			x: 0,
			y: 0,
			z: n
		} : {
	...
	function v4(t, n, e, r, o, s, i, c = x4) {
		...
		for (const d of h) {
			...
			for (let A = 0; A < _.length; A += c) {
				const g = _.slice(A, A + c), p = g.length, y = new Float32Array(p * 3), w = new Float32Array(p), x = new Float32Array(p * 3), R = new Float32Array(p), P = new Float32Array(p), v = new Float32Array(p), z = [];
				for (let T = 0; T < p; T += 1) {
					const U = g[T];
					if (U === void 0) throw new Error("starfieldChunk: missing slice index");
					const B = t[U];
					if (!B) throw new Error("starfieldChunk: missing star row");
					z.push(B), y[T * 3] = n[U * 3], y[T * 3 + 1] = n[U * 3 + 1], y[T * 3 + 2] = n[U * 3 + 2], w[T] = e[U], x[T * 3] = r[U * 3], x[T * 3 + 1] = r[U * 3 + 1], x[T * 3 + 2] = r[U * 3 + 2], R[T] = o[U], P[T] = s[U], v[T] = i[U];
				}
	...
	function D4() {
		const t = [];
		for (let n = 0; n < w4; n += 1) {
			const e = E2(n * .7 % 360, n * .3 % 180 - 90);
			e != null && t.push(e);
		}
		return t;
	}
	...
	function z4(t) {
		const { catalog: n, maxDistanceLevel: e, bounds: r, raInHours: o } = t, s = _4(S4(n, { raInHours: o }).filter((p) => !Wt(p)), r, { maxDistanceLevel: e }), i = () => ({
			chunks: [],
			namedStarsInView: [],
			starsInView: []
		});
		if (s.length === 0) {
			if (e === 0) return i();
			const p = P4(), y = p.length, w = new Float32Array(y * 3), x = new Float32Array(y).fill(Bn), R = new Float32Array(y * 3).fill(1), P = new Float32Array(y).fill(1), v = new Float32Array(y).fill(Qn);
			for (let z = 0; z < y; z += 1) {
				const D = p[z];
				D && (w[z * 3] = D.x, w[z * 3 + 1] = D.y, w[z * 3 + 2] = D.z);
			}
			return {
				chunks: [{
					abbr: "_fallback",
		...
		const c = s.map(E4), a = c.length, u = new Float32Array(a * 3), h = new Float32Array(a), l = new Float32Array(a * 3), d = new Float32Array(a), _ = new Float32Array(a), M = new Float32Array(a), A = [];
		for (let p = 0; p < a; p += 1) {
			const y = c[p], w = s[p];
			u[p * 3] = y.x, u[p * 3 + 1] = y.y, u[p * 3 + 2] = y.z;
			const x = g4(w);
			h[p] = R4(w);
			const [R, P, v] = p4(w?.c != null ? `#${w.c}` : void 0), z = y4(w), D = m4(z), T = 1 - D;
			l[p * 3] = O0(D + T * R), l[p * 3 + 1] = O0(D + T * P), l[p * 3 + 2] = O0(D + T * v), d[p] = z, _[p] = O0(.24 + .46 * (w?.br != null && Number.isFinite(w.br) ? Math.max(0, Math.min(1, w.br)) : 1) + .12 * x), M[p] = w?.mag != null && Number.isFinite(w.mag) ? w.mag : Number.POSITIVE_INFINITY, w?.proper && A.push({
				position: { ...y },
				name: w.proper,
				catalogId: String(w.id)
			});
		}
		const g = v4(s, u, h, l, d, _, M);
		return {
			chunks: g,
			namedStarsInView: A,
