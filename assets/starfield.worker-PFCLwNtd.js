(function() {
	...
	d2(_e);
	d2(Ae);
	const Ee = 25 * D1, W = 299792.458, v1 = .001, P1 = 1e3;
	1 * v1, 10 * v1, 100 * v1, 1 * P1, 10 * P1, 100 * P1, .01 * W, .1 * W, .5 * W, 1 * W, 10 * W, 100 * W, 1e3 * W, 1e4 * W, 1e5 * W, 1e6 * W, 1e7 * W, 1e8 * W, 1e9 * W;
	1 * v1;
	1e9 * W;
	1 * P1;
	function pe(t) {
	...
	const T1 = 173.1446326846693, b = 149597870.69098932, p2 = 63241.07708807546, E = .017453292519943295, St = .26179938779914946, V = 57.29577951308232, _t = 3.819718634205488, ge = 71492, me = 66854, Re = 69911, xe = 1821.6, Ie = 1560.8, De = 2631.2, ve = 2410.3, w2 = 365.24217, y2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), n0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), k0 = 484813681109536e-20, g2 = 10800 * 60, Pe = 2 * g2, m2 = 7292115e-11, ze = g2 / Math.PI, Te = -.17 - 5 * Math.log10(ze), N1 = 29.530588, C1 = 24 * 3600, Ne = C1 * 1e3, R2 = .9972695717592592, i1 = 695700, x2 = i1 / b, s0 = .996647180302104, F0 = s0 * s0, M0 = 6378.1366, Ce = M0 / b, Le = M0 * s0, I2 = 6371, Ue = I2 + 88, Oe = 1738.1 / b, j = 1737.4, D2 = 1736, be = D2 / b, ke = 34 / 60, At = 81.30056, a1 = .0002959122082855911, Et = 4912547451450812e-26, pt = 7243452486162703e-25, c1 = 8887692390113509e-25, wt = 9549535105779258e-26, V0 = 2.825345909524226e-7, H0 = 8.459715185680659e-8, K0 = 1.292024916781969e-8, G0 = 1.524358900784276e-8, Fe = 218869976542597e-26, L1 = c1 / At;
	function yt(t) {
		switch (t) {
	...
	function R0(t) {
		...
		function r(L, k, H, G) {
			const Q = [];
			for (let v0 = 0; v0 <= k - L; ++v0) Q.push(e(H, G));
			return {
				min: L,
				array: Q
			};
		...
		let i, c, a, u, h, l, d, _, M, A, g, p, y, w, x, R, P, D, z, v, N, U, B, u0 = r(-6, 6, 1, 4), o0 = r(-6, 6, 1, 4);
		function E0(L, k) {
			return o(u0, L, k);
		}
		function D0(L, k) {
		...
		var f0 = O(.19833 + .05611 * n), jt = O(.27869 + .04508 * n), Xt = O(.16827 - .36903 * n), Zt = O(.34734 - 5.37261 * n), t2 = O(.10498 - 5.37899 * n), at = O(.42681 - .41855 * n), T4 = O(.14943 - 5.37511 * n);
		for (D = .84 * f0 + .31 * jt + 14.27 * Xt + 7.26 * Zt + .28 * t2 + .24 * at, z = 2.94 * f0 + .31 * jt + 14.27 * Xt + 9.34 * Zt + 1.12 * t2 + .83 * at, v = -6.4 * f0 - 1.89 * at, N = .21 * f0 + .31 * jt + 14.27 * Xt - 88.7 * Zt - 15.3 * t2 + .24 * at - 1.86 * T4, U = D - v, _ = -3332e-9 * O(.59734 - 5.37261 * n) - 539e-9 * O(.35498 - 5.37899 * n) - 64e-9 * O(.39943 - 5.37511 * n), y = n0 * $0(.60643382 + 1336.85522467 * n - 313e-8 * d) + D / y0, w = n0 * $0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + z / y0, x = n0 * $0(.99312619 + 99.99735956 * n - 44e-8 * d) + v / y0, R = n0 * $0(.25909118 + 1342.2278298 * n - 892e-8 * d) + N / y0, P = n0 * $0(.82736186 + 1236.85308708 * n - 397e-8 * d) + U / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (l0(0, h, 1), l0(1, h, Math.cos(a) * u), h0(0, h, 0), h0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) it(E0(l - 1, h), D0(l - 1, h), E0(1, h), D0(1, h), (L, k) => (l0(l, h, L), h0(l, h, k)));
		...
		function qn(L, k, H, G) {
			for (var Q = {
				...
				H,
				G
			], p0 = 1; p0 <= 4; ++p0) v0[p0] !== 0 && it(Q.x, Q.y, E0(v0[p0], p0), D0(v0[p0], p0), (n2, s1) => (Q.x = n2, Q.y = s1));
			return Q;
		}
		function S(L, k, H, G, Q, v0, p0, n2) {
			var s1 = qn(Q, v0, p0, n2);
			M += L * s1.y, B += k * s1.y, g += H * s1.x, p += G * s1.x;
		}
		...
		function d0(L, k, H, G, Q) {
			return L * qn(k, H, G, Q).y;
		}
		...
		let N4 = (1.000002708 + 139.978 * _) * (18519.699999999997 + g) * Math.sin(i) - 6.24 * Math.sin(3 * i) + A;
		return {
			geo_eclip_lon: n0 * $0((y + M / y0) / n0),
			geo_eclip_lat: Math.PI / (180 * 3600) * N4,
	...
	function F1(t, n) {
		...
		r *= k0, o *= k0, s *= k0, i *= k0;
		const c = Math.sin(r), a = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), _ = Math.sin(i), M = Math.cos(i), A = M * h - u * _ * d, g = M * u * a + _ * d * h * a - c * _ * l, p = M * u * c + _ * d * h * c + a * _ * l, y = -_ * h - u * M * d, w = -_ * u * a + M * d * h * a - c * M * l, x = -_ * u * c + M * d * h * c + a * M * l, R = u * l, P = -l * h * a - c * d, D = -l * h * c + d * a;
	...
	function $1(t, n, e, r, o) {
		...
		const P = R[0] * y[0] + R[1] * y[1] + R[2] * y[2], D = R[0] * w[0] + R[1] * w[1] + R[2] * w[2], z = R[0] * x[0] + R[1] * x[1] + R[2] * x[2];
		let v = Math.hypot(D, z), N;
		v > 0 ? (N = -V * Math.atan2(z, D), N < 0 && (N += 360)) : N = 0;
		let U = V * Math.atan2(v, P), B = e, u0 = r;
		if (o) {
			...
			if (U -= E0, E0 > 0 && U > 3e-4) {
				...
				v = Math.hypot(O[0], O[1]), v > 0 ? (B = _t * Math.atan2(O[1], O[0]), B < 0 && (B += 24)) : B = 0, u0 = V * Math.atan2(O[2], v);
			}
		}
	...
	function j0(t, n) {
		const e = n.tt / W0;
		return Tt($2(z0(t[Pt], e, !0), z0(t[zt], e, !1), z0(t[B1], e, !1))).ToAstroVector(n);
	}
	function f1(t, n) {
		const e = n / W0, r = z0(t[Pt], e, !0), o = z0(t[zt], e, !1), s = z0(t[B1], e, !1), i = vt(t[Pt], e), c = vt(t[zt], e), a = vt(t[B1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), _ = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, A = +(a * d) + s * l * c, g = $2(r, o, s), p = [
			_ / W0,
			M / W0,
			A / W0
		];
	...
	const Nt = 51, i3 = 29200, X0 = 146, _0 = 201, T0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function h3(t, n, e) {
		...
		const M = i * h - s * l, A = -s * h - i * l, g = 1 / (1 + A), p = 1 / (1 + Math.sqrt(1 - s * s - i * i)), y = r * (h - s - p * i * M), w = r * (l - i + p * s * M), x = u * g * r * (-l - p * i * A), R = u * g * r * (+h + p * s * A), P = 2 * Math.sqrt(1 - c * c - a * a), D = 1 - 2 * a * a, z = 1 - 2 * c * c, v = 2 * a * c;
		return new K(y * D + w * v, y * v + w * z, (c * w - y * a) * P, x * D + R * v, x * v + R * z, (c * R - x * a) * P, t);
	}
	function Y1(t, n) {
	...
	function W2(t, n, e, r) {
		...
		if (b1(e)) {
			const c = c0(e, o);
			if (r) {
				const u = W1(n, o), h = new C(c.x - u.x, c.y - u.y, c.z - u.z, o), l = T1 / h.Length();
	...
	function J(t, n, e, r) {
		const o = T(r && r.dt_tolerance_seconds || 1), s = Math.abs(o / C1);
	...
	function j1(t, n) {
		...
		if (t === f.Sun) c = Te + 5 * Math.log10(a);
	...
	function p3(t) {
		return tn(new Date(t.time.date.getTime() + 6 * Ne));
	}
	var nn = class {
		constructor(t, n, e) {
	...
	function hn(t, n) {
		if (n = m(n), t === f.Neptune || t === f.Pluto) return N3(t, n);
		function e(a) {
	...
	function l4(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
				const d = E * (125.045 - .0529921 * r), _ = E * (250.089 - .1059842 * r), M = E * (260.008 + 13.0120009 * r), A = E * (176.625 + 13.3407154 * r), g = E * (357.529 + .9856003 * r), p = E * (311.589 + 26.4057084 * r), y = E * (134.963 + 13.064993 * r), w = E * (276.617 + .3287146 * r), x = E * (34.226 + 1.7484877 * r), R = E * (15.134 - .1589763 * r), P = E * (119.743 + .0036096 * r), D = E * (239.961 + .1643573 * r), z = E * (25.053 + 12.9590088 * r);
			...
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const v = E * (99.360714 + 4850.4046 * o), N = E * (175.895369 + 1191.9605 * o), U = E * (300.323162 + 262.5475 * o), B = E * (114.012305 + 6070.2476 * o), u0 = E * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(v) + 938e-6 * Math.sin(N) + .001432 * Math.sin(U) + 3e-5 * Math.sin(B) + .00215 * Math.sin(u0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(v) + 404e-6 * Math.cos(N) + 617e-6 * Math.cos(U) - 13e-6 * Math.cos(B) + 926e-6 * Math.cos(u0), c = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
	...
	function $n(t, n, e, r, o) {
		...
		if (t === 4 || t === 5) {
			...
			const D = t == 4 ? +i : -i, z = .5 * c + D * w, v = .5 * a + D * x, N = .5 * u + D * R, U = .5 * w - D * c, B = .5 * x - D * a, u0 = .5 * R - D * u, o0 = l * z, E0 = l * v, D0 = l * N, l0 = d * c + _ * a + M * u, h0 = d * w + _ * x + M * R;
			A = new K(o0, E0, D0, l0 * z + h0 * U, l0 * v + h0 * B, l0 * N + h0 * u0, n.t);
		} else {
			...
			do {
				const z = P - g, v = P - p;
				D = (y * P + x / (z * z) + R / (v * v)) / (y - 2 * x / (z * z * z) - 2 * R / (v * v * v)), P -= D;
	...
	var f4 = class t0 {
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = X(r, n.tt, f.Mercury, Et), e[f.Venus] = X(r, n.tt, f.Venus, pt), e[f.Earth] = X(r, n.tt, f.Earth, c1 + L1), e[f.Mars] = X(r, n.tt, f.Mars, wt), e[f.Jupiter] = X(r, n.tt, f.Jupiter, V0), e[f.Saturn] = X(r, n.tt, f.Saturn, H0), e[f.Uranus] = X(r, n.tt, f.Uranus, K0), e[f.Neptune] = X(r, n.tt, f.Neptune, G0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new N0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = Y.zero(), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, a1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, Et), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, c1 + L1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, wt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, V0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, H0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, K0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, G0);
		}
	...
	const { MakeTime: r6, SearchRiseSet: o6, Body: s6, Observer: d4 } = st(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: u6, CombineRotation: l6, MakeRotation: h6 } = st(z1);
	Math.PI / (180 * 3600);
	function M4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: M6 } = st(z1);
	function S4(t, n) {
	...
	function D4(t, n, e, r, o, s, i, c = x4) {
		...
		for (const d of h) {
			...
			for (let A = 0; A < _.length; A += c) {
				const g = _.slice(A, A + c), p = g.length, y = new Float32Array(p * 3), w = new Float32Array(p), x = new Float32Array(p * 3), R = new Float32Array(p), P = new Float32Array(p), D = new Float32Array(p), z = [];
				for (let N = 0; N < p; N += 1) {
					const U = g[N];
					if (U === void 0) throw new Error("starfieldChunk: missing slice index");
					const B = t[U];
					if (!B) throw new Error("starfieldChunk: missing star row");
					z.push(B), y[N * 3] = n[U * 3], y[N * 3 + 1] = n[U * 3 + 1], y[N * 3 + 2] = n[U * 3 + 2], w[N] = e[U], x[N * 3] = r[U * 3], x[N * 3 + 1] = r[U * 3 + 1], x[N * 3 + 2] = r[U * 3 + 2], R[N] = o[U], P[N] = s[U], D[N] = i[U];
				}
				const v = _.length <= c ? d : `${d}__part${M}`;
				M += 1, l.push({
					abbr: v,
					positions: y,
					sizes: w,
	...
	function z4(t) {
		...
		if (s.length === 0) {
			if (e === 0) return i();
			const p = P4(), y = p.length, w = new Float32Array(y * 3), x = new Float32Array(y).fill(Bn), R = new Float32Array(y * 3).fill(1), P = new Float32Array(y).fill(1), D = new Float32Array(y).fill(Qn);
			for (let z = 0; z < y; z += 1) {
				const v = p[z];
				v && (w[z * 3] = v.x, w[z * 3 + 1] = v.y, w[z * 3 + 2] = v.z);
			}
			return {
				chunks: [{
					abbr: "_fallback",
		...
		for (let p = 0; p < a; p += 1) {
			...
			const x = g4(w);
			h[p] = R4(w);
			const [R, P, D] = p4(w?.c != null ? `#${w.c}` : void 0), z = y4(w), v = m4(z), N = 1 - v;
			l[p * 3] = O0(v + N * R), l[p * 3 + 1] = O0(v + N * P), l[p * 3 + 2] = O0(v + N * D), d[p] = z, _[p] = O0(.24 + .46 * (w?.br != null && Number.isFinite(w.br) ? Math.max(0, Math.min(1, w.br)) : 1) + .12 * x), M[p] = w?.mag != null && Number.isFinite(w.mag) ? w.mag : Number.POSITIVE_INFINITY, w?.proper && A.push({
				position: { ...y },
				name: w.proper,
				catalogId: String(w.id)
			});
		}
		const g = D4(s, u, h, l, d, _, M);
		return {
			chunks: g,
			namedStarsInView: A,
