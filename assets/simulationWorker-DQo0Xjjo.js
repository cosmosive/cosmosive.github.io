(function() {
	const X = Symbol("Comlink.proxy"), Se = Symbol("Comlink.endpoint"), me = Symbol("Comlink.releaseProxy"), D = Symbol("Comlink.finalizer"), M = Symbol("Comlink.thrown"), z = (e) => typeof e == "object" && e !== null || typeof e == "function", V = new Map([["proxy", {
		canHandle: (e) => z(e) && e[X],
		serialize(e) {
	...
	function K(e, n = globalThis, r = ["*"]) {
		n.addEventListener("message", function o(s) {
			...
			let u;
			try {
				const d = c.slice(0, -1).reduce((m, T) => m[T], e), E = c.reduce((m, T) => m[T], e);
				switch (l) {
					case "GET":
						u = E;
						break;
			...
			Promise.resolve(u).catch((d) => ({
				...
				const [E, m] = g(d);
				n.postMessage(Object.assign(Object.assign({}, E), { id: i }), m), l === "RELEASE" && (n.removeEventListener("message", o), W(n), D in e && typeof e[D] == "function" && e[D]());
			}).catch((d) => {
				const [E, m] = g({
					value: /* @__PURE__ */ new TypeError("Unserializable return value"),
					[M]: 0
				});
				n.postMessage(Object.assign(Object.assign({}, E), { id: i }), m);
			});
		}), n.start && n.start();
	}
	function Ie(e) {
	...
	function L(e, n, r = [], o = function() {}) {
		...
		const i = new Proxy(o, {
			...
			apply(l, c, A) {
				...
				if (u === Se) return R(e, n, { type: "ENDPOINT" }).then(I);
	...
	function q(e) {
		const n = e.map(g);
		return [n.map((r) => r[0]), Te(n.map((r) => r[1]))];
	}
	...
	_ * 3600;
	_ * h;
	const Z = _ * Ue * h, a = 149597870.7, ee = 648e3 / Math.PI, ge = ee * a;
	a / Z;
	const Oe = ge / Z;
	Object.freeze({
		x: 0,
	...
	var O = (e, n, r) => Object.defineProperty(e, n, {
		...
		writable: !0,
		configurable: !0
	}), Ne = Symbol.for("bitecs-relation"), Pe = Symbol.for("bitecs-pairTarget"), xe = Symbol.for("bitecs-isPairComponent"), p = Symbol.for("bitecs-relationData"), te = () => {
		let e = {
			...
			if (!e.pairsMap.has(o)) {
				...
				O(s, Ne, n), O(s, Pe, o), O(s, xe, !0), e.pairsMap.set(o, s);
			}
	...
	const At = 5e4, _t = 2e3, St = At / ae, mt = _t / ae;
	ie(St);
	ie(mt);
	25 * B;
	const w = 1e3;
	1 * w, 1 * _, 10 * _, 100 * _, 1e3 * _, 1e6 * _, 1e9 * _;
	1 * w, 1 * _;
	1e9 * _;
	1 * w;
	const H = 1721545, Y = 3181545;
	function Et(e) {
		return {
			name: e.name,
			type: e.type,
	...
	function _e(e, n) {
		...
		switch (r.type) {
			...
			case "seek": {
				...
				const o = r.jd;
				let s = o;
				(s < 1721545 || s > 3181545) && (console.warn(`[kernel/seek] JD ${s} outside supported range [${H}, ${Y}] (years 0-4000 AD); clamping.`), s = Math.max(H, Math.min(Y, s)));
				const i = {
					...e,
					jd: s
				}, l = [{
			...
			case "step": {
				...
				const s = e.jd + e.speed * o / pt, i = Math.max(H, Math.min(Y, s)), l = {
					...e,
					jd: i
				}, c = [{
					type: "tick",
	...
	function Mt(e) {
		if (e.type === "init") {
			...
			S = J({
				jd: n.jd ?? S.jd,
				playing: n.playing ?? S.playing,
				speed: n.speed ?? S.speed
			}), G(), v({
				type: "workerTick",
				state: S,
				events: []
		...
		if (e.type === "time") {
			const { state: n, events: r } = _e(S, e.cmd);
			S = n, v({
				type: "workerTick",
				state: S,
				events: r
	...
	K({
		...
		subscribe(e) {
			y != null && y !== e && console.warn("[simulationWorker] replacing existing outbound subscriber (double subscribe)"), y = e, G();
		},
		unsubscribe() {
			y = null, F();
		}
