(function() {
	...
	d2(_e);
	d2(Ae);
	const pe = 25 * v1, W = 299792.458, D1 = .001, z1 = 1e3;
	1 * D1, 10 * D1, 100 * D1, 1 * z1, 10 * z1, 100 * z1, .01 * W, .1 * W, .5 * W, 1 * W, 10 * W, 100 * W, 1e3 * W, 1e4 * W, 1e5 * W, 1e6 * W, 1e7 * W, 1e8 * W, 1e9 * W;
	1 * D1;
	1e9 * W;
	1 * z1;
	function Ee(t) {
	...
	const N1 = 173.1446326846693, b = 149597870.69098932, p2 = 63241.07708807546, p = .017453292519943295, St = .26179938779914946, V = 57.29577951308232, _t = 3.819718634205488, ye = 71492, ge = 66854, me = 69911, Re = 1821.6, xe = 1560.8, Ie = 2631.2, ve = 2410.3, E2 = 365.24217, w2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), n0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), k0 = 484813681109536e-20, y2 = 10800 * 60, De = 2 * y2, g2 = 7292115e-11, ze = y2 / Math.PI, Pe = -.17 - 5 * Math.log10(ze), T1 = 29.530588, C1 = 24 * 3600, Ne = C1 * 1e3, m2 = .9972695717592592, i1 = 695700, R2 = i1 / b, s0 = .996647180302104, F0 = s0 * s0, M0 = 6378.1366, Te = M0 / b, Ce = M0 * s0, x2 = 6371, Le = x2 + 88, Ue = 1738.1 / b, j = 1737.4, I2 = 1736, Oe = I2 / b, be = 34 / 60, At = 81.30056, a1 = .0002959122082855911, pt = 4912547451450812e-26, Et = 7243452486162703e-25, c1 = 8887692390113509e-25, wt = 9549535105779258e-26, V0 = 2.825345909524226e-7, H0 = 8.459715185680659e-8, $0 = 1.292024916781969e-8, J0 = 1.524358900784276e-8, ke = 218869976542597e-26, L1 = c1 / At;
	function yt(t) {
		switch (t) {
	...
	function R0(t) {
		...
		for (v = .84 * f0 + .31 * jt + 14.27 * Xt + 7.26 * Zt + .28 * t2 + .24 * at, P = 2.94 * f0 + .31 * jt + 14.27 * Xt + 9.34 * Zt + 1.12 * t2 + .83 * at, D = -6.4 * f0 - 1.89 * at, T = .21 * f0 + .31 * jt + 14.27 * Xt - 88.7 * Zt - 15.3 * t2 + .24 * at - 1.86 * N4, U = v - D, _ = -3332e-9 * O(.59734 - 5.37261 * n) - 539e-9 * O(.35498 - 5.37899 * n) - 64e-9 * O(.39943 - 5.37511 * n), y = n0 * K0(.60643382 + 1336.85522467 * n - 313e-8 * d) + v / y0, w = n0 * K0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + P / y0, x = n0 * K0(.99312619 + 99.99735956 * n - 44e-8 * d) + D / y0, R = n0 * K0(.25909118 + 1342.2278298 * n - 892e-8 * d) + T / y0, z = n0 * K0(.82736186 + 1236.85308708 * n - 397e-8 * d) + U / y0, h = 1; h <= 4; ++h) {
			...
			for (l0(0, h, 1), l0(1, h, Math.cos(a) * u), h0(0, h, 0), h0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) it(p0(l - 1, h), v0(l - 1, h), p0(1, h), v0(1, h), (L, k) => (l0(l, h, L), h0(l, h, k)));
		...
		function qn(L, k, H, J) {
			for (var Q = {
				...
				H,
				J
			], E0 = 1; E0 <= 4; ++E0) D0[E0] !== 0 && it(Q.x, Q.y, p0(D0[E0], E0), v0(D0[E0], E0), (n2, s1) => (Q.x = n2, Q.y = s1));
			return Q;
		}
		function S(L, k, H, J, Q, D0, E0, n2) {
			var s1 = qn(Q, D0, E0, n2);
	...
	function K1(t, n, e, r, o) {
		let s = m(t);
		Y0(n), N(e), N(r);
		const i = Math.sin(n.latitude * p), c = Math.cos(n.latitude * p), a = Math.sin(n.longitude * p), u = Math.cos(n.longitude * p), h = Math.sin(r * p), l = Math.cos(r * p), d = Math.sin(e * St), _ = Math.cos(e * St);
		let M = [
			c * u,
			c * a,
			i
		...
		if (o) {
			...
			if (U -= p0, p0 > 0 && U > 3e-4) {
				const v0 = Math.sin(U * p), l0 = Math.cos(U * p), h0 = Math.sin(o0 * p), it = Math.cos(o0 * p), O = [];
	...
	function $2(t) {
		const n = m(t).AddDays(-1 / N1), e = j0(q.Earth, n), [r, o, s] = $1([
			-e.x,
			-e.y,
			-e.z
	...
	function u1(t, n, e, r, o) {
		...
		return H2(r ? $1(a, s, F.From2000) : a, s);
	}
	function t3(t, n, e) {
		const r = m(t);
		let o = mt(n, S0(r)).pos;
		return e || (o = $1(o, r, F.Into2000)), Rt(o, r);
	}
	function n3(t, n, e) {
		const r = m(t), o = mt(n, S0(r)), s = new $(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
	...
	function G1(t) {
		const n = m(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = m0(n), i = Rt(B0(N2(s.mobl, o), n, F.From2000), n), c = s.tobl * p, a = It(i, Math.cos(c), Math.sin(c));
	...
	const W0 = 365250, zt = 0, Pt = 1, B1 = 2;
	function Nt(t) {
	...
	function j0(t, n) {
		const e = n.tt / W0;
		return Nt(J2(P0(t[zt], e, !0), P0(t[Pt], e, !1), P0(t[B1], e, !1))).ToAstroVector(n);
	}
	function f1(t, n) {
		const e = n / W0, r = P0(t[zt], e, !0), o = P0(t[Pt], e, !1), s = P0(t[B1], e, !1), i = Dt(t[zt], e), c = Dt(t[Pt], e), a = Dt(t[B1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), _ = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, A = +(a * d) + s * l * c, g = J2(r, o, s), E = [
			_ / W0,
			M / W0,
			A / W0
		];
		return new T0(n, Nt(g), Nt(E));
	}
	function Q1(t, n, e, r) {
	...
	const Tt = 51, s3 = 29200, X0 = 146, _0 = 201, N0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function Ut(t, n) {
		...
		return new K2(r, new Ct(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const a3 = [];
	function G2(t, n) {
	...
	function Ot(t) {
		...
		return new K2(e, new Ct(n.tt, r, o, s));
	}
	function c3(t, n) {
		const e = N0[0][0];
		if (n < e || n > N0[Tt - 1][0]) return null;
		const r = G2((n - e) / s3, Tt - 1);
		if (!t[r]) {
			const s = t[r] = [];
	...
	function bt(t, n) {
		...
		const s = c3(a3, t.tt);
		if (s) {
			const i = G2((t.tt - s[0].tt) / X0, _0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = Z0(t.tt - c.tt, c.r, c.v, u), l = Lt(t.tt - c.tt, c.v, u), d = Z0(t.tt - a.tt, a.r, a.v, u), _ = Lt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / X0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(_.mul(M));
		} else {
			let i;
			t.tt < N0[0][0] ? i = B2(N0[0], t.tt, -X0) : i = B2(N0[Tt - 1], t.tt, +X0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function C0(t, n) {
		...
		if (e) return e.dist;
		const r = m(n);
		return t in q ? P0(q[t][B1], r.tt / W0, !1) : c0(t, r).Length();
	}
	function q2(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
	...
	function O3(t, n, e) {
		n = m(n);
		const r = hn(t.lon);
		return X1(new Q0(t.lat + fn(e, t.lat), r, t.dist), n);
	}
	function p1(t, n) {
		let e;
	...
	function B3(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Kt || (Kt = tt(new g0(-45655.74141261017)), yn = new g0(0));
		const e = X1(new Q0(n, 15 * t, 1), yn), r = Ht(E1(Kt, e)), o = 10 / 240, s = o / 15;
		for (let i of G3) {
	...
	function Y3(t) {
		const e = G((r) => Bt(et, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Earth shadow time.";
		return et(e);
	}
	function W3(t) {
		const e = G((r) => Bt(Rn, r), t.AddDays(-.03), t.AddDays(.03));
	...
	function X3(t, n) {
		...
		const i = G((c) => Bt(s, c), r, o);
		if (!i) throw `PeakLocalMoonShadow: search failure for search_center_time = ${t}`;
		return Gt(i, n);
	}
	function Qt(t, n, e) {
		const r = e / 1440, o = t.AddDays(-r), s = t.AddDays(+r), i = G((a) => -(et(a).r - n), o, t), c = G((a) => +(et(a).r - n), t, s);
	...
	function In(t, n) {
		const e = new C(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = xn(Math.asin(R2 / e.Length()), Math.asin(Oe / n.Length()), O1(n, e) * p);
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				...
				if (i.r < i.p + j) {
					...
					return i.r < i.k + j && (c = Z.Partial, h = Qt(i.time, i.k + j, l), i.r + j < i.k ? (c = Z.Total, a = 1, u = Qt(i.time, i.k - j, h)) : a = xn(j, i.k, i.r)), new mn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function Z3(t) {
		...
		const i = tt(t.time), c = E1(i, t.dir), a = E1(i, t.target);
	...
	function e4(t, n) {
		...
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = ot(n, 1, Ln, s, t.time), h = ot(n, -1, Ln, t.time, i), l = zn(t.k)) : l = Z.Partial;
	...
	function $n(t) {
		...
		for (;;) {
			...
			if (e.lat * o.lat <= 0) {
				const s = o.lat > e.lat ? A0.Ascending : A0.Descending, i = G((c) => s * G1(c).lat, n, r);
	...
	function l4(t, n, e, r) {
		...
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new $(0, 0, 0, 0, 0, 0, o), a = h1(o)) : (c = W1(e, o), a = W1(r, o)), Jn(t, c, s, a, i);
	}
	...
	var h4 = class t0 {
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = X(r, n.tt, f.Mercury, pt), e[f.Venus] = X(r, n.tt, f.Venus, Et), e[f.Earth] = X(r, n.tt, f.Earth, c1 + L1), e[f.Mars] = X(r, n.tt, f.Mars, wt), e[f.Jupiter] = X(r, n.tt, f.Jupiter, V0), e[f.Saturn] = X(r, n.tt, f.Saturn, H0), e[f.Uranus] = X(r, n.tt, f.Uranus, $0), e[f.Neptune] = X(r, n.tt, f.Neptune, J0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new T0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = Y.zero(), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, a1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, pt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, Et), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, c1 + L1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, wt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, V0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, H0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, $0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, J0);
		}
	...
	const { MakeTime: W4, SearchRiseSet: j4, Body: X4, Observer: f4 } = st(P1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: e6, CombineRotation: r6, MakeRotation: o6 } = st(P1);
	Math.PI / (180 * 3600);
	function M4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: a6 } = st(P1);
	I1 / dt;
	function S4(t, n) {
	...
	function P4(t) {
		const { catalog: n, maxDistanceLevel: e, bounds: r, raInHours: o } = t, s = _4(S4(n, { raInHours: o }).filter((E) => !Wt(E)), r, { maxDistanceLevel: e }), i = () => ({
			chunks: [],
			namedStarsInView: [],
			starsInView: []
		});
