(function() {
	const V = Symbol("Comlink.proxy"), ht = Symbol("Comlink.endpoint"), pt = Symbol("Comlink.releaseProxy"), k = Symbol("Comlink.finalizer"), b = Symbol("Comlink.thrown"), j = (t) => typeof t == "object" && t !== null || typeof t == "function", B = /* @__PURE__ */ new Map([["proxy", {
		canHandle: (t) => j(t) && t[V],
		serialize(t) {
	...
	function L(t, n, e = [], s = function() {}) {
		...
		const a = new Proxy(s, {
			...
			apply(c, r, l) {
				...
				if (i === ht) return T(t, n, { type: "ENDPOINT" }).then(g);
	...
	function W(t) {
		const n = t.map(D);
		return [n.map((e) => e[0]), Rt(n.map((e) => e[1]))];
	}
	...
	Math.PI / 180;
	180 / Math.PI;
	const w = 86400, yt = 365.25, d = 299792.458, z = d * yt * w, Y = 149597870.7, H = 648e3 / Math.PI, X = H * Y / z, Ct = [
		[
			-.0548755604,
			-.8734371052,
			-.4838350155
	...
	], Dt = 1e3, It = d, xt = d ** Math.sqrt(Math.E), Nt = d ** 2, Ot = d ** Math.E, Ut = [
		1,
		Dt,
		It,
		xt
	...
	var I = (t, n, e) => Object.defineProperty(t, n, {
		...
		writable: !0,
		configurable: !0
	}), Yt = Symbol.for("bitecs-relation"), Ht = Symbol.for("bitecs-pairTarget"), vt = Symbol.for("bitecs-isPairComponent"), Ft = Symbol.for("bitecs-relationData"), nt = () => {
		let t = {
			...
			if (!t.pairsMap.has(s)) {
				...
				I(o, Yt, n), I(o, Ht, s), I(o, vt, !0), t.pairsMap.set(s, o);
			}
			return t.pairsMap.get(s);
		};
		return I(n, Ft, t), n;
	}, Gt = Symbol.for("bitecs-wildcard");
	function Vt() {
		let t = nt();
		return Object.defineProperty(t, Gt, {
			value: !0,
			enumerable: !1,
			writable: !1,
			configurable: !1
	...
	function jt() {
		...
		return globalThis[t] || (globalThis[t] = Vt()), globalThis[t];
	}
	var Bt = jt();
	function Zt() {
		return nt();
	...
	qt();
	const rt = 1e3 / Y, x = rt * z * X, Wt = 695700, Qt = 6378.137;
	function ct(t) {
	...
	function nn(t, n, e) {
		const s = 1 / x, o = t * s / 1e3, a = n * s / 1e3, c = e * s / 1e3, r = it;
		return {
	...
	function sn(t) {
		const n = t.l * Math.PI / 180, e = t.b * Math.PI / 180, s = t.distPc / 1e3, o = t.radiusPc / 1e3, a = Math.max(1, t.elongation ?? 1), c = o * a, r = (t.paDeg ?? 0) * Math.PI / 180, l = Math.sin(n), i = Math.cos(n), _ = Math.sin(e), E = Math.cos(e), f = Math.cos(r), h = Math.sin(r), M = f * -l + h * (-_ * i), S = f * i + h * (-_ * l), m = h * E, N = 1 / (o * o), O = 1 / (c * c) - N, U = t.peakTauCore / (o * Math.sqrt(2 * Math.PI) * a);
		return {
			cx: s * E * i,
			cy: s * E * l,
			cz: s * _,
	...
	const an = on.map(sn), { SQRT1_2: rn } = Math, cn = Math.sqrt(2 * Math.PI);
	function ut(t) {
	...
	function ln(t, n, e) {
		...
		return P.midplaneTauPerKpc * Math.exp(-(s - p.sunRadiusKpc) / P.scaleRKpc) * Math.exp(-Math.abs(o) / P.scaleZKpc);
	}
	function un(t, n, e, s, o, a, c, r) {
		const l = t.cx - n, i = t.cy - e, _ = t.cz - s, E = l * o + i * a + _ * c, f = o * t.dx + a * t.dy + c * t.dz, h = l * t.dx + i * t.dy + _ * t.dz, M = l * l + i * i + _ * _, S = t.invM + t.anis * f * f, m = t.invM * E + t.anis * f * h, N = t.invM * M + t.anis * h * h, O = Math.max(0, N - m * m / S);
		if (O > 25) return 0;
		const U = m / S, Et = Math.sqrt(S) * rn, ft = .5 * (ut((r - U) * Et) - ut(-U * Et));
		if (ft <= 0) return 0;
		const pn = 1 / Math.sqrt(S);
		return t.peakDensity * cn * pn * Math.exp(-.5 * O) * ft;
	}
	function _n(t, n) {
		const e = n.x - t.x, s = n.y - t.y, o = n.z - t.z, a = Math.hypot(e, s, o);
		if (a === 0) return 0;
		let c = 0;
		for (let E = 0; E < P.steps; E += 1) {
			const f = (E + .5) / P.steps;
			c += ln(t.x + e * f, t.y + s * f, t.z + o * f);
		}
		let r = c * a / P.steps;
		const l = e / a, i = s / a, _ = o / a;
		for (const E of an) r += un(E, t.x, t.y, t.z, l, i, _, a);
		return r;
	}
	P.steps;
	function En(t) {
		const n = Math.max(0, t), e = Math.min(n, en);
		return {
			dimMag: 1.086 * n,
			rgbScale: [
				Math.exp(-e * (lt.r - 1)),
	...
	function hn(t) {
		const n = Xt(t.seed), e = t.viewerScenePos, s = e && (e.x !== 0 || e.y !== 0 || e.z !== 0) ? nn(e.x, e.y, e.z) : {
			x: 0,
			y: 0,
			z: 0
		}, o = [], a = [], c = [];
		for (let r = 0; r < t.pointBudget; r += 1) {
			const l = Jt(n), i = Math.hypot(l.x, l.y, l.z);
			if (i >= fn && i <= p.maxSunDistKpc) {
				const _ = i * _t, E = tn(l.x, l.y, l.z), f = _ * x, h = p.packetAbsMag + 5 * Math.log10(_ / 10), S = En(_n(s, l));
				o.push(E.x * f, E.y * f, E.z * f), a.push(h + S.dimMag), c.push(G[0] * S.rgbScale[0], G[1] * S.rgbScale[1], G[2] * S.rgbScale[2]);
			}
		}
		return {
