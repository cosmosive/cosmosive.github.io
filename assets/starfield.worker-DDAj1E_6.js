(function() {
	var Zt = Object.defineProperty, Hn = (t, n) => {
		let e = {};
	...
	const t2 = Symbol("Comlink.proxy"), $n = Symbol("Comlink.endpoint"), Jn = Symbol("Comlink.releaseProxy"), ot = Symbol("Comlink.finalizer"), E1 = Symbol("Comlink.thrown"), n2 = (t) => typeof t == "object" && t !== null || typeof t == "function", e2 = new Map([["proxy", {
	...
	function st(t, n = globalThis, e = ["*"]) {
		n.addEventListener("message", function r(o) {
			...
			const { id: s, type: i, path: a } = Object.assign({ path: [] }, o.data), c = (o.data.argumentList || []).map(D0);
			let u;
			try {
				...
				switch (i) {
					case "GET":
						...
						break;
					case "SET":
						h[a.slice(-1)[0]] = D0(o.data.value), u = !0;
						break;
					case "APPLY":
						u = l.apply(h, c);
						break;
			...
			Promise.resolve(u).catch((h) => ({
				...
				const [l, d] = x1(h);
				n.postMessage(Object.assign(Object.assign({}, l), { id: s }), d), i === "RELEASE" && (n.removeEventListener("message", r), r2(n), ot in t && typeof t[ot] == "function" && t[ot]());
			}).catch((h) => {
				const [l, d] = x1({
	...
	function it(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, a, c) {
				...
				if (u === $n) return C0(t, n, { type: "ENDPOINT" }).then(D0);
	...
	}, r3 = 149597870.7, o3 = 648e3 / Math.PI, at = 1500;
	at / 15;
	100 / o3;
	const ct = {
		x: 0,
	...
	const c2 = Object.fromEntries(Object.entries(e3).map(([t, n]) => [t, i3(n)]));
	z.SUN, z.MERCURY, z.VENUS, z.EARTH, z.MARS, z.JUPITER, z.SATURN, z.URANUS, z.NEPTUNE, z.PLUTO;
	const ut = c2[z.EARTH]?.radiusKm ?? 6371, lt = c2[z.SUN]?.radiusKm ?? 696340, a3 = ut / lt, O0 = .04;
	ut / O0;
	O0 / ut / (15 / r3);
	function m1(t) {
		return !Number.isFinite(t) || t <= 0 ? O0 : t / a3 * O0;
	}
	function c3(t) {
		return !Number.isFinite(t) || t <= 0 ? O0 : m1(t / lt);
	}
	c3(lt);
	const u3 = .008;
	function l3(t, n) {
		if (!Number.isFinite(n) || n <= 0 || t == null || !Number.isFinite(t)) return m1(1);
		const e = t - 5 * Math.log10(n / 10), r = Math.pow(10, (4.83 - e) / 2.5);
		return m1(Math.max(u3, Math.min(100, Math.sqrt(r))));
	}
	var v1 = Hn({
		AU_PER_LY: () => u2,
		AngleBetween: () => N1,
		...
		DEG2RAD: () => p,
		DefineStar: () => T3,
		DeltaT_EspenakMeeus: () => At,
		DeltaT_JplHorizons: () => L3,
		EUROPA_RADIUS_KM: () => S3,
		EclipseEvent: () => gn,
		EclipseKind: () => j,
		Ecliptic: () => u1,
		...
		HelioVector: () => a0,
		Horizon: () => V1,
		HorizonFromVector: () => me,
		HorizontalCoordinates: () => P2,
		HourAngle: () => _e,
		HourAngleEvent: () => K2,
		IO_RADIUS_KM: () => M3,
		IdentityMatrix: () => Re,
		...
		InverseRotation: () => r1,
		JUPITER_EQUATORIAL_RADIUS_KM: () => h3,
		JUPITER_MEAN_RADIUS_KM: () => d3,
		JUPITER_POLAR_RADIUS_KM: () => f3,
		JupiterMoons: () => ne,
		JupiterMoonsInfo: () => k2,
		KM_PER_AU: () => U,
		LagrangePoint: () => t4,
		LagrangePointFast: () => Un,
		Libration: () => k3,
		LibrationInfo: () => x2,
		...
		SearchMaxElongation: () => Ae,
		SearchMoonNode: () => Ln,
		SearchMoonPhase: () => S1,
		SearchMoonQuarter: () => G2,
		SearchPeakMagnitude: () => we,
		...
		TransitInfo: () => Dn,
		Vector: () => P,
		VectorFromHorizon: () => ve,
		VectorFromSphere: () => K1,
		VectorObserver: () => q3,
		e_tilt: () => g0
	});
	const z1 = 173.1446326846693, U = 149597870.69098932, u2 = 63241.07708807546, p = .017453292519943295, ht = .26179938779914946, k = 57.29577951308232, ft = 3.819718634205488, h3 = 71492, f3 = 66854, d3 = 69911, M3 = 1821.6, S3 = 1560.8, _3 = 2631.2, p3 = 2410.3, l2 = 365.24217, h2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, E0 = 3600 * (180 / Math.PI), b0 = 484813681109536e-20, f2 = 10800 * 60, A3 = 2 * f2, d2 = 7292115e-11, w3 = f2 / Math.PI, E3 = -.17 - 5 * Math.log10(w3), D1 = 29.530588, I1 = 24 * 3600, y3 = I1 * 1e3, M2 = .9972695717592592, s1 = 695700, S2 = s1 / U, o0 = .996647180302104, k0 = o0 * o0, d0 = 6378.1366, g3 = d0 / U, R3 = d0 * o0, _2 = 6371, x3 = _2 + 88, m3 = 1738.1 / U, W = 1737.4, p2 = 1736, v3 = p2 / U, z3 = 34 / 60, dt = 81.30056, i1 = .0002959122082855911, Mt = 4912547451450812e-26, St = 7243452486162703e-25, a1 = 8887692390113509e-25, _t = 9549535105779258e-26, F0 = 2.825345909524226e-7, V0 = 8.459715185680659e-8, H0 = 1.292024916781969e-8, $0 = 1.524358900784276e-8, D3 = 218869976542597e-26, P1 = a1 / dt;
	function pt(t) {
		switch (t) {
	...
	var y0 = class rt {
		constructor(n) {
			...
			if (n instanceof Date && Number.isFinite(n.getTime())) {
				this.date = n, this.ut = (n.getTime() - h2.getTime()) / e, this.tt = E2(this.ut);
				return;
			}
			if (Number.isFinite(n)) {
				this.date = new Date(h2.getTime() + n * e), this.ut = n, this.tt = E2(this.ut);
				return;
			}
	...
	function g0(t) {
		if (!U1 || Math.abs(U1.tt - t.tt) > 1e-6) {
			const n = O3(t), e = y2(t), r = e + n.deps / 3600;
			U1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function R0(t) {
		...
		function r(T, C, F, H) {
			const Q = [];
			for (let z0 = 0; z0 <= C - T; ++z0) Q.push(e(F, H));
			return {
				min: T,
				array: Q
			};
		...
		function s(T, C, F, H) {
			const Q = T.array[C - T.min];
			Q.array[F - Q.min] = H;
		}
		let i, a, c, u, h, l, d, A, M, _, w, E, x, g, m, R, D, I, b, N, q, G, Y, c0 = r(-6, 6, 1, 4), r0 = r(-6, 6, 1, 4);
		function p0(T, C) {
			return o(c0, T, C);
		}
		function v0(T, C) {
		...
		function nt(T, C, F, H, Q) {
			Q(T * F - C * H, C * F + T * H);
		}
		function L(T) {
			return Math.sin(t0 * T);
		}
		d = n * n, M = 0, Y = 0, w = 0, E = 3422.7;
		var h0 = L(.19833 + .05611 * n), Bt = L(.27869 + .04508 * n), Kt = L(.16827 - .36903 * n), Yt = L(.34734 - 5.37261 * n), Wt = L(.10498 - 5.37899 * n), et = L(.42681 - .41855 * n), M4 = L(.14943 - 5.37511 * n);
		for (I = .84 * h0 + .31 * Bt + 14.27 * Kt + 7.26 * Yt + .28 * Wt + .24 * et, b = 2.94 * h0 + .31 * Bt + 14.27 * Kt + 9.34 * Yt + 1.12 * Wt + .83 * et, N = -6.4 * h0 - 1.89 * et, q = .21 * h0 + .31 * Bt + 14.27 * Kt - 88.7 * Yt - 15.3 * Wt + .24 * et - 1.86 * M4, G = I - N, A = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), x = t0 * J0(.60643382 + 1336.85522467 * n - 313e-8 * d) + I / E0, g = t0 * J0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + b / E0, m = t0 * J0(.99312619 + 99.99735956 * n - 44e-8 * d) + N / E0, R = t0 * J0(.25909118 + 1342.2278298 * n - 892e-8 * d) + q / E0, D = t0 * J0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / E0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
					c = g, a = 4, u = 1.000002208;
					break;
				case 2:
					c = m, a = 3, u = .997504612 - .002495388 * n;
					break;
				case 3:
					c = R, a = 4, u = 1.000002708 + 139.978 * A;
					break;
				case 4:
					c = D, a = 6, u = 1;
					break;
			...
			for (u0(0, h, 1), u0(1, h, Math.cos(c) * u), l0(0, h, 0), l0(1, h, Math.sin(c) * u), l = 2; l <= a; ++l) nt(p0(l - 1, h), v0(l - 1, h), p0(1, h), v0(1, h), (T, C) => (u0(l, h, T), l0(l, h, C)));
		...
		function Fn(T, C, F, H) {
			for (var Q = {
				...
				F,
				H
			], A0 = 1; A0 <= 4; ++A0) z0[A0] !== 0 && nt(Q.x, Q.y, p0(z0[A0], A0), v0(z0[A0], A0), (Xt, o1) => (Q.x = Xt, Q.y = o1));
			return Q;
		}
		function S(T, C, F, H, Q, z0, A0, Xt) {
			var o1 = Fn(Q, z0, A0, Xt);
			M += T * o1.y, Y += C * o1.y, w += F * o1.x, E += H * o1.x;
		}
		...
		function f0(T, C, F, H, Q) {
			return T * Fn(C, F, H, Q).y;
		}
		_ = 0, _ += f0(-526.069, 0, 0, 1, -2), _ += f0(-3.352, 0, 0, 1, -4), _ += f0(44.297, 1, 0, 1, -2), _ += f0(-6, 1, 0, 1, -4), _ += f0(20.599, -1, 0, 1, 0), _ += f0(-30.598, -1, 0, 1, -2), _ += f0(-24.649, -2, 0, 1, 0), _ += f0(-2, -2, 0, 1, -2), _ += f0(-22.571, 0, 1, 1, -2), _ += f0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = R + Y / E0;
		let S4 = (1.000002708 + 139.978 * A) * (18519.699999999997 + w) * Math.sin(i) - 6.24 * Math.sin(3 * i) + _;
		return {
			geo_eclip_lon: t0 * J0((x + M / E0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * S4,
			distance_au: E0 * g3 / (.999953253 * E)
		};
	}
	var x2 = class {
	...
	function C1(t, n) {
		...
		r *= b0, o *= b0, s *= b0, i *= b0;
		const a = Math.sin(r), c = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), A = Math.sin(i), M = Math.cos(i), _ = M * h - u * A * d, w = M * u * c + A * d * h * c - a * A * l, E = M * u * a + A * d * h * a + c * A * l, x = -A * h - u * M * d, g = -A * u * c + M * d * h * c - a * M * l, m = -A * u * a + M * d * h * a + c * M * l, R = u * l, D = -l * h * c - a * d, I = -l * h * a + d * c;
	...
	function V3(t, n) {
		...
		let i, a, c;
		if (s < 1e-6) i = 0, a = o > 0 ? 90 : -90, c = Math.abs(o) - R3;
		else {
			...
			for (;;) {
				...
				const _ = (k0 - 1) * d0, w = h * h, E = l * l, x = w + k0 * E;
				d = Math.sqrt(x);
				const g = _ * l * h / d - o * h + s * l;
				if (Math.abs(g) < 1e-8) break;
				const m = _ * ((w - E) / d - E * w * (k0 - 1) / (_ * x)) + o * l + s * h;
				u -= g / m;
			}
			a = k * u;
			const M = d0 / d;
		...
		return new yt(a, i, 1e3 * c);
	}
	function wt(t, n) {
		const e = t.latitude * p, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, o0 * r), i = k0 * s, a = t.height / 1e3, c = d0 * s + a, u = d0 * i + a, h = (15 * n + t.longitude) * p, l = Math.sin(h), d = Math.cos(h);
		return {
			pos: [
				c * o * d / U,
				c * o * l / U,
	...
	function b1(t, n) {
		const e = g0(t), r = e.mobl * p, o = e.tobl * p, s = e.dpsi * b0, i = Math.cos(r), a = Math.sin(r), c = Math.cos(o), u = Math.sin(o), h = Math.cos(s), l = Math.sin(s), d = h, A = -l * i, M = -l * a, _ = l * c, w = h * i * c + a * u, E = h * a * c - i * u, x = l * u, g = h * i * u - a * c, m = h * a * u + i * c;
		if (n === O.From2000) return new $([
			[
				d,
				_,
	...
	function H3(t, n, e) {
		return e === O.Into2000 ? v2(D2(t, n, e), n, e) : D2(v2(t, n, e), n, e);
	}
	function I2(t, n) {
		const e = wt(n, M0(t)).pos;
		return k1(e, t, O.Into2000);
	}
	var P = class {
		constructor(t, n, e, r) {
	...
	function J3(t) {
		if (!$3(t)) throw "Argument must be a [3][3] array of numbers";
		return new $(t);
	}
	var P2 = class {
		constructor(t, n, e, r) {
	...
	function V1(t, n, e, r, o) {
		let s = y(t);
		K0(n), v(e), v(r);
		const i = Math.sin(n.latitude * p), a = Math.cos(n.latitude * p), c = Math.sin(n.longitude * p), u = Math.cos(n.longitude * p), h = Math.sin(r * p), l = Math.cos(r * p), d = Math.sin(e * ht), A = Math.cos(e * ht);
		let M = [
			a * u,
			a * c,
			i
		...
		const E = -15 * M0(s);
		let x = B0(E, M), g = B0(E, _), m = B0(E, w), R = [
			l * A,
			l * d,
			h
		];
		const D = R[0] * x[0] + R[1] * x[1] + R[2] * x[2], I = R[0] * g[0] + R[1] * g[1] + R[2] * g[2], b = R[0] * m[0] + R[1] * m[1] + R[2] * m[2];
		let N = Math.hypot(I, b), q;
		N > 0 ? (q = -k * Math.atan2(b, I), q < 0 && (q += 360)) : q = 0;
		let G = k * Math.atan2(N, D), Y = e, c0 = r;
		if (o) {
			let r0 = G, p0 = p1(o, 90 - G);
			if (G -= p0, p0 > 0 && G > 3e-4) {
				const v0 = Math.sin(G * p), u0 = Math.cos(G * p), l0 = Math.sin(r0 * p), nt = Math.cos(r0 * p), L = [];
				for (let h0 = 0; h0 < 3; ++h0) L.push((R[h0] - nt * x[h0]) / l0 * v0 + x[h0] * u0);
				N = Math.hypot(L[0], L[1]), N > 0 ? (Y = ft * Math.atan2(L[1], L[0]), Y < 0 && (Y += 24)) : Y = 0, c0 = k * Math.atan2(L[2], N);
			}
		}
		return new P2(q, 90 - G, Y, c0);
	}
	function K0(t) {
	...
	function L2(t) {
		const n = y(t).AddDays(-1 / z1), e = W0(B.Earth, n), [r, o, s] = k1([
			-e.x,
			-e.y,
			-e.z
		], n, O.From2000), i = p * g0(n).tobl, a = Math.cos(i), c = Math.sin(i);
		return gt(new P(r, o, s, n), a, c);
	}
	function c1(t, n, e, r, o) {
		K0(e), T1(r), T1(o);
		const s = y(n), i = I2(s, e), a = n0(t, s, o), c = [
			a.x - i[0],
			a.y - i[1],
			a.z - i[2]
		];
		return N2(r ? k1(c, s, O.From2000) : c, s);
	}
	function G3(t, n, e) {
		const r = y(t);
		let o = wt(n, M0(r)).pos;
		return e || (o = k1(o, r, O.Into2000)), Et(o, r);
	}
	function Q3(t, n, e) {
		const r = y(t), o = wt(n, M0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
		return e ? s : H3(s, r, O.Into2000);
	}
	function q3(t, n) {
		const e = M0(t.t);
		let r = [
		...
		return n || (r = G0(r, t.t, O.From2000), r = Q0(r, t.t, O.From2000)), V3(r, e);
	}
	function B3(t, n) {
	...
	function gt(t, n, e) {
		...
		let a = 0;
		i > 0 && (a = k * Math.atan2(o, r), a < 0 && (a += 360));
		let c = k * Math.atan2(s, i);
		return new T2(new P(r, o, s, t.t), c, a);
	}
	function u1(t) {
		const n = g0(t.t), [e, r, o] = Q0(G0([
			t.x,
			t.y,
			t.z
	...
	function i0(t) {
		const n = y(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = G0(b3(n, [
	...
	function H1(t) {
		const n = y(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = Et(Q0(g2(s.mobl, o), n, O.From2000), n), a = s.tobl * p, c = gt(i, Math.cos(a), Math.sin(a));
	...
	function W0(t, n) {
		const e = n.tt / Y0;
		return zt(U2(I0(t[mt], e, !0), I0(t[vt], e, !1), I0(t[$1], e, !1))).ToAstroVector(n);
	}
	function h1(t, n) {
		const e = n / Y0, r = I0(t[mt], e, !0), o = I0(t[vt], e, !1), s = I0(t[$1], e, !1), i = xt(t[mt], e), a = xt(t[vt], e), c = xt(t[$1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), A = +(c * l * u) - s * d * u * a - s * l * h * i, M = +(c * l * h) - s * d * h * a + s * l * u * i, _ = +(c * d) + s * l * a, w = U2(r, o, s), E = [
			A / Y0,
			M / Y0,
			_ / Y0
		];
		return new T0(n, zt(w), zt(E));
	}
	function J1(t, n, e, r) {
		const o = r / (r + i1), s = W0(B[e], n);
	...
	const Dt = 51, Y3 = 29200, X0 = 146, S0 = 201, P0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	var d1 = class {
		...
		clone() {
			return new Vn(this.tt, this.r.clone(), this.v.clone(), this.a.clone());
		}
	}, C2 = class {
		constructor(t, n) {
	...
	function Tt(t, n) {
		...
		return new C2(r, new It(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const X3 = [];
	function O2(t, n) {
	...
	function Nt(t) {
		const n = W3(t), e = new d1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new C2(e, new It(n.tt, r, o, s));
	}
	function j3(t, n) {
		const e = P0[0][0];
		if (n < e || n > P0[Dt - 1][0]) return null;
		const r = O2((n - e) / Y3, Dt - 1);
		if (!t[r]) {
			const s = t[r] = [];
			s[0] = Nt(P0[r]).grav, s[S0 - 1] = Nt(P0[r + 1]).grav;
			let i, a = s[0].tt;
			for (i = 1; i < S0 - 1; ++i) s[i] = Tt(a += X0, s[i - 1]).grav;
			a = s[S0 - 1].tt;
			var o = [];
			for (o[S0 - 1] = s[S0 - 1], i = S0 - 2; i > 0; --i) o[i] = Tt(a -= X0, o[i + 1]).grav;
			for (i = S0 - 2; i > 0; --i) {
				const c = i / (S0 - 1);
	...
	function Lt(t, n) {
		...
		const s = j3(X3, t.tt);
		if (s) {
			const i = O2((t.tt - s[0].tt) / X0, S0 - 1), a = s[i], c = s[i + 1], u = a.a.mean(c.a), h = j0(t.tt - a.tt, a.r, a.v, u), l = Pt(t.tt - a.tt, a.v, u), d = j0(t.tt - c.tt, c.r, c.v, u), A = Pt(t.tt - c.tt, c.v, u), M = (t.tt - a.tt) / X0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(A.mul(M));
		} else {
			let i;
			t.tt < P0[0][0] ? i = b2(P0[0], t.tt, -X0) : i = b2(P0[Dt - 1], t.tt, +X0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
		return n && (o || (o = new d1(t.tt)), e = e.sub(o.Sun.r), r = r.sub(o.Sun.v)), new V(e.x, e.y, e.z, r.x, r.y, r.z, t);
	}
	const Z3 = new $([
		[
			.999432765338654,
	...
	function te(t, n, e) {
		...
		const M = i * h - s * l, _ = -s * h - i * l, w = 1 / (1 + _), E = 1 / (1 + Math.sqrt(1 - s * s - i * i)), x = r * (h - s - E * i * M), g = r * (l - i + E * s * M), m = u * w * r * (-l - E * i * _), R = u * w * r * (+h + E * s * _), D = 2 * Math.sqrt(1 - a * a - c * c), I = 1 - 2 * c * c, b = 1 - 2 * a * a, N = 2 * c * a;
		return new V(x * I + g * N, x * N + g * b, (a * g - x * c) * D, m * I + R * N, m * N + R * b, (a * R - m * c) * D, t);
	}
	function Q1(t, n) {
		...
		return Y1(Z3, te(t, n.mu, r));
	}
	function ne(t) {
		const n = new y0(t);
		return new k2(Q1(n, G1[0]), Q1(n, G1[1]), Q1(n, G1[2]), Q1(n, G1[3]));
	}
	function a0(t, n) {
		var e = y(n);
		if (t in B) return W0(B[t], e);
		if (t === f.Pluto) {
			const i = Lt(e, !0);
	...
	function N0(t, n) {
		...
		if (e) return e.dist;
		const r = y(n);
		return t in B ? I0(B[t][$1], r.tt / Y0, !1) : a0(t, r).Length();
	}
	function F2(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
	...
	function re(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
			case f.EMB:
				const o = h1(B[f.Earth], e.tt), s = t === f.Moon ? l1(e) : Rt(e);
	...
	function q1(t, n) {
		...
		switch (t) {
			...
			case f.Saturn:
			case f.Uranus:
			case f.Neptune: return L0(h1(B[t], e.tt), e);
			case f.Pluto: return Lt(e, !0);
			case f.Moon:
			case f.EMB:
				const o = h1(B.Earth, e.tt), s = t == f.Moon ? l1(e) : Rt(e);
	...
	function J(t, n, e, r) {
		const o = v(r && r.dt_tolerance_seconds || 1), s = Math.abs(o / I1);
		let i = r && r.init_f1 || t(n), a = r && r.init_f2 || t(e), c = NaN, u = 0, h = r && r.iter_limit || 20, l = !0;
		for (;;) {
			if (++u > h) throw "Excessive iteration in Search()";
			let d = C3(n, e, .5), A = d.ut - n.ut;
			if (Math.abs(A) < s) return d;
			l ? c = t(d) : l = !0;
			let M = oe(d.ut, e.ut - d.ut, i, c, a);
			if (M) {
				let _ = y(M.t), w = t(_);
				if (M.df_dt !== 0) {
					if (Math.abs(w / M.df_dt) < s) return _;
					let E = 1.2 * Math.abs(w / M.df_dt);
					if (E < A / 10) {
						let x = _.AddDays(-E), g = _.AddDays(+E);
						if ((x.ut - n.ut) * (x.ut - e.ut) < 0 && (g.ut - n.ut) * (g.ut - e.ut) < 0) {
							let m = t(x), R = t(g);
							if (m < 0 && R >= 0) {
								i = m, a = R, n = x, e = g, c = w, l = !1;
								continue;
							}
						}
					}
	...
	function Ut(t, n, e) {
		...
		const r = y(e), o = u1(n0(t, r, !1)), s = u1(n0(n, r, !1));
	...
	function ie(t, n, e, r, o) {
		const s = u1(r), i = p * 28.06, a = p * (169.51 + 382e-7 * o.tt), c = p * s.elat, u = p * s.elon, h = Math.asin(Math.sin(c) * Math.cos(i) - Math.cos(c) * Math.sin(i) * Math.sin(u - a)), l = Math.sin(Math.abs(h));
		let d = -9 + .044 * t;
	...
	function B1(t, n) {
		...
		if (t === f.Sun) a = E3 + 5 * Math.log10(c);
		else if (t === f.Moon) a = ae(o, u, c);
		else if (t === f.Saturn) {
			const l = ie(o, u, c, i, e);
	...
	function e1(t, n, e) {
		...
		let i = M1(t), a = y(e), c = s(a);
		c > 0 && (c -= 360);
		for (let u = 0; u < 100; ++u) {
			let h = -c / 360 * i;
			if (a = a.AddDays(h), Math.abs(h) * I1 < 1) return a;
			let l = c;
	...
	function ce(t) {
		return G2(new Date(t.time.date.getTime() + 6 * y3));
	}
	var Q2 = class {
		constructor(t, n, e) {
	...
	function ue(t, n) {
		const e = t.latitude * p, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, r * o0), i = s * (o0 * o0), a = (t.height - n) / 1e3, c = d0 * s + a, u = d0 * i + a, h = 1e3 * Math.hypot(c * o, u * r), l = .175 * Math.pow(1 - .0065 / 283.15 * (t.height - 2 / 3 * n), 3.256);
		return k * -(Math.sqrt(2 * (1 - l) * n / h) / (1 - l));
	}
	function le(t) {
		switch (t) {
			case f.Sun: return S2;
			case f.Moon: return m3;
			default: return 0;
		}
	}
	function he(t, n, e, r, o, s = 0) {
		if (!Number.isFinite(s) || s < 0) throw `Invalid value for metersAboveGround: ${s}`;
		const i = le(t), a = q2(n.height - s);
		return B2(t, n, e, r, o, i, ue(n, s) - z3 * a.density);
	}
	function fe(t, n, e, r, o, s) {
		if (!Number.isFinite(s) || s < -90 || s > 90) throw `Invalid altitude angle: ${s}`;
		return B2(t, n, e, r, o, 0, s);
	}
	var de = class {
		constructor(t, n, e, r) {
	...
	function Ot(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new de(r, o, s, i);
		if (s >= 0 && i < 0) return null;
		if (t > 17) throw "Excessive recursion in rise/set ascent search.";
		const a = o.ut - r.ut;
		if (a * I1 < 1 || Math.min(Math.abs(s), Math.abs(i)) > e * (a / 2)) return null;
	...
	function B2(t, n, e, r, o, s, i) {
		if (K0(n), v(o), v(s), v(i), i < -90 || i > 90) throw `Invalid target altitude angle: ${i}`;
		const a = .42, c = Me(t, n.latitude);
		function u(_) {
			const w = c1(t, _, n, !0, !0);
			return e * (V1(_, n, w.ra, w.dec).altitude + k * Math.asin(s / w.dist) - i);
		}
		const h = y(r);
		let l = h, d = h, A = u(l), M = A;
		for (;;) {
			o < 0 ? (l = d.AddDays(-a), A = u(l)) : (d = l.AddDays(+a), M = u(d));
			const _ = Ot(0, u, c, l, d, A, M);
			if (_) {
				const w = J(u, _.tx, _.ty, {
	...
	function _e(t, n, e) {
		const r = y(n), o = z2(r), s = c1(t, r, e, !0, !0);
	...
	function Ae(t, n) {
		...
		for (; ++i <= 2;) {
			...
			let l = o.AddDays(h), d = e1(t, c, l), A = e1(t, u, d), M = r(d);
			...
			if (!w) throw `SearchMaxElongation: failed search iter ${i} (t1=${d.toString()}, t2=${A.toString()})`;
	...
	function we(t, n) {
		...
		for (; ++a <= 2;) {
			...
			if (!E) throw `SearchPeakMagnitude: failed search iter ${a} (t1=${A.toString()}, t2=${M.toString()})`;
			if (E.tt >= o.tt) return B1(t, E);
			o = M.AddDays(1);
		}
	...
	function ye(t, n) {
		...
		for (let d = 0; d < 100; ++d) {
			const A = r.AddDays(d * u), M = N0(t, A);
	...
	function tn(t, n) {
		if (n = y(n), t === f.Neptune || t === f.Pluto) return ye(t, n);
		function e(c) {
	...
	function ve(t, n, e) {
		n = y(n);
		const r = nn(t.lon);
		return K1(new q0(t.lat + en(e, t.lat), r, t.dist), n);
	}
	function p1(t, n) {
		let e;
	...
	function W1(t) {
		return t = y(t), m0(C1(t, O.From2000), b1(t, O.From2000));
	}
	function De(t) {
		const n = y(t);
		return m0(W1(n), hn(n));
	...
	function X1(t) {
		return t = y(t), m0(b1(t, O.Into2000), C1(t, O.Into2000));
	}
	function Ft(t, n) {
		t = y(t);
	...
	function ln(t) {
		const n = g0(y(t)).tobl * p, e = Math.cos(n), r = Math.sin(n);
		return new $([
			[
				1,
				0,
	...
	function Oe(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Vt || (Vt = W1(new y0(-45655.74141261017)), fn = new y0(0));
		const e = K1(new q0(n, 15 * t, 1), fn), r = bt(A1(Vt, e)), o = 10 / 240, s = o / 15;
		for (let i of Ce) {
	...
	function w1(t, n, e, r) {
		...
		return new be(n, o, U * Math.hypot(s, i, a), +s1 - (1 + o) * (s1 - t), -s1 + (1 + o) * (s1 + t), e, r);
	}
	function j1(t) {
		const n = n0(f.Sun, t, !0), e = new P(-n.x, -n.y, -n.z, n.t);
		return w1(x3, t, i0(t), e);
	}
	function Sn(t) {
	...
	function Fe(t) {
		const e = J((r) => $t(j1, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Earth shadow time.";
		return j1(e);
	}
	function Ve(t) {
		const e = J((r) => $t(Sn, r), t.AddDays(-.03), t.AddDays(.03));
	...
	function pn(t, n) {
		const e = new P(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = _n(Math.asin(S2 / e.Length()), Math.asin(v3 / n.Length()), N1(n, e) * p);
	...
	function An(t) {
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				const i = Fe(o);
				if (i.r < i.p + W) {
					let a = j.Penumbral, c = 0, u = 0, h = 0, l = Jt(i.time, i.p + W, 200);
					return i.r < i.k + W && (a = j.Partial, h = Jt(i.time, i.k + W, l), i.r + W < i.k ? (a = j.Total, c = 1, u = Jt(i.time, i.k - W, h)) : c = _n(W, i.k, i.r)), new Mn(a, c, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function Je(t) {
		...
		a.x *= U, a.y *= U, a.z *= U / o0, c.x *= U, c.y *= U, c.z *= U / o0;
		const u = d0, h = a.x * a.x + a.y * a.y + a.z * a.z, l = -2 * (a.x * c.x + a.y * c.y + a.z * c.z), d = c.x * c.x + c.y * c.y + c.z * c.z - u * u, A = l * l - 4 * h * d;
		let M;
		if (A > 0) {
			const _ = (-l - Math.sqrt(A)) / (2 * h), w = _ * a.x - c.x, E = _ * a.y - c.y, x = (_ * a.z - c.z) * o0, g = Math.hypot(w, E) * k0;
			g == 0 ? o = x > 0 ? 90 : -90 : o = k * Math.atan(x / g);
			const m = M0(e);
			s = (k * Math.atan2(E, w) - 15 * m) % 360, s <= -180 ? s += 360 : s > 180 && (s -= 360);
			const R = r1(i);
			let D = new P(w / U, E / U, x / U, t.time);
			D = A1(R, D), D.x += t.target.x, D.y += t.target.y, D.z += t.target.z;
			const I = w1(p2, t.time, D, t.dir);
			if (I.r > 1e-9 || I.r < 0) throw `Unexpected shadow distance from geoid intersection = ${I.r}`;
			n = En(I.k), M = n === j.Total ? 1 : pn(t.dir, D);
		} else M = void 0;
		return new wn(n, M, e, r, o, s);
	}
	function Ge(t) {
		return t = y(t), An(t.AddDays(10));
	}
	function yn(t) {
		t = y(t);
		const n = 1.8;
	...
	function qe(t, n) {
		...
		const a = tt(n, 1, xn, s, t.time), c = tt(n, -1, xn, t.time, i);
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = tt(n, 1, mn, s, t.time), h = tt(n, -1, mn, t.time, i), l = En(t.k)) : l = j.Partial;
		const d = l === j.Total ? 1 : pn(t.dir, t.target);
	...
	function Ke(t, n) {
		return t = y(t), zn(t.AddDays(10), n);
	}
	var Dn = class {
		constructor(t, n, e, r) {
	...
	function Pn(t, n) {
		...
		for (;;) {
			...
			if (n1(t, i) < e) {
				const a = He(t, o, i);
				if (a.r < a.p) {
					const c = a.time.AddDays(-r), u = In(t, o, c, a.time, -1), h = a.time.AddDays(+r), l = In(t, o, a.time, h, 1), d = 60 * n1(t, a.time);
					return new Dn(u, a.time, l, d);
				}
			}
			s = i.AddDays(10);
		}
	...
	function We(t, n) {
		return n = y(n), Pn(t, n.AddDays(100));
	}
	var _0;
	(function(t) {
	...
	function Ze(t, n) {
		...
		switch (t) {
			...
			case f.Venus:
				...
				break;
			case f.Earth: return je(e);
			case f.Moon:
				const d = p * (125.045 - .0529921 * r), A = p * (250.089 - .1059842 * r), M = p * (260.008 + 13.0120009 * r), _ = p * (176.625 + 13.3407154 * r), w = p * (357.529 + .9856003 * r), E = p * (311.589 + 26.4057084 * r), x = p * (134.963 + 13.064993 * r), g = p * (276.617 + .3287146 * r), m = p * (34.226 + 1.7484877 * r), R = p * (15.134 - .1589763 * r), D = p * (119.743 + .0036096 * r), I = p * (239.961 + .1643573 * r), b = p * (25.053 + 12.9590088 * r);
				s = 269.9949 + .0031 * o - 3.8787 * Math.sin(d) - .1204 * Math.sin(A) + .07 * Math.sin(M) - .0172 * Math.sin(_) + .0072 * Math.sin(E) - .0052 * Math.sin(R) + .0043 * Math.sin(b), i = 66.5392 + .013 * o + 1.5419 * Math.cos(d) + .0239 * Math.cos(A) - .0278 * Math.cos(M) + .0068 * Math.cos(_) - .0029 * Math.cos(E) + 9e-4 * Math.cos(x) + 8e-4 * Math.cos(R) - 9e-4 * Math.cos(b), a = 38.3213 + (13.17635815 - 14e-13 * r) * r + 3.561 * Math.sin(d) + .1208 * Math.sin(A) - .0642 * Math.sin(M) + .0158 * Math.sin(_) + .0252 * Math.sin(w) - .0066 * Math.sin(E) - .0047 * Math.sin(x) - .0046 * Math.sin(g) + .0028 * Math.sin(m) + .0052 * Math.sin(R) + .004 * Math.sin(D) + .0019 * Math.sin(I) - .0044 * Math.sin(b);
				break;
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const N = p * (99.360714 + 4850.4046 * o), q = p * (175.895369 + 1191.9605 * o), G = p * (300.323162 + 262.5475 * o), Y = p * (114.012305 + 6070.2476 * o), c0 = p * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(N) + 938e-6 * Math.sin(q) + .001432 * Math.sin(G) + 3e-5 * Math.sin(Y) + .00215 * Math.sin(c0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(N) + 404e-6 * Math.cos(q) + 617e-6 * Math.cos(G) - 13e-6 * Math.cos(Y) + 926e-6 * Math.cos(c0), a = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
	...
	function t4(t, n, e, r) {
		const o = y(n), s = pt(e), i = pt(r);
		let a, c;
		return e === f.Earth && r === f.Moon ? (a = new V(0, 0, 0, 0, 0, 0, o), c = l1(o)) : (a = q1(e, o), c = q1(r, o)), Un(t, a, s, c, i);
	}
	function Un(t, n, e, r, o) {
		...
		const h = a * a + c * c + u * u, l = Math.sqrt(h), d = r.vx - n.vx, A = r.vy - n.vy, M = r.vz - n.vz;
		let _;
		if (t === 4 || t === 5) {
			const w = c * M - u * A, E = u * d - a * M, x = a * A - c * d;
			let g = E * u - x * c, m = x * a - w * u, R = w * c - E * a;
			const D = Math.sqrt(g * g + m * m + R * R);
			g /= D, m /= D, R /= D, a /= l, c /= l, u /= l;
			const I = t == 4 ? +i : -i, b = .5 * a + I * g, N = .5 * c + I * m, q = .5 * u + I * R, G = .5 * g - I * a, Y = .5 * m - I * c, c0 = .5 * R - I * u, r0 = l * b, p0 = l * N, v0 = l * q, u0 = d * a + A * c + M * u, l0 = d * g + A * m + M * R;
			_ = new V(r0, p0, v0, u0 * b + l0 * G, u0 * N + l0 * Y, u0 * q + l0 * c0, n.t);
		} else {
			const w = -l * (o / (e + o)), E = +l * (e / (e + o)), x = (e + o) / (h * l);
			let g, m, R;
			...
			do {
				const b = D - w, N = D - E;
				I = (x * D + m / (b * b) + R / (N * N)) / (x - 2 * m / (b * b * b) - 2 * R / (N * N * N)), D -= I;
			} while (Math.abs(I / l) > 1e-14);
			g = (D - w) / l, _ = new V(g * a, g * c, g * u, g * d, g * A, g * M, n.t);
		}
		return _;
	}
	var n4 = class Z {
	...
	const { MakeTime: z4, SearchRiseSet: D4, Body: I4, Observer: e4 } = v1;
	new e4(40.7128, -74.006, 10);
	function On(t, n) {
	...
	const { Body: e0 } = v1;
	z.SUN, e0.Sun, z.MERCURY, e0.Mercury, z.VENUS, e0.Venus, z.EARTH, e0.Earth, z.MARS, e0.Mars, z.JUPITER, e0.Jupiter, z.SATURN, e0.Saturn, z.URANUS, e0.Uranus, z.NEPTUNE, e0.Neptune, z.PLUTO, e0.Pluto, z.MOON, e0.Moon;
	2 * Math.PI;
	const { Rotation_EQD_EQJ: L4, CombineRotation: U4, MakeRotation: C4 } = v1;
	Math.PI / (180 * 3600);
	function o4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: k4 } = v1;
	function s4(t, n) {
	...
	function bn(t) {
		...
		return n != null && e != null && r != null && Number.isFinite(n) && Number.isFinite(e) && Number.isFinite(r) ? jn({
			x: n,
			y: e,
			z: r
		}) : r4(o) ?? 25;
	...
	function a4(t) {
		...
		const n = at + bn(t) * 20, { x: e, y: r, z: o, ra: s, dec: i } = t, a = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let c = null;
		return a && (c = n3({
			x: e,
			y: r,
	...
	function f4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = i4(s4(n, { raInHours: s }).filter((_) => !qt(_)), o, { maxDistanceLevel: e }), a = (_, w, E, x) => ({
			positions: _,
			sizes: w,
			colors: E,
			brightness: x,
		...
		if (i.length === 0) {
			if (e === 0) return a(new Float32Array(0), new Float32Array(0), new Float32Array(0), new Float32Array(0));
			const _ = h4(), w = _.length, E = new Float32Array(w * 3), x = new Float32Array(w).fill(kn), g = new Float32Array(w * 3).fill(1), m = new Float32Array(w).fill(1);
			for (let R = 0; R < w; R++) E[R * 3] = _[R].x, E[R * 3 + 1] = _[R].y, E[R * 3 + 2] = _[R].z;
			return a(E, x, g, m);
		}
		const c = i.map(a4), u = c.length, h = new Float32Array(u * 3), l = new Float32Array(u), d = new Float32Array(u * 3), A = new Float32Array(u), M = [];
		for (let _ = 0; _ < u; _++) {
			const w = c[_], E = i[_];
			h[_ * 3] = w.x, h[_ * 3 + 1] = w.y, h[_ * 3 + 2] = w.z;
			const x = E ? l3(E.mag, bn(E)) : O0;
			l[_] = r ? x : kn;
			const g = E?.c != null ? `#${E.c}` : void 0, [m, R, D] = r ? c4(g) : [
				1,
				1,
				1
			];
			d[_ * 3] = m, d[_ * 3 + 1] = R, d[_ * 3 + 2] = D, A[_] = r && E?.br != null && Number.isFinite(E.br) ? Math.max(0, Math.min(1, E.br)) : 1, E?.proper && M.push({
				position: { ...w },
				name: E.proper,
