(function() {
	...
	const r2 = Symbol("Comlink.proxy"), Bn = Symbol("Comlink.endpoint"), Yn = Symbol("Comlink.releaseProxy"), ct = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), o2 = (t) => typeof t == "object" && t !== null || typeof t == "function", s2 = new Map([["proxy", {
	...
	}, c3 = 149597870.7, u3 = 648e3 / Math.PI, z0 = 1e4, b0 = 100 / u3, l3 = 16777216, h2 = 3.2615637771694, ht = {
		x: 0,
		y: 0,
		z: 0
	};
	...
	const ft = d2[v.EARTH]?.radiusKm ?? 6371, dt = d2[v.SUN]?.radiusKm ?? 696340, p3 = ft / dt, k0 = .04 * (100 / 15);
	ft / k0;
	k0 / ft / (100 / c3);
	function D1(t) {
	...
	function R0(t) {
		...
		let i, c, a, u, h, l, d, p, M, _, E, w, x, g, m, R, D, z, b, N, q, G, K, c0 = r(-6, 6, 1, 4), r0 = r(-6, 6, 1, 4);
		function A0(T, U) {
			return o(c0, T, U);
		}
		function I0(T, U) {
		...
		function S(T, U, F, H, Q, v0, p0, t2) {
			...
			M += T * i1.y, K += U * i1.y, E += F * i1.x, w += H * i1.x;
		}
		...
		_ = 0, _ += f0(-526.069, 0, 0, 1, -2), _ += f0(-3.352, 0, 0, 1, -4), _ += f0(44.297, 1, 0, 1, -2), _ += f0(-6, 1, 0, 1, -4), _ += f0(20.599, -1, 0, 1, 0), _ += f0(-30.598, -1, 0, 1, -2), _ += f0(-24.649, -2, 0, 1, 0), _ += f0(-2, -2, 0, 1, -2), _ += f0(-22.571, 0, 1, 1, -2), _ += f0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = R + K / w0;
	...
	function J1(t, n, e, r, o) {
		...
		let G = k * Math.atan2(N, D), K = e, c0 = r;
		if (o) {
			...
			if (G -= A0, A0 > 0 && G > 3e-4) {
				...
				N = Math.hypot(L[0], L[1]), N > 0 ? (K = St * Math.atan2(L[1], L[0]), K < 0 && (K += 24)) : K = 0, c0 = k * Math.atan2(L[2], N);
			}
		}
		return new U2(q, 90 - G, K, c0);
	}
	function W0(t) {
	...
	function i0(t) {
		const n = y(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = q0(Y3(n, [
	...
	function fe(t) {
		const n = new y0(t);
		return new J2(Y1(n, B1[0]), Y1(n, B1[1]), Y1(n, B1[2]), Y1(n, B1[3]));
	}
	function a0(t, n) {
		var e = y(n);
		...
		const s = O1(t);
		if (s) return X1(new Y0(s.dec, 15 * s.ra, s.dist), e);
	...
	function Q2(t, n, e, r) {
		...
		if (O1(e)) {
			const c = a0(e, o);
			if (r) {
				const u = K1(n, o), h = new P(c.x - u.x, c.y - u.y, c.z - u.z, o), l = P1 / h.Length();
	...
	function Ee(t) {
		return K2(new Date(t.time.date.getTime() + 6 * N3));
	}
	var W2 = class {
		constructor(t, n, e) {
	...
	function be(t, n, e) {
		n = y(n);
		const r = an(t.lon);
		return X1(new Y0(t.lat + cn(e, t.lat), r, t.dist), n);
	}
	function E1(t, n) {
		let e;
	...
	function Be(t, n) {
		...
		const e = X1(new Y0(n, 15 * t, 1), An), r = Vt(w1(Jt, e)), o = 10 / 240, s = o / 15;
		for (let i of qe) {
	...
	function y1(t, n, e, r) {
		...
		return new Ye(n, o, C * Math.hypot(s, i, c), +a1 - (1 + o) * (a1 - t), -a1 + (1 + o) * (a1 + t), e, r);
	}
	function nt(t) {
	...
	function l4(t, n) {
		...
		switch (t) {
			...
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const N = A * (99.360714 + 4850.4046 * o), q = A * (175.895369 + 1191.9605 * o), G = A * (300.323162 + 262.5475 * o), K = A * (114.012305 + 6070.2476 * o), c0 = A * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(N) + 938e-6 * Math.sin(q) + .001432 * Math.sin(G) + 3e-5 * Math.sin(K) + .00215 * Math.sin(c0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(N) + 404e-6 * Math.cos(q) + 617e-6 * Math.cos(G) - 13e-6 * Math.cos(K) + 926e-6 * Math.cos(c0), c = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
	...
	function h4(t, n, e, r) {
		...
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new V(0, 0, 0, 0, 0, 0, o), a = f1(o)) : (c = K1(e, o), a = K1(r, o)), Fn(t, c, s, a, i);
	}
	function Fn(t, n, e, r, o) {
		...
		if (t === 4 || t === 5) {
			...
			const z = t == 4 ? +i : -i, b = .5 * c + z * g, N = .5 * a + z * m, q = .5 * u + z * R, G = .5 * g - z * c, K = .5 * m - z * a, c0 = .5 * R - z * u, r0 = l * b, A0 = l * N, I0 = l * q, u0 = d * c + p * a + M * u, l0 = d * g + p * m + M * R;
			_ = new V(r0, A0, I0, u0 * b + l0 * G, u0 * N + l0 * K, u0 * q + l0 * c0, n.t);
		} else {
	...
	var f4 = class Z {
		...
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = Y.zero(), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, c1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, At), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, u1 + L1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, Et), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, H0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, $0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, J0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, G0);
		}
	...
	const { MakeTime: H4, SearchRiseSet: $4, Body: J4, Observer: d4 } = ot(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: B4, CombineRotation: Y4, MakeRotation: K4 } = ot(z1);
	Math.PI / (180 * 3600);
	function S4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: j4 } = ot(z1);
	function _4(t, n) {
	...
	function R4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = A4(_4(n, { raInHours: s }).filter((_) => !Kt(_)), o, { maxDistanceLevel: e }), c = (_, E, w, x) => ({
			positions: _,
			sizes: E,
			colors: w,
			brightness: x,
