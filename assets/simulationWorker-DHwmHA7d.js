(function() {
	...
	function C(e, t = globalThis, n = ["*"]) {
		t.addEventListener("message", function s(o) {
			...
			const { id: a, type: u, path: i } = Object.assign({ path: [] }, o.data), _ = (o.data.argumentList || []).map(y);
			let c;
			try {
				const l = i.slice(0, -1).reduce((S, f) => S[f], e), p = i.reduce((S, f) => S[f], e);
				switch (u) {
					case "GET":
						c = p;
						break;
					case "SET":
						l[i.slice(-1)[0]] = y(o.data.value), c = !0;
						break;
					case "APPLY":
						c = p.apply(l, _);
						break;
			...
			Promise.resolve(c).catch((l) => ({
				...
				const [p, S] = M(l);
				t.postMessage(Object.assign(Object.assign({}, p), { id: a }), S), u === "RELEASE" && (t.removeEventListener("message", s), V(t), O in e && typeof e[O] == "function" && e[O]());
			}).catch((l) => {
				const [p, S] = M({
	...
	Q / Z;
	function ee(e) {
		return !Number.isFinite(e) || e <= 0 ? 0 : e * v;
	}
	ee(Q);
	ee(Z);
	function Ie(e) {
		...
		switch (n.type) {
			...
			case "seek": {
				...
				if (s < 1721545 || s > 3181545) {
					const i = e.jd > 1721545 && e.jd < 3181545, _ = e.jd === 1721545 && s < 1721545;
					(i || _) && u.push({
			...
			case "step": {
				...
				const o = e.jd + e.speed * s / ge, a = Math.max(k, Math.min(x, o)), u = {
					...e,
					jd: a
				}, i = [{
					type: "tick",
