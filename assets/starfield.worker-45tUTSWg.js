(function() {
	var e2 = Object.defineProperty, Qn = (t, n) => {
		let e = {};
	...
	const r2 = Symbol("Comlink.proxy"), qn = Symbol("Comlink.endpoint"), Yn = Symbol("Comlink.releaseProxy"), ct = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), o2 = (t) => typeof t == "object" && t !== null || typeof t == "function", s2 = new Map([["proxy", {
	...
	function c2(t) {
		const n = t.map(I1);
		return [n.map((e) => e[0]), n3(n.map((e) => e[1]))];
	}
	...
	const x = {
		...
		[x.MOON]: {
			...
			massKg: 7342e19,
			semiMajorAxisAu: c3,
			parentBodyId: x.EARTH
		}
	}, l3 = 149597870.7, h3 = 648e3 / Math.PI, h2 = 1e3, z0 = 100 * h2, b0 = 100 / h3, f3 = 16777216, f2 = 3.2615637771694, ht = {
		x: 0,
		y: 0,
		z: 0
	};
	...
	const d3 = 5e4, M3 = 2e3, S3 = d3 / f2, _3 = M3 / f2;
	d2(S3);
	d2(_3);
	let v1;
	b0 <= 0 ? v1 = z0 : 25 >= b0 ? v1 = z0 + (25 - b0) * 20 : v1 = 25 / b0 * z0;
	const A3 = v1;
	function p3(t) {
		return {
			name: t.name,
	...
	const M2 = Object.fromEntries(Object.entries(u3).map(([t, n]) => [t, p3(n)]));
	x.SUN, x.MERCURY, x.VENUS, x.EARTH, x.MARS, x.JUPITER, x.SATURN, x.URANUS, x.NEPTUNE, x.PLUTO, x.MOON;
	function E3(t) {
		if (!Number.isFinite(t) || t < 0) return 0;
		const n = b0;
		return n <= 0 ? z0 : t >= n ? z0 + (t - n) * 20 : t / n * z0;
	}
	const ft = M2[x.EARTH]?.radiusKm ?? 6371, dt = M2[x.SUN]?.radiusKm ?? 696340, y3 = ft / dt, k0 = .04 * (100 / 15);
	ft / k0;
	k0 / ft / (h2 / l3);
	function D1(t) {
		return !Number.isFinite(t) || t <= 0 ? k0 : t / y3 * k0;
	}
	function w3(t) {
		return !Number.isFinite(t) || t <= 0 ? k0 : D1(t / dt);
	}
	w3(dt);
	const g3 = .008;
	function m3(t, n) {
		if (!Number.isFinite(n) || n <= 0 || t == null || !Number.isFinite(t)) return D1(1);
		const e = 10 ** ((4.83 - (t - 5 * Math.log10(n / 10))) / 2.5);
		return D1(Math.max(g3, Math.min(100, Math.sqrt(e))));
	}
	var z1 = Qn({
		AU_PER_LY: () => S2,
		AngleBetween: () => U1,
		...
		InverseRotation: () => s1,
		JUPITER_EQUATORIAL_RADIUS_KM: () => R3,
		...
		SearchMaxElongation: () => Ne,
		SearchMoonNode: () => Fn,
		SearchMoonPhase: () => A1,
		SearchMoonQuarter: () => W2,
		SearchPeakMagnitude: () => Te,
		...
		TransitInfo: () => Cn,
		Vector: () => P,
		VectorFromHorizon: () => Fe,
		VectorFromSphere: () => j1,
		VectorObserver: () => se,
		e_tilt: () => g0
	});
	const P1 = 173.1446326846693, C = 149597870.69098932, S2 = 63241.07708807546, A = .017453292519943295, Mt = .26179938779914946, k = 57.29577951308232, St = 3.819718634205488, R3 = 71492, x3 = 66854, I3 = 69911, v3 = 1821.6, D3 = 1560.8, z3 = 2631.2, P3 = 2410.3, _2 = 365.24217, A2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), F0 = 484813681109536e-20, p2 = 10800 * 60, N3 = 2 * p2, E2 = 7292115e-11, T3 = p2 / Math.PI, L3 = -.17 - 5 * Math.log10(T3), N1 = 29.530588, T1 = 24 * 3600, C3 = T1 * 1e3, y2 = .9972695717592592, a1 = 695700, w2 = a1 / C, o0 = .996647180302104, V0 = o0 * o0, d0 = 6378.1366, U3 = d0 / C, O3 = d0 * o0, g2 = 6371, b3 = g2 + 88, k3 = 1738.1 / C, W = 1737.4, m2 = 1736, F3 = m2 / C, V3 = 34 / 60, _t = 81.30056, c1 = .0002959122082855911, At = 4912547451450812e-26, pt = 7243452486162703e-25, u1 = 8887692390113509e-25, Et = 9549535105779258e-26, H0 = 2.825345909524226e-7, $0 = 8.459715185680659e-8, J0 = 1.292024916781969e-8, G0 = 1.524358900784276e-8, H3 = 218869976542597e-26, L1 = u1 / _t;
	function yt(t) {
		switch (t) {
			...
			case f.Saturn: return $0;
			case f.Uranus: return J0;
			case f.Neptune: return G0;
			case f.Pluto: return H3;
	...
	var w0 = class at {
		constructor(n) {
			...
			if (n instanceof Date && Number.isFinite(n.getTime())) {
				this.date = n, this.ut = (n.getTime() - A2.getTime()) / e, this.tt = I2(this.ut);
				return;
			}
			if (Number.isFinite(n)) {
				this.date = new Date(A2.getTime() + n * e), this.ut = n, this.tt = I2(this.ut);
				return;
			}
	...
	function q3(t, n, e) {
		return new w0(t.ut + e * (n.ut - t.ut));
	}
	function w(t) {
		return t instanceof w0 ? t : new w0(t);
	}
	function Y3(t) {
		function n(d) {
			return d % N3 * F0;
	...
	function g0(t) {
		if (!b1 || Math.abs(b1.tt - t.tt) > 1e-6) {
			const n = Y3(t), e = v2(t), r = e + n.deps / 3600;
			b1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function m0(t) {
		...
		function r(N, U, F, H) {
			const B = [];
			for (let v0 = 0; v0 <= U - N; ++v0) B.push(e(F, H));
			return {
				min: N,
				array: B
			};
		...
		function o(N, U, F) {
			const H = N.array[U - N.min];
			return H.array[F - H.min];
		}
		function s(N, U, F, H) {
			const B = N.array[U - N.min];
			B.array[F - B.min] = H;
		}
		let i, c, a, u, h, l, d, p, M, _, E, y, R, g, I, m, D, z, b, T, K, G, Y, c0 = r(-6, 6, 1, 4), r0 = r(-6, 6, 1, 4);
		function A0(N, U) {
			return o(c0, N, U);
		}
		function I0(N, U) {
		...
		function st(N, U, F, H, B) {
			B(N * F - U * H, U * F + N * H);
		}
		function L(N) {
			return Math.sin(t0 * N);
		}
		d = n * n, M = 0, Y = 0, E = 0, y = 3422.7;
		var h0 = L(.19833 + .05611 * n), Wt = L(.27869 + .04508 * n), jt = L(.16827 - .36903 * n), Xt = L(.34734 - 5.37261 * n), Zt = L(.10498 - 5.37899 * n), it = L(.42681 - .41855 * n), v4 = L(.14943 - 5.37511 * n);
		for (z = .84 * h0 + .31 * Wt + 14.27 * jt + 7.26 * Xt + .28 * Zt + .24 * it, b = 2.94 * h0 + .31 * Wt + 14.27 * jt + 9.34 * Xt + 1.12 * Zt + .83 * it, T = -6.4 * h0 - 1.89 * it, K = .21 * h0 + .31 * Wt + 14.27 * jt - 88.7 * Xt - 15.3 * Zt + .24 * it - 1.86 * v4, G = z - T, p = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), R = t0 * B0(.60643382 + 1336.85522467 * n - 313e-8 * d) + z / y0, g = t0 * B0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + b / y0, I = t0 * B0(.99312619 + 99.99735956 * n - 44e-8 * d) + T / y0, m = t0 * B0(.25909118 + 1342.2278298 * n - 892e-8 * d) + K / y0, D = t0 * B0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
					a = g, c = 4, u = 1.000002208;
					break;
				case 2:
					a = I, c = 3, u = .997504612 - .002495388 * n;
					break;
				case 3:
					a = m, c = 4, u = 1.000002708 + 139.978 * p;
					break;
				case 4:
					a = D, c = 6, u = 1;
					break;
			...
			for (u0(0, h, 1), u0(1, h, Math.cos(a) * u), l0(0, h, 0), l0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) st(A0(l - 1, h), I0(l - 1, h), A0(1, h), I0(1, h), (N, U) => (u0(l, h, N), l0(l, h, U)));
		...
		function Bn(N, U, F, H) {
			for (var B = {
				...
				F,
				H
			], p0 = 1; p0 <= 4; ++p0) v0[p0] !== 0 && st(B.x, B.y, A0(v0[p0], p0), I0(v0[p0], p0), (t2, i1) => (B.x = t2, B.y = i1));
			return B;
		}
		function S(N, U, F, H, B, v0, p0, t2) {
			var i1 = Bn(B, v0, p0, t2);
			M += N * i1.y, Y += U * i1.y, E += F * i1.x, y += H * i1.x;
		}
		...
		function f0(N, U, F, H, B) {
			return N * Bn(U, F, H, B).y;
		}
		_ = 0, _ += f0(-526.069, 0, 0, 1, -2), _ += f0(-3.352, 0, 0, 1, -4), _ += f0(44.297, 1, 0, 1, -2), _ += f0(-6, 1, 0, 1, -4), _ += f0(20.599, -1, 0, 1, 0), _ += f0(-30.598, -1, 0, 1, -2), _ += f0(-24.649, -2, 0, 1, 0), _ += f0(-2, -2, 0, 1, -2), _ += f0(-22.571, 0, 1, 1, -2), _ += f0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = m + Y / y0;
		let D4 = (1.000002708 + 139.978 * p) * (18519.699999999997 + E) * Math.sin(i) - 6.24 * Math.sin(3 * i) + _;
		return {
			geo_eclip_lon: t0 * B0((R + M / y0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * D4,
			distance_au: y0 * U3 / (.999953253 * y)
		};
	}
	var P2 = class {
	...
	function j3(t) {
		const n = w(t), e = n.tt / 36525, r = e * e, o = r * e, s = r * r, i = m0(n), c = i.geo_eclip_lon, a = i.geo_eclip_lat, u = i.distance_au * C, h = A * 1.543, l = A * e1(93.272095 + 483202.0175233 * e - .0036539 * r - o / 3526e3 + s / 86331e4), d = A * e1(125.0445479 - 1934.1362891 * e + .0020754 * r + o / 467441 - s / 60616e3), p = A * e1(357.5291092 + 35999.0502909 * e - 1536e-7 * r + o / 2449e4), M = A * e1(134.9633964 + 477198.8675055 * e + .0087414 * r + o / 69699 - s / 14712e3), _ = A * e1(297.8501921 + 445267.1114034 * e - .0018819 * r + o / 545868 - s / 113065e3), E = 1 - .002516 * e - 74e-7 * r, y = c - d, R = Math.atan2(Math.sin(y) * Math.cos(a) * Math.cos(h) - Math.sin(a) * Math.sin(h), Math.cos(y) * Math.cos(a)), g = n1(k * (R - l)), I = Math.asin(-Math.sin(y) * Math.cos(a) * Math.sin(h) - Math.sin(a) * Math.cos(h)), m = A * (119.75 + 131.849 * e), D = A * (72.56 + 20.186 * e), z = -.02752 * Math.cos(M) + -.02245 * Math.sin(l) + .00684 * Math.cos(M - 2 * l) + -.00293 * Math.cos(2 * l) + -85e-5 * Math.cos(2 * l - 2 * _) + -54e-5 * Math.cos(M - 2 * _) + -2e-4 * Math.sin(M + l) + -2e-4 * Math.cos(M + 2 * l) + -2e-4 * Math.cos(M - l) + 14e-5 * Math.cos(M + 2 * l - 2 * _), b = -.02816 * Math.sin(M) + .02244 * Math.cos(l) + -.00682 * Math.sin(M - 2 * l) + -.00279 * Math.sin(2 * l) + -83e-5 * Math.sin(2 * l - 2 * _) + 69e-5 * Math.sin(M - 2 * _) + 4e-4 * Math.cos(M + l) + -25e-5 * Math.sin(2 * M) + -23e-5 * Math.sin(M + 2 * l) + 2e-4 * Math.cos(M - l) + 19e-5 * Math.sin(M - l) + 13e-5 * Math.sin(M + 2 * l - 2 * _) + -1e-4 * Math.cos(M - 3 * l), T = -(.0252 * E * Math.sin(p) + .00473 * Math.sin(2 * M - 2 * l) + -.00467 * Math.sin(M) + .00396 * Math.sin(m) + .00276 * Math.sin(2 * M - 2 * _) + .00196 * Math.sin(d) + -.00183 * Math.cos(M - l) + .00115 * Math.sin(M - 2 * _) + -96e-5 * Math.sin(M - _) + 46e-5 * Math.sin(2 * l - 2 * _) + -39e-5 * Math.sin(M - l) + -32e-5 * Math.sin(M - p - _) + 27e-5 * Math.sin(2 * M - p - 2 * _) + 23e-5 * Math.sin(D) + -14e-5 * Math.sin(2 * _) + 14e-5 * Math.cos(2 * M - 2 * l) + -12e-5 * Math.sin(M - 2 * l) + -12e-5 * Math.sin(2 * M) + 11e-5 * Math.sin(2 * M - 2 * p - 2 * _)) + (z * Math.cos(R) + b * Math.sin(R)) * Math.tan(I), K = b * Math.cos(R) - z * Math.sin(R), G = 2 * k * Math.atan(W / Math.sqrt(u * u - W * W));
		return new P2(k * I + K, g + T, k * a, k * c, u, G);
	}
	function N2(t, n) {
		return [
	...
	function k1(t, n) {
		...
		r *= F0, o *= F0, s *= F0, i *= F0;
		const c = Math.sin(r), a = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), p = Math.sin(i), M = Math.cos(i), _ = M * h - u * p * d, E = M * u * a + p * d * h * a - c * p * l, y = M * u * c + p * d * h * c + a * p * l, R = -p * h - u * M * d, g = -p * u * a + M * d * h * a - c * M * l, I = -p * u * c + M * d * h * c + a * M * l, m = u * l, D = -l * h * a - c * d, z = -l * h * c + d * a;
	...
	function Z3(t, n) {
		...
		let i, c, a;
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - O3;
		else {
			...
			for (;;) {
				...
				const _ = (V0 - 1) * d0, E = h * h, y = l * l, R = E + V0 * y;
				d = Math.sqrt(R);
				const g = _ * l * h / d - o * h + s * l;
				if (Math.abs(g) < 1e-8) break;
				const I = _ * ((E - y) / d - y * E * (V0 - 1) / (_ * R)) + o * l + s * h;
				u -= g / I;
			}
			c = k * u;
			const M = d0 / d;
			Math.abs(l) > Math.abs(h) ? a = o / l - V0 * M : a = s / h - M;
		}
		return new Rt(c, i, 1e3 * a);
	}
	function gt(t, n) {
		const e = t.latitude * A, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, o0 * r), i = V0 * s, c = t.height / 1e3, a = d0 * s + c, u = d0 * i + c, h = (15 * n + t.longitude) * A, l = Math.sin(h), d = Math.cos(h);
		return {
			pos: [
				a * o * d / C,
				a * o * l / C,
	...
	function V1(t, n) {
		const e = g0(t), r = e.mobl * A, o = e.tobl * A, s = e.dpsi * F0, i = Math.cos(r), c = Math.sin(r), a = Math.cos(o), u = Math.sin(o), h = Math.cos(s), l = Math.sin(s), d = h, p = -l * i, M = -l * c, _ = l * a, E = h * i * a + c * u, y = h * c * a - i * u, R = l * u, g = h * i * u - c * a, I = h * c * u + i * a;
		if (n === O.From2000) return new $([
			[
				d,
				_,
	...
	function H1(t, n, e) {
		return e === O.Into2000 ? K0(Q0(t, n, e), n, e) : Q0(K0(t, n, e), n, e);
	}
	function te(t, n, e) {
		return e === O.Into2000 ? T2(C2(t, n, e), n, e) : C2(T2(t, n, e), n, e);
	}
	function U2(t, n) {
		const e = gt(n, M0(t)).pos;
	...
	function ee(t) {
		if (!ne(t)) throw "Argument must be a [3][3] array of numbers";
		return new $(t);
	}
	var O2 = class {
		constructor(t, n, e, r) {
	...
	function J1(t, n, e, r, o) {
		let s = w(t);
		W0(n), v(e), v(r);
		const i = Math.sin(n.latitude * A), c = Math.cos(n.latitude * A), a = Math.sin(n.longitude * A), u = Math.cos(n.longitude * A), h = Math.sin(r * A), l = Math.cos(r * A), d = Math.sin(e * Mt), p = Math.cos(e * Mt);
		let M = [
			c * u,
			c * a,
			i
		...
		const y = -15 * M0(s);
		let R = Y0(y, M), g = Y0(y, _), I = Y0(y, E), m = [
			l * p,
			l * d,
			h
		];
		const D = m[0] * R[0] + m[1] * R[1] + m[2] * R[2], z = m[0] * g[0] + m[1] * g[1] + m[2] * g[2], b = m[0] * I[0] + m[1] * I[1] + m[2] * I[2];
		let T = Math.hypot(z, b), K;
		T > 0 ? (K = -k * Math.atan2(b, z), K < 0 && (K += 360)) : K = 0;
		let G = k * Math.atan2(T, D), Y = e, c0 = r;
		if (o) {
			let r0 = G, A0 = E1(o, 90 - G);
			if (G -= A0, A0 > 0 && G > 3e-4) {
				const I0 = Math.sin(G * A), u0 = Math.cos(G * A), l0 = Math.sin(r0 * A), st = Math.cos(r0 * A), L = [];
				for (let h0 = 0; h0 < 3; ++h0) L.push((m[h0] - st * R[h0]) / l0 * I0 + R[h0] * u0);
				T = Math.hypot(L[0], L[1]), T > 0 ? (Y = St * Math.atan2(L[1], L[0]), Y < 0 && (Y += 24)) : Y = 0, c0 = k * Math.atan2(L[2], T);
			}
		}
		return new O2(K, 90 - G, Y, c0);
	}
	function W0(t) {
	...
	var Rt = class {
		constructor(t, n, e) {
			this.latitude = t, this.longitude = n, this.height = e, W0(this);
		}
	};
	function F2(t) {
		const n = w(t).AddDays(-1 / P1), e = X0(Q.Earth, n), [r, o, s] = H1([
			-e.x,
			-e.y,
			-e.z
	...
	function l1(t, n, e, r, o) {
		W0(e), C1(r), C1(o);
		const s = w(n), i = U2(s, e), c = n0(t, s, o), a = [
			c.x - i[0],
			c.y - i[1],
			c.z - i[2]
		];
		return k2(r ? H1(a, s, O.From2000) : a, s);
	}
	function re(t, n, e) {
		const r = w(t);
		let o = gt(n, M0(r)).pos;
		return e || (o = H1(o, r, O.Into2000)), mt(o, r);
	}
	function oe(t, n, e) {
		const r = w(t), o = gt(n, M0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
		return e ? s : te(s, r, O.Into2000);
	}
	function se(t, n) {
		const e = M0(t.t);
		let r = [
		...
		return n || (r = K0(r, t.t, O.From2000), r = Q0(r, t.t, O.From2000)), Z3(r, e);
	}
	function ie(t, n) {
	...
	function xt(t, n, e) {
		...
		return new b2(new P(r, o, s, t.t), a, c);
	}
	function h1(t) {
		const n = g0(t.t), [e, r, o] = Q0(K0([
			t.x,
			t.y,
			t.z
	...
	function i0(t) {
		const n = w(t), e = m0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = K0(W3(n, [
	...
	function G1(t) {
		const n = w(t), e = m0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			r * Math.cos(e.geo_eclip_lon),
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = mt(Q0(D2(s.mobl, o), n, O.From2000), n), c = s.tobl * A, a = xt(i, Math.cos(c), Math.sin(c));
		return new q0(a.elat, a.elon, e.distance_au);
	}
	function f1(t) {
		const n = w(t), e = 1e-5, r = n.AddDays(-e), o = n.AddDays(+e), s = i0(r), i = i0(o);
		return new V((s.x + i.x) / 2, (s.y + i.y) / 2, (s.z + i.z) / 2, (i.x - s.x) / (2 * e), (i.y - s.y) / (2 * e), (i.z - s.z) / (2 * e), n);
	}
	function It(t) {
		const n = w(t), e = f1(n), r = 1 + _t;
	...
	const j0 = 365250, Dt = 0, zt = 1, B1 = 2;
	function Pt(t) {
	...
	function X0(t, n) {
		const e = n.tt / j0;
		return Pt(V2(P0(t[Dt], e, !0), P0(t[zt], e, !1), P0(t[B1], e, !1))).ToAstroVector(n);
	}
	function d1(t, n) {
		const e = n / j0, r = P0(t[Dt], e, !0), o = P0(t[zt], e, !1), s = P0(t[B1], e, !1), i = vt(t[Dt], e), c = vt(t[zt], e), a = vt(t[B1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), p = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, _ = +(a * d) + s * l * c, E = V2(r, o, s), y = [
			p / j0,
			M / j0,
			_ / j0
		];
		return new T0(n, Pt(E), Pt(y));
	}
	function K1(t, n, e, r) {
	...
	function ae(t) {
		...
		return K1(n, t, f.Jupiter, H0), K1(n, t, f.Saturn, $0), K1(n, t, f.Uranus, J0), K1(n, t, f.Neptune, G0), n;
	}
	const Nt = 51, ce = 29200, Z0 = 146, S0 = 201, N0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function ue(t) {
		let [n, [e, r, o], [s, i, c]] = t;
		return new T0(n, new q(e, r, o), new q(s, i, c));
	}
	function j(t, n, e, r) {
	...
	var S1 = class {
		constructor(t) {
			let n = new T0(t, new q(0, 0, 0), new q(0, 0, 0));
			this.Jupiter = j(n, t, f.Jupiter, H0), this.Saturn = j(n, t, f.Saturn, $0), this.Uranus = j(n, t, f.Uranus, J0), this.Neptune = j(n, t, f.Neptune, G0), this.Jupiter.r.decr(n.r), this.Jupiter.v.decr(n.v), this.Saturn.r.decr(n.r), this.Saturn.v.decr(n.v), this.Uranus.r.decr(n.r), this.Uranus.v.decr(n.v), this.Neptune.r.decr(n.r), this.Neptune.v.decr(n.v), this.Sun = new T0(t, n.r.mul(-1), n.v.mul(-1));
		}
		Acceleration(t) {
			let n = M1(t, c1, this.Sun.r);
			return n.incr(M1(t, H0, this.Jupiter.r)), n.incr(M1(t, $0, this.Saturn.r)), n.incr(M1(t, J0, this.Uranus.r)), n.incr(M1(t, G0, this.Neptune.r)), n;
		}
	}, Tt = class Kn {
		constructor(n, e, r, o) {
	...
	function Ct(t, n) {
		const e = t - n.tt, r = new S1(t), o = t1(e, n.r, n.v, n.a), s = r.Acceleration(o).mean(n.a), i = t1(e, n.r, n.v, s);
	...
	function Ut(t) {
		const n = ue(t), e = new S1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new H2(e, new Tt(n.tt, r, o, s));
	}
	function he(t, n) {
		const e = N0[0][0];
		if (n < e || n > N0[Nt - 1][0]) return null;
		const r = $2((n - e) / ce, Nt - 1);
		if (!t[r]) {
			const s = t[r] = [];
			s[0] = Ut(N0[r]).grav, s[S0 - 1] = Ut(N0[r + 1]).grav;
			let i, c = s[0].tt;
			for (i = 1; i < S0 - 1; ++i) s[i] = Ct(c += Z0, s[i - 1]).grav;
			c = s[S0 - 1].tt;
			var o = [];
			for (o[S0 - 1] = s[S0 - 1], i = S0 - 2; i > 0; --i) o[i] = Ct(c -= Z0, o[i + 1]).grav;
			for (i = S0 - 2; i > 0; --i) {
				const a = i / (S0 - 1);
	...
	function J2(t, n, e) {
		...
		for (let s = 0; s < o; ++s) r = Ct(s + 1 === o ? n : r.grav.tt + e, r.grav);
		return r;
	}
	function Ot(t, n) {
		let e, r, o;
		const s = he(le, t.tt);
		if (s) {
			const i = $2((t.tt - s[0].tt) / Z0, S0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = t1(t.tt - c.tt, c.r, c.v, u), l = Lt(t.tt - c.tt, c.v, u), d = t1(t.tt - a.tt, a.r, a.v, u), p = Lt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / Z0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(p.mul(M));
		} else {
			let i;
			t.tt < N0[0][0] ? i = J2(N0[0], t.tt, -Z0) : i = J2(N0[Nt - 1], t.tt, +Z0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function de(t, n, e) {
		...
		const M = i * h - s * l, _ = -s * h - i * l, E = 1 / (1 + _), y = 1 / (1 + Math.sqrt(1 - s * s - i * i)), R = r * (h - s - y * i * M), g = r * (l - i + y * s * M), I = u * E * r * (-l - y * i * _), m = u * E * r * (+h + y * s * _), D = 2 * Math.sqrt(1 - c * c - a * a), z = 1 - 2 * a * a, b = 1 - 2 * c * c, T = 2 * a * c;
		return new V(R * z + g * T, R * T + g * b, (c * g - R * a) * D, I * z + m * T, I * T + m * b, (c * m - I * a) * D, t);
	}
	function q1(t, n) {
		...
		return X1(fe, de(t, n.mu, r));
	}
	function Me(t) {
		const n = new w0(t);
		return new G2(q1(n, Q1[0]), q1(n, Q1[1]), q1(n, Q1[2]), q1(n, Q1[3]));
	}
	function a0(t, n) {
		var e = w(n);
		...
		if (t === f.EMB) {
			const i = X0(Q.Earth, e), c = i0(e), a = 1 + _t;
			return new P(i.x + c.x / a, i.y + c.y / a, i.z + c.z / a, e);
		}
		if (t === f.SSB) return ae(e);
		const s = O1(t);
	...
	function L0(t, n) {
		...
		if (e) return e.dist;
		const r = w(n);
		return t in Q ? P0(Q[t][B1], r.tt / j0, !1) : a0(t, r).Length();
	}
	function B2(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
	...
	function K2(t, n, e, r) {
		...
		if (O1(e)) {
			const c = a0(e, o);
			if (r) {
				const u = Y1(n, o), h = new P(c.x - u.x, c.y - u.y, c.z - u.z, o), l = P1 / h.Length();
		...
		const i = new Se(n, e, r, s);
		return B2((c) => i.Position(c), o);
	}
	function n0(t, n, e) {
		C1(e);
		const r = w(n);
	...
	function Y1(t, n) {
		...
		switch (t) {
			...
			case f.Saturn:
			case f.Uranus:
			case f.Neptune: return C0(d1(Q[t], e.tt), e);
	...
	function J(t, n, e, r) {
		...
		for (;;) {
			...
			let d = q3(n, e, .5), p = d.ut - n.ut;
			if (Math.abs(p) < s) return d;
			l ? a = t(d) : l = !0;
			let M = Ae(d.ut, e.ut - d.ut, i, a, c);
			if (M) {
				let _ = w(M.t), E = t(_);
				if (M.df_dt !== 0) {
					if (Math.abs(E / M.df_dt) < s) return _;
					let y = 1.2 * Math.abs(E / M.df_dt);
					if (y < p / 10) {
						let R = _.AddDays(-y), g = _.AddDays(+y);
						if ((R.ut - n.ut) * (R.ut - e.ut) < 0 && (g.ut - n.ut) * (g.ut - e.ut) < 0) {
							let I = t(R), m = t(g);
							if (I < 0 && m >= 0) {
								i = I, c = m, n = R, e = g, a = E, l = !1;
								continue;
							}
						}
					}
	...
	function bt(t, n, e) {
		...
		const r = w(e), o = h1(n0(t, r, !1)), s = h1(n0(n, r, !1));
		return e1(o.elon - s.elon);
	}
	function r1(t, n) {
	...
	function W1(t, n) {
		...
		let o, s, i, c;
		t === f.Sun ? (i = new P(-r.x, -r.y, -r.z, e), s = new P(0, 0, 0, e), o = 0) : (t === f.Moon ? (i = i0(e), s = new P(r.x + i.x, r.y + i.y, r.z + i.z, e)) : (s = a0(t, n), i = new P(s.x - r.x, s.y - r.y, s.z - r.z, e)), o = U1(i, s));
		let a = i.Length(), u = s.Length(), h;
		if (t === f.Sun) c = L3 + 5 * Math.log10(a);
		else if (t === f.Moon) c = ye(o, u, a);
		else if (t === f.Saturn) {
			const l = Ee(o, u, a, i, e);
	...
	function o1(t, n, e) {
		...
		function s(u) {
			const h = U0(t, u);
			return n1(o * (U0(f.Earth, u) - h) - n);
		}
		let i = _1(t), c = w(e), a = s(c);
		a > 0 && (a -= 360);
		for (let u = 0; u < 100; ++u) {
			let h = -a / 360 * i;
	...
	function we(t) {
		return W2(new Date(t.time.date.getTime() + 6 * C3));
	}
	var j2 = class {
		constructor(t, n, e) {
	...
	function me(t) {
		switch (t) {
			...
			case f.Moon: return k3;
			default: return 0;
		}
	}
	function Re(t, n, e, r, o, s = 0) {
		if (!Number.isFinite(s) || s < 0) throw `Invalid value for metersAboveGround: ${s}`;
		const i = me(t), c = X2(n.height - s);
		return Z2(t, n, e, r, o, i, ge(n, s) - V3 * c.density);
	}
	function xe(t, n, e, r, o, s) {
		if (!Number.isFinite(s) || s < -90 || s > 90) throw `Invalid altitude angle: ${s}`;
		return Z2(t, n, e, r, o, 0, s);
	}
	var Ie = class {
		constructor(t, n, e, r) {
	...
	function Ft(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new Ie(r, o, s, i);
		...
		const a = new w0((r.ut + o.ut) / 2), u = n(a);
	...
	function ve(t, n) {
		...
		const o = A * n;
		return Math.abs((360 / y2 - e) * Math.cos(o)) + Math.abs(r * Math.sin(o));
	}
	function Z2(t, n, e, r, o, s, i) {
		if (W0(n), v(o), v(s), v(i), i < -90 || i > 90) throw `Invalid target altitude angle: ${i}`;
		const c = .42, a = ve(t, n.latitude);
		function u(_) {
			const E = l1(t, _, n, !0, !0);
			return e * (J1(_, n, E.ra, E.dec).altitude + k * Math.asin(s / E.dist) - i);
		}
		const h = w(r);
		let l = h, d = h, p = u(l), M = p;
		for (;;) {
			o < 0 ? (l = d.AddDays(-c), p = u(l)) : (d = l.AddDays(+c), M = u(d));
			const _ = Ft(0, u, a, l, d, p, M);
			if (_) {
	...
	function ze(t, n, e) {
		const r = w(n), o = L2(r), s = l1(t, r, e, !0, !0);
	...
	function Te(t, n) {
		...
		let c = 0;
		for (; ++c <= 2;) {
			let a = n1(U0(t, o) - U0(f.Earth, o)), u, h, l;
			a >= -s && a < +s ? (l = 0, u = +s, h = +i) : a >= +i || a < -i ? (l = 0, u = -i, h = -s) : a >= 0 ? (l = -_1(t) / 4, u = +s, h = +i) : (l = -_1(t) / 4, u = -i, h = -s);
			let d = o.AddDays(l), p = o1(t, u, d), M = o1(t, h, p), _ = r(p);
			...
			if (!y) throw `SearchPeakMagnitude: failed search iter ${c} (t1=${p.toString()}, t2=${M.toString()})`;
			if (y.tt >= o.tt) return W1(t, y);
			o = M.AddDays(1);
		}
	...
	function Ce(t, n) {
		...
		for (let d = 0; d < 100; ++d) {
			const p = r.AddDays(d * u), M = L0(t, p);
	...
	function an(t, n) {
		if (n = w(n), t === f.Neptune || t === f.Pluto) return Ce(t, n);
		function e(a) {
	...
	function Ue(t, n) {
		...
		const e = .25 * s0[t].OrbitalPeriod, r = an(t, n.time.AddDays(e));
	...
	function be(t, n, e) {
		...
		const r = v(e) * A, o = Math.cos(r), s = Math.sin(r), i = (n + 1) % 3, c = (n + 2) % 3, a = n;
		let u = [
			[
				0,
				0,
	...
	function ke(t, n) {
		const e = Ht(t);
		return e.lon = cn(e.lon), e.lat += E1(n, e.lat), e;
	}
	function Fe(t, n, e) {
		n = w(n);
		const r = cn(t.lon);
		return j1(new q0(t.lat + un(e, t.lat), r, t.dist), n);
	}
	function E1(t, n) {
		let e;
	...
	function Z1(t) {
		return t = w(t), x0(k1(t, O.From2000), V1(t, O.From2000));
	}
	function He(t) {
		const n = w(t);
		return x0(Z1(n), An(n));
	...
	function tt(t) {
		return t = w(t), x0(V1(t, O.Into2000), k1(t, O.Into2000));
	}
	function $t(t, n) {
		t = w(t);
		const e = Math.sin(n.latitude * A), r = Math.cos(n.latitude * A), o = Math.sin(n.longitude * A), s = Math.cos(n.longitude * A), i = [
			...
			-s,
			0
		], u = -15 * M0(t), h = Y0(u, i), l = Y0(u, c), d = Y0(u, a);
		return new $([
			[
				l[0],
				d[0],
	...
	function _n(t) {
		const n = g0(w(t)).tobl * A, e = Math.cos(n), r = Math.sin(n);
		return new $([
			[
				1,
				0,
	...
	function Ye(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Jt || (Jt = Z1(new w0(-45655.74141261017)), pn = new w0(0));
		const e = j1(new q0(n, 15 * t, 1), pn), r = Vt(y1(Jt, e)), o = 10 / 240, s = o / 15;
		for (let i of qe) {
			const c = i[3] * o, a = i[1] * s, u = i[2] * s;
			if (c <= r.dec && a <= r.ra && r.ra < u) {
				const h = Qe[i[0]];
				return new En(h[0], h[1], r.ra, r.dec);
			}
		}
	...
	function w1(t, n, e, r) {
		...
		return new We(n, o, C * Math.hypot(s, i, c), +a1 - (1 + o) * (a1 - t), -a1 + (1 + o) * (a1 + t), e, r);
	}
	function nt(t) {
		const n = n0(f.Sun, t, !0), e = new P(-n.x, -n.y, -n.z, n.t);
		return w1(b3, t, i0(t), e);
	}
	function wn(t) {
	...
	function Gt(t, n) {
		const e = U2(t, n), r = n0(f.Sun, t, !0), o = i0(t), s = new P(e[0] - o.x, e[1] - o.y, e[2] - o.z, t);
	...
	function Xe(t) {
		const e = J((r) => Bt(nt, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Earth shadow time.";
		return nt(e);
	}
	function Ze(t) {
		const e = J((r) => Bt(wn, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Moon shadow time.";
		return wn(e);
	}
	function t4(t, n, e) {
		const o = J((s) => je(t, n, s), e.AddDays(-1), e.AddDays(1));
	...
	function mn(t, n) {
		const e = new P(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = gn(Math.asin(w2 / e.Length()), Math.asin(F3 / n.Length()), U1(n, e) * A);
	...
	function Rn(t) {
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				const i = Xe(o);
				if (i.r < i.p + W) {
					let c = X.Penumbral, a = 0, u = 0, h = 0, l = Kt(i.time, i.p + W, 200);
					return i.r < i.k + W && (c = X.Partial, h = Kt(i.time, i.k + W, l), i.r + W < i.k ? (c = X.Total, a = 1, u = Kt(i.time, i.k - W, h)) : a = gn(W, i.k, i.r)), new yn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function e4(t) {
		...
		let M;
		if (p > 0) {
			const _ = (-l - Math.sqrt(p)) / (2 * h), E = _ * c.x - a.x, y = _ * c.y - a.y, R = (_ * c.z - a.z) * o0, g = Math.hypot(E, y) * V0;
			g == 0 ? o = R > 0 ? 90 : -90 : o = k * Math.atan(R / g);
			const I = M0(e);
			s = (k * Math.atan2(y, E) - 15 * I) % 360, s <= -180 ? s += 360 : s > 180 && (s -= 360);
			const m = s1(i);
			let D = new P(E / C, y / C, R / C, t.time);
			D = y1(m, D), D.x += t.target.x, D.y += t.target.y, D.z += t.target.z;
			const z = w1(m2, t.time, D, t.dir);
			if (z.r > 1e-9 || z.r < 0) throw `Unexpected shadow distance from geoid intersection = ${z.r}`;
			n = In(z.k), M = n === X.Total ? 1 : mn(t.dir, D);
		} else M = void 0;
		return new xn(n, M, e, r, o, s);
	}
	function r4(t) {
		return t = w(t), Rn(t.AddDays(10));
	}
	function vn(t) {
		t = w(t);
		const n = 1.8;
	...
	function s4(t, n) {
		...
		const c = rt(n, 1, Pn, s, t.time), a = rt(n, -1, Pn, t.time, i);
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = rt(n, 1, Nn, s, t.time), h = rt(n, -1, Nn, t.time, i), l = In(t.k)) : l = X.Partial;
		const d = l === X.Total ? 1 : mn(t.dir, t.target);
		return new zn(l, d, c, u, o, h, a);
	}
	...
	function a4(t, n) {
		return t = w(t), Ln(t.AddDays(10), n);
	}
	var Cn = class {
		constructor(t, n, e, r) {
	...
	function On(t, n) {
		...
		for (;;) {
			...
			if (r1(t, i) < e) {
				const c = t4(t, o, i);
				if (c.r < c.p) {
					const a = c.time.AddDays(-r), u = Un(t, o, a, c.time, -1), h = c.time.AddDays(+r), l = Un(t, o, c.time, h, 1), d = 60 * r1(t, c.time);
					return new Cn(u, c.time, l, d);
				}
			}
			s = i.AddDays(10);
		}
	...
	function u4(t, n) {
		return n = w(n), On(t, n.AddDays(100));
	}
	var _0;
	(function(t) {
	...
	function h4(t) {
		const n = K0(Q0([
			...
			0,
			1
		], t, O.Into2000), t, O.Into2000), e = new P(n[0], n[1], n[2], t), r = Vt(e), o = 190.41375788700253 + 360.9856122880876 * t.ut;
	...
	function f4(t, n) {
		...
		switch (t) {
			...
			case f.Venus:
				...
				break;
			case f.Earth: return h4(e);
			case f.Moon:
				const d = A * (125.045 - .0529921 * r), p = A * (250.089 - .1059842 * r), M = A * (260.008 + 13.0120009 * r), _ = A * (176.625 + 13.3407154 * r), E = A * (357.529 + .9856003 * r), y = A * (311.589 + 26.4057084 * r), R = A * (134.963 + 13.064993 * r), g = A * (276.617 + .3287146 * r), I = A * (34.226 + 1.7484877 * r), m = A * (15.134 - .1589763 * r), D = A * (119.743 + .0036096 * r), z = A * (239.961 + .1643573 * r), b = A * (25.053 + 12.9590088 * r);
				s = 269.9949 + .0031 * o - 3.8787 * Math.sin(d) - .1204 * Math.sin(p) + .07 * Math.sin(M) - .0172 * Math.sin(_) + .0072 * Math.sin(y) - .0052 * Math.sin(m) + .0043 * Math.sin(b), i = 66.5392 + .013 * o + 1.5419 * Math.cos(d) + .0239 * Math.cos(p) - .0278 * Math.cos(M) + .0068 * Math.cos(_) - .0029 * Math.cos(y) + 9e-4 * Math.cos(R) + 8e-4 * Math.cos(m) - 9e-4 * Math.cos(b), c = 38.3213 + (13.17635815 - 14e-13 * r) * r + 3.561 * Math.sin(d) + .1208 * Math.sin(p) - .0642 * Math.sin(M) + .0158 * Math.sin(_) + .0252 * Math.sin(E) - .0066 * Math.sin(y) - .0047 * Math.sin(R) - .0046 * Math.sin(g) + .0028 * Math.sin(I) + .0052 * Math.sin(m) + .004 * Math.sin(D) + .0019 * Math.sin(z) - .0044 * Math.sin(b);
				break;
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const T = A * (99.360714 + 4850.4046 * o), K = A * (175.895369 + 1191.9605 * o), G = A * (300.323162 + 262.5475 * o), Y = A * (114.012305 + 6070.2476 * o), c0 = A * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(T) + 938e-6 * Math.sin(K) + .001432 * Math.sin(G) + 3e-5 * Math.sin(Y) + .00215 * Math.sin(c0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(T) + 404e-6 * Math.cos(K) + 617e-6 * Math.cos(G) - 13e-6 * Math.cos(Y) + 926e-6 * Math.cos(c0), c = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
	...
	function d4(t, n, e, r) {
		const o = w(n), s = yt(e), i = yt(r);
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new V(0, 0, 0, 0, 0, 0, o), a = f1(o)) : (c = Y1(e, o), a = Y1(r, o)), Vn(t, c, s, a, i);
	}
	function Vn(t, n, e, r, o) {
		...
		let _;
		if (t === 4 || t === 5) {
			const E = a * M - u * p, y = u * d - c * M, R = c * p - a * d;
			let g = y * u - R * a, I = R * c - E * u, m = E * a - y * c;
			const D = Math.sqrt(g * g + I * I + m * m);
			g /= D, I /= D, m /= D, c /= l, a /= l, u /= l;
			const z = t == 4 ? +i : -i, b = .5 * c + z * g, T = .5 * a + z * I, K = .5 * u + z * m, G = .5 * g - z * c, Y = .5 * I - z * a, c0 = .5 * m - z * u, r0 = l * b, A0 = l * T, I0 = l * K, u0 = d * c + p * a + M * u, l0 = d * g + p * I + M * m;
			_ = new V(r0, A0, I0, u0 * b + l0 * G, u0 * T + l0 * Y, u0 * K + l0 * c0, n.t);
		} else {
			const E = -l * (o / (e + o)), y = +l * (e / (e + o)), R = (e + o) / (h * l);
			let g, I, m;
			if (t === 1 || t === 2) g = e / (e + o) * Math.cbrt(o / (3 * e)), I = -e, t == 1 ? (g = 1 - g, m = +o) : (g = 1 + g, m = -o);
			else if (t === 3) g = (7 / 12 * o - e) / (o + e), I = +e, m = +o;
			else throw `Invalid Langrage point ${t}. Must be an integer 1..5.`;
			let D = l * g - E, z;
			do {
				const b = D - E, T = D - y;
				z = (R * D + I / (b * b) + m / (T * T)) / (R - 2 * I / (b * b * b) - 2 * m / (T * T * T)), D -= z;
	...
	var M4 = class Z {
		...
		InternalBodyState(n) {
			if (n === f.SSB) return new T0(this.curr.time.tt, q.zero(), q.zero());
			const e = this.curr.gravitators[n];
			if (e) return e;
			throw `Invalid body: ${n}`;
		}
		static CalcSolarSystem(n) {
			const e = {}, r = new T0(n.tt, q.zero(), q.zero());
			e[f.Mercury] = j(r, n.tt, f.Mercury, At), e[f.Venus] = j(r, n.tt, f.Venus, pt), e[f.Earth] = j(r, n.tt, f.Earth, u1 + L1), e[f.Mars] = j(r, n.tt, f.Mars, Et), e[f.Jupiter] = j(r, n.tt, f.Jupiter, H0), e[f.Saturn] = j(r, n.tt, f.Saturn, $0), e[f.Uranus] = j(r, n.tt, f.Uranus, J0), e[f.Neptune] = j(r, n.tt, f.Neptune, G0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new T0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = q.zero(), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, c1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, At), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, u1 + L1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, Et), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, H0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, $0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, J0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, G0);
		}
	...
	const { MakeTime: H4, SearchRiseSet: $4, Body: J4, Observer: S4 } = ot(z1);
	new S4(40.7128, -74.006, 10);
	function $n(t, n) {
	...
	const { Body: e0 } = ot(z1);
	x.SUN, e0.Sun, x.MERCURY, e0.Mercury, x.VENUS, e0.Venus, x.EARTH, e0.Earth, x.MARS, e0.Mars, x.JUPITER, e0.Jupiter, x.SATURN, e0.Saturn, x.URANUS, e0.Uranus, x.NEPTUNE, e0.Neptune, x.PLUTO, e0.Pluto, x.MOON, e0.Moon;
	2 * Math.PI;
	const { Rotation_EQD_EQJ: Q4, CombineRotation: q4, MakeRotation: Y4 } = ot(z1);
	Math.PI / (180 * 3600);
	function A4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: X4 } = ot(z1);
	function p4(t, n) {
	...
	function Jn(t) {
		...
		return n != null && e != null && r != null && Number.isFinite(n) && Number.isFinite(e) && Number.isFinite(r) ? o3({
			x: n,
			y: e,
			z: r
		}) : _4(o) ?? 25;
	...
	function y4(t) {
		...
		const n = E3(Jn(t)), { x: e, y: r, z: o, ra: s, dec: i } = t, c = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let a = null;
		return c && (a = a3({
			x: e,
			y: r,
			z: o
		})), a == null && (a = $n(s, i)), a == null ? {
			x: 0,
			y: 0,
			z: n
		} : {
	...
	function x4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = E4(p4(n, { raInHours: s }).filter((_) => !Yt(_)), o, { maxDistanceLevel: e }), c = (_, E, y, R) => ({
			positions: _,
			sizes: E,
			colors: y,
			brightness: R,
		...
		if (i.length === 0) {
			...
			const _ = R4(), E = _.length, y = new Float32Array(E * 3), R = new Float32Array(E).fill(Gn), g = new Float32Array(E * 3).fill(1), I = new Float32Array(E).fill(1);
			for (let m = 0; m < E; m += 1) y[m * 3] = _[m].x, y[m * 3 + 1] = _[m].y, y[m * 3 + 2] = _[m].z;
			return c(y, R, g, I);
		}
		const a = i.map(y4), u = a.length, h = new Float32Array(u * 3), l = new Float32Array(u), d = new Float32Array(u * 3), p = new Float32Array(u), M = [];
		for (let _ = 0; _ < u; _ += 1) {
			const E = a[_], y = i[_];
			h[_ * 3] = E.x, h[_ * 3 + 1] = E.y, h[_ * 3 + 2] = E.z;
			const R = y ? m3(y.mag, Jn(y)) : k0;
			l[_] = r ? R : Gn;
			const g = y?.c != null ? `#${y.c}` : void 0, [I, m, D] = r ? w4(g) : [
				1,
				1,
				1
			];
			d[_ * 3] = I, d[_ * 3 + 1] = m, d[_ * 3 + 2] = D, r ? p[_] = y?.br != null && Number.isFinite(y.br) ? Math.max(0, Math.min(1, y.br)) : 1 : p[_] = 1, y?.proper && M.push({
				position: { ...E },
				name: y.proper,
