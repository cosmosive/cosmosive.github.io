...
`;function st(e){return new q({uniforms:{uPointSizeMax:{value:e?.pointSizeMax??at},uTime:{value:0},uDtYears:{value:0},uTwinkleAmp:{value:e?.twinkleAmp??0},uTwinkleFreq:{value:e?.twinkleFreq??.8},uSizeMultiplier:{value:e?.sizeMultiplier??1},uPointSizeFloor:{value:e?.pointSizeFloor??1},uMagLimit:{value:6.5},uMagFadeRange:{value:2.5},uMagRef:{value:0},uFluxClampMax:{value:4},uFluxGamma:{value:.6},uPsfMaxMult:{value:5},uFaintSizeFade:{value:.5},uFlareSatLo:{value:.42},uFlareSatHi:{value:.66},uFlareMaxScale:{value:5},uFlareSpikeAmt:{value:.5},uPaletteHot:{value:new U(1,.78,.56)},uPaletteFlare:{value:new U(1,.92,.75)},uStreakAmt:{value:0},uCoreSizeRatio:{value:e?.coreSizeRatio??1}},vertexShader:nt,fragmentShader:it,transparent:!0,depthWrite:!1,blending:2,toneMapped:!1})}function lt(){return st({sizeMultiplier:1.6,pointSizeMax:ot,pointSizeFloor:1,twinkleAmp:.03,twinkleFreq:.9,coreSizeRatio:1.6})}function ut(e){const{layers:t}=ve(e);return{createChunk(r){const o=t[r.layer],a=new Re;for(const u of r.attributes)a.setAttribute(u.name,new ae(u.data,u.components));a.computeBoundingSphere();const n=r.material,c=new Ve(a,n);c.frustumCulled=!0,c.raycast=()=>{},o.add(c);const s=new Map(r.attributes.map(u=>[u.name,u.components]));return{meshRef:c,updateAttribute(u,i){const l=s.get(u);l!=null&&a.setAttribute(u,new ae(i,l))},setVertexCount(u){a.computeBoundingSphere()},dispose(){o.remove(c),a.dispose()}}}}}function ct(e){return ut(e)}function Se(e,t){return t?e.depthCam!==t.depthCam?e.depthCam<t.depthCam:e.renderedPx!==t.renderedPx?e.renderedPx>t.renderedPx:e.distSqPx<t.distSqPx:!0}function ft(e,t){let r=null,o=null;for(let a=0;a<e.length;a+=1){const n=e[a],c=t(n,a);c&&Se(c,r)&&(r=c,o=n.starsInChunk[c.index]??null)}return o}function mt(e){const t=e.proper??(e.hip!=null?`HIP ${e.hip}`:`Star ${e.id}`);return Ee.select(Fe("hyg",e.id),t,"star"),!0}function ht(e){if(e.chunks.length===0)return!1;const t=e.prepared;if(t==null)return!1;const r=ft(e.chunks,(o,a)=>e.pickInChunk(o,a,t));return r?mt(r):!1}function pt(e,t){return!Number.isFinite(e)||!Number.isFinite(t)||e<=0||t<=0?0:e/(2*Math.tan(t*.5))}function vt(){return 1}function dt(){return 8}var Z={px:0,py:0};function gt(e){const{positions:t,velocities:r,dtYears:o,sizes:a,distancesKm:n,starCount:c,camFrustum:s,tanHalfV:u,viewpointHelioKm:i,maxDistSqKm:l,mouseX:m,mouseY:v,projectStarToScreenPx:p}=e,S=o??0,d=Math.min(c,t.length/3,a.length,n.length);let y=null;for(let k=0;k<d;k+=1){const w=k*3,F=t[w]+(r?r[w]*S:0),C=t[w+1]+(r?r[w+1]*S:0),T=t[w+2]+(r?r[w+2]*S:0);if(!et(F,C,T,n[k]??0,i.x,i.y,i.z,l))continue;const R=F-s.position.x,I=C-s.position.y,N=T-s.position.z,B=R*s.forward.x+I*s.forward.y+N*s.forward.z;if(B<=1e-6)continue;const J=R*s.right.x+I*s.right.y+N*s.right.z,h=R*s.up.x+I*s.up.y+N*s.up.z,f=u*s.aspect*B;if(Math.abs(J)>f)continue;const g=u*B;if(Math.abs(h)>g)continue;const x=B,P=vt(),b=dt();if(b<=0)continue;p(F,C,T,Z);const D=Z.px-m,L=Z.py-v,G=D*D+L*L;if(G>b*b)continue;const O={index:k,distSqPx:G,depthCam:x,renderedPx:P};Se(O,y)&&(y=O)}return y}var Q=new H,ee=new H;function St(e,t,r,o,a,n,c,s,u,i,l,m,v,p,S){const d=r.getBoundingClientRect();return gt({positions:n,velocities:p,dtYears:S,sizes:c,distancesKm:s,starCount:u,uScale:pt(d.height,o.fov*Math.PI/180),camFrustum:i,tanHalfV:l,viewpointHelioKm:m,maxDistSqKm:v,mouseX:e-d.left,mouseY:t-d.top,projectStarToScreenPx:(y,k,w,F)=>{Q.set(y,k,w),Q.applyMatrix4(a.matrixWorld),ee.copy(Q).project(o),F.px=(ee.x*.5+.5)*d.width,F.py=(-ee.y*.5+.5)*d.height}})}function xt(e){const{cameraThree:t,pickerScratch:r,syncScratch:o}=e;Ne(t,r.frustum,o.forward,o.right,o.up);const a=Ae();a&&(r.frustum.position.x+=a.x,r.frustum.position.y+=a.y,r.frustum.position.z+=a.z);const{tanHalfV:n,maxDistSqKm:c}=rt(r);return ht({chunks:e.chunks,prepared:{tanHalfV:n,maxDistSqKm:c},pickInChunk:(s,u,i)=>{const l=e.pointsMeshes[u],m=e.chunkDistancesKm[u],v=e.chunkOriginalSizes[u];return!l||!m||!v?null:St(e.clientX,e.clientY,e.canvas,t,l,s.positions,v,m,s.starsInChunk.length,r.frustum,i.tanHalfV,r.viewpointHelioKm,i.maxDistSqKm,e.chunkVelocities[u]??null,e.dtYears)}})}function yt(e){return xt(e)}var _={chunks:[],pointsMeshes:[],starsInView:[]};function rr(){return _}function ar(e,t){const r=ve(e),{starsInViewRef:o,getStarData:a,getStatus:n}=t,c=ct(e),s=[],u=[],i=[],l=[],m=[],v=[],p=lt();let S=0,d=0;const y=new Map,k=new Te,w=new De,F=new Ie,C=tt(),T=new H,R=new H,I=new H;function N(){for(const h of s)h.dispose();s.length=0,u.length=0,i.length=0,l.length=0,m.length=0,v.length=0;for(const h of y.values())h.ref=null}function B(h){_.chunks=h,_.pointsMeshes=u,_.starsInView=o.current}function J(h){const f=_;for(let g=0;g<f.chunks.length;g+=1){const x=f.chunks[g],P=f.pointsMeshes[g];if(!(!x||!P))for(let b=0;b<x.starsInChunk.length;b+=1){const D=x.starsInChunk[b];if(!D||D.proper!==h)continue;const L=P.geometry.getAttribute("ignite");return L instanceof ae?{attribute:L,index:b}:null}}return null}return{refresh(){N();const h=Je(a,n,i);for(const f of h){l.push(new Float32Array(f.sizes));const g=_e(f);m.push(g);const x=c.createChunk({layer:"starfield",meshName:`cosmosive-starfield-points-${f.abbr.replace(/[^a-zA-Z0-9_-]/g,"_")}`,material:p,attributes:[{name:"position",data:f.positions,components:3},{name:"size",data:f.sizes,components:1},{name:"color",data:f.colors,components:3},{name:"warm",data:f.warm,components:1},{name:"brightness",data:f.brightness,components:1},{name:"mag",data:f.mag,components:1},{name:"ignite",data:new Float32Array(f.sizes.length).fill(1),components:1},{name:"velocity",data:g,components:3}]});s.push(x);const P=x.meshRef;u.push(P)}v.length=0;for(const f of u)v.push(f);B(h)},advanceTime(h){S+=h,p.uniforms.uTime&&(p.uniforms.uTime.value=S);const f=(ce().jd-le)/ue;p.uniforms.uDtYears&&(p.uniforms.uDtYears.value=f);const g=4.5,x=7.5,P=.8,b=E.effectiveSpeedKmPerS,D=P*Math.max(0,Math.min(1,((b>1?Math.log10(b):0)-g)/(x-g))),L=1-Math.exp(-h/.35);d+=(D-d)*L;const G=Math.max(d,Le()*P);p.uniforms.uStreakAmt&&(p.uniforms.uStreakAmt.value=G);const O=Me(),xe=1-Math.exp(-h/.7);if(p.uniforms.uMagLimit){const V=p.uniforms.uMagLimit.value;p.uniforms.uMagLimit.value=V+(O.magLimit-V)*xe}const ye=1-Math.exp(-h/.5),se=O.ignitedStarProperNames;for(const V of se)y.has(V)||y.set(V,{value:1,ref:null});for(const[V,z]of y){const we=se.includes(V)?6:1;z.ref===null&&(z.ref=J(V)),z.value+=(we-z.value)*ye,z.ref!==null&&(z.ref.attribute.setX(z.ref.index,z.value),z.ref.attribute.needsUpdate=!0)}},countFrustum(){let h=0,f=0;if(v.length>0){const g=r.cameraThree;g.updateMatrixWorld(!0),k.multiplyMatrices(g.projectionMatrix,g.matrixWorldInverse),w.setFromProjectionMatrix(k);for(const x of v){const P=x.geometry;P.boundingSphere||P.computeBoundingSphere();const b=P.boundingSphere;b&&(F.copy(b).applyMatrix4(x.matrixWorld),w.intersectsSphere(F)&&(h+=1))}for(const x of u)f+=x.geometry.getAttribute("position")?.count??0}return{total:v.length,visible:h,verts:f}},pickFromClick({clientX:h,clientY:f,canvas:g}){return yt({kind:"r3f",clientX:h,clientY:f,canvas:g,chunks:_.chunks,chunkDistancesKm:i,chunkOriginalSizes:l,chunkVelocities:m,dtYears:(ce().jd-le)/ue,cameraThree:r.cameraThree,pointsMeshes:u,pickerScratch:C,syncScratch:{forward:T,right:R,up:I}})},dispose(){N(),p.dispose(),_.chunks=[],_.pointsMeshes=[],_.starsInView=[]}}}var M={star1:Y("#fff3a8",1),star2:Y("#f5c238",1),star3:Y("#e07b1a",1),star4:Y("#a8380c",1)};function j(e){return new U(e[0],e[1],e[2])}function wt(){return{cool:j(M.star4),warm:j(M.star3),hot:j(M.star2),flare:j(M.star1)}}var bt=`
#include <common>
