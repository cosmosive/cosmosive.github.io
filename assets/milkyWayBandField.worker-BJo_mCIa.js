(function() {
	const Z = Symbol("Comlink.proxy"), pt = Symbol("Comlink.endpoint"), St = Symbol("Comlink.releaseProxy"), Y = Symbol("Comlink.finalizer"), A = Symbol("Comlink.thrown"), W = (t) => typeof t == "object" && t !== null || typeof t == "function", Q = new Map([["proxy", {
		canHandle: (t) => W(t) && t[Z],
		serialize(t) {
	...
	function H(t, e = globalThis, n = ["*"]) {
		e.addEventListener("message", function s(o) {
			...
			Promise.resolve(i).catch((_) => ({
				...
				const [E, h] = N(_);
				e.postMessage(Object.assign(Object.assign({}, E), { id: a }), h), c === "RELEASE" && (e.removeEventListener("message", s), X(e), Y in t && typeof t[Y] == "function" && t[Y]());
			}).catch((_) => {
				const [E, h] = N({
	...
	const D = /* @__PURE__ */ new WeakMap(), I = "FinalizationRegistry" in globalThis && new FinalizationRegistry((t) => {
		const e = (D.get(t) || 0) - 1;
		D.set(t, e), e === 0 && $(t);
	});
	function Pt(t, e) {
		const n = (D.get(e) || 0) + 1;
		D.set(e, n), I && I.register(t, e, t);
	}
	function Tt(t) {
		I && I.unregister(t);
	}
	function F(t, e, n = [], s = function() {}) {
		let o = !1;
		const a = new Proxy(s, {
			get(c, r) {
				if (C(o), r === St) return () => {
			...
			apply(c, r, l) {
				...
				if (i === "bind") return F(t, e, n.slice(0, -1));
				const [_, E] = J(l);
				return P(t, e, {
					type: "APPLY",
	...
	Math.PI / 180;
	180 / Math.PI;
	const v = 86400, At = 365.25, S = 299792.458, m = S * At * v, x = 149597870.7, G = 648e3 / Math.PI, et = G * x / m, Dt = [
		[
			-.0548755604,
			-.8734371052,
			-.4838350155
	...
	var O = (t, e, n) => Object.defineProperty(t, e, {
		value: n,
		enumerable: !1,
		writable: !0,
		configurable: !0
	}), Ft = Symbol.for("bitecs-relation"), vt = Symbol.for("bitecs-pairTarget"), Gt = Symbol.for("bitecs-isPairComponent"), y = Symbol.for("bitecs-relationData"), ot = () => {
		let t = {
			...
			if (!t.pairsMap.has(s)) {
				...
				O(o, Ft, e), O(o, vt, s), O(o, Gt, !0), t.pairsMap.set(s, o);
			}
	...
	Wt();
	const rt = 1e3 / x, k = rt * m * et, se = 695700, ae = 6378.137;
	function ct(t) {
	...
	function he(t) {
		const e = t.l * Math.PI / 180, n = t.b * Math.PI / 180, s = t.distPc / 1e3, o = t.radiusPc / 1e3, a = Math.max(1, t.elongation ?? 1), c = o * a, r = (t.paDeg ?? 0) * Math.PI / 180, l = Math.sin(e), i = Math.cos(e), _ = Math.sin(n), E = Math.cos(n), h = Math.cos(r), f = Math.sin(r), d = h * -l + f * (-_ * i), b = h * i + f * (-_ * l), M = f * E, L = 1 / (o * o), w = 1 / (c * c) - L, z = t.peakTauCore / (o * Math.sqrt(2 * Math.PI) * a);
		return {
			cx: s * E * i,
			cy: s * E * l,
			cz: s * _,
	...
	const fe = Ee.map(he), ut = 5, { SQRT1_2: pe } = Math, Se = Math.sqrt(2 * Math.PI);
	function _t(t) {
	...
	function ge(t, e, n, s, o, a, c, r) {
		const l = t.cx - e, i = t.cy - n, _ = t.cz - s, E = l * o + i * a + _ * c, h = o * t.dx + a * t.dy + c * t.dz, f = l * t.dx + i * t.dy + _ * t.dz, d = l * l + i * i + _ * _, b = t.invM + t.anis * h * h, M = t.invM * E + t.anis * h * f, L = t.invM * d + t.anis * f * f, w = Math.max(0, L - M * M / b);
		if (w > ut * ut) return 0;
		const z = M / b, ht = Math.sqrt(b) * pe, ft = .5 * (_t((r - z) * ht) - _t(-z * ht));
		if (ft <= 0) return 0;
		const me = 1 / Math.sqrt(b);
		return t.peakDensity * Se * me * Math.exp(-.5 * w) * ft;
	}
	function Re(t, e) {
		...
		for (let E = 0; E < R.steps; E += 1) {
			const h = (E + .5) / R.steps;
			c += de(t.x + n * h, t.y + s * h, t.z + o * h);
		}
		let r = c * a / R.steps;
		const l = n / a, i = s / a, _ = o / a;
		for (const E of fe) r += ge(E, t.x, t.y, t.z, l, i, _, a);
		return r;
	}
	R.steps;
	function Pe(t) {
	...
	function be(t) {
		...
		for (let r = 0; r < t.pointBudget; r += 1) {
			...
			if (i >= Te && i <= p.maxSunDistKpc) {
				const _ = i * Et, E = le(l.x, l.y, l.z), h = _ * k, f = p.packetAbsMag + 5 * Math.log10(_ / 10), d = Pe(Re(s, l));
