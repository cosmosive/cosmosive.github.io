(function() {
	...
	const n2 = Symbol("Comlink.proxy"), Jn = Symbol("Comlink.endpoint"), Gn = Symbol("Comlink.releaseProxy"), st = Symbol("Comlink.finalizer"), y1 = Symbol("Comlink.thrown"), e2 = (t) => typeof t == "object" && t !== null || typeof t == "function", r2 = new Map([["proxy", {
	...
	const z1 = 173.1446326846693, U = 149597870.69098932, l2 = 63241.07708807546, p = .017453292519943295, ft = .26179938779914946, k = 57.29577951308232, dt = 3.819718634205488, f3 = 71492, d3 = 66854, M3 = 69911, S3 = 1821.6, _3 = 1560.8, p3 = 2631.2, A3 = 2410.3, h2 = 365.24217, f2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), b0 = 484813681109536e-20, d2 = 10800 * 60, w3 = 2 * d2, M2 = 7292115e-11, y3 = d2 / Math.PI, E3 = -.17 - 5 * Math.log10(y3), D1 = 29.530588, I1 = 24 * 3600, g3 = I1 * 1e3, S2 = .9972695717592592, s1 = 695700, _2 = s1 / U, o0 = .996647180302104, k0 = o0 * o0, d0 = 6378.1366, R3 = d0 / U, x3 = d0 * o0, p2 = 6371, m3 = p2 + 88, v3 = 1738.1 / U, W = 1737.4, A2 = 1736, z3 = A2 / U, D3 = 34 / 60, Mt = 81.30056, i1 = .0002959122082855911, St = 4912547451450812e-26, _t = 7243452486162703e-25, a1 = 8887692390113509e-25, pt = 9549535105779258e-26, F0 = 2.825345909524226e-7, V0 = 8.459715185680659e-8, H0 = 1.292024916781969e-8, $0 = 1.524358900784276e-8, I3 = 218869976542597e-26, P1 = a1 / Mt;
	function At(t) {
		switch (t) {
	...
	var E0 = class ot {
		constructor(n) {
			...
			if (n instanceof Date && Number.isFinite(n.getTime())) {
				this.date = n, this.ut = (n.getTime() - f2.getTime()) / e, this.tt = E2(this.ut);
				return;
			}
			if (Number.isFinite(n)) {
				this.date = new Date(f2.getTime() + n * e), this.ut = n, this.tt = E2(this.ut);
				return;
			}
	...
	function O3(t, n, e) {
		return new E0(t.ut + e * (n.ut - t.ut));
	}
	function E(t) {
		return t instanceof E0 ? t : new E0(t);
	}
	function b3(t) {
		function n(d) {
			return d % w3 * b0;
	...
	function R0(t) {
		...
		let i, c, a, u, h, l, d, A, M, _, w, y, x, g, m, R, D, I, b, N, q, G, Y, c0 = r(-6, 6, 1, 4), r0 = r(-6, 6, 1, 4);
		function p0(T, C) {
			return o(c0, T, C);
		}
		function v0(T, C) {
		...
		for (I = .84 * h0 + .31 * Kt + 14.27 * Yt + 7.26 * Wt + .28 * Xt + .24 * rt, b = 2.94 * h0 + .31 * Kt + 14.27 * Yt + 9.34 * Wt + 1.12 * Xt + .83 * rt, N = -6.4 * h0 - 1.89 * rt, q = .21 * h0 + .31 * Kt + 14.27 * Yt - 88.7 * Wt - 15.3 * Xt + .24 * rt - 1.86 * S4, G = I - N, A = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), x = t0 * J0(.60643382 + 1336.85522467 * n - 313e-8 * d) + I / y0, g = t0 * J0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + b / y0, m = t0 * J0(.99312619 + 99.99735956 * n - 44e-8 * d) + N / y0, R = t0 * J0(.25909118 + 1342.2278298 * n - 892e-8 * d) + q / y0, D = t0 * J0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
		...
		function S(T, C, F, H, Q, z0, A0, jt) {
			...
			M += T * o1.y, Y += C * o1.y, w += F * o1.x, y += H * o1.x;
		}
		...
		_ = 0, _ += f0(-526.069, 0, 0, 1, -2), _ += f0(-3.352, 0, 0, 1, -4), _ += f0(44.297, 1, 0, 1, -2), _ += f0(-6, 1, 0, 1, -4), _ += f0(20.599, -1, 0, 1, 0), _ += f0(-30.598, -1, 0, 1, -2), _ += f0(-24.649, -2, 0, 1, 0), _ += f0(-2, -2, 0, 1, -2), _ += f0(-22.571, 0, 1, 1, -2), _ += f0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = R + Y / y0;
		let _4 = (1.000002708 + 139.978 * A) * (18519.699999999997 + w) * Math.sin(i) - 6.24 * Math.sin(3 * i) + _;
		return {
			geo_eclip_lon: t0 * J0((x + M / y0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * _4,
			distance_au: y0 * R3 / (.999953253 * y)
		};
	}
	var m2 = class {
	...
	function C1(t, n) {
		...
		r *= b0, o *= b0, s *= b0, i *= b0;
		const c = Math.sin(r), a = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), A = Math.sin(i), M = Math.cos(i), _ = M * h - u * A * d, w = M * u * a + A * d * h * a - c * A * l, y = M * u * c + A * d * h * c + a * A * l, x = -A * h - u * M * d, g = -A * u * a + M * d * h * a - c * M * l, m = -A * u * c + M * d * h * c + a * M * l, R = u * l, D = -l * h * a - c * d, I = -l * h * c + d * a;
	...
	function H3(t, n) {
		...
		let i, c, a;
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - x3;
		else {
			...
			for (;;) {
				...
				const _ = (k0 - 1) * d0, w = h * h, y = l * l, x = w + k0 * y;
				d = Math.sqrt(x);
				const g = _ * l * h / d - o * h + s * l;
				if (Math.abs(g) < 1e-8) break;
				const m = _ * ((w - y) / d - y * w * (k0 - 1) / (_ * x)) + o * l + s * h;
				u -= g / m;
			}
			c = k * u;
			const M = d0 / d;
	...
	function b1(t, n) {
		const e = g0(t), r = e.mobl * p, o = e.tobl * p, s = e.dpsi * b0, i = Math.cos(r), c = Math.sin(r), a = Math.cos(o), u = Math.sin(o), h = Math.cos(s), l = Math.sin(s), d = h, A = -l * i, M = -l * c, _ = l * a, w = h * i * a + c * u, y = h * c * a - i * u, x = l * u, g = h * i * u - c * a, m = h * c * u + i * a;
		if (n === O.From2000) return new $([
			[
				d,
				_,
	...
	function V1(t, n, e, r, o) {
		let s = E(t);
		K0(n), v(e), v(r);
		const i = Math.sin(n.latitude * p), c = Math.cos(n.latitude * p), a = Math.sin(n.longitude * p), u = Math.cos(n.longitude * p), h = Math.sin(r * p), l = Math.cos(r * p), d = Math.sin(e * ft), A = Math.cos(e * ft);
		let M = [
			c * u,
			c * a,
			i
		...
		const y = -15 * M0(s);
		let x = B0(y, M), g = B0(y, _), m = B0(y, w), R = [
			l * A,
			l * d,
			h
		];
	...
	function U2(t) {
		const n = E(t).AddDays(-1 / z1), e = W0(B.Earth, n), [r, o, s] = k1([
			-e.x,
			-e.y,
			-e.z
		], n, O.From2000), i = p * g0(n).tobl, c = Math.cos(i), a = Math.sin(i);
		return Rt(new P(r, o, s, n), c, a);
	}
	function c1(t, n, e, r, o) {
		K0(e), T1(r), T1(o);
		const s = E(n), i = P2(s, e), c = n0(t, s, o), a = [
			c.x - i[0],
			c.y - i[1],
			c.z - i[2]
		];
		return L2(r ? k1(a, s, O.From2000) : a, s);
	}
	function Q3(t, n, e) {
		const r = E(t);
		let o = yt(n, M0(r)).pos;
	...
	function q3(t, n, e) {
		const r = E(t), o = yt(n, M0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
	...
	function Rt(t, n, e) {
		...
		return new N2(new P(r, o, s, t.t), a, c);
	}
	function u1(t) {
	...
	function i0(t) {
		const n = E(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = G0(k3(n, [
	...
	function H1(t) {
		const n = E(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = Et(Q0(R2(s.mobl, o), n, O.From2000), n), c = s.tobl * p, a = Rt(i, Math.cos(c), Math.sin(c));
	...
	function h1(t, n) {
		const e = n / Y0, r = I0(t[vt], e, !0), o = I0(t[zt], e, !1), s = I0(t[$1], e, !1), i = mt(t[vt], e), c = mt(t[zt], e), a = mt(t[$1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), A = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, _ = +(a * d) + s * l * c, w = C2(r, o, s), y = [
			A / Y0,
			M / Y0,
			_ / Y0
		];
		return new T0(n, Dt(w), Dt(y));
	}
	function J1(t, n, e, r) {
	...
	function Z3(t, n) {
		...
		if (!t[r]) {
			...
			let i, c = s[0].tt;
			for (i = 1; i < S0 - 1; ++i) s[i] = Nt(c += X0, s[i - 1]).grav;
			c = s[S0 - 1].tt;
			var o = [];
			for (o[S0 - 1] = s[S0 - 1], i = S0 - 2; i > 0; --i) o[i] = Nt(c -= X0, o[i + 1]).grav;
			for (i = S0 - 2; i > 0; --i) {
				const a = i / (S0 - 1);
	...
	function Ut(t, n) {
		...
		const s = Z3(j3, t.tt);
		if (s) {
			const i = b2((t.tt - s[0].tt) / X0, S0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = j0(t.tt - c.tt, c.r, c.v, u), l = Tt(t.tt - c.tt, c.v, u), d = j0(t.tt - a.tt, a.r, a.v, u), A = Tt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / X0;
	...
	function ne(t, n, e) {
		...
		const M = i * h - s * l, _ = -s * h - i * l, w = 1 / (1 + _), y = 1 / (1 + Math.sqrt(1 - s * s - i * i)), x = r * (h - s - y * i * M), g = r * (l - i + y * s * M), m = u * w * r * (-l - y * i * _), R = u * w * r * (+h + y * s * _), D = 2 * Math.sqrt(1 - c * c - a * a), I = 1 - 2 * a * a, b = 1 - 2 * c * c, N = 2 * a * c;
	...
	function a0(t, n) {
		...
		if (t === f.EMB) {
			const i = W0(B.Earth, e), c = i0(e), a = 1 + Mt;
		...
		for (;;) {
			...
			if (M) {
				...
				if (M.df_dt !== 0) {
					...
					let y = 1.2 * Math.abs(w / M.df_dt);
					if (y < A / 10) {
						let x = _.AddDays(-y), g = _.AddDays(+y);
	...
	function B1(t, n) {
		...
		if (t === f.Sun) c = E3 + 5 * Math.log10(a);
		else if (t === f.Moon) c = ce(o, u, a);
		else if (t === f.Saturn) {
	...
	function fe(t, n, e, r, o, s = 0) {
		...
		const i = he(t), c = B2(n.height - s);
		return K2(t, n, e, r, o, i, le(n, s) - D3 * c.density);
	}
	...
	function bt(t, n, e, r, o, s, i) {
		...
		const a = new E0((r.ut + o.ut) / 2), u = n(a);
		return bt(1 + t, n, e, r, a, s, u) || bt(1 + t, n, e, a, o, u, i);
	}
	function Se(t, n) {
	...
	function ye(t, n) {
		...
		for (; ++c <= 2;) {
			...
			if (!y) throw `SearchPeakMagnitude: failed search iter ${c} (t1=${A.toString()}, t2=${M.toString()})`;
			if (y.tt >= o.tt) return B1(t, y);
			o = M.AddDays(1);
		}
	...
	function nn(t, n) {
		if (n = E(n), t === f.Neptune || t === f.Pluto) return ge(t, n);
		function e(a) {
	...
	function W1(t) {
		return t = E(t), m0(C1(t, O.From2000), b1(t, O.From2000));
	}
	function Ie(t) {
		const n = E(t);
		return m0(W1(n), fn(n));
	...
	function X1(t) {
		return t = E(t), m0(b1(t, O.Into2000), C1(t, O.Into2000));
	}
	function Vt(t, n) {
		t = E(t);
	...
	function hn(t) {
		const n = g0(E(t)).tobl * p, e = Math.cos(n), r = Math.sin(n);
		return new $([
			[
				1,
				0,
	...
	function be(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Ht || (Ht = W1(new E0(-45655.74141261017)), dn = new E0(0));
	...
	function w1(t, n, e, r) {
		...
		return new ke(n, o, U * Math.hypot(s, i, c), +s1 - (1 + o) * (s1 - t), -s1 + (1 + o) * (s1 + t), e, r);
	}
	function j1(t) {
	...
	function wn(t) {
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				const i = Ve(o);
				if (i.r < i.p + W) {
					let c = j.Penumbral, a = 0, u = 0, h = 0, l = Gt(i.time, i.p + W, 200);
					return i.r < i.k + W && (c = j.Partial, h = Gt(i.time, i.k + W, l), i.r + W < i.k ? (c = j.Total, a = 1, u = Gt(i.time, i.k - W, h)) : a = pn(W, i.k, i.r)), new Sn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function Ge(t) {
		...
		let M;
		if (A > 0) {
			const _ = (-l - Math.sqrt(A)) / (2 * h), w = _ * c.x - a.x, y = _ * c.y - a.y, x = (_ * c.z - a.z) * o0, g = Math.hypot(w, y) * k0;
			g == 0 ? o = x > 0 ? 90 : -90 : o = k * Math.atan(x / g);
			const m = M0(e);
			s = (k * Math.atan2(y, w) - 15 * m) % 360, s <= -180 ? s += 360 : s > 180 && (s -= 360);
			const R = r1(i);
			let D = new P(w / U, y / U, x / U, t.time);
			...
			n = En(I.k), M = n === j.Total ? 1 : An(t.dir, D);
		} else M = void 0;
		return new yn(n, M, e, r, o, s);
	}
	function Qe(t) {
		return t = E(t), wn(t.AddDays(10));
	}
	function gn(t) {
		t = E(t);
		const n = 1.8;
	...
	function Be(t, n) {
		...
		const c = tt(n, 1, mn, s, t.time), a = tt(n, -1, mn, t.time, i);
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = tt(n, 1, vn, s, t.time), h = tt(n, -1, vn, t.time, i), l = En(t.k)) : l = j.Partial;
		const d = l === j.Total ? 1 : An(t.dir, t.target);
		return new xn(l, d, c, u, o, h, a);
	}
	...
	function Tn(t, n) {
		...
		for (;;) {
			...
			if (n1(t, i) < e) {
				const c = $e(t, o, i);
				if (c.r < c.p) {
					const a = c.time.AddDays(-r), u = Pn(t, o, a, c.time, -1), h = c.time.AddDays(+r), l = Pn(t, o, c.time, h, 1), d = 60 * n1(t, c.time);
					return new In(u, c.time, l, d);
				}
			}
			s = i.AddDays(10);
		}
	...
	function t4(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
				const d = p * (125.045 - .0529921 * r), A = p * (250.089 - .1059842 * r), M = p * (260.008 + 13.0120009 * r), _ = p * (176.625 + 13.3407154 * r), w = p * (357.529 + .9856003 * r), y = p * (311.589 + 26.4057084 * r), x = p * (134.963 + 13.064993 * r), g = p * (276.617 + .3287146 * r), m = p * (34.226 + 1.7484877 * r), R = p * (15.134 - .1589763 * r), D = p * (119.743 + .0036096 * r), I = p * (239.961 + .1643573 * r), b = p * (25.053 + 12.9590088 * r);
				s = 269.9949 + .0031 * o - 3.8787 * Math.sin(d) - .1204 * Math.sin(A) + .07 * Math.sin(M) - .0172 * Math.sin(_) + .0072 * Math.sin(y) - .0052 * Math.sin(R) + .0043 * Math.sin(b), i = 66.5392 + .013 * o + 1.5419 * Math.cos(d) + .0239 * Math.cos(A) - .0278 * Math.cos(M) + .0068 * Math.cos(_) - .0029 * Math.cos(y) + 9e-4 * Math.cos(x) + 8e-4 * Math.cos(R) - 9e-4 * Math.cos(b), c = 38.3213 + (13.17635815 - 14e-13 * r) * r + 3.561 * Math.sin(d) + .1208 * Math.sin(A) - .0642 * Math.sin(M) + .0158 * Math.sin(_) + .0252 * Math.sin(w) - .0066 * Math.sin(y) - .0047 * Math.sin(x) - .0046 * Math.sin(g) + .0028 * Math.sin(m) + .0052 * Math.sin(R) + .004 * Math.sin(D) + .0019 * Math.sin(I) - .0044 * Math.sin(b);
				break;
			case f.Mars:
	...
	function n4(t, n, e, r) {
		...
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new V(0, 0, 0, 0, 0, 0, o), a = l1(o)) : (c = q1(e, o), a = q1(r, o)), Cn(t, c, s, a, i);
	}
	function Cn(t, n, e, r, o) {
		...
		let _;
		if (t === 4 || t === 5) {
			const w = a * M - u * A, y = u * d - c * M, x = c * A - a * d;
			let g = y * u - x * a, m = x * c - w * u, R = w * a - y * c;
			...
			const w = -l * (o / (e + o)), y = +l * (e / (e + o)), x = (e + o) / (h * l);
			let g, m, R;
		...
		Update(n) {
			...
			if (r === 0) this.prev = this.Duplicate();
				...
				for (let i = 0; i < this.curr.bodies.length; ++i) {
					...
					a.tt = e.tt, a.r = j0(r, c.r, c.v, u), a.v = Tt(r, c.v, u);
				}
				this.CalcBodyAccelerations();
			}
	...
	function c4(t) {
		...
		const n = ct + kn(t) * 20, { x: e, y: r, z: o, ra: s, dec: i } = t, c = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let a = null;
		return c && (a = e3({
			x: e,
			y: r,
	...
	function d4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = a4(i4(n, { raInHours: s }).filter((_) => !Bt(_)), o, { maxDistanceLevel: e }), c = (_, w, y, x) => ({
			positions: _,
			sizes: w,
			colors: y,
			brightness: x,
		...
		if (i.length === 0) {
			...
			const _ = f4(), w = _.length, y = new Float32Array(w * 3), x = new Float32Array(w).fill(Fn), g = new Float32Array(w * 3).fill(1), m = new Float32Array(w).fill(1);
			for (let R = 0; R < w; R++) y[R * 3] = _[R].x, y[R * 3 + 1] = _[R].y, y[R * 3 + 2] = _[R].z;
			return c(y, x, g, m);
		}
		const a = i.map(c4), u = a.length, h = new Float32Array(u * 3), l = new Float32Array(u), d = new Float32Array(u * 3), A = new Float32Array(u), M = [];
		for (let _ = 0; _ < u; _++) {
			const w = a[_], y = i[_];
			h[_ * 3] = w.x, h[_ * 3 + 1] = w.y, h[_ * 3 + 2] = w.z;
			const x = y ? h3(y.mag, kn(y)) : O0;
			l[_] = r ? x : Fn;
			const g = y?.c != null ? `#${y.c}` : void 0, [m, R, D] = r ? u4(g) : [
				1,
				1,
				1
			];
			d[_ * 3] = m, d[_ * 3 + 1] = R, d[_ * 3 + 2] = D, A[_] = r && y?.br != null && Number.isFinite(y.br) ? Math.max(0, Math.min(1, y.br)) : 1, y?.proper && M.push({
				position: { ...w },
				name: y.proper,
