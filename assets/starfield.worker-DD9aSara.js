(function() {
	...
	const r2 = Symbol("Comlink.proxy"), jn = Symbol("Comlink.endpoint"), Xn = Symbol("Comlink.releaseProxy"), at = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), o2 = (t) => typeof t == "object" && t !== null || typeof t == "function", s2 = new Map([["proxy", {
	...
	function ct(t, n = globalThis, e = ["*"]) {
		n.addEventListener("message", function r(o) {
			...
			Promise.resolve(u).catch((h) => ({
				...
				const [l, d] = I1(h);
				n.postMessage(Object.assign(Object.assign({}, l), { id: s }), d), i === "RELEASE" && (n.removeEventListener("message", r), i2(n), at in t && typeof t[at] == "function" && t[at]());
			}).catch((h) => {
				const [l, d] = I1({
	...
	const m1 = /* @__PURE__ */ new WeakMap(), x1 = "FinalizationRegistry" in globalThis && new FinalizationRegistry((t) => {
		const n = (m1.get(t) || 0) - 1;
		m1.set(t, n), n === 0 && a2(t);
	});
	function ee(t, n) {
		const e = (m1.get(n) || 0) + 1;
		m1.set(n, e), x1 && x1.register(t, n, t);
	}
	function re(t) {
		x1 && x1.unregister(t);
	}
	function ut(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, c, a) {
				...
				if (u === jn) return O0(t, n, { type: "ENDPOINT" }).then(v0);
	...
	}, de = 149597870.7, Me = 648e3 / Math.PI, h2 = 1e3, z0 = 100 * h2, b0 = 100 / Me, Se = 16777216, f2 = 3.2615637771694, lt = {
		x: 0,
		y: 0,
		z: 0
	};
	...
	const _e = 5e4, Ae = 2e3, pe = _e / f2, Ee = Ae / f2;
	d2(pe);
	d2(Ee);
	let D1;
	b0 <= 0 ? D1 = z0 : 25 >= b0 ? D1 = z0 + (25 - b0) * 20 : D1 = 25 / b0 * z0;
	const ye = D1;
	function we(t) {
		return {
			name: t.name,
	...
	const M2 = Object.fromEntries(Object.entries(fe).map(([t, n]) => [t, we(n)]));
	...
	const ht = M2[x.EARTH]?.radiusKm ?? 6371, ft = M2[x.SUN]?.radiusKm ?? 696340, Re = ht / ft, k0 = .04 * (100 / 15);
	ht / k0;
	k0 / ht / (h2 / de);
	function S2(t) {
		return !Number.isFinite(t) || t <= 0 ? k0 : t / Re * k0;
	}
	function me(t) {
		return !Number.isFinite(t) || t <= 0 ? k0 : S2(t / ft);
	}
	me(ft);
	const _2 = .005, xe = 1e3, Ie = 1.42;
	function De(t) {
		return Math.max(_2, Math.min(xe, t));
	}
	function ve(t) {
		const n = De(t), e = Math.min(n, 32), r = Math.log(_2), o = Math.log(32), s = (Math.log(e) - r) / (o - r), i = Math.max(0, Math.min(1, s)) ** Ie;
	...
	function ze(t, n) {
		...
		return dt(Math.sqrt(e));
	}
	function Pe(t, n) {
		const e = t.radiusSolar;
		return e != null && Number.isFinite(e) && e > 0 ? dt(e) : ze(t.mag, n);
	}
	var v1 = Wn({
		AU_PER_LY: () => p2,
		AngleBetween: () => C1,
		...
		InverseRotation: () => s1,
		JUPITER_EQUATORIAL_RADIUS_KM: () => Ne,
		JUPITER_MEAN_RADIUS_KM: () => Le,
		JUPITER_POLAR_RADIUS_KM: () => Te,
		JupiterMoons: () => w3,
		JupiterMoonsInfo: () => Q2,
		KM_PER_AU: () => U,
		LagrangePoint: () => y4,
		...
		SearchMaxElongation: () => k3,
		SearchMoonNode: () => $n,
		SearchMoonPhase: () => A1,
		SearchMoonQuarter: () => Z2,
		SearchPeakMagnitude: () => F3,
	...
	const z1 = 173.1446326846693, U = 149597870.69098932, p2 = 63241.07708807546, A = .017453292519943295, Mt = .26179938779914946, k = 57.29577951308232, St = 3.819718634205488, Ne = 71492, Te = 66854, Le = 69911, Ce = 1821.6, Ue = 1560.8, Oe = 2631.2, be = 2410.3, E2 = 365.24217, y2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), F0 = 484813681109536e-20, w2 = 10800 * 60, ke = 2 * w2, g2 = 7292115e-11, Fe = w2 / Math.PI, Ve = -.17 - 5 * Math.log10(Fe), P1 = 29.530588, N1 = 24 * 3600, He = N1 * 1e3, R2 = .9972695717592592, a1 = 695700, m2 = a1 / U, o0 = .996647180302104, V0 = o0 * o0, d0 = 6378.1366, $e = d0 / U, Je = d0 * o0, x2 = 6371, Ge = x2 + 88, Be = 1738.1 / U, W = 1737.4, I2 = 1736, Ke = I2 / U, Qe = 34 / 60, _t = 81.30056, c1 = .0002959122082855911, At = 4912547451450812e-26, pt = 7243452486162703e-25, u1 = 8887692390113509e-25, Et = 9549535105779258e-26, H0 = 2.825345909524226e-7, $0 = 8.459715185680659e-8, J0 = 1.292024916781969e-8, G0 = 1.524358900784276e-8, qe = 218869976542597e-26, T1 = u1 / _t;
	function yt(t) {
		switch (t) {
	...
	function D2(t) {
		const n = Ye.indexOf(t);
		return n >= 0 ? We[n] : null;
	}
	function U1(t) {
		const n = D2(t);
	...
	function g0(t) {
		if (!O1 || Math.abs(O1.tt - t.tt) > 1e-6) {
			const n = e3(t), e = P2(t), r = e + n.deps / 3600;
			O1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function R0(t) {
		...
		function r(N, O, F, H) {
			const B = [];
			for (let D0 = 0; D0 <= O - N; ++D0) B.push(e(F, H));
			return {
				min: N,
				array: B
			};
		...
		function o(N, O, F) {
			const H = N.array[O - N.min];
			return H.array[F - H.min];
		}
		function s(N, O, F, H) {
			const B = N.array[O - N.min];
			B.array[F - B.min] = H;
		}
		let i, c, a, u, h, l, d, p, M, S, E, y, m, g, I, R, z, D, T, L, K, G, Y, c0 = r(-6, 6, 1, 4), r0 = r(-6, 6, 1, 4);
		function A0(N, O) {
			return o(c0, N, O);
		}
		function I0(N, O) {
		...
		var h0 = C(.19833 + .05611 * n), Wt = C(.27869 + .04508 * n), jt = C(.16827 - .36903 * n), Xt = C(.34734 - 5.37261 * n), Zt = C(.10498 - 5.37899 * n), st = C(.42681 - .41855 * n), C4 = C(.14943 - 5.37511 * n);
		for (D = .84 * h0 + .31 * Wt + 14.27 * jt + 7.26 * Xt + .28 * Zt + .24 * st, T = 2.94 * h0 + .31 * Wt + 14.27 * jt + 9.34 * Xt + 1.12 * Zt + .83 * st, L = -6.4 * h0 - 1.89 * st, K = .21 * h0 + .31 * Wt + 14.27 * jt - 88.7 * Xt - 15.3 * Zt + .24 * st - 1.86 * C4, G = D - L, p = -3332e-9 * C(.59734 - 5.37261 * n) - 539e-9 * C(.35498 - 5.37899 * n) - 64e-9 * C(.39943 - 5.37511 * n), m = t0 * B0(.60643382 + 1336.85522467 * n - 313e-8 * d) + D / y0, g = t0 * B0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + T / y0, I = t0 * B0(.99312619 + 99.99735956 * n - 44e-8 * d) + L / y0, R = t0 * B0(.25909118 + 1342.2278298 * n - 892e-8 * d) + K / y0, z = t0 * B0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (u0(0, h, 1), u0(1, h, Math.cos(a) * u), l0(0, h, 0), l0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) ot(A0(l - 1, h), I0(l - 1, h), A0(1, h), I0(1, h), (N, O) => (u0(l, h, N), l0(l, h, O)));
		...
		function qn(N, O, F, H) {
			for (var B = {
				...
				F,
				H
			], p0 = 1; p0 <= 4; ++p0) D0[p0] !== 0 && ot(B.x, B.y, A0(D0[p0], p0), I0(D0[p0], p0), (t2, i1) => (B.x = t2, B.y = i1));
			return B;
		}
		function _(N, O, F, H, B, D0, p0, t2) {
			var i1 = qn(B, D0, p0, t2);
			M += N * i1.y, Y += O * i1.y, E += F * i1.x, y += H * i1.x;
		}
		...
		function f0(N, O, F, H, B) {
			return N * qn(O, F, H, B).y;
		}
		S = 0, S += f0(-526.069, 0, 0, 1, -2), S += f0(-3.352, 0, 0, 1, -4), S += f0(44.297, 1, 0, 1, -2), S += f0(-6, 1, 0, 1, -4), S += f0(20.599, -1, 0, 1, 0), S += f0(-30.598, -1, 0, 1, -2), S += f0(-24.649, -2, 0, 1, 0), S += f0(-2, -2, 0, 1, -2), S += f0(-22.571, 0, 1, 1, -2), S += f0(10.985, 0, -1, 1, -2), M += .82 * C(.7736 - 62.5512 * n) + .31 * C(.0466 - 125.1025 * n) + .35 * C(.5785 - 25.1042 * n) + .66 * C(.4591 + 1335.8075 * n) + .64 * C(.313 - 91.568 * n) + 1.14 * C(.148 + 1331.2898 * n) + .21 * C(.5918 + 1056.5859 * n) + .44 * C(.5784 + 1322.8595 * n) + .24 * C(.2275 - 5.7374 * n) + .28 * C(.2965 + 2.6929 * n) + .33 * C(.3132 + 6.3368 * n), i = R + Y / y0;
		let U4 = (1.000002708 + 139.978 * p) * (18519.699999999997 + E) * Math.sin(i) - 6.24 * Math.sin(3 * i) + S;
		return {
			geo_eclip_lon: t0 * B0((m + M / y0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * U4,
			distance_au: y0 * $e / (.999953253 * y)
		};
	}
	var L2 = class {
	...
	function o3(t) {
		const n = w(t), e = n.tt / 36525, r = e * e, o = r * e, s = r * r, i = R0(n), c = i.geo_eclip_lon, a = i.geo_eclip_lat, u = i.distance_au * U, h = A * 1.543, l = A * e1(93.272095 + 483202.0175233 * e - .0036539 * r - o / 3526e3 + s / 86331e4), d = A * e1(125.0445479 - 1934.1362891 * e + .0020754 * r + o / 467441 - s / 60616e3), p = A * e1(357.5291092 + 35999.0502909 * e - 1536e-7 * r + o / 2449e4), M = A * e1(134.9633964 + 477198.8675055 * e + .0087414 * r + o / 69699 - s / 14712e3), S = A * e1(297.8501921 + 445267.1114034 * e - .0018819 * r + o / 545868 - s / 113065e3), E = 1 - .002516 * e - 74e-7 * r, y = c - d, m = Math.atan2(Math.sin(y) * Math.cos(a) * Math.cos(h) - Math.sin(a) * Math.sin(h), Math.cos(y) * Math.cos(a)), g = n1(k * (m - l)), I = Math.asin(-Math.sin(y) * Math.cos(a) * Math.sin(h) - Math.sin(a) * Math.cos(h)), R = A * (119.75 + 131.849 * e), z = A * (72.56 + 20.186 * e), D = -.02752 * Math.cos(M) + -.02245 * Math.sin(l) + .00684 * Math.cos(M - 2 * l) + -.00293 * Math.cos(2 * l) + -85e-5 * Math.cos(2 * l - 2 * S) + -54e-5 * Math.cos(M - 2 * S) + -2e-4 * Math.sin(M + l) + -2e-4 * Math.cos(M + 2 * l) + -2e-4 * Math.cos(M - l) + 14e-5 * Math.cos(M + 2 * l - 2 * S), T = -.02816 * Math.sin(M) + .02244 * Math.cos(l) + -.00682 * Math.sin(M - 2 * l) + -.00279 * Math.sin(2 * l) + -83e-5 * Math.sin(2 * l - 2 * S) + 69e-5 * Math.sin(M - 2 * S) + 4e-4 * Math.cos(M + l) + -25e-5 * Math.sin(2 * M) + -23e-5 * Math.sin(M + 2 * l) + 2e-4 * Math.cos(M - l) + 19e-5 * Math.sin(M - l) + 13e-5 * Math.sin(M + 2 * l - 2 * S) + -1e-4 * Math.cos(M - 3 * l), L = -(.0252 * E * Math.sin(p) + .00473 * Math.sin(2 * M - 2 * l) + -.00467 * Math.sin(M) + .00396 * Math.sin(R) + .00276 * Math.sin(2 * M - 2 * S) + .00196 * Math.sin(d) + -.00183 * Math.cos(M - l) + .00115 * Math.sin(M - 2 * S) + -96e-5 * Math.sin(M - S) + 46e-5 * Math.sin(2 * l - 2 * S) + -39e-5 * Math.sin(M - l) + -32e-5 * Math.sin(M - p - S) + 27e-5 * Math.sin(2 * M - p - 2 * S) + 23e-5 * Math.sin(z) + -14e-5 * Math.sin(2 * S) + 14e-5 * Math.cos(2 * M - 2 * l) + -12e-5 * Math.sin(M - 2 * l) + -12e-5 * Math.sin(2 * M) + 11e-5 * Math.sin(2 * M - 2 * p - 2 * S)) + (D * Math.cos(m) + T * Math.sin(m)) * Math.tan(I), K = T * Math.cos(m) - D * Math.sin(m), G = 2 * k * Math.atan(W / Math.sqrt(u * u - W * W));
		return new L2(k * I + K, g + L, k * a, k * c, u, G);
	}
	function C2(t, n) {
		return [
	...
	function b1(t, n) {
		...
		r *= F0, o *= F0, s *= F0, i *= F0;
		const c = Math.sin(r), a = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), p = Math.sin(i), M = Math.cos(i), S = M * h - u * p * d, E = M * u * a + p * d * h * a - c * p * l, y = M * u * c + p * d * h * c + a * p * l, m = -p * h - u * M * d, g = -p * u * a + M * d * h * a - c * M * l, I = -p * u * c + M * d * h * c + a * M * l, R = u * l, z = -l * h * a - c * d, D = -l * h * c + d * a;
	...
	function i3(t, n) {
		...
		let i, c, a;
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - Je;
		else {
			...
			for (;;) {
				...
				const S = (V0 - 1) * d0, E = h * h, y = l * l, m = E + V0 * y;
				d = Math.sqrt(m);
				const g = S * l * h / d - o * h + s * l;
				if (Math.abs(g) < 1e-8) break;
				const I = S * ((E - y) / d - y * E * (V0 - 1) / (S * m)) + o * l + s * h;
				u -= g / I;
			}
			c = k * u;
			const M = d0 / d;
	...
	function F1(t, n) {
		const e = g0(t), r = e.mobl * A, o = e.tobl * A, s = e.dpsi * F0, i = Math.cos(r), c = Math.sin(r), a = Math.cos(o), u = Math.sin(o), h = Math.cos(s), l = Math.sin(s), d = h, p = -l * i, M = -l * c, S = l * a, E = h * i * a + c * u, y = h * c * a - i * u, m = l * u, g = h * i * u - c * a, I = h * c * u + i * a;
		if (n === b.From2000) return new $([
			[
				d,
				S,
	...
	function V1(t, n, e) {
		return e === b.Into2000 ? K0(Q0(t, n, e), n, e) : Q0(K0(t, n, e), n, e);
	}
	function a3(t, n, e) {
	...
	function $1(t, n, e, r, o) {
		...
		const y = -15 * M0(s);
		let m = Y0(y, M), g = Y0(y, S), I = Y0(y, E), R = [
			l * p,
			l * d,
			h
		];
		const z = R[0] * m[0] + R[1] * m[1] + R[2] * m[2], D = R[0] * g[0] + R[1] * g[1] + R[2] * g[2], T = R[0] * I[0] + R[1] * I[1] + R[2] * I[2];
		let L = Math.hypot(D, T), K;
		L > 0 ? (K = -k * Math.atan2(T, D), K < 0 && (K += 360)) : K = 0;
		let G = k * Math.atan2(L, z), Y = e, c0 = r;
		if (o) {
			let r0 = G, A0 = E1(o, 90 - G);
			if (G -= A0, A0 > 0 && G > 3e-4) {
				const I0 = Math.sin(G * A), u0 = Math.cos(G * A), l0 = Math.sin(r0 * A), ot = Math.cos(r0 * A), C = [];
				for (let h0 = 0; h0 < 3; ++h0) C.push((R[h0] - ot * m[h0]) / l0 * I0 + m[h0] * u0);
				L = Math.hypot(C[0], C[1]), L > 0 ? (Y = St * Math.atan2(C[1], C[0]), Y < 0 && (Y += 24)) : Y = 0, c0 = k * Math.atan2(C[2], L);
			}
		}
	...
	function $2(t) {
		const n = w(t).AddDays(-1 / z1), e = X0(Q.Earth, n), [r, o, s] = V1([
			-e.x,
			-e.y,
			-e.z
		], n, b.From2000), i = A * g0(n).tobl, c = Math.cos(i), a = Math.sin(i);
		return xt(new P(r, o, s, n), c, a);
	}
	function l1(t, n, e, r, o) {
		W0(e), L1(r), L1(o);
		const s = w(n), i = k2(s, e), c = n0(t, s, o), a = [
			c.x - i[0],
			c.y - i[1],
			c.z - i[2]
		];
		return H2(r ? V1(a, s, b.From2000) : a, s);
	}
	function l3(t, n, e) {
		const r = w(t);
		let o = gt(n, M0(r)).pos;
		return e || (o = V1(o, r, b.Into2000)), Rt(o, r);
	}
	function h3(t, n, e) {
		const r = w(t), o = gt(n, M0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
		return e ? s : a3(s, r, b.Into2000);
	}
	function f3(t, n) {
		const e = M0(t.t);
		let r = [
		...
		return n || (r = K0(r, t.t, b.From2000), r = Q0(r, t.t, b.From2000)), i3(r, e);
	}
	function d3(t, n) {
	...
	function xt(t, n, e) {
		...
		return new V2(new P(r, o, s, t.t), a, c);
	}
	function h1(t) {
		const n = g0(t.t), [e, r, o] = Q0(K0([
			...
			t.y,
			t.z
		], t.t, b.From2000), t.t, b.From2000), s = new P(e, r, o, t.t), i = n.tobl * A;
		return xt(s, Math.cos(i), Math.sin(i));
	}
	function i0(t) {
		const n = w(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = K0(r3(n, [
	...
	function J1(t) {
		const n = w(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			r * Math.cos(e.geo_eclip_lon),
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = Rt(Q0(N2(s.mobl, o), n, b.From2000), n), c = s.tobl * A, a = xt(i, Math.cos(c), Math.sin(c));
	...
	const j0 = 365250, vt = 0, zt = 1, G1 = 2;
	function Pt(t) {
	...
	function X0(t, n) {
		const e = n.tt / j0;
		return Pt(J2(P0(t[vt], e, !0), P0(t[zt], e, !1), P0(t[G1], e, !1))).ToAstroVector(n);
	}
	function d1(t, n) {
		const e = n / j0, r = P0(t[vt], e, !0), o = P0(t[zt], e, !1), s = P0(t[G1], e, !1), i = Dt(t[vt], e), c = Dt(t[zt], e), a = Dt(t[G1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), p = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, S = +(a * d) + s * l * c, E = J2(r, o, s), y = [
			p / j0,
			M / j0,
			S / j0
		];
	...
	function M3(t) {
		...
		return B1(n, t, f.Jupiter, H0), B1(n, t, f.Saturn, $0), B1(n, t, f.Uranus, J0), B1(n, t, f.Neptune, G0), n;
	}
	const Nt = 51, S3 = 29200, Z0 = 146, S0 = 201, N0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function Ct(t, n) {
		...
		return new G2(r, new Tt(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const A3 = [];
	function B2(t, n) {
	...
	function Ut(t) {
		const n = _3(t), e = new S1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new G2(e, new Tt(n.tt, r, o, s));
	}
	function p3(t, n) {
		const e = N0[0][0];
		if (n < e || n > N0[Nt - 1][0]) return null;
		const r = B2((n - e) / S3, Nt - 1);
		if (!t[r]) {
			const s = t[r] = [];
	...
	function Ot(t, n) {
		...
		const s = p3(A3, t.tt);
		if (s) {
			const i = B2((t.tt - s[0].tt) / Z0, S0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = t1(t.tt - c.tt, c.r, c.v, u), l = Lt(t.tt - c.tt, c.v, u), d = t1(t.tt - a.tt, a.r, a.v, u), p = Lt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / Z0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(p.mul(M));
		} else {
			let i;
			t.tt < N0[0][0] ? i = K2(N0[0], t.tt, -Z0) : i = K2(N0[Nt - 1], t.tt, +Z0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function y3(t, n, e) {
		...
		const M = i * h - s * l, S = -s * h - i * l, E = 1 / (1 + S), y = 1 / (1 + Math.sqrt(1 - s * s - i * i)), m = r * (h - s - y * i * M), g = r * (l - i + y * s * M), I = u * E * r * (-l - y * i * S), R = u * E * r * (+h + y * s * S), z = 2 * Math.sqrt(1 - c * c - a * a), D = 1 - 2 * a * a, T = 1 - 2 * c * c, L = 2 * a * c;
		return new V(m * D + g * L, m * L + g * T, (c * g - m * a) * z, I * D + R * L, I * L + R * T, (c * R - I * a) * z, t);
	}
	function Q1(t, n) {
		...
		return j1(E3, y3(t, n.mu, r));
	}
	function w3(t) {
		const n = new w0(t);
		return new Q2(Q1(n, K1[0]), Q1(n, K1[1]), Q1(n, K1[2]), Q1(n, K1[3]));
	}
	function a0(t, n) {
		var e = w(n);
		...
		if (t === f.SSB) return M3(e);
		const s = U1(t);
		if (s) return W1(new q0(s.dec, 15 * s.ra, s.dist), e);
	...
	function L0(t, n) {
		...
		if (e) return e.dist;
		const r = w(n);
		return t in Q ? P0(Q[t][G1], r.tt / j0, !1) : a0(t, r).Length();
	}
	function q2(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
	...
	function Y2(t, n, e, r) {
		...
		if (U1(e)) {
			const c = a0(e, o);
			if (r) {
				const u = q1(n, o), h = new P(c.x - u.x, c.y - u.y, c.z - u.z, o), l = z1 / h.Length();
		...
		for (;;) {
			...
			if (M) {
				...
				if (M.df_dt !== 0) {
					...
					if (y < p / 10) {
						let m = S.AddDays(-y), g = S.AddDays(+y);
						if ((m.ut - n.ut) * (m.ut - e.ut) < 0 && (g.ut - n.ut) * (g.ut - e.ut) < 0) {
							let I = t(m), R = t(g);
							if (I < 0 && R >= 0) {
								i = I, c = R, n = m, e = g, a = E, l = !1;
								continue;
							}
						}
					}
	...
	function D3(t, n, e) {
		let r = t * A, o = r * r, s = o * o, i = -12.717 + 1.49 * Math.abs(r) + .0431 * s, c = e / (385000.6 / U);
	...
	function Y1(t, n) {
		...
		else if (t === f.Moon) c = D3(o, u, a);
		else if (t === f.Saturn) {
			const l = I3(o, u, a, i, e);
	...
	function o1(t, n, e) {
		...
		for (let u = 0; u < 100; ++u) {
			let h = -a / 360 * i;
			if (c = c.AddDays(h), Math.abs(h) * N1 < 1) return c;
			let l = a;
	...
	function v3(t) {
		return Z2(new Date(t.time.date.getTime() + 6 * He));
	}
	var tn = class {
		constructor(t, n, e) {
	...
	function N3(t, n, e, r, o, s = 0) {
		if (!Number.isFinite(s) || s < 0) throw `Invalid value for metersAboveGround: ${s}`;
		const i = P3(t), c = nn(n.height - s);
		return en(t, n, e, r, o, i, z3(n, s) - Qe * c.density);
	}
	function T3(t, n, e, r, o, s) {
	...
	function Ft(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new L3(r, o, s, i);
		if (s >= 0 && i < 0) return null;
		if (t > 17) throw "Excessive recursion in rise/set ascent search.";
		const c = o.ut - r.ut;
		if (c * N1 < 1 || Math.min(Math.abs(s), Math.abs(i)) > e * (c / 2)) return null;
	...
	function en(t, n, e, r, o, s, i) {
		...
		const c = .42, a = C3(t, n.latitude);
		function u(S) {
			const E = l1(t, S, n, !0, !0);
			return e * ($1(S, n, E.ra, E.dec).altitude + k * Math.asin(s / E.dist) - i);
		}
		const h = w(r);
		let l = h, d = h, p = u(l), M = p;
		for (;;) {
			o < 0 ? (l = d.AddDays(-c), p = u(l)) : (d = l.AddDays(+c), M = u(d));
			const S = Ft(0, u, a, l, d, p, M);
			if (S) {
	...
	function O3(t, n, e) {
		const r = w(n), o = O2(r), s = l1(t, r, e, !0, !0);
	...
	function F3(t, n) {
		...
		for (; ++c <= 2;) {
			...
			let d = o.AddDays(l), p = o1(t, u, d), M = o1(t, h, p), S = r(p);
	...
	function ln(t, n) {
		if (n = w(n), t === f.Neptune || t === f.Pluto) return H3(t, n);
		function e(a) {
		...
		for (let a = 0; a * s < 2 * o; ++a) {
			...
			if (c * h <= 0) {
				let l, d;
				if (c < 0 || h > 0) l = e, d = m0.Pericenter;
				else if (c > 0 || h < 0) l = r, d = m0.Apocenter;
	...
	function B3(t, n) {
		const e = Ht(t);
		return e.lon = hn(e.lon), e.lat += E1(n, e.lat), e;
	}
	function K3(t, n, e) {
		n = w(n);
		const r = hn(t.lon);
		return W1(new q0(t.lat + fn(e, t.lat), r, t.dist), n);
	}
	function E1(t, n) {
		let e;
	...
	function X1(t) {
		return t = w(t), x0(b1(t, b.From2000), F1(t, b.From2000));
	}
	function q3(t) {
		const n = w(t);
		return x0(X1(n), yn(n));
	...
	function Z1(t) {
		return t = w(t), x0(F1(t, b.Into2000), b1(t, b.Into2000));
	}
	function $t(t, n) {
		t = w(t);
	...
	function e4(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Jt || (Jt = X1(new w0(-45655.74141261017)), wn = new w0(0));
		const e = W1(new q0(n, 15 * t, 1), wn), r = Vt(y1(Jt, e)), o = 10 / 240, s = o / 15;
		for (let i of n4) {
	...
	function w1(t, n, e, r) {
		...
		return new r4(n, o, U * Math.hypot(s, i, c), +a1 - (1 + o) * (a1 - t), -a1 + (1 + o) * (a1 + t), e, r);
	}
	function tt(t) {
		const n = n0(f.Sun, t, !0), e = new P(-n.x, -n.y, -n.z, n.t);
		return w1(Ge, t, i0(t), e);
	}
	function mn(t) {
	...
	function Gt(t, n) {
		const e = k2(t, n), r = n0(f.Sun, t, !0), o = i0(t), s = new P(e[0] - o.x, e[1] - o.y, e[2] - o.z, t);
	...
	function s4(t) {
		const e = J((r) => Bt(tt, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Earth shadow time.";
		return tt(e);
	}
	function i4(t) {
		const e = J((r) => Bt(mn, r), t.AddDays(-.03), t.AddDays(.03));
	...
	function In(t, n) {
		const e = new P(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = xn(Math.asin(m2 / e.Length()), Math.asin(Ke / n.Length()), C1(n, e) * A);
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				...
				if (i.r < i.p + W) {
					...
					return i.r < i.k + W && (c = X.Partial, h = Kt(i.time, i.k + W, l), i.r + W < i.k ? (c = X.Total, a = 1, u = Kt(i.time, i.k - W, h)) : a = xn(W, i.k, i.r)), new Rn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function u4(t) {
		...
		const i = X1(t.time), c = y1(i, t.dir), a = y1(i, t.target);
		...
		let M;
		if (p > 0) {
			const S = (-l - Math.sqrt(p)) / (2 * h), E = S * c.x - a.x, y = S * c.y - a.y, m = (S * c.z - a.z) * o0, g = Math.hypot(E, y) * V0;
			g == 0 ? o = m > 0 ? 90 : -90 : o = k * Math.atan(m / g);
			const I = M0(e);
			s = (k * Math.atan2(y, E) - 15 * I) % 360, s <= -180 ? s += 360 : s > 180 && (s -= 360);
			const R = s1(i);
			let z = new P(E / U, y / U, m / U, t.time);
			z = y1(R, z), z.x += t.target.x, z.y += t.target.y, z.z += t.target.z;
			const D = w1(I2, t.time, z, t.dir);
			if (D.r > 1e-9 || D.r < 0) throw `Unexpected shadow distance from geoid intersection = ${D.r}`;
			n = zn(D.k), M = n === X.Total ? 1 : In(t.dir, z);
		} else M = void 0;
		return new vn(n, M, e, r, o, s);
	}
	function l4(t) {
		return t = w(t), Dn(t.AddDays(10));
	}
	function Pn(t) {
		t = w(t);
		const n = 1.8;
	...
	function f4(t, n) {
		...
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = et(n, 1, Cn, s, t.time), h = et(n, -1, Cn, t.time, i), l = zn(t.k)) : l = X.Partial;
	...
	function M4(t, n) {
		return t = w(t), On(t.AddDays(10), n);
	}
	var bn = class {
		constructor(t, n, e, r) {
	...
	function Fn(t, n) {
		...
		for (;;) {
			...
			if (r1(t, i) < e) {
				const c = a4(t, o, i);
				if (c.r < c.p) {
					const a = c.time.AddDays(-r), u = kn(t, o, a, c.time, -1), h = c.time.AddDays(+r), l = kn(t, o, c.time, h, 1), d = 60 * r1(t, c.time);
	...
	function _4(t, n) {
		return n = w(n), Fn(t, n.AddDays(100));
	}
	var _0;
	(function(t) {
	...
	function p4(t) {
		const n = K0(Q0([
			...
			0,
			1
		], t, b.Into2000), t, b.Into2000), e = new P(n[0], n[1], n[2], t), r = Vt(e), o = 190.41375788700253 + 360.9856122880876 * t.ut;
	...
	function E4(t, n) {
		...
		switch (t) {
			...
			case f.Venus:
				...
				break;
			case f.Earth: return p4(e);
			case f.Moon:
				const d = A * (125.045 - .0529921 * r), p = A * (250.089 - .1059842 * r), M = A * (260.008 + 13.0120009 * r), S = A * (176.625 + 13.3407154 * r), E = A * (357.529 + .9856003 * r), y = A * (311.589 + 26.4057084 * r), m = A * (134.963 + 13.064993 * r), g = A * (276.617 + .3287146 * r), I = A * (34.226 + 1.7484877 * r), R = A * (15.134 - .1589763 * r), z = A * (119.743 + .0036096 * r), D = A * (239.961 + .1643573 * r), T = A * (25.053 + 12.9590088 * r);
				s = 269.9949 + .0031 * o - 3.8787 * Math.sin(d) - .1204 * Math.sin(p) + .07 * Math.sin(M) - .0172 * Math.sin(S) + .0072 * Math.sin(y) - .0052 * Math.sin(R) + .0043 * Math.sin(T), i = 66.5392 + .013 * o + 1.5419 * Math.cos(d) + .0239 * Math.cos(p) - .0278 * Math.cos(M) + .0068 * Math.cos(S) - .0029 * Math.cos(y) + 9e-4 * Math.cos(m) + 8e-4 * Math.cos(R) - 9e-4 * Math.cos(T), c = 38.3213 + (13.17635815 - 14e-13 * r) * r + 3.561 * Math.sin(d) + .1208 * Math.sin(p) - .0642 * Math.sin(M) + .0158 * Math.sin(S) + .0252 * Math.sin(E) - .0066 * Math.sin(y) - .0047 * Math.sin(m) - .0046 * Math.sin(g) + .0028 * Math.sin(I) + .0052 * Math.sin(R) + .004 * Math.sin(z) + .0019 * Math.sin(D) - .0044 * Math.sin(T);
				break;
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const L = A * (99.360714 + 4850.4046 * o), K = A * (175.895369 + 1191.9605 * o), G = A * (300.323162 + 262.5475 * o), Y = A * (114.012305 + 6070.2476 * o), c0 = A * (49.511251 + 64.3 * o);
	...
	function y4(t, n, e, r) {
		...
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new V(0, 0, 0, 0, 0, 0, o), a = f1(o)) : (c = q1(e, o), a = q1(r, o)), Jn(t, c, s, a, i);
	}
	function Jn(t, n, e, r, o) {
		...
		let S;
		if (t === 4 || t === 5) {
			const E = a * M - u * p, y = u * d - c * M, m = c * p - a * d;
			let g = y * u - m * a, I = m * c - E * u, R = E * a - y * c;
			const z = Math.sqrt(g * g + I * I + R * R);
			g /= z, I /= z, R /= z, c /= l, a /= l, u /= l;
			const D = t == 4 ? +i : -i, T = .5 * c + D * g, L = .5 * a + D * I, K = .5 * u + D * R, G = .5 * g - D * c, Y = .5 * I - D * a, c0 = .5 * R - D * u, r0 = l * T, A0 = l * L, I0 = l * K, u0 = d * c + p * a + M * u, l0 = d * g + p * I + M * R;
			S = new V(r0, A0, I0, u0 * T + l0 * G, u0 * L + l0 * Y, u0 * K + l0 * c0, n.t);
		} else {
			const E = -l * (o / (e + o)), y = +l * (e / (e + o)), m = (e + o) / (h * l);
			let g, I, R;
			if (t === 1 || t === 2) g = e / (e + o) * Math.cbrt(o / (3 * e)), I = -e, t == 1 ? (g = 1 - g, R = +o) : (g = 1 + g, R = -o);
			else if (t === 3) g = (7 / 12 * o - e) / (o + e), I = +e, R = +o;
			else throw `Invalid Langrage point ${t}. Must be an integer 1..5.`;
			let z = l * g - E, D;
			do {
				const T = z - E, L = z - y;
				D = (m * z + I / (T * T) + R / (L * L)) / (m - 2 * I / (T * T * T) - 2 * R / (L * L * L)), z -= D;
			} while (Math.abs(D / l) > 1e-14);
			g = (z - E) / l, S = new V(g * c, g * a, g * u, g * d, g * p, g * M, n.t);
		}
		return S;
	}
	var w4 = class Z {
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = j(r, n.tt, f.Mercury, At), e[f.Venus] = j(r, n.tt, f.Venus, pt), e[f.Earth] = j(r, n.tt, f.Earth, u1 + T1), e[f.Mars] = j(r, n.tt, f.Mars, Et), e[f.Jupiter] = j(r, n.tt, f.Jupiter, H0), e[f.Saturn] = j(r, n.tt, f.Saturn, $0), e[f.Uranus] = j(r, n.tt, f.Uranus, J0), e[f.Neptune] = j(r, n.tt, f.Neptune, G0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new T0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = q.zero(), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, c1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, At), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, u1 + T1), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, Et), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, H0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, $0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, J0), Z.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, G0);
		}
	...
	const { MakeTime: q4, SearchRiseSet: Y4, Body: W4, Observer: g4 } = rt(v1);
	new g4(40.7128, -74.006, 10);
	function Bn(t, n) {
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: t6, CombineRotation: n6, MakeRotation: e6 } = rt(v1);
	Math.PI / (180 * 3600);
	function m4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: s6 } = rt(v1);
	function x4(t, n) {
	...
	function T4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = I4(x4(n, { raInHours: s }).filter((S) => !Yt(S)), o, { maxDistanceLevel: e }), c = (S, E, y, m) => ({
			positions: S,
			sizes: E,
			colors: y,
			brightness: m,
		...
		if (i.length === 0) {
			...
			const S = N4(), E = S.length, y = new Float32Array(E * 3), m = new Float32Array(E).fill(Qn), g = new Float32Array(E * 3).fill(1), I = new Float32Array(E).fill(1);
			for (let R = 0; R < E; R += 1) y[R * 3] = S[R].x, y[R * 3 + 1] = S[R].y, y[R * 3 + 2] = S[R].z;
			return c(y, m, g, I);
		}
		const a = i.map(D4), u = a.length, h = new Float32Array(u * 3), l = new Float32Array(u), d = new Float32Array(u * 3), p = new Float32Array(u), M = [];
		for (let S = 0; S < u; S += 1) {
			const E = a[S], y = i[S];
			h[S * 3] = E.x, h[S * 3 + 1] = E.y, h[S * 3 + 2] = E.z;
			const m = y ? Pe(y, Kn(y)) : k0;
			l[S] = r ? m : Qn;
			const g = y?.c != null ? `#${y.c}` : void 0;
			if (r) {
				const [I, R, z] = v4(g), D = .88, T = 1 - D;
				d[S * 3] = D + T * I, d[S * 3 + 1] = D + T * R, d[S * 3 + 2] = D + T * z, p[S] = .5 + .5 * (y?.br != null && Number.isFinite(y.br) ? Math.max(0, Math.min(1, y.br)) : 1);
			} else d[S * 3] = 1, d[S * 3 + 1] = 1, d[S * 3 + 2] = 1, p[S] = 1;
			y?.proper && M.push({
				position: { ...E },
				name: y.proper,
