(function() {
	...
	M2(h3);
	25 * Mt;
	const q = 299792.458, v1 = .001, P1 = 1e3;
	1 * v1, 10 * v1, 100 * v1, 1 * P1, 10 * P1, 100 * P1, .01 * q, .1 * q, .5 * q, 1 * q, 10 * q, 100 * q, 1e3 * q, 1e4 * q, 1e5 * q, 1e6 * q, 1e7 * q, 1e8 * q, 1e9 * q;
	1 * v1;
	1e9 * q;
	1 * P1;
	function f3(t) {
	...
	const T1 = 173.1446326846693, U = 149597870.69098932, p2 = 63241.07708807546, _ = .017453292519943295, St = .26179938779914946, b = 57.29577951308232, _t = 3.819718634205488, M3 = 71492, S3 = 66854, _3 = 69911, A3 = 1821.6, E3 = 1560.8, p3 = 2631.2, y3 = 2410.3, y2 = 365.24217, w2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), n0 = 2 * Math.PI, w0 = 3600 * (180 / Math.PI), b0 = 484813681109536e-20, g2 = 10800 * 60, w3 = 2 * g2, R2 = 7292115e-11, g3 = g2 / Math.PI, R3 = -.17 - 5 * Math.log10(g3), N1 = 29.530588, L1 = 24 * 3600, m3 = L1 * 1e3, m2 = .9972695717592592, i1 = 695700, x2 = i1 / U, s0 = .996647180302104, H0 = s0 * s0, M0 = 6378.1366, x3 = M0 / U, I3 = M0 * s0, I2 = 6371, v3 = I2 + 88, P3 = 1738.1 / U, j = 1737.4, v2 = 1736, D3 = v2 / U, z3 = 34 / 60, At = 81.30056, a1 = .0002959122082855911, Et = 4912547451450812e-26, pt = 7243452486162703e-25, c1 = 8887692390113509e-25, yt = 9549535105779258e-26, V0 = 2.825345909524226e-7, F0 = 8.459715185680659e-8, G0 = 1.292024916781969e-8, K0 = 1.524358900784276e-8, T3 = 218869976542597e-26, U1 = c1 / At;
	function wt(t) {
		switch (t) {
	...
	function R0(t) {
		if (!b1 || Math.abs(b1.tt - t.tt) > 1e-6) {
			const n = H3(t), e = T2(t), o = e + n.deps / 3600;
			b1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function m0(t) {
		...
		function o(T, C, H, F) {
			const $ = [];
			for (let P0 = 0; P0 <= C - T; ++P0) $.push(e(H, F));
			return {
				min: T,
				array: $
			};
		...
		let i, a, c, u, h, l, d, A, M, E, y, g, m, w, I, x, z, P, k, N, B, J, W, u0 = o(-6, 6, 1, 4), r0 = o(-6, 6, 1, 4);
		function E0(T, C) {
			return r(u0, T, C);
		}
		function v0(T, C) {
		...
		var f0 = L(.19833 + .05611 * n), Xt = L(.27869 + .04508 * n), Zt = L(.16827 - .36903 * n), t2 = L(.34734 - 5.37261 * n), n2 = L(.10498 - 5.37899 * n), ct = L(.42681 - .41855 * n), he = L(.14943 - 5.37511 * n);
		for (P = .84 * f0 + .31 * Xt + 14.27 * Zt + 7.26 * t2 + .28 * n2 + .24 * ct, k = 2.94 * f0 + .31 * Xt + 14.27 * Zt + 9.34 * t2 + 1.12 * n2 + .83 * ct, N = -6.4 * f0 - 1.89 * ct, B = .21 * f0 + .31 * Xt + 14.27 * Zt - 88.7 * t2 - 15.3 * n2 + .24 * ct - 1.86 * he, J = P - N, A = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), m = n0 * J0(.60643382 + 1336.85522467 * n - 313e-8 * d) + P / w0, w = n0 * J0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + k / w0, I = n0 * J0(.99312619 + 99.99735956 * n - 44e-8 * d) + N / w0, x = n0 * J0(.25909118 + 1342.2278298 * n - 892e-8 * d) + B / w0, z = n0 * J0(.82736186 + 1236.85308708 * n - 397e-8 * d) + J / w0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (l0(0, h, 1), l0(1, h, Math.cos(c) * u), h0(0, h, 0), h0(1, h, Math.sin(c) * u), l = 2; l <= a; ++l) at(E0(l - 1, h), v0(l - 1, h), E0(1, h), v0(1, h), (T, C) => (l0(l, h, T), h0(l, h, C)));
			for (l = 1; l <= a; ++l) l0(-l, h, E0(l, h)), h0(-l, h, -v0(l, h));
		}
		function Bn(T, C, H, F) {
			for (var $ = {
				x: 1,
				...
				H,
				F
			], p0 = 1; p0 <= 4; ++p0) P0[p0] !== 0 && at($.x, $.y, E0(P0[p0], p0), v0(P0[p0], p0), (e2, s1) => ($.x = e2, $.y = s1));
			return $;
		}
		function S(T, C, H, F, $, P0, p0, e2) {
			var s1 = Bn($, P0, p0, e2);
		...
		E = 0, E += d0(-526.069, 0, 0, 1, -2), E += d0(-3.352, 0, 0, 1, -4), E += d0(44.297, 1, 0, 1, -2), E += d0(-6, 1, 0, 1, -4), E += d0(20.599, -1, 0, 1, 0), E += d0(-30.598, -1, 0, 1, -2), E += d0(-24.649, -2, 0, 1, 0), E += d0(-2, -2, 0, 1, -2), E += d0(-22.571, 0, 1, 1, -2), E += d0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = x + W / w0;
		let fe = (1.000002708 + 139.978 * A) * (18519.699999999997 + y) * Math.sin(i) - 6.24 * Math.sin(3 * i) + E;
		return {
	...
	function F3(t) {
		const n = p(t), e = n.tt / 36525, o = e * e, r = o * e, s = o * o, i = m0(n), a = i.geo_eclip_lon, c = i.geo_eclip_lat, u = i.distance_au * U, h = _ * 1.543, l = _ * n1(93.272095 + 483202.0175233 * e - .0036539 * o - r / 3526e3 + s / 86331e4), d = _ * n1(125.0445479 - 1934.1362891 * e + .0020754 * o + r / 467441 - s / 60616e3), A = _ * n1(357.5291092 + 35999.0502909 * e - 1536e-7 * o + r / 2449e4), M = _ * n1(134.9633964 + 477198.8675055 * e + .0087414 * o + r / 69699 - s / 14712e3), E = _ * n1(297.8501921 + 445267.1114034 * e - .0018819 * o + r / 545868 - s / 113065e3), y = 1 - .002516 * e - 74e-7 * o, g = a - d, m = Math.atan2(Math.sin(g) * Math.cos(c) * Math.cos(h) - Math.sin(c) * Math.sin(h), Math.cos(g) * Math.cos(c)), w = t1(b * (m - l)), I = Math.asin(-Math.sin(g) * Math.cos(c) * Math.sin(h) - Math.sin(c) * Math.cos(h)), x = _ * (119.75 + 131.849 * e), z = _ * (72.56 + 20.186 * e), P = -.02752 * Math.cos(M) + -.02245 * Math.sin(l) + .00684 * Math.cos(M - 2 * l) + -.00293 * Math.cos(2 * l) + -85e-5 * Math.cos(2 * l - 2 * E) + -54e-5 * Math.cos(M - 2 * E) + -2e-4 * Math.sin(M + l) + -2e-4 * Math.cos(M + 2 * l) + -2e-4 * Math.cos(M - l) + 14e-5 * Math.cos(M + 2 * l - 2 * E), k = -.02816 * Math.sin(M) + .02244 * Math.cos(l) + -.00682 * Math.sin(M - 2 * l) + -.00279 * Math.sin(2 * l) + -83e-5 * Math.sin(2 * l - 2 * E) + 69e-5 * Math.sin(M - 2 * E) + 4e-4 * Math.cos(M + l) + -25e-5 * Math.sin(2 * M) + -23e-5 * Math.sin(M + 2 * l) + 2e-4 * Math.cos(M - l) + 19e-5 * Math.sin(M - l) + 13e-5 * Math.sin(M + 2 * l - 2 * E) + -1e-4 * Math.cos(M - 3 * l), N = -(.0252 * y * Math.sin(A) + .00473 * Math.sin(2 * M - 2 * l) + -.00467 * Math.sin(M) + .00396 * Math.sin(x) + .00276 * Math.sin(2 * M - 2 * E) + .00196 * Math.sin(d) + -.00183 * Math.cos(M - l) + .00115 * Math.sin(M - 2 * E) + -96e-5 * Math.sin(M - E) + 46e-5 * Math.sin(2 * l - 2 * E) + -39e-5 * Math.sin(M - l) + -32e-5 * Math.sin(M - A - E) + 27e-5 * Math.sin(2 * M - A - 2 * E) + 23e-5 * Math.sin(z) + -14e-5 * Math.sin(2 * E) + 14e-5 * Math.cos(2 * M - 2 * l) + -12e-5 * Math.sin(M - 2 * l) + -12e-5 * Math.sin(2 * M) + 11e-5 * Math.sin(2 * M - 2 * A - 2 * E)) + (P * Math.cos(m) + k * Math.sin(m)) * Math.tan(I), B = k * Math.cos(m) - P * Math.sin(m), J = 2 * b * Math.atan(j / Math.sqrt(u * u - j * j));
	...
	function H1(t, n) {
		...
		o *= b0, r *= b0, s *= b0, i *= b0;
		const a = Math.sin(o), c = Math.cos(o), u = Math.sin(-r), h = Math.cos(-r), l = Math.sin(-s), d = Math.cos(-s), A = Math.sin(i), M = Math.cos(i), E = M * h - u * A * d, y = M * u * c + A * d * h * c - a * A * l, g = M * u * a + A * d * h * a + c * A * l, m = -A * h - u * M * d, w = -A * u * c + M * d * h * c - a * M * l, I = -A * u * a + M * d * h * a + c * M * l, x = u * l, z = -l * h * c - a * d, P = -l * h * a + d * c;
	...
	function K3(t, n) {
		...
		if (s < 1e-6) i = 0, a = r > 0 ? 90 : -90, c = Math.abs(r) - I3;
			...
			for (;;) {
				...
				const E = (H0 - 1) * M0, y = h * h, g = l * l, m = y + H0 * g;
				d = Math.sqrt(m);
				const w = E * l * h / d - r * h + s * l;
				if (Math.abs(w) < 1e-8) break;
				const I = E * ((y - g) / d - g * y * (H0 - 1) / (E * m)) + r * l + s * h;
				u -= w / I;
			}
			a = b * u;
			const M = M0 / d;
	...
	function F1(t, n) {
		const e = R0(t), o = e.mobl * _, r = e.tobl * _, s = e.dpsi * b0, i = Math.cos(o), a = Math.sin(o), c = Math.cos(r), u = Math.sin(r), h = Math.cos(s), l = Math.sin(s), d = h, A = -l * i, M = -l * a, E = l * c, y = h * i * c + a * u, g = h * a * c - i * u, m = l * u, w = h * i * u - a * c, I = h * a * u + i * c;
		if (n === O.From2000) return new G([
			[
				d,
				E,
	...
	function J1(t, n, e, o, r) {
		...
		const g = -15 * S0(s);
		let m = Y0(g, M), w = Y0(g, E), I = Y0(g, y), x = [
			l * A,
			l * d,
			h
		];
		...
		if (r) {
			let r0 = J, E0 = E1(r, 90 - J);
			if (J -= E0, E0 > 0 && J > 3e-4) {
				const v0 = Math.sin(J * _), l0 = Math.cos(J * _), h0 = Math.sin(r0 * _), at = Math.cos(r0 * _), L = [];
				for (let f0 = 0; f0 < 3; ++f0) L.push((x[f0] - at * m[f0]) / h0 * v0 + m[f0] * l0);
	...
	function K2(t) {
		const n = p(t).AddDays(-1 / T1), e = j0(Q.Earth, n), [o, r, s] = G1([
			-e.x,
			-e.y,
			-e.z
	...
	function u1(t, n, e, o, r) {
		q0(e), C1(o), C1(r);
		const s = p(n), i = H2(s, e), a = e0(t, s, r), c = [
			a.x - i[0],
			a.y - i[1],
			a.z - i[2]
		];
	...
	function Y3(t, n, e) {
		const o = p(t), r = Rt(n, S0(o)), s = new V(r.pos[0], r.pos[1], r.pos[2], r.vel[0], r.vel[1], r.vel[2], o);
	...
	function q3(t, n) {
		...
		return n || (o = $0(o, t.t, O.From2000), o = B0(o, t.t, O.From2000)), K3(o, e);
	}
	function W3(t, n) {
	...
	function a0(t) {
		const n = p(t), e = m0(n), o = e.distance_au * Math.cos(e.geo_eclip_lat), r = $0(V3(n, [
	...
	function $1(t) {
		const n = p(t), e = m0(n), o = e.distance_au * Math.cos(e.geo_eclip_lat), r = [
			o * Math.cos(e.geo_eclip_lon),
			o * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
	...
	function h1(t) {
		const n = p(t), e = 1e-5, o = n.AddDays(-e), r = n.AddDays(+e), s = a0(o), i = a0(r);
		return new V((s.x + i.x) / 2, (s.y + i.y) / 2, (s.z + i.z) / 2, (i.x - s.x) / (2 * e), (i.y - s.y) / (2 * e), (i.z - s.z) / (2 * e), n);
	}
	function vt(t) {
		const n = p(t), e = h1(n), o = 1 + At;
	...
	function f1(t, n) {
		const e = n / W0, o = z0(t[Dt], e, !0), r = z0(t[zt], e, !1), s = z0(t[B1], e, !1), i = Pt(t[Dt], e), a = Pt(t[zt], e), c = Pt(t[B1], e), u = Math.cos(o), h = Math.sin(o), l = Math.cos(r), d = Math.sin(r), A = +(c * l * u) - s * d * u * a - s * l * h * i, M = +(c * l * h) - s * d * h * a + s * l * u * i, E = +(c * d) + s * l * a, y = J2(o, r, s), g = [
			A / W0,
			M / W0,
			E / W0
		];
	...
	function o4(t, n, e) {
		...
		const M = i * h - s * l, E = -s * h - i * l, y = 1 / (1 + E), g = 1 / (1 + Math.sqrt(1 - s * s - i * i)), m = o * (h - s - g * i * M), w = o * (l - i + g * s * M), I = u * y * o * (-l - g * i * E), x = u * y * o * (+h + g * s * E), z = 2 * Math.sqrt(1 - a * a - c * c), P = 1 - 2 * c * c, k = 1 - 2 * a * a, N = 2 * c * a;
	...
	function K(t, n, e, o) {
		const r = v(o && o.dt_tolerance_seconds || 1), s = Math.abs(r / L1);
		...
		for (;;) {
			...
			if (M) {
				let E = p(M.t), y = t(E);
				if (M.df_dt !== 0) {
					if (Math.abs(y / M.df_dt) < s) return E;
					let g = 1.2 * Math.abs(y / M.df_dt);
					if (g < A / 10) {
						let m = E.AddDays(-g), w = E.AddDays(+g);
	...
	function bt(t, n, e) {
		...
		const o = p(e), r = l1(e0(t, o, !1)), s = l1(e0(n, o, !1));
	...
	function on(t, n, e, o, r, s, i) {
		...
		function u(E) {
			const y = u1(t, E, n, !0, !0);
			return e * (J1(E, n, y.ra, y.dec).altitude + b * Math.asin(s / y.dist) - i);
		}
		const h = p(o);
		let l = h, d = h, A = u(l), M = A;
		for (;;) {
			r < 0 ? (l = d.AddDays(-a), A = u(l)) : (d = l.AddDays(+a), M = u(d));
			const E = Vt(0, u, c, l, d, A, M);
			if (E) {
				const y = K(u, E.tx, E.ty, {
	...
	function p4(t, n, e) {
		const o = p(n), r = k2(o), s = u1(t, o, e, !0, !0);
	...
	function g4(t, n) {
		...
		for (; ++a <= 2;) {
			...
			let d = r.AddDays(l), A = o1(t, u, d), M = o1(t, h, A), E = o(A);
	...
	function hn(t, n) {
		if (n = p(n), t === f.Neptune || t === f.Pluto) return m4(t, n);
		function e(c) {
	...
	function tt(t) {
		return t = p(t), I0(H1(t, O.From2000), F1(t, O.From2000));
	}
	function T4(t) {
		const n = p(t);
		return I0(tt(n), wn(n));
	...
	function nt(t) {
		return t = p(t), I0(F1(t, O.Into2000), H1(t, O.Into2000));
	}
	function Kt(t, n) {
		t = p(t);
	...
	function yn(t) {
		const n = R0(p(t)).tobl * _, e = Math.cos(n), o = Math.sin(n);
		return new G([
			[
				1,
				0,
	...
	function H4(t, n) {
		...
		const e = X1(new Q0(n, 15 * t, 1), gn), o = Ft(p1(Jt, e)), r = 10 / 240, s = r / 15;
		for (let i of b4) {
	...
	function y1(t, n, e, o) {
		...
		return new V4(n, r, U * Math.hypot(s, i, a), +i1 - (1 + r) * (i1 - t), -i1 + (1 + r) * (i1 + t), e, o);
	}
	function et(t) {
	...
	function J4(t, n, e) {
		const r = K((s) => F4(t, n, s), e.AddDays(-1), e.AddDays(1));
	...
	function vn(t, n) {
		const e = new D(t.x + n.x, t.y + n.y, t.z + n.z, t.t), o = In(Math.asin(x2 / e.Length()), Math.asin(D3 / n.Length()), O1(n, e) * _);
	...
	function B4(t) {
		...
		const i = tt(t.time), a = p1(i, t.dir), c = p1(i, t.target);
		...
		let M;
		if (A > 0) {
			const E = (-l - Math.sqrt(A)) / (2 * h), y = E * a.x - c.x, g = E * a.y - c.y, m = (E * a.z - c.z) * s0, w = Math.hypot(y, g) * H0;
	...
	function j4(t, n) {
		return t = p(t), kn(t.AddDays(10), n);
	}
	var bn = class {
		constructor(t, n, e, o) {
	...
	function Z4(t, n) {
		return n = p(n), Vn(t, n.AddDays(100));
	}
	var A0;
	(function(t) {
	...
	function ee(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
				...
				s = 269.9949 + .0031 * r - 3.8787 * Math.sin(d) - .1204 * Math.sin(A) + .07 * Math.sin(M) - .0172 * Math.sin(E) + .0072 * Math.sin(g) - .0052 * Math.sin(x) + .0043 * Math.sin(k), i = 66.5392 + .013 * r + 1.5419 * Math.cos(d) + .0239 * Math.cos(A) - .0278 * Math.cos(M) + .0068 * Math.cos(E) - .0029 * Math.cos(g) + 9e-4 * Math.cos(m) + 8e-4 * Math.cos(x) - 9e-4 * Math.cos(k), a = 38.3213 + (13.17635815 - 14e-13 * o) * o + 3.561 * Math.sin(d) + .1208 * Math.sin(A) - .0642 * Math.sin(M) + .0158 * Math.sin(E) + .0252 * Math.sin(y) - .0066 * Math.sin(g) - .0047 * Math.sin(m) - .0046 * Math.sin(w) + .0028 * Math.sin(I) + .0052 * Math.sin(x) + .004 * Math.sin(z) + .0019 * Math.sin(P) - .0044 * Math.sin(k);
				break;
			case f.Mars:
	...
	function Jn(t, n, e, o, r) {
		...
		if (t === 4 || t === 5) {
			...
			const P = t == 4 ? +i : -i, k = .5 * a + P * w, N = .5 * c + P * I, B = .5 * u + P * x, J = .5 * w - P * a, W = .5 * I - P * c, u0 = .5 * x - P * u, r0 = l * k, E0 = l * N, v0 = l * B, l0 = d * a + A * c + M * u, h0 = d * w + A * I + M * x;
			E = new V(r0, E0, v0, l0 * k + h0 * J, l0 * N + h0 * W, l0 * B + h0 * u0, n.t);
		} else {
			...
			w = (z - y) / l, E = new V(w * a, w * c, w * u, w * d, w * A, w * M, n.t);
		}
		return E;
	}
	var re = class t0 {
		...
		Update(n) {
			const e = p(n), o = e.tt - this.curr.time.tt;
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = X(o, n.tt, f.Mercury, Et), e[f.Venus] = X(o, n.tt, f.Venus, pt), e[f.Earth] = X(o, n.tt, f.Earth, c1 + U1), e[f.Mars] = X(o, n.tt, f.Mars, yt), e[f.Jupiter] = X(o, n.tt, f.Jupiter, V0), e[f.Saturn] = X(o, n.tt, f.Saturn, F0), e[f.Uranus] = X(o, n.tt, f.Uranus, G0), e[f.Neptune] = X(o, n.tt, f.Neptune, K0);
			for (let r in e) e[r].r.decr(o.r), e[r].v.decr(o.v);
			return e[f.Sun] = new N0(n.tt, o.r.neg(), o.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = Y.zero(), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, a1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, Et), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, pt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, c1 + U1), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, yt), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, V0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, F0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, G0), t0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, K0);
		}
	...
	const { MakeTime: ke, SearchRiseSet: be, Body: He, Observer: se } = st(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: Ke, CombineRotation: Je, MakeRotation: $e } = st(z1);
	Math.PI / (180 * 3600);
	function it(t, n) {
