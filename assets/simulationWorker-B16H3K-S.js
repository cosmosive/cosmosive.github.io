(function() {
	var u2 = Object.defineProperty, ne = (t, n) => {
		let e = {};
	...
	const l2 = Symbol("Comlink.proxy"), ee = Symbol("Comlink.endpoint"), re = Symbol("Comlink.releaseProxy"), lt = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), h2 = (t) => typeof t == "object" && t !== null || typeof t == "function", f2 = new Map([["proxy", {
	...
	function ht(t, n = globalThis, e = ["*"]) {
		n.addEventListener("message", function r(o) {
			...
			const { id: s, type: i, path: a } = Object.assign({ path: [] }, o.data), c = (o.data.argumentList || []).map(T0);
			let u;
			try {
				...
				switch (i) {
					case "GET":
						...
						break;
					case "SET":
						h[a.slice(-1)[0]] = T0(o.data.value), u = !0;
						break;
					case "APPLY":
						u = l.apply(h, c);
						break;
			...
			Promise.resolve(u).catch((h) => ({
				...
				const [l, d] = v1(h);
				n.postMessage(Object.assign(Object.assign({}, l), { id: s }), d), i === "RELEASE" && (n.removeEventListener("message", r), d2(n), lt in t && typeof t[lt] == "function" && t[lt]());
			}).catch((h) => {
				const [l, d] = v1({
	...
	function ft(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, a, c) {
				...
				if (u === ee) return k0(t, n, { type: "ENDPOINT" }).then(T0);
	...
	function S2(t) {
		const n = t.map(v1);
		return [n.map((e) => e[0]), ue(n.map((e) => e[1]))];
	}
	...
	const p2 = 1e3 / I1, Mt = I1 * p2;
	100 * Mt;
	100 / dt;
	const A2 = 3.2615637771694, St = Mt * dt;
	25 * St;
	function E2(t) {
		return !Number.isFinite(t) || t < 0 ? 0 : t * St;
	}
	const Se = 5e4, _e = 2e3, pe = Se / A2, Ae = _e / A2;
	E2(pe);
	E2(Ae);
	25 * St;
	const j = 299792.458, D1 = .001, P1 = 1e3;
	1 * D1, 10 * D1, 100 * D1, 1 * P1, 10 * P1, 100 * P1, .01 * j, .1 * j, .5 * j, 1 * j, 10 * j, 100 * j, 1e3 * j, 1e4 * j, 1e5 * j, 1e6 * j, 1e7 * j, 1e8 * j, 1e9 * j;
	1 * D1;
	1e9 * j;
	1 * P1;
	const _t = 1721545, pt = 3181545;
	function Ee(t) {
		return {
			name: t.name,
			type: t.type,
	...
	const y2 = Object.fromEntries(Object.entries(Me).map(([t, n]) => [t, Ee(n)]));
	R.SUN, R.MERCURY, R.VENUS, R.EARTH, R.MARS, R.JUPITER, R.SATURN, R.URANUS, R.NEPTUNE, R.PLUTO, R.MOON;
	const w2 = y2[R.EARTH]?.radiusKm ?? 6371, g2 = y2[R.SUN]?.radiusKm ?? 696340;
	w2 / g2;
	function R2(t) {
		return !Number.isFinite(t) || t <= 0 ? 0 : t * p2;
	}
	R2(w2);
	R2(g2);
	var T1 = ne({
		...
		TransitInfo: () => $n,
		Vector: () => P,
		VectorFromHorizon: () => U3,
		VectorFromSphere: () => X1,
		VectorObserver: () => n3,
		e_tilt: () => m0
	});
	const z1 = 173.1446326846693, C = 149597870.69098932, m2 = 63241.07708807546, _ = .017453292519943295, At = .26179938779914946, b = 57.29577951308232, Et = 3.819718634205488, ye = 71492, we = 66854, ge = 69911, Re = 1821.6, me = 1560.8, xe = 2631.2, ve = 2410.3, x2 = 365.24217, v2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), e0 = 2 * Math.PI, g0 = 3600 * (180 / Math.PI), b0 = 484813681109536e-20, I2 = 10800 * 60, Ie = 2 * I2, D2 = 7292115e-11, De = I2 / Math.PI, Pe = -.17 - 5 * Math.log10(De), N1 = 29.530588, L1 = 24 * 3600, Te = L1 * 1e3, P2 = .9972695717592592, i1 = 695700, T2 = i1 / C, i0 = .996647180302104, V0 = i0 * i0, S0 = 6378.1366, ze = S0 / C, Ne = S0 * i0, z2 = 6371, Le = z2 + 88, Ce = 1738.1 / C, W = 1737.4, N2 = 1736, Ue = N2 / C, Oe = 34 / 60, yt = 81.30056, a1 = .0002959122082855911, wt = 4912547451450812e-26, gt = 7243452486162703e-25, c1 = 8887692390113509e-25, Rt = 9549535105779258e-26, H0 = 2.825345909524226e-7, F0 = 8.459715185680659e-8, J0 = 1.292024916781969e-8, $0 = 1.524358900784276e-8, ke = 218869976542597e-26, C1 = c1 / yt;
	function mt(t) {
		switch (t) {
	...
	var R0 = class ut {
		constructor(n) {
			...
			if (n instanceof Date && Number.isFinite(n.getTime())) {
				this.date = n, this.ut = (n.getTime() - v2.getTime()) / e, this.tt = U2(this.ut);
				return;
			}
			if (Number.isFinite(n)) {
				this.date = new Date(v2.getTime() + n * e), this.ut = n, this.tt = U2(this.ut);
				return;
			}
	...
	function m0(t) {
		if (!b1 || Math.abs(b1.tt - t.tt) > 1e-6) {
			const n = Ge(t), e = O2(t), r = e + n.deps / 3600;
			b1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function x0(t) {
		...
		function r(z, U, V, F) {
			const G = [];
			for (let P0 = 0; P0 <= U - z; ++P0) G.push(e(V, F));
			return {
				min: z,
				array: G
			};
		...
		function o(z, U, V) {
			const F = z.array[U - z.min];
			return F.array[V - F.min];
		}
		function s(z, U, V, F) {
			const G = z.array[U - z.min];
			G.array[V - G.min] = F;
		}
		let i, a, c, u, h, l, d, p, M, A, y, g, m, w, v, x, T, D, k, N, B, K, Y, l0 = r(-6, 6, 1, 4), s0 = r(-6, 6, 1, 4);
		function E0(z, U) {
			return o(l0, z, U);
		}
		function D0(z, U) {
		...
		var d0 = L(.19833 + .05611 * n), r2 = L(.27869 + .04508 * n), o2 = L(.16827 - .36903 * n), s2 = L(.34734 - 5.37261 * n), i2 = L(.10498 - 5.37899 * n), ct = L(.42681 - .41855 * n), p4 = L(.14943 - 5.37511 * n);
		for (D = .84 * d0 + .31 * r2 + 14.27 * o2 + 7.26 * s2 + .28 * i2 + .24 * ct, k = 2.94 * d0 + .31 * r2 + 14.27 * o2 + 9.34 * s2 + 1.12 * i2 + .83 * ct, N = -6.4 * d0 - 1.89 * ct, B = .21 * d0 + .31 * r2 + 14.27 * o2 - 88.7 * s2 - 15.3 * i2 + .24 * ct - 1.86 * p4, K = D - N, p = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), m = e0 * K0(.60643382 + 1336.85522467 * n - 313e-8 * d) + D / g0, w = e0 * K0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + k / g0, v = e0 * K0(.99312619 + 99.99735956 * n - 44e-8 * d) + N / g0, x = e0 * K0(.25909118 + 1342.2278298 * n - 892e-8 * d) + B / g0, T = e0 * K0(.82736186 + 1236.85308708 * n - 397e-8 * d) + K / g0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (h0(0, h, 1), h0(1, h, Math.cos(c) * u), f0(0, h, 0), f0(1, h, Math.sin(c) * u), l = 2; l <= a; ++l) at(E0(l - 1, h), D0(l - 1, h), E0(1, h), D0(1, h), (z, U) => (h0(l, h, z), f0(l, h, U)));
			for (l = 1; l <= a; ++l) h0(-l, h, E0(l, h)), f0(-l, h, -D0(l, h));
		}
		function Zn(z, U, V, F) {
			for (var G = {
				x: 1,
				...
				V,
				F
			], y0 = 1; y0 <= 4; ++y0) P0[y0] !== 0 && at(G.x, G.y, E0(P0[y0], y0), D0(P0[y0], y0), (a2, s1) => (G.x = a2, G.y = s1));
			return G;
		}
		function S(z, U, V, F, G, P0, y0, a2) {
			var s1 = Zn(G, P0, y0, a2);
			M += z * s1.y, Y += U * s1.y, y += V * s1.x, g += F * s1.x;
		}
		...
		function M0(z, U, V, F, G) {
			return z * Zn(U, V, F, G).y;
		}
		A = 0, A += M0(-526.069, 0, 0, 1, -2), A += M0(-3.352, 0, 0, 1, -4), A += M0(44.297, 1, 0, 1, -2), A += M0(-6, 1, 0, 1, -4), A += M0(20.599, -1, 0, 1, 0), A += M0(-30.598, -1, 0, 1, -2), A += M0(-24.649, -2, 0, 1, 0), A += M0(-2, -2, 0, 1, -2), A += M0(-22.571, 0, 1, 1, -2), A += M0(10.985, 0, -1, 1, -2), M += .82 * L(.7736 - 62.5512 * n) + .31 * L(.0466 - 125.1025 * n) + .35 * L(.5785 - 25.1042 * n) + .66 * L(.4591 + 1335.8075 * n) + .64 * L(.313 - 91.568 * n) + 1.14 * L(.148 + 1331.2898 * n) + .21 * L(.5918 + 1056.5859 * n) + .44 * L(.5784 + 1322.8595 * n) + .24 * L(.2275 - 5.7374 * n) + .28 * L(.2965 + 2.6929 * n) + .33 * L(.3132 + 6.3368 * n), i = x + Y / g0;
		let A4 = (1.000002708 + 139.978 * p) * (18519.699999999997 + y) * Math.sin(i) - 6.24 * Math.sin(3 * i) + A;
		return {
			geo_eclip_lon: e0 * K0((m + M / g0) / e0),
			geo_eclip_lat: Math.PI / (180 * 3600) * A4,
			distance_au: g0 * ze / (.999953253 * g)
		};
	}
	var V2 = class {
	...
	function Qe(t) {
		const n = E(t), e = n.tt / 36525, r = e * e, o = r * e, s = r * r, i = x0(n), a = i.geo_eclip_lon, c = i.geo_eclip_lat, u = i.distance_au * C, h = _ * 1.543, l = _ * n1(93.272095 + 483202.0175233 * e - .0036539 * r - o / 3526e3 + s / 86331e4), d = _ * n1(125.0445479 - 1934.1362891 * e + .0020754 * r + o / 467441 - s / 60616e3), p = _ * n1(357.5291092 + 35999.0502909 * e - 1536e-7 * r + o / 2449e4), M = _ * n1(134.9633964 + 477198.8675055 * e + .0087414 * r + o / 69699 - s / 14712e3), A = _ * n1(297.8501921 + 445267.1114034 * e - .0018819 * r + o / 545868 - s / 113065e3), y = 1 - .002516 * e - 74e-7 * r, g = a - d, m = Math.atan2(Math.sin(g) * Math.cos(c) * Math.cos(h) - Math.sin(c) * Math.sin(h), Math.cos(g) * Math.cos(c)), w = t1(b * (m - l)), v = Math.asin(-Math.sin(g) * Math.cos(c) * Math.sin(h) - Math.sin(c) * Math.cos(h)), x = _ * (119.75 + 131.849 * e), T = _ * (72.56 + 20.186 * e), D = -.02752 * Math.cos(M) + -.02245 * Math.sin(l) + .00684 * Math.cos(M - 2 * l) + -.00293 * Math.cos(2 * l) + -85e-5 * Math.cos(2 * l - 2 * A) + -54e-5 * Math.cos(M - 2 * A) + -2e-4 * Math.sin(M + l) + -2e-4 * Math.cos(M + 2 * l) + -2e-4 * Math.cos(M - l) + 14e-5 * Math.cos(M + 2 * l - 2 * A), k = -.02816 * Math.sin(M) + .02244 * Math.cos(l) + -.00682 * Math.sin(M - 2 * l) + -.00279 * Math.sin(2 * l) + -83e-5 * Math.sin(2 * l - 2 * A) + 69e-5 * Math.sin(M - 2 * A) + 4e-4 * Math.cos(M + l) + -25e-5 * Math.sin(2 * M) + -23e-5 * Math.sin(M + 2 * l) + 2e-4 * Math.cos(M - l) + 19e-5 * Math.sin(M - l) + 13e-5 * Math.sin(M + 2 * l - 2 * A) + -1e-4 * Math.cos(M - 3 * l), N = -(.0252 * y * Math.sin(p) + .00473 * Math.sin(2 * M - 2 * l) + -.00467 * Math.sin(M) + .00396 * Math.sin(x) + .00276 * Math.sin(2 * M - 2 * A) + .00196 * Math.sin(d) + -.00183 * Math.cos(M - l) + .00115 * Math.sin(M - 2 * A) + -96e-5 * Math.sin(M - A) + 46e-5 * Math.sin(2 * l - 2 * A) + -39e-5 * Math.sin(M - l) + -32e-5 * Math.sin(M - p - A) + 27e-5 * Math.sin(2 * M - p - 2 * A) + 23e-5 * Math.sin(T) + -14e-5 * Math.sin(2 * A) + 14e-5 * Math.cos(2 * M - 2 * l) + -12e-5 * Math.sin(M - 2 * l) + -12e-5 * Math.sin(2 * M) + 11e-5 * Math.sin(2 * M - 2 * p - 2 * A)) + (D * Math.cos(m) + k * Math.sin(m)) * Math.tan(v), B = k * Math.cos(m) - D * Math.sin(m), K = 2 * b * Math.atan(W / Math.sqrt(u * u - W * W));
		return new V2(b * v + B, w + N, b * c, b * a, u, K);
	}
	function H2(t, n) {
		return [
	...
	function V1(t, n) {
		...
		r *= b0, o *= b0, s *= b0, i *= b0;
		const a = Math.sin(r), c = Math.cos(r), u = Math.sin(-o), h = Math.cos(-o), l = Math.sin(-s), d = Math.cos(-s), p = Math.sin(i), M = Math.cos(i), A = M * h - u * p * d, y = M * u * c + p * d * h * c - a * p * l, g = M * u * a + p * d * h * a + c * p * l, m = -p * h - u * M * d, w = -p * u * c + M * d * h * c - a * M * l, v = -p * u * a + M * d * h * a + c * M * l, x = u * l, T = -l * h * c - a * d, D = -l * h * a + d * c;
	...
	function je(t, n) {
		...
		let i, a, c;
		if (s < 1e-6) i = 0, a = o > 0 ? 90 : -90, c = Math.abs(o) - Ne;
		else {
			...
			for (;;) {
				...
				const A = (V0 - 1) * S0, y = h * h, g = l * l, m = y + V0 * g;
				d = Math.sqrt(m);
				const w = A * l * h / d - o * h + s * l;
				if (Math.abs(w) < 1e-8) break;
				const v = A * ((y - g) / d - g * y * (V0 - 1) / (A * m)) + o * l + s * h;
				u -= w / v;
			}
			a = b * u;
			const M = S0 / d;
			Math.abs(l) > Math.abs(h) ? c = o / l - V0 * M : c = s / h - M;
		}
		return new Dt(a, i, 1e3 * c);
	}
	function vt(t, n) {
		const e = t.latitude * _, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, i0 * r), i = V0 * s, a = t.height / 1e3, c = S0 * s + a, u = S0 * i + a, h = (15 * n + t.longitude) * _, l = Math.sin(h), d = Math.cos(h);
		return {
			pos: [
				c * o * d / C,
				c * o * l / C,
	...
	function F1(t, n) {
		const e = m0(t), r = e.mobl * _, o = e.tobl * _, s = e.dpsi * b0, i = Math.cos(r), a = Math.sin(r), c = Math.cos(o), u = Math.sin(o), h = Math.cos(s), l = Math.sin(s), d = h, p = -l * i, M = -l * a, A = l * c, y = h * i * c + a * u, g = h * a * c - i * u, m = l * u, w = h * i * u - a * c, v = h * a * u + i * c;
		if (n === O.From2000) return new J([
			[
				d,
				A,
	...
	function Ye(t, n, e) {
		return e === O.Into2000 ? F2($2(t, n, e), n, e) : $2(F2(t, n, e), n, e);
	}
	function K2(t, n) {
		const e = vt(n, _0(t)).pos;
		return J1(e, t, O.Into2000);
	}
	var P = class {
		constructor(t, n, e, r) {
	...
	function Xe(t) {
		if (!We(t)) throw "Argument must be a [3][3] array of numbers";
		return new J(t);
	}
	var G2 = class {
		constructor(t, n, e, r) {
	...
	function K1(t, n, e, r, o) {
		let s = E(t);
		j0(n), I(e), I(r);
		const i = Math.sin(n.latitude * _), a = Math.cos(n.latitude * _), c = Math.sin(n.longitude * _), u = Math.cos(n.longitude * _), h = Math.sin(r * _), l = Math.cos(r * _), d = Math.sin(e * At), p = Math.cos(e * At);
		let M = [
			a * u,
			a * c,
			i
		...
		const g = -15 * _0(s);
		let m = q0(g, M), w = q0(g, A), v = q0(g, y), x = [
			l * p,
			l * d,
			h
		];
		const T = x[0] * m[0] + x[1] * m[1] + x[2] * m[2], D = x[0] * w[0] + x[1] * w[1] + x[2] * w[2], k = x[0] * v[0] + x[1] * v[1] + x[2] * v[2];
		let N = Math.hypot(D, k), B;
		N > 0 ? (B = -b * Math.atan2(k, D), B < 0 && (B += 360)) : B = 0;
		let K = b * Math.atan2(N, T), Y = e, l0 = r;
		if (o) {
			let s0 = K, E0 = A1(o, 90 - K);
			if (K -= E0, E0 > 0 && K > 3e-4) {
				const D0 = Math.sin(K * _), h0 = Math.cos(K * _), f0 = Math.sin(s0 * _), at = Math.cos(s0 * _), L = [];
				for (let d0 = 0; d0 < 3; ++d0) L.push((x[d0] - at * m[d0]) / f0 * D0 + m[d0] * h0);
				N = Math.hypot(L[0], L[1]), N > 0 ? (Y = Et * Math.atan2(L[1], L[0]), Y < 0 && (Y += 24)) : Y = 0, l0 = b * Math.atan2(L[2], N);
			}
		}
		return new G2(B, 90 - K, Y, l0);
	}
	function j0(t) {
	...
	function q2(t) {
		const n = E(t).AddDays(-1 / z1), e = W0(Q.Earth, n), [r, o, s] = J1([
			-e.x,
			-e.y,
			-e.z
		], n, O.From2000), i = _ * m0(n).tobl, a = Math.cos(i), c = Math.sin(i);
		return Pt(new P(r, o, s, n), a, c);
	}
	function u1(t, n, e, r, o) {
		j0(e), U1(r), U1(o);
		const s = E(n), i = K2(s, e), a = r0(t, s, o), c = [
			a.x - i[0],
			a.y - i[1],
			a.z - i[2]
		];
		return Q2(r ? J1(c, s, O.From2000) : c, s);
	}
	function Ze(t, n, e) {
		const r = E(t);
		let o = vt(n, _0(r)).pos;
		return e || (o = J1(o, r, O.Into2000)), It(o, r);
	}
	function t3(t, n, e) {
		const r = E(t), o = vt(n, _0(r)), s = new H(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
	...
	function n3(t, n) {
		...
		return n || (r = G0(r, t.t, O.From2000), r = B0(r, t.t, O.From2000)), je(r, e);
	}
	function e3(t, n) {
	...
	function Pt(t, n, e) {
		...
		return new B2(new P(r, o, s, t.t), c, a);
	}
	function l1(t) {
		const n = m0(t.t), [e, r, o] = B0(G0([
			...
			t.y,
			t.z
		], t.t, O.From2000), t.t, O.From2000), s = new P(e, r, o, t.t), i = n.tobl * _;
		return Pt(s, Math.cos(i), Math.sin(i));
	}
	function c0(t) {
		const n = E(t), e = x0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = G0(Be(n, [
	...
	function G1(t) {
		const n = E(t), e = x0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = m0(n), i = It(B0(k2(s.mobl, o), n, O.From2000), n), a = s.tobl * _, c = Pt(i, Math.cos(a), Math.sin(a));
		return new Q0(c.elat, c.elon, e.distance_au);
	}
	function h1(t) {
		const n = E(t), e = 1e-5, r = n.AddDays(-e), o = n.AddDays(+e), s = c0(r), i = c0(o);
		return new H((s.x + i.x) / 2, (s.y + i.y) / 2, (s.z + i.z) / 2, (i.x - s.x) / (2 * e), (i.y - s.y) / (2 * e), (i.z - s.z) / (2 * e), n);
	}
	function Tt(t) {
		const n = E(t), e = h1(n), r = 1 + yt;
	...
	const Y0 = 365250, Nt = 0, Lt = 1, B1 = 2;
	function Ct(t) {
	...
	function W0(t, n) {
		const e = n.tt / Y0;
		return Ct(j2(z0(t[Nt], e, !0), z0(t[Lt], e, !1), z0(t[B1], e, !1))).ToAstroVector(n);
	}
	function f1(t, n) {
		const e = n / Y0, r = z0(t[Nt], e, !0), o = z0(t[Lt], e, !1), s = z0(t[B1], e, !1), i = zt(t[Nt], e), a = zt(t[Lt], e), c = zt(t[B1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), p = +(c * l * u) - s * d * u * a - s * l * h * i, M = +(c * l * h) - s * d * h * a + s * l * u * i, A = +(c * d) + s * l * a, y = j2(r, o, s), g = [
			p / Y0,
			M / Y0,
			A / Y0
		];
		return new L0(n, Ct(y), Ct(g));
	}
	function Q1(t, n, e, r) {
	...
	function r3(t) {
		...
		return Q1(n, t, f.Jupiter, H0), Q1(n, t, f.Saturn, F0), Q1(n, t, f.Uranus, J0), Q1(n, t, f.Neptune, $0), n;
	}
	const Ut = 51, o3 = 29200, X0 = 146, p0 = 201, N0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function s3(t) {
		let [n, [e, r, o], [s, i, a]] = t;
		return new L0(n, new q(e, r, o), new q(s, i, a));
	}
	function X(t, n, e, r) {
	...
	var M1 = class {
		constructor(t) {
			...
			this.Jupiter = X(n, t, f.Jupiter, H0), this.Saturn = X(n, t, f.Saturn, F0), this.Uranus = X(n, t, f.Uranus, J0), this.Neptune = X(n, t, f.Neptune, $0), this.Jupiter.r.decr(n.r), this.Jupiter.v.decr(n.v), this.Saturn.r.decr(n.r), this.Saturn.v.decr(n.v), this.Uranus.r.decr(n.r), this.Uranus.v.decr(n.v), this.Neptune.r.decr(n.r), this.Neptune.v.decr(n.v), this.Sun = new L0(t, n.r.mul(-1), n.v.mul(-1));
		}
		Acceleration(t) {
			let n = d1(t, a1, this.Sun.r);
			return n.incr(d1(t, H0, this.Jupiter.r)), n.incr(d1(t, F0, this.Saturn.r)), n.incr(d1(t, J0, this.Uranus.r)), n.incr(d1(t, $0, this.Neptune.r)), n;
		}
	}, Ot = class te {
		constructor(n, e, r, o) {
		...
		clone() {
			return new te(this.tt, this.r.clone(), this.v.clone(), this.a.clone());
		}
	}, Y2 = class {
		constructor(t, n) {
	...
	function bt(t, n) {
		...
		return new Y2(r, new Ot(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const i3 = [];
	function W2(t, n) {
	...
	function Vt(t) {
		const n = s3(t), e = new M1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new Y2(e, new Ot(n.tt, r, o, s));
	}
	function a3(t, n) {
		const e = N0[0][0];
		if (n < e || n > N0[Ut - 1][0]) return null;
		const r = W2((n - e) / o3, Ut - 1);
		if (!t[r]) {
			const s = t[r] = [];
			s[0] = Vt(N0[r]).grav, s[p0 - 1] = Vt(N0[r + 1]).grav;
			let i, a = s[0].tt;
			for (i = 1; i < p0 - 1; ++i) s[i] = bt(a += X0, s[i - 1]).grav;
			a = s[p0 - 1].tt;
			var o = [];
			for (o[p0 - 1] = s[p0 - 1], i = p0 - 2; i > 0; --i) o[i] = bt(a -= X0, o[i + 1]).grav;
			for (i = p0 - 2; i > 0; --i) {
				const c = i / (p0 - 1);
	...
	function Ht(t, n) {
		...
		const s = a3(i3, t.tt);
		if (s) {
			const i = W2((t.tt - s[0].tt) / X0, p0 - 1), a = s[i], c = s[i + 1], u = a.a.mean(c.a), h = Z0(t.tt - a.tt, a.r, a.v, u), l = kt(t.tt - a.tt, a.v, u), d = Z0(t.tt - c.tt, c.r, c.v, u), p = kt(t.tt - c.tt, c.v, u), M = (t.tt - a.tt) / X0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(p.mul(M));
		} else {
			let i;
			t.tt < N0[0][0] ? i = X2(N0[0], t.tt, -X0) : i = X2(N0[Ut - 1], t.tt, +X0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
		return n && (o || (o = new M1(t.tt)), e = e.sub(o.Sun.r), r = r.sub(o.Sun.v)), new H(e.x, e.y, e.z, r.x, r.y, r.z, t);
	}
	const c3 = new J([
		[
			.999432765338654,
	...
	function u3(t, n, e) {
		...
		const M = i * h - s * l, A = -s * h - i * l, y = 1 / (1 + A), g = 1 / (1 + Math.sqrt(1 - s * s - i * i)), m = r * (h - s - g * i * M), w = r * (l - i + g * s * M), v = u * y * r * (-l - g * i * A), x = u * y * r * (+h + g * s * A), T = 2 * Math.sqrt(1 - a * a - c * c), D = 1 - 2 * c * c, k = 1 - 2 * a * a, N = 2 * c * a;
		return new H(m * D + w * N, m * N + w * k, (a * w - m * c) * T, v * D + x * N, v * N + x * k, (a * x - v * c) * T, t);
	}
	function j1(t, n) {
		...
		return Z1(c3, u3(t, n.mu, r));
	}
	function l3(t) {
		const n = new R0(t);
		return new Z2(j1(n, q1[0]), j1(n, q1[1]), j1(n, q1[2]), j1(n, q1[3]));
	}
	function u0(t, n) {
		var e = E(n);
	...
	function C0(t, n) {
		...
		if (e) return e.dist;
		const r = E(n);
		return t in Q ? z0(Q[t][B1], r.tt / Y0, !1) : u0(t, r).Length();
	}
	function tn(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
	...
	function nn(t, n, e, r) {
		...
		if (k1(e)) {
			const a = u0(e, o);
			if (r) {
				const u = Y1(n, o), h = new P(a.x - u.x, a.y - u.y, a.z - u.z, o), l = z1 / h.Length();
		...
		let s;
		r ? s = new P(0, 0, 0, o) : s = u0(n, o);
		const i = new h3(n, e, r, s);
		return tn((a) => i.Position(a), o);
	}
	function r0(t, n, e) {
		U1(e);
		const r = E(n);
	...
	function f3(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
			case f.EMB:
				const o = f1(Q[f.Earth], e.tt), s = t === f.Moon ? h1(e) : Tt(e);
	...
	function Y1(t, n) {
		...
		switch (t) {
			...
			case f.Pluto: return Ht(e, !0);
			case f.Moon:
			case f.EMB:
				const o = f1(Q.Earth, e.tt), s = t == f.Moon ? h1(e) : Tt(e);
	...
	function S3(t, n, e, r, o) {
		const s = l1(r), i = _ * 28.06, a = _ * (169.51 + 382e-7 * o.tt), c = _ * s.elat, u = _ * s.elon, h = Math.asin(Math.sin(c) * Math.cos(i) - Math.cos(c) * Math.sin(i) * Math.sin(u - a)), l = Math.sin(Math.abs(h));
		let d = -9 + .044 * t;
	...
	function W1(t, n) {
		...
		const e = E(n), r = W0(Q.Earth, e);
		let o, s, i, a;
		t === f.Sun ? (i = new P(-r.x, -r.y, -r.z, e), s = new P(0, 0, 0, e), o = 0) : (t === f.Moon ? (i = c0(e), s = new P(r.x + i.x, r.y + i.y, r.z + i.z, e)) : (s = u0(t, n), i = new P(s.x - r.x, s.y - r.y, s.z - r.z, e)), o = O1(i, s));
		let c = i.Length(), u = s.Length(), h;
		if (t === f.Sun) a = Pe + 5 * Math.log10(c);
		else if (t === f.Moon) a = _3(o, u, c);
		else if (t === f.Saturn) {
			const l = S3(o, u, c, i, e);
			a = l.mag, h = l.ring_tilt;
		} else a = M3(t, o, u, c);
		return new rn(e, a, o, u, c, i, s, h);
	}
	function S1(t) {
	...
	function _1(t, n, e) {
		...
		let i = r(s), a, c, u;
		if (e < 0) {
			if (i < 0 && (i += 360), a = -(N1 * i) / 360, u = a + o, u < e) return null;
			c = Math.max(e, a - o);
		} else {
			if (i > 0 && (i -= 360), a = -(N1 * i) / 360, c = a - o, c > e) return null;
			u = Math.min(e, a + o);
		}
	...
	function p3(t) {
		return sn(new Date(t.time.date.getTime() + 6 * Te));
	}
	var an = class {
		constructor(t, n, e) {
	...
	function A3(t, n) {
		const e = t.latitude * _, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, r * i0), i = s * (i0 * i0), a = (t.height - n) / 1e3, c = S0 * s + a, u = S0 * i + a, h = 1e3 * Math.hypot(c * o, u * r), l = .175 * Math.pow(1 - .0065 / 283.15 * (t.height - 2 / 3 * n), 3.256);
		return b * -(Math.sqrt(2 * (1 - l) * n / h) / (1 - l));
	}
	function E3(t) {
		switch (t) {
			case f.Sun: return T2;
			case f.Moon: return Ce;
			default: return 0;
		}
	}
	function y3(t, n, e, r, o, s = 0) {
		if (!Number.isFinite(s) || s < 0) throw `Invalid value for metersAboveGround: ${s}`;
		const i = E3(t), a = cn(n.height - s);
		return un(t, n, e, r, o, i, A3(n, s) - Oe * a.density);
	}
	function w3(t, n, e, r, o, s) {
		if (!Number.isFinite(s) || s < -90 || s > 90) throw `Invalid altitude angle: ${s}`;
		return un(t, n, e, r, o, 0, s);
	}
	var g3 = class {
		constructor(t, n, e, r) {
	...
	function $t(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new g3(r, o, s, i);
		...
		const c = new R0((r.ut + o.ut) / 2), u = n(c);
	...
	function un(t, n, e, r, o, s, i) {
		if (j0(n), I(o), I(s), I(i), i < -90 || i > 90) throw `Invalid target altitude angle: ${i}`;
		const a = .42, c = R3(t, n.latitude);
		function u(A) {
			const y = u1(t, A, n, !0, !0);
			return e * (K1(A, n, y.ra, y.dec).altitude + b * Math.asin(s / y.dist) - i);
		}
		const h = E(r);
		let l = h, d = h, p = u(l), M = p;
		for (;;) {
			o < 0 ? (l = d.AddDays(-a), p = u(l)) : (d = l.AddDays(+a), M = u(d));
			const A = $t(0, u, c, l, d, p, M);
			if (A) {
				const y = $(u, A.tx, A.ty, {
	...
	function m3(t, n, e, r, o = 1) {
		...
		for (;;) {
			++i;
			let a = _0(s), c = u1(t, s, n, !0, !0), u = (e + c.ra - n.longitude / 15 - a) % 24;
			if (i === 1 ? o > 0 ? u < 0 && (u += 24) : u > 0 && (u -= 24) : u < -12 ? u += 24 : u > 12 && (u -= 24), Math.abs(u) * 3600 < .1) {
				const l = K1(s, n, c.ra, c.dec, "normal");
				return new ln(s, l);
			}
			let h = u / 24 * P2;
			s = s.AddDays(h);
	...
	function x3(t, n, e) {
		const r = E(n), o = J2(r), s = u1(t, r, e, !0, !0);
	...
	function v3(t) {
		function n(e, r, o) {
			let s = new Date(Date.UTC(t, r - 1, o)), i = en(e, s, 20);
	...
	function dn(t, n) {
		let e = E(n), r = Ft(t, f.Sun, e), o;
		r > 180 ? (o = "morning", r = 360 - r) : o = "evening";
		let s = e1(t, e);
		return new fn(e, o, s, r);
	}
	function I3(t, n) {
		function r(a) {
		...
		for (; ++i <= 2;) {
			...
			if (!y) throw `SearchMaxElongation: failed search iter ${i} (t1=${d.toString()}, t2=${p.toString()})`;
			if (y.tt >= o.tt) return dn(t, y);
			o = p.AddDays(1);
		}
	...
	function D3(t, n) {
		...
		const e = .01;
		function r(c) {
			const u = c.AddDays(-e / 2), h = c.AddDays(+e / 2), l = W1(t, u).mag;
		...
		let a = 0;
		for (; ++a <= 2;) {
			let c = t1(O0(t, o) - O0(f.Earth, o)), u, h, l;
			c >= -s && c < +s ? (l = 0, u = +s, h = +i) : c >= +i || c < -i ? (l = 0, u = -i, h = -s) : c >= 0 ? (l = -S1(t) / 4, u = +s, h = +i) : (l = -S1(t) / 4, u = -i, h = -s);
			let d = o.AddDays(l), p = r1(t, u, d), M = r1(t, h, p), A = r(p);
	...
	function Sn(t, n, e, r) {
		const o = n === v0.Apocenter ? 1 : -1, s = 10;
		for (;;) {
			const i = r / (s - 1);
			if (i < 1 / 1440) {
	...
	function T3(t, n) {
		...
		for (let d = 0; d < 100; ++d) {
			const p = r.AddDays(d * u), M = C0(t, p);
	...
	function _n(t, n) {
		if (n = E(n), t === f.Neptune || t === f.Pluto) return T3(t, n);
		function e(c) {
			let h = c.AddDays(-5e-4), l = c.AddDays(.001 / 2), d = C0(t, h);
			return (C0(t, l) - d) / .001;
		}
		function r(c) {
			return -e(c);
		...
		for (let c = 0; c * s < 2 * o; ++c) {
			...
			if (a * h <= 0) {
				let l, d;
				if (a < 0 || h > 0) l = e, d = v0.Pericenter;
				else if (a > 0 || h < 0) l = r, d = v0.Apocenter;
	...
	function z3(t, n) {
		if (n.kind !== v0.Pericenter && n.kind !== v0.Apocenter) throw `Invalid apsis kind: ${n.kind}`;
		const e = .25 * a0[t].OrbitalPeriod, r = _n(t, n.time.AddDays(e));
	...
	function L3(t, n, e) {
		...
		const r = I(e) * _, o = Math.cos(r), s = Math.sin(r), i = (n + 1) % 3, a = (n + 2) % 3, c = n;
		let u = [
			[
				0,
				0,
	...
	function C3(t, n) {
		const e = Gt(t);
		return e.lon = pn(e.lon), e.lat += A1(n, e.lat), e;
	}
	function U3(t, n, e) {
		n = E(n);
		const r = pn(t.lon);
		return X1(new Q0(t.lat + An(e, t.lat), r, t.dist), n);
	}
	function A1(t, n) {
		let e;
	...
	function tt(t) {
		return t = E(t), I0(V1(t, O.From2000), F1(t, O.From2000));
	}
	function k3(t) {
		const n = E(t);
		return I0(tt(n), vn(n));
	...
	function nt(t) {
		return t = E(t), I0(F1(t, O.Into2000), V1(t, O.Into2000));
	}
	function Bt(t, n) {
		t = E(t);
		const e = Math.sin(n.latitude * _), r = Math.cos(n.latitude * _), o = Math.sin(n.longitude * _), s = Math.cos(n.longitude * _), i = [
			...
			-s,
			0
		], u = -15 * _0(t), h = q0(u, i), l = q0(u, a), d = q0(u, c);
		return new J([
			[
				l[0],
				d[0],
	...
	function xn(t) {
		const n = m0(E(t)).tobl * _, e = Math.cos(n), r = Math.sin(n);
		return new J([
			[
				1,
				0,
	...
	function G3(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Qt || (Qt = tt(new R0(-45655.74141261017)), In = new R0(0));
		const e = X1(new Q0(n, 15 * t, 1), In), r = Kt(E1(Qt, e)), o = 10 / 240, s = o / 15;
		for (let i of K3) {
	...
	function y1(t, n, e, r) {
		...
		return new B3(n, o, C * Math.hypot(s, i, a), +i1 - (1 + o) * (i1 - t), -i1 + (1 + o) * (i1 + t), e, r);
	}
	function et(t) {
		const n = r0(f.Sun, t, !0), e = new P(-n.x, -n.y, -n.z, n.t);
		return y1(Le, t, c0(t), e);
	}
	function Tn(t) {
		const n = r0(f.Sun, t, !0), e = c0(t), r = new P(-e.x, -e.y, -e.z, e.t);
		return e.x -= n.x, e.y -= n.y, e.z -= n.z, y1(W, t, r, e);
	}
	function qt(t, n) {
		const e = K2(t, n), r = r0(f.Sun, t, !0), o = c0(t), s = new P(e[0] - o.x, e[1] - o.y, e[2] - o.z, t);
		return o.x -= r.x, o.y -= r.y, o.z -= r.z, y1(W, t, s, o);
	}
	function rt(t, n, e) {
		const r = r0(t, e, !0), o = r0(f.Sun, e, !0), s = new P(r.x - o.x, r.y - o.y, r.z - o.z, e);
	...
	function q3(t) {
		const e = $((r) => jt(et, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Earth shadow time.";
		return et(e);
	}
	function j3(t) {
		const e = $((r) => jt(Tn, r), t.AddDays(-.03), t.AddDays(.03));
	...
	function W3(t, n) {
		...
		const i = $((a) => jt(s, a), r, o);
		if (!i) throw `PeakLocalMoonShadow: search failure for search_center_time = ${t}`;
		return qt(i, n);
	}
	function Yt(t, n, e) {
		const r = e / 1440, o = t.AddDays(-r), s = t.AddDays(+r), i = $((c) => -(et(c).r - n), o, t), a = $((c) => +(et(c).r - n), t, s);
	...
	function Nn(t, n) {
		const e = new P(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = zn(Math.asin(T2 / e.Length()), Math.asin(Ue / n.Length()), O1(n, e) * _);
	...
	function Ln(t) {
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				const i = q3(o);
				if (i.r < i.p + W) {
					let a = Z.Penumbral, c = 0, u = 0, h = 0, l = Yt(i.time, i.p + W, 200);
					return i.r < i.k + W && (a = Z.Partial, h = Yt(i.time, i.k + W, l), i.r + W < i.k ? (a = Z.Total, c = 1, u = Yt(i.time, i.k - W, h)) : c = zn(W, i.k, i.r)), new Pn(a, c, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function X3(t) {
		...
		a.x *= C, a.y *= C, a.z *= C / i0, c.x *= C, c.y *= C, c.z *= C / i0;
		const u = S0, h = a.x * a.x + a.y * a.y + a.z * a.z, l = -2 * (a.x * c.x + a.y * c.y + a.z * c.z), d = c.x * c.x + c.y * c.y + c.z * c.z - u * u, p = l * l - 4 * h * d;
		let M;
		if (p > 0) {
			const A = (-l - Math.sqrt(p)) / (2 * h), y = A * a.x - c.x, g = A * a.y - c.y, m = (A * a.z - c.z) * i0, w = Math.hypot(y, g) * V0;
			w == 0 ? o = m > 0 ? 90 : -90 : o = b * Math.atan(m / w);
			const v = _0(e);
			s = (b * Math.atan2(g, y) - 15 * v) % 360, s <= -180 ? s += 360 : s > 180 && (s -= 360);
			const x = o1(i);
			let T = new P(y / C, g / C, m / C, t.time);
			T = E1(x, T), T.x += t.target.x, T.y += t.target.y, T.z += t.target.z;
			const D = y1(N2, t.time, T, t.dir);
			if (D.r > 1e-9 || D.r < 0) throw `Unexpected shadow distance from geoid intersection = ${D.r}`;
			n = Un(D.k), M = n === Z.Total ? 1 : Nn(t.dir, T);
		} else M = void 0;
		return new Cn(n, M, e, r, o, s);
	}
	function Z3(t) {
		return t = E(t), Ln(t.AddDays(10));
	}
	function On(t) {
		t = E(t);
		const n = 1.8;
	...
	function n4(t, n) {
		...
		const a = ot(n, 1, Vn, s, t.time), c = ot(n, -1, Vn, t.time, i);
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = ot(n, 1, Hn, s, t.time), h = ot(n, -1, Hn, t.time, i), l = Un(t.k)) : l = Z.Partial;
		const d = l === Z.Total ? 1 : Nn(t.dir, t.target);
		return new bn(l, d, a, u, o, h, c);
	}
	...
	function r4(t, n) {
		return t = E(t), Jn(t.AddDays(10), n);
	}
	var $n = class {
		constructor(t, n, e, r) {
	...
	function Gn(t, n) {
		...
		for (;;) {
			...
			if (e1(t, i) < e) {
				const a = Y3(t, o, i);
				if (a.r < a.p) {
					const c = a.time.AddDays(-r), u = Kn(t, o, c, a.time, -1), h = a.time.AddDays(+r), l = Kn(t, o, a.time, h, 1), d = 60 * e1(t, a.time);
					return new $n(u, a.time, l, d);
				}
			}
			s = i.AddDays(10);
		}
	...
	function s4(t, n) {
		return n = E(n), Gn(t, n.AddDays(100));
	}
	var A0;
	(function(t) {
	...
	function qn(t) {
		let n = E(t), e = G1(n);
		for (;;) {
			const r = n.AddDays(Qn), o = G1(r);
			if (e.lat * o.lat <= 0) {
				const s = o.lat > e.lat ? A0.Ascending : A0.Descending, i = $((a) => s * G1(a).lat, n, r);
	...
	function c4(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
				const d = _ * (125.045 - .0529921 * r), p = _ * (250.089 - .1059842 * r), M = _ * (260.008 + 13.0120009 * r), A = _ * (176.625 + 13.3407154 * r), y = _ * (357.529 + .9856003 * r), g = _ * (311.589 + 26.4057084 * r), m = _ * (134.963 + 13.064993 * r), w = _ * (276.617 + .3287146 * r), v = _ * (34.226 + 1.7484877 * r), x = _ * (15.134 - .1589763 * r), T = _ * (119.743 + .0036096 * r), D = _ * (239.961 + .1643573 * r), k = _ * (25.053 + 12.9590088 * r);
				s = 269.9949 + .0031 * o - 3.8787 * Math.sin(d) - .1204 * Math.sin(p) + .07 * Math.sin(M) - .0172 * Math.sin(A) + .0072 * Math.sin(g) - .0052 * Math.sin(x) + .0043 * Math.sin(k), i = 66.5392 + .013 * o + 1.5419 * Math.cos(d) + .0239 * Math.cos(p) - .0278 * Math.cos(M) + .0068 * Math.cos(A) - .0029 * Math.cos(g) + 9e-4 * Math.cos(m) + 8e-4 * Math.cos(x) - 9e-4 * Math.cos(k), a = 38.3213 + (13.17635815 - 14e-13 * r) * r + 3.561 * Math.sin(d) + .1208 * Math.sin(p) - .0642 * Math.sin(M) + .0158 * Math.sin(A) + .0252 * Math.sin(y) - .0066 * Math.sin(g) - .0047 * Math.sin(m) - .0046 * Math.sin(w) + .0028 * Math.sin(v) + .0052 * Math.sin(x) + .004 * Math.sin(T) + .0019 * Math.sin(D) - .0044 * Math.sin(k);
				break;
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const N = _ * (99.360714 + 4850.4046 * o), B = _ * (175.895369 + 1191.9605 * o), K = _ * (300.323162 + 262.5475 * o), Y = _ * (114.012305 + 6070.2476 * o), l0 = _ * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(N) + 938e-6 * Math.sin(B) + .001432 * Math.sin(K) + 3e-5 * Math.sin(Y) + .00215 * Math.sin(l0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(N) + 404e-6 * Math.cos(B) + 617e-6 * Math.cos(K) - 13e-6 * Math.cos(Y) + 926e-6 * Math.cos(l0), a = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
			...
			case f.Uranus:
				...
				break;
			case f.Neptune:
				const s0 = _ * (357.85 + 52.316 * o);
				s = 299.36 + .7 * Math.sin(s0), i = 43.46 - .51 * Math.cos(s0), a = 249.978 + 541.1397757 * r - .48 * Math.sin(s0);
				break;
			case f.Pluto:
	...
	function u4(t, n, e, r) {
		const o = E(n), s = mt(e), i = mt(r);
		let a, c;
		return e === f.Earth && r === f.Moon ? (a = new H(0, 0, 0, 0, 0, 0, o), c = h1(o)) : (a = Y1(e, o), c = Y1(r, o)), jn(t, a, s, c, i);
	}
	function jn(t, n, e, r, o) {
		...
		let A;
		if (t === 4 || t === 5) {
			const y = c * M - u * p, g = u * d - a * M, m = a * p - c * d;
			let w = g * u - m * c, v = m * a - y * u, x = y * c - g * a;
			const T = Math.sqrt(w * w + v * v + x * x);
			w /= T, v /= T, x /= T, a /= l, c /= l, u /= l;
			const D = t == 4 ? +i : -i, k = .5 * a + D * w, N = .5 * c + D * v, B = .5 * u + D * x, K = .5 * w - D * a, Y = .5 * v - D * c, l0 = .5 * x - D * u, s0 = l * k, E0 = l * N, D0 = l * B, h0 = d * a + p * c + M * u, f0 = d * w + p * v + M * x;
			A = new H(s0, E0, D0, h0 * k + f0 * K, h0 * N + f0 * Y, h0 * B + f0 * l0, n.t);
		} else {
			const y = -l * (o / (e + o)), g = +l * (e / (e + o)), m = (e + o) / (h * l);
			let w, v, x;
			...
			do {
				const k = T - y, N = T - g;
				D = (m * T + v / (k * k) + x / (N * N)) / (m - 2 * v / (k * k * k) - 2 * x / (N * N * N)), T -= D;
			} while (Math.abs(D / l) > 1e-14);
			w = (T - y) / l, A = new H(w * a, w * c, w * u, w * d, w * p, w * M, n.t);
		}
		return A;
	}
	var l4 = class n0 {
		...
		static CalcSolarSystem(n) {
			...
			e[f.Mercury] = X(r, n.tt, f.Mercury, wt), e[f.Venus] = X(r, n.tt, f.Venus, gt), e[f.Earth] = X(r, n.tt, f.Earth, c1 + C1), e[f.Mars] = X(r, n.tt, f.Mars, Rt), e[f.Jupiter] = X(r, n.tt, f.Jupiter, H0), e[f.Saturn] = X(r, n.tt, f.Saturn, F0), e[f.Uranus] = X(r, n.tt, f.Uranus, J0), e[f.Neptune] = X(r, n.tt, f.Neptune, $0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new L0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = q.zero(), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, a1), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, wt), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, gt), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, c1 + C1), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, Rt), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, H0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, F0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, J0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, $0);
		}
		...
		Duplicate() {
			...
			return new Yn(this.curr.time, n, e);
		}
	}, Yn = class {
		constructor(t, n, e) {
	...
	const { MakeTime: k4, SearchRiseSet: b4, Body: V4, Observer: h4 } = st(T1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: $4, CombineRotation: K4, MakeRotation: G4 } = st(T1);
	Math.PI / (180 * 3600);
	const { HelioState: Q4 } = st(T1), f4 = 26050901, d4 = 86400;
	function Wn(t) {
		return t.type === "init" ? {
			type: "init",
	...
	function Xn(t, n) {
		...
		switch (e.type) {
			...
			case "seek": {
				...
				const r = e.jd;
				let o = r;
				(o < 1721545 || o > 3181545) && (console.warn(`[kernel/seek] JD ${o} outside supported range [${_t}, ${pt}] (years 0-4000 AD); clamping.`), o = Math.max(_t, Math.min(pt, o)));
				const s = {
					...t,
					jd: o
				}, i = [{
			...
			case "step": {
				...
				const o = t.jd + t.speed * r / d4, s = Math.max(_t, Math.min(pt, o)), i = {
					...t,
					jd: s
				}, a = [{
					type: "tick",
	...
	I1 / Mt;
	let t0 = Zt(), it = null, w1 = null;
	function t2(t) {
		w1?.(t);
	}
	function n2() {
		it != null && (clearInterval(it), it = null);
	}
	function S4() {
		n2(), it = setInterval(() => {
			const { state: t, events: n } = Xn(t0, {
				type: "step",
				deltaSeconds: .5
			});
			t0 = t, t2({
	...
	function _4(t) {
		if (t.type === "init") {
			...
			t0 = Zt({
				jd: n.jd ?? t0.jd,
				playing: n.playing ?? t0.playing,
				speed: n.speed ?? t0.speed
			}), e2(), t2({
				type: "workerTick",
				state: t0,
				events: []
		...
		if (t.type === "time") {
			const { state: n, events: e } = Xn(t0, t.cmd);
			t0 = n, t2({
				type: "workerTick",
				state: t0,
				events: e
	...
	ht({
		...
		subscribe(t) {
			w1 != null && w1 !== t && console.warn("[simulationWorker] replacing existing outbound subscriber (double subscribe)"), w1 = t, e2();
		},
		unsubscribe() {
			w1 = null, n2();
		}
