(function() {
	...
	const s2 = Symbol("Comlink.proxy"), Wn = Symbol("Comlink.endpoint"), jn = Symbol("Comlink.releaseProxy"), ct = Symbol("Comlink.finalizer"), m1 = Symbol("Comlink.thrown"), i2 = (t) => typeof t == "object" && t !== null || typeof t == "function", a2 = new Map([["proxy", {
	...
	function lt(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, c, a) {
				...
				if (u === Wn) return b0(t, n, { type: "ENDPOINT" }).then(z0);
	...
	}, de = 149597870.7, Me = 648e3 / Math.PI, ht = 1e3, N0 = 100 * ht, k0 = 100 / Me, Se = 16777216, f2 = 3.2615637771694, ft = {
		x: 0,
		y: 0,
		z: 0
	};
	...
	d2(Ee) * 10;
	let v1;
	k0 <= 0 ? v1 = N0 : 25 >= k0 ? v1 = N0 + (25 - k0) * 20 : v1 = 25 / k0 * N0;
	const we = v1;
	function ye(t) {
		return {
			name: t.name,
	...
	const M2 = Object.fromEntries(Object.entries(fe).map(([t, n]) => [t, ye(n)]));
	v.SUN, v.MERCURY, v.VENUS, v.EARTH, v.MARS, v.JUPITER, v.SATURN, v.URANUS, v.NEPTUNE, v.PLUTO, v.MOON;
	function ge(t) {
		if (!Number.isFinite(t) || t < 0) return 0;
		const n = k0;
		return n <= 0 ? N0 : t >= n ? N0 + (t - n) * 20 : t / n * N0;
	}
	const dt = M2[v.EARTH]?.radiusKm ?? 6371, Mt = M2[v.SUN]?.radiusKm ?? 696340, me = dt / Mt, F0 = .04 * (100 / 15);
	dt / F0;
	F0 / dt / (ht / de);
	function S2(t) {
		return !Number.isFinite(t) || t <= 0 ? F0 : t / me * F0;
	}
	function Re(t) {
		return !Number.isFinite(t) || t <= 0 ? F0 : S2(t / Mt);
	}
	Re(Mt);
	const _2 = .005, xe = 1e3, Ie = 1.9;
	function De(t) {
		return Math.max(_2, Math.min(xe, t));
	}
	function ve(t) {
		const n = De(t), e = Math.min(n, 72), r = Math.log(_2), o = Math.log(72), s = (Math.log(e) - r) / (o - r), i = Math.max(0, Math.min(1, s)) ** Ie;
	...
	function Ne(t, n) {
		const e = t.solrad;
		return e != null && Number.isFinite(e) && e > 0 ? St(e) : ze(t.mag, n);
	}
	var z1 = Yn({
		AU_PER_LY: () => p2,
		AngleBetween: () => U1,
		...
		InverseRotation: () => i1,
		JUPITER_EQUATORIAL_RADIUS_KM: () => Te,
	...
	const N1 = 173.1446326846693, U = 149597870.69098932, p2 = 63241.07708807546, p = .017453292519943295, _t = .26179938779914946, k = 57.29577951308232, At = 3.819718634205488, Te = 71492, Pe = 66854, Le = 69911, Ce = 1821.6, Ue = 1560.8, Oe = 2631.2, be = 2410.3, E2 = 365.24217, w2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, w0 = 3600 * (180 / Math.PI), V0 = 484813681109536e-20, y2 = 10800 * 60, ke = 2 * y2, g2 = 7292115e-11, Fe = y2 / Math.PI, Ve = -.17 - 5 * Math.log10(Fe), T1 = 29.530588, P1 = 24 * 3600, He = P1 * 1e3, m2 = .9972695717592592, c1 = 695700, R2 = c1 / U, o0 = .996647180302104, H0 = o0 * o0, d0 = 6378.1366, $e = d0 / U, Je = d0 * o0, x2 = 6371, Ge = x2 + 88, Be = 1738.1 / U, W = 1737.4, I2 = 1736, Ke = I2 / U, Qe = 34 / 60, pt = 81.30056, u1 = .0002959122082855911, Et = 4912547451450812e-26, wt = 7243452486162703e-25, l1 = 8887692390113509e-25, yt = 9549535105779258e-26, $0 = 2.825345909524226e-7, J0 = 8.459715185680659e-8, G0 = 1.292024916781969e-8, B0 = 1.524358900784276e-8, qe = 218869976542597e-26, L1 = l1 / pt;
	function gt(t) {
		switch (t) {
	...
	function m0(t) {
		...
		let J4 = (1.000002708 + 139.978 * A) * (18519.699999999997 + E) * Math.sin(i) - 6.24 * Math.sin(3 * i) + S;
		return {
			geo_eclip_lon: t0 * K0((R + M / w0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * J4,
			distance_au: w0 * $e / (.999953253 * w)
		};
	}
	var L2 = class {
	...
	function f3(t, n) {
		...
		return n || (r = Q0(r, t.t, b.From2000), r = q0(r, t.t, b.From2000)), i3(r, e);
	}
	function d3(t, n) {
	...
	function i0(t) {
		const n = g(t), e = m0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = Q0(r3(n, [
	...
	const Lt = 51, S3 = 29200, t1 = 146, S0 = 201, P0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function bt(t) {
		const n = _3(t), e = new _1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new G2(e, new Ct(n.tt, r, o, s));
	}
	function p3(t, n) {
		const e = P0[0][0];
		if (n < e || n > P0[Lt - 1][0]) return null;
		const r = B2((n - e) / S3, Lt - 1);
		if (!t[r]) {
			const s = t[r] = [];
	...
	function ln(t, n) {
		if (n = g(n), t === f.Neptune || t === f.Pluto) return H3(t, n);
		function e(a) {
	...
	function g1(t, n, e, r) {
		...
		return new r4(n, o, U * Math.hypot(s, i, c), +c1 - (1 + o) * (c1 - t), -c1 + (1 + o) * (c1 + t), e, r);
	}
	function nt(t) {
		const n = n0(f.Sun, t, !0), e = new N(-n.x, -n.y, -n.z, n.t);
		return g1(Ge, t, i0(t), e);
	}
	function Rn(t) {
	...
	function In(t, n) {
		const e = new N(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = xn(Math.asin(R2 / e.Length()), Math.asin(Ke / n.Length()), U1(n, e) * p);
	...
	const { MakeTime: o6, SearchRiseSet: s6, Body: i6, Observer: g4 } = ot(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: l6, CombineRotation: h6, MakeRotation: f6 } = ot(z1);
	Math.PI / (180 * 3600);
	function R4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: S6 } = ot(z1);
	function x4(t, n) {
	...
	const z4 = 50, N4 = 500, T4 = .01;
	function I0(t) {
		return Math.max(0, Math.min(1, t));
	}
	function P4(t) {
		return t?.warm != null && Number.isFinite(t.warm) ? I0(t.warm) : T4;
	}
	function L4(t) {
	...
	function H4(t) {
		const { catalog: n, maxDistanceLevel: e, bounds: r, raInHours: o } = t, s = I4(x4(n, { raInHours: o }).filter((S) => !jt(S)), r, { maxDistanceLevel: e }), i = () => ({
			chunks: [],
			namedStarsInView: [],
			starsInView: []
		});
		if (s.length === 0) {
			if (e === 0) return i();
			const S = V4(), E = S.length, w = new Float32Array(E * 3), R = new Float32Array(E).fill(z4), y = new Float32Array(E * 3).fill(1), D = new Float32Array(E).fill(1);
			for (let m = 0; m < E; m += 1) {
				const x = S[m];
		...
		const c = s.map(D4), a = c.length, u = new Float32Array(a * 3), h = new Float32Array(a), l = new Float32Array(a * 3), d = new Float32Array(a), A = [];
		for (let S = 0; S < a; S += 1) {
			const E = c[S], w = s[S];
			u[S * 3] = E.x, u[S * 3 + 1] = E.y, u[S * 3 + 2] = E.z;
			const R = w ? Ne(w, Kn(w)) : F0, y = L4(w);
			h[S] = R * U4(y);
			const [D, m, x] = v4(w?.c != null ? `#${w.c}` : void 0), I = C4(P4(w)), P = 1 - I;
