(function() {
	...
	const o2 = Symbol("Comlink.proxy"), jn = Symbol("Comlink.endpoint"), Xn = Symbol("Comlink.releaseProxy"), at = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), s2 = (t) => typeof t == "object" && t !== null || typeof t == "function", i2 = new Map([["proxy", {
	...
	}, de = 149597870.7, Me = 648e3 / Math.PI, lt = 1e3, z0 = 100 * lt, b0 = 100 / Me, Se = 16777216, f2 = 3.2615637771694, ht = {
		x: 0,
		y: 0,
		z: 0
	};
	...
	const ft = M2[x.EARTH]?.radiusKm ?? 6371, dt = M2[x.SUN]?.radiusKm ?? 696340, Re = ft / dt, k0 = .04 * (100 / 15);
	ft / k0;
	k0 / ft / (lt / de);
	function S2(t) {
	...
	function me(t) {
		return !Number.isFinite(t) || t <= 0 ? k0 : S2(t / dt);
	}
	me(dt);
	...
	const z1 = 173.1446326846693, C = 149597870.69098932, p2 = 63241.07708807546, A = .017453292519943295, St = .26179938779914946, k = 57.29577951308232, _t = 3.819718634205488, Pe = 71492, Te = 66854, Le = 69911, Ue = 1821.6, Ce = 1560.8, Oe = 2631.2, be = 2410.3, E2 = 365.24217, y2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, y0 = 3600 * (180 / Math.PI), F0 = 484813681109536e-20, w2 = 10800 * 60, ke = 2 * w2, g2 = 7292115e-11, Fe = w2 / Math.PI, Ve = -.17 - 5 * Math.log10(Fe), N1 = 29.530588, P1 = 24 * 3600, He = P1 * 1e3, R2 = .9972695717592592, a1 = 695700, m2 = a1 / C, o0 = .996647180302104, V0 = o0 * o0, d0 = 6378.1366, $e = d0 / C, Je = d0 * o0, x2 = 6371, Ge = x2 + 88, Be = 1738.1 / C, W = 1737.4, I2 = 1736, Ke = I2 / C, Qe = 34 / 60, At = 81.30056, c1 = .0002959122082855911, pt = 4912547451450812e-26, Et = 7243452486162703e-25, u1 = 8887692390113509e-25, yt = 9549535105779258e-26, H0 = 2.825345909524226e-7, $0 = 8.459715185680659e-8, J0 = 1.292024916781969e-8, G0 = 1.524358900784276e-8, qe = 218869976542597e-26, T1 = u1 / At;
	function wt(t) {
		switch (t) {
	...
	function g0(t) {
		if (!O1 || Math.abs(O1.tt - t.tt) > 1e-6) {
			const n = e3(t), e = N2(t), r = e + n.deps / 3600;
			O1 = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function R0(t) {
		...
		function r(P, O, F, H) {
			const B = [];
			for (let D0 = 0; D0 <= O - P; ++D0) B.push(e(F, H));
			return {
				min: P,
				array: B
			};
		...
		function o(P, O, F) {
			const H = P.array[O - P.min];
			return H.array[F - H.min];
		}
		function s(P, O, F, H) {
			const B = P.array[O - P.min];
			B.array[F - B.min] = H;
		}
		...
		function ot(P, O, F, H, B) {
			B(P * F - O * H, O * F + P * H);
		}
		function U(P) {
			return Math.sin(t0 * P);
		}
		d = n * n, M = 0, Y = 0, E = 0, y = 3422.7;
		var h0 = U(.19833 + .05611 * n), jt = U(.27869 + .04508 * n), Xt = U(.16827 - .36903 * n), Zt = U(.34734 - 5.37261 * n), t2 = U(.10498 - 5.37899 * n), st = U(.42681 - .41855 * n), U4 = U(.14943 - 5.37511 * n);
		for (D = .84 * h0 + .31 * jt + 14.27 * Xt + 7.26 * Zt + .28 * t2 + .24 * st, T = 2.94 * h0 + .31 * jt + 14.27 * Xt + 9.34 * Zt + 1.12 * t2 + .83 * st, L = -6.4 * h0 - 1.89 * st, K = .21 * h0 + .31 * jt + 14.27 * Xt - 88.7 * Zt - 15.3 * t2 + .24 * st - 1.86 * U4, G = D - L, p = -3332e-9 * U(.59734 - 5.37261 * n) - 539e-9 * U(.35498 - 5.37899 * n) - 64e-9 * U(.39943 - 5.37511 * n), m = t0 * B0(.60643382 + 1336.85522467 * n - 313e-8 * d) + D / y0, g = t0 * B0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + T / y0, I = t0 * B0(.99312619 + 99.99735956 * n - 44e-8 * d) + L / y0, R = t0 * B0(.25909118 + 1342.2278298 * n - 892e-8 * d) + K / y0, z = t0 * B0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / y0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
			...
			for (u0(0, h, 1), u0(1, h, Math.cos(a) * u), l0(0, h, 0), l0(1, h, Math.sin(a) * u), l = 2; l <= c; ++l) ot(A0(l - 1, h), I0(l - 1, h), A0(1, h), I0(1, h), (P, O) => (u0(l, h, P), l0(l, h, O)));
		...
		function _(P, O, F, H, B, D0, p0, n2) {
			var i1 = qn(B, D0, p0, n2);
			M += P * i1.y, Y += O * i1.y, E += F * i1.x, y += H * i1.x;
		}
		...
		function f0(P, O, F, H, B) {
			return P * qn(O, F, H, B).y;
		}
		S = 0, S += f0(-526.069, 0, 0, 1, -2), S += f0(-3.352, 0, 0, 1, -4), S += f0(44.297, 1, 0, 1, -2), S += f0(-6, 1, 0, 1, -4), S += f0(20.599, -1, 0, 1, 0), S += f0(-30.598, -1, 0, 1, -2), S += f0(-24.649, -2, 0, 1, 0), S += f0(-2, -2, 0, 1, -2), S += f0(-22.571, 0, 1, 1, -2), S += f0(10.985, 0, -1, 1, -2), M += .82 * U(.7736 - 62.5512 * n) + .31 * U(.0466 - 125.1025 * n) + .35 * U(.5785 - 25.1042 * n) + .66 * U(.4591 + 1335.8075 * n) + .64 * U(.313 - 91.568 * n) + 1.14 * U(.148 + 1331.2898 * n) + .21 * U(.5918 + 1056.5859 * n) + .44 * U(.5784 + 1322.8595 * n) + .24 * U(.2275 - 5.7374 * n) + .28 * U(.2965 + 2.6929 * n) + .33 * U(.3132 + 6.3368 * n), i = R + Y / y0;
		let C4 = (1.000002708 + 139.978 * p) * (18519.699999999997 + E) * Math.sin(i) - 6.24 * Math.sin(3 * i) + S;
		return {
	...
	function a3(t, n, e) {
		return e === b.Into2000 ? C2(b2(t, n, e), n, e) : b2(C2(t, n, e), n, e);
	}
	function k2(t, n) {
	...
	function $1(t, n, e, r, o) {
		let s = w(t);
		W0(n), v(e), v(r);
		const i = Math.sin(n.latitude * A), c = Math.cos(n.latitude * A), a = Math.sin(n.longitude * A), u = Math.cos(n.longitude * A), h = Math.sin(r * A), l = Math.cos(r * A), d = Math.sin(e * St), p = Math.cos(e * St);
		let M = [
			c * u,
			c * a,
			i
		...
		if (o) {
			...
			if (G -= A0, A0 > 0 && G > 3e-4) {
				const I0 = Math.sin(G * A), u0 = Math.cos(G * A), l0 = Math.sin(r0 * A), ot = Math.cos(r0 * A), U = [];
				for (let h0 = 0; h0 < 3; ++h0) U.push((R[h0] - ot * m[h0]) / l0 * I0 + m[h0] * u0);
				L = Math.hypot(U[0], U[1]), L > 0 ? (Y = _t * Math.atan2(U[1], U[0]), Y < 0 && (Y += 24)) : Y = 0, c0 = k * Math.atan2(U[2], L);
			}
		}
	...
	function h3(t, n, e) {
		const r = w(t), o = Rt(n, M0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
	...
	function J1(t) {
		const n = w(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = mt(Q0(P2(s.mobl, o), n, b.From2000), n), c = s.tobl * A, a = It(i, Math.cos(c), Math.sin(c));
	...
	function X0(t, n) {
		const e = n.tt / j0;
		return Pt(J2(N0(t[zt], e, !0), N0(t[Nt], e, !1), N0(t[G1], e, !1))).ToAstroVector(n);
	}
	function d1(t, n) {
		const e = n / j0, r = N0(t[zt], e, !0), o = N0(t[Nt], e, !1), s = N0(t[G1], e, !1), i = vt(t[zt], e), c = vt(t[Nt], e), a = vt(t[G1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), p = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, S = +(a * d) + s * l * c, E = J2(r, o, s), y = [
			p / j0,
			M / j0,
			S / j0
		];
	...
	function Ct(t, n) {
		...
		return new G2(r, new Lt(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const A3 = [];
	function B2(t, n) {
	...
	function Ot(t) {
		...
		return new G2(e, new Lt(n.tt, r, o, s));
	}
	function p3(t, n) {
		const e = P0[0][0];
		if (n < e || n > P0[Tt - 1][0]) return null;
		const r = B2((n - e) / S3, Tt - 1);
		if (!t[r]) {
			const s = t[r] = [];
	...
	function bt(t, n) {
		...
		const s = p3(A3, t.tt);
		if (s) {
			const i = B2((t.tt - s[0].tt) / Z0, S0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = t1(t.tt - c.tt, c.r, c.v, u), l = Ut(t.tt - c.tt, c.v, u), d = t1(t.tt - a.tt, a.r, a.v, u), p = Ut(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / Z0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(p.mul(M));
		} else {
			let i;
			t.tt < P0[0][0] ? i = K2(P0[0], t.tt, -Z0) : i = K2(P0[Tt - 1], t.tt, +Z0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function k3(t, n) {
		...
		let i = 0;
		for (; ++i <= 2;) {
			let c = n1(C0(t, o) - C0(f.Earth, o)), a, u, h;
	...
	function F3(t, n) {
		...
		let c = 0;
		for (; ++c <= 2;) {
			let a = n1(C0(t, o) - C0(f.Earth, o)), u, h, l;
	...
	function e4(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Gt || (Gt = X1(new w0(-45655.74141261017)), wn = new w0(0));
		const e = W1(new q0(n, 15 * t, 1), wn), r = Ht(y1(Gt, e)), o = 10 / 240, s = o / 15;
		for (let i of n4) {
	...
	function w1(t, n, e, r) {
		...
		return new r4(n, o, C * Math.hypot(s, i, c), +a1 - (1 + o) * (a1 - t), -a1 + (1 + o) * (a1 + t), e, r);
	}
	function tt(t) {
	...
	function i4(t) {
		const e = J((r) => Kt(mn, r), t.AddDays(-.03), t.AddDays(.03));
	...
	function In(t, n) {
		const e = new N(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = xn(Math.asin(m2 / e.Length()), Math.asin(Ke / n.Length()), U1(n, e) * A);
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				...
				if (i.r < i.p + W) {
					...
					return i.r < i.k + W && (c = X.Partial, h = Qt(i.time, i.k + W, l), i.r + W < i.k ? (c = X.Total, a = 1, u = Qt(i.time, i.k - W, h)) : a = xn(W, i.k, i.r)), new Rn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function u4(t) {
		...
		if (p > 0) {
			...
			const R = s1(i);
			let z = new N(E / C, y / C, m / C, t.time);
		...
		Update(n) {
			...
			if (r === 0) this.prev = this.Duplicate();
				...
				for (let i = 0; i < this.curr.bodies.length; ++i) {
					...
					a.tt = e.tt, a.r = t1(r, c.r, c.v, u), a.v = Ut(r, c.v, u);
				}
				this.CalcBodyAccelerations();
			}
	...
	const { MakeTime: W4, SearchRiseSet: j4, Body: X4, Observer: g4 } = rt(v1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: e6, CombineRotation: r6, MakeRotation: o6 } = rt(v1);
	Math.PI / (180 * 3600);
	function m4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: a6 } = rt(v1);
	function x4(t, n) {
	...
	function T4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = I4(x4(n, { raInHours: s }).filter((S) => !Wt(S)), o, { maxDistanceLevel: e }), c = (S, E, y, m) => ({
			positions: S,
			sizes: E,
			colors: y,
			brightness: m,
		...
		if (i.length === 0) {
			...
			const S = P4(), E = S.length, y = new Float32Array(E * 3), m = new Float32Array(E).fill(Qn), g = new Float32Array(E * 3).fill(1), I = new Float32Array(E).fill(1);
