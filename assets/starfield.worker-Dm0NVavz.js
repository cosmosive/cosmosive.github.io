(function() {
	var e2 = Object.defineProperty, Bn = (t, n) => {
		let e = {};
	...
	const r2 = Symbol("Comlink.proxy"), Kn = Symbol("Comlink.endpoint"), Yn = Symbol("Comlink.releaseProxy"), ct = Symbol("Comlink.finalizer"), g1 = Symbol("Comlink.thrown"), o2 = (t) => typeof t == "object" && t !== null || typeof t == "function", s2 = new Map([["proxy", {
	...
	function lt(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, c, a) {
				...
				if (u === Kn) return U0(t, n, { type: "ENDPOINT" }).then(D0);
	...
	}, u3 = 149597870.7, l3 = 648e3 / Math.PI, O0 = 1500, i1 = 100 / l3, h2 = 16777216;
	...
	const h3 = 5e4, f3 = 2e3, d3 = h3 / f2, M3 = f3 / f2;
	d2(d3);
	d2(M3);
	let v1;
	...
	const M2 = Object.fromEntries(Object.entries(c3).map(([t, n]) => [t, _3(n)]));
	...
	const ft = M2[v.EARTH]?.radiusKm ?? 6371, dt = M2[v.SUN]?.radiusKm ?? 696340, p3 = ft / dt, b0 = .04;
	ft / b0;
	b0 / ft / (15 / u3);
	function D1(t) {
		return !Number.isFinite(t) || t <= 0 ? b0 : t / p3 * b0;
	}
	function E3(t) {
	...
	function y3(t, n) {
		...
		return D1(Math.max(w3, Math.min(100, Math.sqrt(e))));
	}
	var z1 = Bn({
		AU_PER_LY: () => S2,
		AngleBetween: () => U1,
		...
		InverseRotation: () => o1,
		JUPITER_EQUATORIAL_RADIUS_KM: () => g3,
		JUPITER_MEAN_RADIUS_KM: () => x3,
		JUPITER_POLAR_RADIUS_KM: () => R3,
		JupiterMoons: () => fe,
		JupiterMoonsInfo: () => G2,
		KM_PER_AU: () => C,
		LagrangePoint: () => h4,
		...
		SearchMaxElongation: () => ze,
		SearchMoonNode: () => Fn,
		SearchMoonPhase: () => A1,
		SearchMoonQuarter: () => W2,
		SearchPeakMagnitude: () => Pe,
		...
		TransitInfo: () => Cn,
		Vector: () => P,
		VectorFromHorizon: () => be,
		VectorFromSphere: () => X1,
		VectorObserver: () => re,
		e_tilt: () => g0
	});
	const P1 = 173.1446326846693, C = 149597870.69098932, S2 = 63241.07708807546, A = .017453292519943295, Mt = .26179938779914946, k = 57.29577951308232, St = 3.819718634205488, g3 = 71492, R3 = 66854, x3 = 69911, m3 = 1821.6, I3 = 1560.8, v3 = 2631.2, D3 = 2410.3, _2 = 365.24217, A2 = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), t0 = 2 * Math.PI, w0 = 3600 * (180 / Math.PI), k0 = 484813681109536e-20, p2 = 10800 * 60, z3 = 2 * p2, E2 = 7292115e-11, P3 = p2 / Math.PI, T3 = -.17 - 5 * Math.log10(P3), T1 = 29.530588, N1 = 24 * 3600, N3 = N1 * 1e3, w2 = .9972695717592592, a1 = 695700, y2 = a1 / C, o0 = .996647180302104, F0 = o0 * o0, d0 = 6378.1366, L3 = d0 / C, C3 = d0 * o0, g2 = 6371, U3 = g2 + 88, O3 = 1738.1 / C, W = 1737.4, R2 = 1736, b3 = R2 / C, k3 = 34 / 60, _t = 81.30056, c1 = .0002959122082855911, At = 4912547451450812e-26, pt = 7243452486162703e-25, u1 = 8887692390113509e-25, Et = 9549535105779258e-26, V0 = 2.825345909524226e-7, H0 = 8.459715185680659e-8, $0 = 1.292024916781969e-8, J0 = 1.524358900784276e-8, F3 = 218869976542597e-26, L1 = u1 / _t;
	function wt(t) {
		switch (t) {
	...
	function R0(t) {
		...
		var h0 = L(.19833 + .05611 * n), Wt = L(.27869 + .04508 * n), Xt = L(.16827 - .36903 * n), jt = L(.34734 - 5.37261 * n), Zt = L(.10498 - 5.37899 * n), it = L(.42681 - .41855 * n), m4 = L(.14943 - 5.37511 * n);
		for (z = .84 * h0 + .31 * Wt + 14.27 * Xt + 7.26 * jt + .28 * Zt + .24 * it, b = 2.94 * h0 + .31 * Wt + 14.27 * Xt + 9.34 * jt + 1.12 * Zt + .83 * it, N = -6.4 * h0 - 1.89 * it, q = .21 * h0 + .31 * Wt + 14.27 * Xt - 88.7 * jt - 15.3 * Zt + .24 * it - 1.86 * m4, G = z - N, p = -3332e-9 * L(.59734 - 5.37261 * n) - 539e-9 * L(.35498 - 5.37899 * n) - 64e-9 * L(.39943 - 5.37511 * n), x = t0 * G0(.60643382 + 1336.85522467 * n - 313e-8 * d) + z / w0, g = t0 * G0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + b / w0, m = t0 * G0(.99312619 + 99.99735956 * n - 44e-8 * d) + N / w0, R = t0 * G0(.25909118 + 1342.2278298 * n - 892e-8 * d) + q / w0, D = t0 * G0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / w0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
		...
		let I4 = (1.000002708 + 139.978 * p) * (18519.699999999997 + E) * Math.sin(i) - 6.24 * Math.sin(3 * i) + _;
		return {
			geo_eclip_lon: t0 * G0((x + M / w0) / t0),
			geo_eclip_lat: Math.PI / (180 * 3600) * I4,
			distance_au: w0 * L3 / (.999953253 * w)
		};
	}
	var P2 = class {
	...
	function Y3(t) {
		...
		return new P2(k * m + q, g + N, k * a, k * c, u, G);
	}
	function T2(t, n) {
		return [
	...
	function X3(t, n) {
		...
		let i, c, a;
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - C3;
		else {
	...
	function l1(t, n, e, r, o) {
		Y0(e), C1(r), C1(o);
		const s = y(n), i = U2(s, e), c = n0(t, s, o), a = [
			c.x - i[0],
			c.y - i[1],
			c.z - i[2]
		];
	...
	function re(t, n) {
		...
		return n || (r = Q0(r, t.t, O.From2000), r = q0(r, t.t, O.From2000)), X3(r, e);
	}
	function oe(t, n) {
	...
	function i0(t) {
		const n = y(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = Q0(K3(n, [
	...
	function G1(t) {
		const n = y(t), e = R0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = g0(n), i = Rt(q0(D2(s.mobl, o), n, O.From2000), n), c = s.tobl * A, a = mt(i, Math.cos(c), Math.sin(c));
	...
	function X0(t, n) {
		const e = n.tt / W0;
		return Pt(V2(z0(t[Dt], e, !0), z0(t[zt], e, !1), z0(t[Q1], e, !1))).ToAstroVector(n);
	}
	function d1(t, n) {
		const e = n / W0, r = z0(t[Dt], e, !0), o = z0(t[zt], e, !1), s = z0(t[Q1], e, !1), i = vt(t[Dt], e), c = vt(t[zt], e), a = vt(t[Q1], e), u = Math.cos(r), h = Math.sin(r), l = Math.cos(o), d = Math.sin(o), p = +(a * l * u) - s * d * u * c - s * l * h * i, M = +(a * l * h) - s * d * h * c + s * l * u * i, _ = +(a * d) + s * l * c, E = V2(r, o, s), w = [
			p / W0,
			M / W0,
			_ / W0
		];
	...
	const Tt = 51, ie = 29200, j0 = 146, S0 = 201, P0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	function Ct(t, n) {
		...
		return new H2(r, new Nt(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const ce = [];
	function $2(t, n) {
	...
	function Ut(t) {
		const n = ae(t), e = new S1(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new H2(e, new Nt(n.tt, r, o, s));
	}
	function ue(t, n) {
		const e = P0[0][0];
		if (n < e || n > P0[Tt - 1][0]) return null;
		const r = $2((n - e) / ie, Tt - 1);
		if (!t[r]) {
			const s = t[r] = [];
	...
	function Ot(t, n) {
		...
		const s = ue(ce, t.tt);
		if (s) {
			const i = $2((t.tt - s[0].tt) / j0, S0 - 1), c = s[i], a = s[i + 1], u = c.a.mean(a.a), h = Z0(t.tt - c.tt, c.r, c.v, u), l = Lt(t.tt - c.tt, c.v, u), d = Z0(t.tt - a.tt, a.r, a.v, u), p = Lt(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / j0;
			e = h.mul(1 - M).add(d.mul(M)), r = l.mul(1 - M).add(p.mul(M));
		} else {
			let i;
			t.tt < P0[0][0] ? i = J2(P0[0], t.tt, -j0) : i = J2(P0[Tt - 1], t.tt, +j0), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function K1(t, n) {
		...
		return j1(le, he(t, n.mu, r));
	}
	function fe(t) {
		const n = new y0(t);
		return new G2(K1(n, B1[0]), K1(n, B1[1]), K1(n, B1[2]), K1(n, B1[3]));
	}
	function a0(t, n) {
		var e = y(n);
	...
	function W1(t, n) {
		...
		if (t === f.Sun) c = T3 + 5 * Math.log10(a);
		else if (t === f.Moon) c = pe(o, u, a);
		else if (t === f.Saturn) {
			const l = Ae(o, u, a, i, e);
			c = l.mag, h = l.ring_tilt;
		} else c = _e(t, o, u, a);
		return new K2(e, c, o, u, a, i, s, h);
	}
	function _1(t) {
	...
	function Ee(t) {
		return W2(new Date(t.time.date.getTime() + 6 * N3));
	}
	var X2 = class {
		constructor(t, n, e) {
	...
	function ye(t) {
		switch (t) {
			...
			case f.Moon: return O3;
			default: return 0;
		}
	}
	function ge(t, n, e, r, o, s = 0) {
		if (!Number.isFinite(s) || s < 0) throw `Invalid value for metersAboveGround: ${s}`;
		const i = ye(t), c = j2(n.height - s);
		return Z2(t, n, e, r, o, i, we(n, s) - k3 * c.density);
	}
	function Re(t, n, e, r, o, s) {
	...
	function Ft(t, n, e, r, o, s, i) {
		if (s < 0 && i >= 0) return new xe(r, o, s, i);
	...
	function ve(t, n, e) {
		const r = y(n), o = L2(r), s = l1(t, r, e, !0, !0);
	...
	function an(t, n) {
		if (n = y(n), t === f.Neptune || t === f.Pluto) return Ne(t, n);
		function e(a) {
	...
	function be(t, n, e) {
		n = y(n);
		const r = cn(t.lon);
		return X1(new B0(t.lat + un(e, t.lat), r, t.dist), n);
	}
	function E1(t, n) {
		let e;
	...
	function Be(t, n) {
		...
		t %= 24, t < 0 && (t += 24), Jt || (Jt = Z1(new y0(-45655.74141261017)), pn = new y0(0));
		const e = X1(new B0(n, 15 * t, 1), pn), r = Vt(w1(Jt, e)), o = 10 / 240, s = o / 15;
		for (let i of qe) {
	...
	function y1(t, n, e, r) {
		...
		return new Ke(n, o, C * Math.hypot(s, i, c), +a1 - (1 + o) * (a1 - t), -a1 + (1 + o) * (a1 + t), e, r);
	}
	function nt(t) {
		const n = n0(f.Sun, t, !0), e = new P(-n.x, -n.y, -n.z, n.t);
		return y1(U3, t, i0(t), e);
	}
	function yn(t) {
	...
	function Xe(t) {
		const e = J((r) => Qt(yn, r), t.AddDays(-.03), t.AddDays(.03));
		if (!e) throw "Failed to find peak Moon shadow time.";
		return yn(e);
	}
	function je(t, n, e) {
		const o = J((s) => Ye(t, n, s), e.AddDays(-1), e.AddDays(1));
	...
	function Rn(t, n) {
		const e = new P(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = gn(Math.asin(y2 / e.Length()), Math.asin(b3 / n.Length()), U1(n, e) * A);
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				...
				if (i.r < i.p + W) {
					...
					return i.r < i.k + W && (c = j.Partial, h = qt(i.time, i.k + W, l), i.r + W < i.k ? (c = j.Total, a = 1, u = qt(i.time, i.k - W, h)) : a = gn(W, i.k, i.r)), new wn(c, a, i.time, l, h, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function r4(t, n) {
		...
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = rt(n, 1, Tn, s, t.time), h = rt(n, -1, Tn, t.time, i), l = In(t.k)) : l = j.Partial;
		const d = l === j.Total ? 1 : Rn(t.dir, t.target);
	...
	function s4(t, n) {
		return t = y(t), Ln(t.AddDays(10), n);
	}
	var Cn = class {
		constructor(t, n, e, r) {
	...
	function On(t, n) {
		...
		for (;;) {
			...
			if (e1(t, i) < e) {
				const c = je(t, o, i);
				if (c.r < c.p) {
					const a = c.time.AddDays(-r), u = Un(t, o, a, c.time, -1), h = c.time.AddDays(+r), l = Un(t, o, c.time, h, 1), d = 60 * e1(t, c.time);
	...
	function a4(t, n) {
		return n = y(n), On(t, n.AddDays(100));
	}
	var _0;
	(function(t) {
	...
	const { MakeTime: F4, SearchRiseSet: V4, Body: H4, Observer: d4 } = ot(z1);
	...
	2 * Math.PI;
	const { Rotation_EQD_EQJ: Q4, CombineRotation: q4, MakeRotation: B4 } = ot(z1);
	Math.PI / (180 * 3600);
	function S4(t) {
		return t == null || !Number.isFinite(t) || t < 1 || t > 4 ? 4 : Math.floor(t);
	}
	const { HelioState: W4 } = ot(z1);
	function _4(t, n) {
	...
	function p4(t) {
		...
		const n = A3(Jn(t)), { x: e, y: r, z: o, ra: s, dec: i } = t, c = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let a = null;
		return c && (a = a3({
			x: e,
			y: r,
	...
	function R4(t) {
		const { catalog: n, maxDistanceLevel: e, complexStars: r, bounds: o, raInHours: s } = t, i = A4(_4(n, { raInHours: s }).filter((_) => !Yt(_)), o, { maxDistanceLevel: e }), c = (_, E, w, x) => ({
			positions: _,
			sizes: E,
			colors: w,
			brightness: x,
		...
		if (i.length === 0) {
			...
			const _ = g4(), E = _.length, w = new Float32Array(E * 3), x = new Float32Array(E).fill(Gn), g = new Float32Array(E * 3).fill(1), m = new Float32Array(E).fill(1);
			for (let R = 0; R < E; R += 1) w[R * 3] = _[R].x, w[R * 3 + 1] = _[R].y, w[R * 3 + 2] = _[R].z;
			return c(w, x, g, m);
		}
		const a = i.map(p4), u = a.length, h = new Float32Array(u * 3), l = new Float32Array(u), d = new Float32Array(u * 3), p = new Float32Array(u), M = [];
		for (let _ = 0; _ < u; _ += 1) {
			const E = a[_], w = i[_];
			h[_ * 3] = E.x, h[_ * 3 + 1] = E.y, h[_ * 3 + 2] = E.z;
			const x = w ? y3(w.mag, Jn(w)) : b0;
			l[_] = r ? x : Gn;
			const g = w?.c != null ? `#${w.c}` : void 0, [m, R, D] = r ? E4(g) : [
				1,
				1,
				1
			];
