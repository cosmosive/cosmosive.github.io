(function() {
	const F = Symbol("Comlink.proxy"), Ae = Symbol("Comlink.endpoint"), de = Symbol("Comlink.releaseProxy"), K = Symbol("Comlink.finalizer"), M = Symbol("Comlink.thrown"), z = (e) => typeof e == "object" && e !== null || typeof e == "function", W = new Map([["proxy", {
		canHandle: (e) => z(e) && e[F],
		serialize(e) {
	...
	function D(e, n = globalThis, r = ["*"]) {
		n.addEventListener("message", function o(s) {
			...
			Promise.resolve(u).catch((A) => ({
				...
				const [S, E] = b(A);
				n.postMessage(Object.assign(Object.assign({}, S), { id: i }), E), l === "RELEASE" && (n.removeEventListener("message", o), $(n), K in e && typeof e[K] == "function" && e[K]());
			}).catch((A) => {
				const [S, E] = b({
	...
	Math.PI / 180;
	180 / Math.PI;
	const L = 86400, Me = 365.25, m = 299792.458, h = m * Me * L, a = 149597870.7, j = 648e3 / Math.PI, ee = j * a / h, Ne = 23.4392911, Ue = 1e3, be = m, Oe = m ** Math.sqrt(Math.E), ge = m ** 2, xe = m ** Math.E, Pe = [
		1,
		Ue,
		be,
		Oe
	...
	const se = 1e3 / a, B = se * h * ee, mt = 695700, Rt = 6378.137;
	function ae(e) {
	...
	function le(e, n) {
		...
		switch (r.type) {
			...
			case "step": {
				...
				const s = e.jd + e.speed * o / Ut, i = Math.max(H, Math.min(J, s)), l = {
					...e,
					jd: i
				}, c = [{
					type: "tick",
	...
	D({
		...
		subscribe(e) {
			y != null && y !== e && console.warn("[simulationWorker] replacing existing outbound subscriber (double subscribe)"), y = e, V();
		},
		unsubscribe() {
			y = null, X();
		}
