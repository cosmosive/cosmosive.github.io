(function() {
	...
	function jt(t) {
		const { catalog: n, maxDistanceLevel: e, simplifyStars: r, bounds: a, raInHours: o } = t, R = Vt(Ht(n, { raInHours: o }).filter((l) => !V(l)), a, { maxDistanceLevel: e }), A = (l, m, f, g) => ({
			positions: l,
			sizes: m,
			colors: f,
			brightness: g,
		...
		for (let l = 0; l < i; l++) {
			...
			_[l] = r ? ct : g;
			const k = f?.c != null ? `#${f.c}` : void 0, [K, M, Wt] = r ? [
				1,
				1,
				1
			] : Kt(k);
			E[l * 3] = K, E[l * 3 + 1] = M, E[l * 3 + 2] = Wt, N[l] = r ? 1 : f?.br != null && Number.isFinite(f.br) ? Math.max(0, Math.min(1, f.br)) : 1, f?.proper && ut.push({
				position: { ...m },
				name: f.proper,
