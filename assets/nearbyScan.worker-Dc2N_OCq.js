(function() {
	var Tn = Object.defineProperty, V2 = (t, n) => {
		let e = {};
		for (var r in t) Tn(e, r, {
			get: t[r],
			enumerable: !0
		});
		return n || Tn(e, Symbol.toStringTag, { value: "Module" }), e;
	};
	const Dn = Symbol("Comlink.proxy"), J2 = Symbol("Comlink.endpoint"), Y2 = Symbol("Comlink.releaseProxy"), E1 = Symbol("Comlink.finalizer"), Nt = Symbol("Comlink.thrown"), Cn = (t) => typeof t == "object" && t !== null || typeof t == "function", vn = /* @__PURE__ */ new Map([["proxy", {
		canHandle: (t) => Cn(t) && t[Dn],
		serialize(t) {
	...
	function y1(t, n = globalThis, e = ["*"]) {
		n.addEventListener("message", function r(o) {
			...
			Promise.resolve(u).catch((l) => ({
				...
				const [h, d] = vt(l);
				n.postMessage(Object.assign(Object.assign({}, h), { id: s }), d), i === "RELEASE" && (n.removeEventListener("message", r), zn(n), E1 in t && typeof t[E1] == "function" && t[E1]());
			}).catch((l) => {
				const [h, d] = vt({
	...
	function p1(t, n, e = [], r = function() {}) {
		...
		const s = new Proxy(r, {
			...
			apply(i, c, a) {
				...
				if (u === J2) return k0(t, n, { type: "ENDPOINT" }).then(D0);
	...
	function Un(t) {
		const n = t.map(vt);
		return [n.map((e) => e[0]), W2(n.map((e) => e[1]))];
	}
	...
	function tr(t) {
		return Object.assign(t, { [Dn]: !0 });
	}
	function vt(t) {
		for (const [n, e] of vn) if (e.canHandle(t)) {
	...
	function D0(t) {
		switch (t.type) {
			case "HANDLER": return vn.get(t.name).deserialize(t.value);
	...
	2 * Math.PI;
	const er = Math.PI / 180;
	180 / Math.PI;
	const zt = 86400, R1 = 365.25, Ln = 2451545, p0 = 299792.458, g1 = p0 * R1 * zt, F = 149597870.7, x1 = 648e3 / Math.PI, bt = x1 * F, I1 = bt / g1, kn = 23.4392911, rr = 1e3, or = p0, sr = p0 ** Math.sqrt(Math.E), ir = p0 ** 2, ar = p0 ** Math.E, cr = [
		1,
		rr,
		or,
		sr
	...
	const Kn = .999999999;
	function ur(t) {
		const n = Math.min(Math.max(t, 0), Kn);
	...
	var Ut = (t, n, e) => Object.defineProperty(t, n, {
		...
		writable: !0,
		configurable: !0
	}), lr = Symbol.for("bitecs-relation"), hr = Symbol.for("bitecs-pairTarget"), fr = Symbol.for("bitecs-isPairComponent"), dr = Symbol.for("bitecs-relationData"), Bn = () => {
		let t = {
			...
			if (!t.pairsMap.has(r)) {
				...
				Ut(o, lr, n), Ut(o, hr, r), Ut(o, fr, !0), t.pairsMap.set(r, o);
			}
	...
	function $n(t) {
		return t == null ? K.Barren : t >= Ir ? K.Molten : t >= wr ? K.Desert : t >= Pr ? K.Oceanic : K.Barren;
	}
	function Nr(t) {
		const n = T1(t.radiusEarth), e = T1(t.massEarth), r = T1(t.eqTempK);
		return n != null ? n > Rr ? K.GasGiant : n > pr ? K.IceGiant : $n(r) : e != null ? e > xr ? K.GasGiant : e > gr ? K.IceGiant : $n(r) : K.Barren;
	}
	const Tr = 384399 / F, Dr = 421800 / F, Cr = 671034 / F, vr = 1070400 / F, zr = 1882700 / F, br = 9376 / F, Ur = 23463 / F, Or = 185540 / F, Lr = 237948 / F, kr = 294619 / F, Fr = 377396 / F, Kr = 527108 / F, Hr = 1221870 / F, Br = 3560820 / F, Gr = 129390 / F, Vr = 190900 / F, Jr = 266e3 / F, Yr = 436300 / F, $r = 583520 / F, jr = 354759 / F, qr = {
		[S.SUN]: {
			name: "Sun",
			type: "star",
			color: "#f5e042",
	...
	const D1 = Object.fromEntries(Object.entries(qr).map(([t, n]) => [t, Xr(n)]));
	function Qr(t) {
		return D1[t]?.name ?? t;
	}
	function Wr(t) {
	...
	const lt = 1e3, C0 = lt / F, F0 = C0 * g1 * I1, C1 = 695700, jn = 6378.137;
	function qn(t) {
	...
	function uo(t) {
		switch (ne(t)) {
			case "local_system": return K0.local * ht + ao(t.source, t.bodyType, t.rawId);
			case "exo_host_star": return K0.exoHost * ht + io(te - t.exoCount, 0, te);
			case "dso": return K0.dso * ht + co(t.bodyType);
	...
	const ft = (t) => t;
	function lo(t, n) {
		const r = z1(Number.isFinite(t.distPc) && t.distPc > 0 ? t.distPc : 0);
		if (n) {
	...
	function ho(t, n) {
		...
		const r = z1(ee(t)), { x: o, y: s, z: i, ra: c, dec: a } = t, u = o != null && s != null && i != null && Number.isFinite(o) && Number.isFinite(s) && Number.isFinite(i);
		let l = null;
		return u && (l = Hn({
			x: o,
			y: s,
			z: i
		})), l == null && (l = v1(c, a)), l == null ? ft({
			x: 0,
			y: 0,
			z: r
		}) : ft({
	...
	function fo(t, n) {
		...
		const r = z1(ee(t)), { x: o, y: s, z: i, ra: c, dec: a } = t;
	...
	function re(t, n, e) {
		return ho(t, e), Mo(t, e.x, e.y, e.z, Ot), e.x += Ot.x * n, e.y += Ot.y * n, e.z += Ot.z * n, e;
	}
	const Lt = {
		x: 0,
		y: 0,
	...
	function kt(t) {
		const n = po(t.objectScenePosition, t.viewpointScenePosition), e = se(n);
		...
		const s = Math.acos(yo(o)), i = Wn(t.viewpointScenePosition, t.objectScenePosition), { source: c, rawId: a } = Yn(t.entityId), u = {
			source: c,
			bodyType: t.bodyType,
			rawId: a,
			isLocal: t.isLocal,
	...
	function go(t) {
		const n = Math.max(1, Math.floor(t.maxCount ?? mo)), e = se(t.viewForward);
		if (!Number.isFinite(e) || e <= 0) return [];
		const r = {
			x: t.viewForward.x / e,
			y: t.viewForward.y / e,
			z: t.viewForward.z / e
		}, o = Number.isFinite(t.halfAngleRad) ? Math.max(0, t.halfAngleRad) : 0, s = Math.cos(o), i = t.activeSystem ?? Ao, c = i.originId != null ? Yn(i.originId).rawId : null, { exoCountByStarId: a } = t, u = [], l = i.kind === "sol";
		...
		const h = t.viewpointScenePosition, d = t.maxDistancePc != null && Number.isFinite(t.maxDistancePc) ? t.maxDistancePc : null, _ = (p) => {
			...
			return !(!Number.isFinite(C) || C <= 0 || !((I * r.x + T * r.y + w * r.z) / C >= s) || d != null && Wn(h, p) > d);
		};
		for (const p of t.deepSkyObjects) {
			const I = lo(p);
			if (_(I)) {
				const T = ut("dso", p.catalogId), w = kt({
					entityId: T,
					name: p.name,
					category: "dso",
					bodyType: p.type,
		...
		for (const p of t.stars) if (R(p) && (re(p, M, E), _(E))) {
			const I = ut("hyg", p.id), T = a?.get(String(p.id)) ?? 0, w = kt({
				entityId: I,
				name: so(p),
				category: Zn(p),
				bodyType: "star",
				...
				exoCount: T,
				hasProperName: Zn(p) === "proper_star",
		...
		return t.selection === "priority" ? [...N].sort(ie).slice(0, n) : [...N].sort(Ro).slice(0, n).sort(ie);
	}
	kn * er;
	var xo = V2({
		AU_PER_LY: () => ae,
		...
		CombineRotation: () => P0,
		Constellation: () => t3,
		ConstellationInfo: () => u2,
		CorrectLightTravel: () => Oe,
		DEG2RAD: () => y,
		DefineStar: () => qo,
		...
		EclipseKind: () => t0,
		Ecliptic: () => St,
		EclipticCoordinates: () => Ne,
		EclipticGeoMoon: () => Zt,
		EclipticLongitude: () => L0,
		Elongation: () => je,
		ElongationEvent: () => $e,
		Equator: () => Mt,
		EquatorFromVector: () => un,
		...
		KM_PER_AU: () => U,
		LagrangePoint: () => m3,
		LagrangePointFast: () => D2,
		Libration: () => es,
		LibrationInfo: () => pe,
		LocalSolarEclipseInfo: () => E2,
		LunarEclipseInfo: () => l2,
		MakeRotation: () => as,
		MakeTime: () => x,
		MassProduct: () => G1,
		...
		Search: () => $,
		SearchAltitude: () => Ds,
		SearchGlobalSolarEclipse: () => A2,
		SearchHourAngle: () => zs,
		SearchLocalSolarEclipse: () => g2,
		SearchLunarApsis: () => qe,
		SearchLunarEclipse: () => M2,
		SearchMaxElongation: () => Os,
		SearchMoonNode: () => T2,
		SearchMoonPhase: () => pt,
		SearchMoonQuarter: () => He,
		SearchPeakMagnitude: () => Ls,
		SearchPlanetApsis: () => Qe,
		SearchRelativeLongitude: () => ot,
		SearchRiseSet: () => Ts,
		SearchSunLongitude: () => ke,
		SearchTransit: () => w2,
		SeasonInfo: () => Ye,
		Seasons: () => Us,
	...
	const Ft = 173.1446326846693, U = 149597870.69098932, ae = 63241.07708807546, y = .017453292519943295, L1 = .26179938779914946, H = 57.29577951308232, k1 = 3.819718634205488, Io = 71492, wo = 66854, Po = 69911, No = 1821.6, To = 1560.8, Do = 2631.2, Co = 2410.3, ce = 365.24217, ue = /* @__PURE__ */ new Date("2000-01-01T12:00:00Z"), e0 = 2 * Math.PI, R0 = 3600 * (180 / Math.PI), H0 = 484813681109536e-20, le = 648e3, vo = 2 * le, zo = 7292115e-11, bo = le / Math.PI, Uo = -.17 - 5 * Math.log10(bo), Kt = 29.530588, Ht = 86400, Oo = Ht * 1e3, he = .9972695717592592, F1 = 695700, fe = F1 / U, s0 = .996647180302104, Bt = s0 * s0, S0 = 6378.1366, Lo = S0 / U, ko = S0 * s0, Fo = 6371, Ko = 6459, Ho = 1738.1 / U, W = 1737.4, de = 1736, Bo = de / U, Go = 34 / 60, Vo = 81.30056, dt = .0002959122082855911, K1 = 4912547451450812e-26, H1 = 7243452486162703e-25, Me = 8887692390113509e-25, B1 = 9549535105779258e-26, B0 = 2.825345909524226e-7, G0 = 8.459715185680659e-8, V0 = 1.292024916781969e-8, J0 = 1.524358900784276e-8, Jo = 218869976542597e-26, Yo = Me / Vo;
	function G1(t) {
		switch (t) {
			case f.Sun: return dt;
			case f.Mercury: return K1;
			case f.Venus: return H1;
			case f.Earth: return Me;
	...
	var g0 = class m1 {
		constructor(n) {
			...
			if (n instanceof Date && Number.isFinite(n.getTime())) {
				this.date = n, this.ut = (n.getTime() - ue.getTime()) / e, this.tt = Ae(this.ut);
				return;
			}
			if (Number.isFinite(n)) {
				this.date = new Date(ue.getTime() + n * e), this.ut = n, this.tt = Ae(this.ut);
				return;
			}
	...
	function x0(t) {
		if (!Yt || Math.abs(Yt.tt - t.tt) > 1e-6) {
			const n = ts(t), e = me(t), r = e + n.deps / 3600;
			Yt = {
				tt: t.tt,
				dpsi: n.dpsi,
				deps: n.deps,
	...
	function I0(t) {
		...
		let i, c, a, u, l, h, d, _, M, m, g, R, E, P, N, p, I, T, w, C, k, G, Q, u0 = r(-6, 6, 1, 4), o0 = r(-6, 6, 1, 4);
		function m0(z, O) {
			return o(u0, z, O);
		}
		function l0(z, O) {
		...
		d = n * n, M = 0, Q = 0, g = 0, R = 3422.7;
		var f0 = b(.19833 + .05611 * n), gn = b(.27869 + .04508 * n), xn = b(.16827 - .36903 * n), In = b(.34734 - 5.37261 * n), wn = b(.10498 - 5.37899 * n), A1 = b(.42681 - .41855 * n), d4 = b(.14943 - 5.37511 * n);
		for (T = .84 * f0 + .31 * gn + 14.27 * xn + 7.26 * In + .28 * wn + .24 * A1, w = 2.94 * f0 + .31 * gn + 14.27 * xn + 9.34 * In + 1.12 * wn + .83 * A1, C = -6.4 * f0 - 1.89 * A1, k = .21 * f0 + .31 * gn + 14.27 * xn - 88.7 * In - 15.3 * wn + .24 * A1 - 1.86 * d4, G = T - C, _ = -3332e-9 * b(.59734 - 5.37261 * n) - 539e-9 * b(.35498 - 5.37899 * n) - 64e-9 * b(.39943 - 5.37511 * n), E = e0 * Y0(.60643382 + 1336.85522467 * n - 313e-8 * d) + T / R0, P = e0 * Y0(.37489701 + 1325.55240982 * n + 2565e-8 * d) + w / R0, N = e0 * Y0(.99312619 + 99.99735956 * n - 44e-8 * d) + C / R0, p = e0 * Y0(.25909118 + 1342.2278298 * n - 892e-8 * d) + k / R0, I = e0 * Y0(.82736186 + 1236.85308708 * n - 397e-8 * d) + G / R0, l = 1; l <= 4; ++l) {
			switch (l) {
				case 1:
		...
		function B2(z, O, B, J) {
			for (var j = {
				...
				B,
				J
			], E0 = 1; E0 <= 4; ++E0) T0[E0] !== 0 && at(j.x, j.y, m0(T0[E0], E0), l0(T0[E0], E0), (Pn, ct) => (j.x = Pn, j.y = ct));
			return j;
		}
		function A(z, O, B, J, j, T0, E0, Pn) {
			var ct = B2(j, T0, E0, Pn);
			M += z * ct.y, Q += O * ct.y, g += B * ct.x, R += J * ct.x;
		}
		...
		function d0(z, O, B, J, j) {
			return z * B2(O, B, J, j).y;
		}
		m = 0, m += d0(-526.069, 0, 0, 1, -2), m += d0(-3.352, 0, 0, 1, -4), m += d0(44.297, 1, 0, 1, -2), m += d0(-6, 1, 0, 1, -4), m += d0(20.599, -1, 0, 1, 0), m += d0(-30.598, -1, 0, 1, -2), m += d0(-24.649, -2, 0, 1, 0), m += d0(-2, -2, 0, 1, -2), m += d0(-22.571, 0, 1, 1, -2), m += d0(10.985, 0, -1, 1, -2), M += .82 * b(.7736 - 62.5512 * n) + .31 * b(.0466 - 125.1025 * n) + .35 * b(.5785 - 25.1042 * n) + .66 * b(.4591 + 1335.8075 * n) + .64 * b(.313 - 91.568 * n) + 1.14 * b(.148 + 1331.2898 * n) + .21 * b(.5918 + 1056.5859 * n) + .44 * b(.5784 + 1322.8595 * n) + .24 * b(.2275 - 5.7374 * n) + .28 * b(.2965 + 2.6929 * n) + .33 * b(.3132 + 6.3368 * n), i = p + Q / R0;
		let M4 = (1.000002708 + 139.978 * _) * (18518.511 + 1.189 + g) * Math.sin(i) - 6.24 * Math.sin(3 * i) + m;
		return {
			geo_eclip_lon: e0 * Y0((E + M / R0) / e0),
			geo_eclip_lat: Math.PI / 648e3 * M4,
			distance_au: R0 * Lo / (.999953253 * R)
		};
	}
	var pe = class {
	...
	function es(t) {
		...
		return new pe(H * N + k, P + C, H * a, H * c, u, G);
	}
	function Re(t, n) {
		return [
	...
	function os(t, n) {
		...
		if (s < 1e-6) i = 0, c = o > 0 ? 90 : -90, a = Math.abs(o) - ko;
			...
			for (;;) {
				...
				const m = -.006694397995865464 * S0, g = l * l, R = h * h, E = g + Bt * R;
				d = Math.sqrt(E);
			...
			c = H * u;
			const M = S0 / d;
			Math.abs(h) > Math.abs(l) ? a = o / h - Bt * M : a = s / l - M;
		}
		return new $1(c, i, 1e3 * a);
	}
	function J1(t, n) {
		const e = t.latitude * y, r = Math.sin(e), o = Math.cos(e), s = 1 / Math.hypot(o, s0 * r), i = Bt * s, c = t.height / 1e3, a = S0 * s + c, u = S0 * i + c, l = (15 * n + t.longitude) * y, h = Math.sin(l), d = Math.cos(l);
		return {
			pos: [
				a * o * d / U,
				a * o * h / U,
	...
	function ss(t, n, e) {
		return e === L.Into2000 ? ge(Ie(t, n, e), n, e) : Ie(ge(t, n, e), n, e);
	}
	function we(t, n) {
		const e = J1(n, _0(t)).pos;
		return Xt(e, t, L.Into2000);
	}
	var v = class {
		constructor(t, n, e, r) {
	...
	function Wt(t, n, e, r, o) {
		...
		const R = -15 * _0(s);
		let E = X0(R, M), P = X0(R, m), N = X0(R, g), p = [
			h * _,
			h * d,
			l
		];
		...
		let G = H * Math.atan2(C, I), Q = e, u0 = r;
		if (o) {
			...
			if (G -= m0, m0 > 0 && G > 3e-4) {
				...
				C = Math.hypot(b[0], b[1]), C > 0 ? (Q = k1 * Math.atan2(b[1], b[0]), Q < 0 && (Q += 24)) : Q = 0, u0 = H * Math.atan2(b[2], C);
			}
		}
		return new Pe(k, 90 - G, Q, u0);
	}
	function Q0(t) {
	...
	function De(t) {
		const n = x(t).AddDays(-1 / Ft), e = Z0(q.Earth, n), [r, o, s] = Xt([
			-e.x,
			-e.y,
			-e.z
		], n, L.From2000), i = y * x0(n).tobl, c = Math.cos(i), a = Math.sin(i);
		return j1(new v(r, o, s, n), c, a);
	}
	function Mt(t, n, e, r, o) {
		Q0(e), Gt(r), Gt(o);
		const s = x(n), i = we(s, e), c = r0(t, s, o), a = [
			c.x - i[0],
			c.y - i[1],
			c.z - i[2]
		];
		return Te(r ? Xt(a, s, L.From2000) : a, s);
	}
	function cs(t, n, e) {
		const r = x(t);
		let o = J1(n, _0(r)).pos;
		return e || (o = Xt(o, r, L.Into2000)), Y1(o, r);
	}
	function us(t, n, e) {
		const r = x(t), o = J1(n, _0(r)), s = new V(o.pos[0], o.pos[1], o.pos[2], o.vel[0], o.vel[1], o.vel[2], r);
	...
	function ls(t, n) {
		...
		return n || (r = $0(r, t.t, L.From2000), r = j0(r, t.t, L.From2000)), os(r, e);
	}
	function hs(t, n) {
	...
	function j1(t, n, e) {
		...
		return new Ne(new v(r, o, s, t.t), a, c);
	}
	function St(t) {
	...
	function a0(t) {
		const n = x(t), e = I0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = $0(ns(n, [
	...
	function Zt(t) {
		const n = x(t), e = I0(n), r = e.distance_au * Math.cos(e.geo_eclip_lat), o = [
			...
			r * Math.sin(e.geo_eclip_lon),
			e.distance_au * Math.sin(e.geo_eclip_lat)
		], s = x0(n), i = Y1(j0(Ee(s.mobl, o), n, L.From2000), n), c = s.tobl * y, a = j1(i, Math.cos(c), Math.sin(c));
	...
	const W0 = 365250, Q1 = 0, W1 = 1, t1 = 2;
	function Z1(t) {
		return new X(t[0] + 4.4036e-7 * t[1] - 1.90919e-7 * t[2], -4.79966e-7 * t[0] + .917482137087 * t[1] - .397776982902 * t[2], .397776982902 * t[1] + .917482137087 * t[2]);
	}
	function Ce(t, n, e) {
	...
	function Z0(t, n) {
		const e = n.tt / W0;
		return Z1(Ce(v0(t[Q1], e, !0), v0(t[W1], e, !1), v0(t[t1], e, !1))).ToAstroVector(n);
	}
	function At(t, n) {
		const e = n / W0, r = v0(t[Q1], e, !0), o = v0(t[W1], e, !1), s = v0(t[t1], e, !1), i = X1(t[Q1], e), c = X1(t[W1], e), a = X1(t[t1], e), u = Math.cos(r), l = Math.sin(r), h = Math.cos(o), d = Math.sin(o), _ = +(a * h * u) - s * d * u * c - s * h * l * i, M = +(a * h * l) - s * d * l * c + s * h * u * i, m = +(a * d) + s * h * c, g = Ce(r, o, s), R = [
			_ / W0,
			M / W0,
			m / W0
		];
		return new b0(n, Z1(g), Z1(R));
	}
	function n1(t, n, e, r) {
	...
	function fs(t) {
		...
		return n1(n, t, f.Jupiter, B0), n1(n, t, f.Saturn, G0), n1(n, t, f.Uranus, V0), n1(n, t, f.Neptune, J0), n;
	}
	const ds = 29200, e1 = 146, z0 = [
		[
			-73e4,
			[
				-26.118207232108,
	...
	var X = class y0 {
		...
		clone() {
			return new Nn(this.tt, this.r, this.v);
		}
		sub(n) {
			return new Nn(this.tt, this.r.sub(n.r), this.v.sub(n.v));
		}
	};
	function Ms(t) {
		let [n, [e, r, o], [s, i, c]] = t;
		return new b0(n, new X(e, r, o), new X(s, i, c));
	}
	function Z(t, n, e, r) {
	...
	var Et = class {
		constructor(t) {
			let n = new b0(t, new X(0, 0, 0), new X(0, 0, 0));
		...
		clone() {
			return new G2(this.tt, this.r.clone(), this.v.clone(), this.a.clone());
		}
	}, ve = class {
		constructor(t, n) {
	...
	function tt(t, n, e, r) {
		return new X(n.x + t * (e.x + t * r.x / 2), n.y + t * (e.y + t * r.y / 2), n.z + t * (e.z + t * r.z / 2));
	}
	function nn(t, n, e) {
		return new X(n.x + t * e.x, n.y + t * e.y, n.z + t * e.z);
	}
	function en(t, n) {
		const e = t - n.tt, r = new Et(t), o = tt(e, n.r, n.v, n.a), s = r.Acceleration(o).mean(n.a), i = tt(e, n.r, n.v, s);
		return new ve(r, new tn(t, i, n.v.add(s.mul(e)), r.Acceleration(i)));
	}
	const Ss = [];
	function ze(t, n) {
	...
	function rn(t) {
		const n = Ms(t), e = new Et(n.tt), r = n.r.add(e.Sun.r), o = n.v.add(e.Sun.v), s = e.Acceleration(r);
		return new ve(e, new tn(n.tt, r, o, s));
	}
	function _s(t, n) {
		const e = z0[0][0];
		if (n < e || n > z0[50][0]) return null;
		const r = ze((n - e) / ds, 50);
		if (!t[r]) {
			const s = t[r] = [];
			s[0] = rn(z0[r]).grav, s[200] = rn(z0[r + 1]).grav;
			let i, c = s[0].tt;
			for (i = 1; i < 200; ++i) s[i] = en(c += e1, s[i - 1]).grav;
			c = s[200].tt;
			var o = [];
			for (o[200] = s[200], i = 199; i > 0; --i) o[i] = en(c -= e1, o[i + 1]).grav;
			for (i = 199; i > 0; --i) {
				const a = i / 200;
	...
	function be(t, n, e) {
		...
		for (let s = 0; s < o; ++s) r = en(s + 1 === o ? n : r.grav.tt + e, r.grav);
		return r;
	}
	function on(t, n) {
		let e, r, o;
		const s = _s(Ss, t.tt);
		if (s) {
			const i = ze((t.tt - s[0].tt) / e1, 200), c = s[i], a = s[i + 1], u = c.a.mean(a.a), l = tt(t.tt - c.tt, c.r, c.v, u), h = nn(t.tt - c.tt, c.v, u), d = tt(t.tt - a.tt, a.r, a.v, u), _ = nn(t.tt - a.tt, a.v, u), M = (t.tt - c.tt) / e1;
			e = l.mul(1 - M).add(d.mul(M)), r = h.mul(1 - M).add(_.mul(M));
		} else {
			let i;
			t.tt < z0[0][0] ? i = be(z0[0], t.tt, -146) : i = be(z0[50], t.tt, 146), e = i.grav.r, r = i.grav.v, o = i.bary;
		}
	...
	function Es(t) {
		const n = new g0(t);
		return new Ue(o1(n, r1[0]), o1(n, r1[1]), o1(n, r1[2]), o1(n, r1[3]));
	}
	function c0(t, n) {
		var e = x(n);
		...
		const s = Jt(t);
		if (s) return a1(new q0(s.dec, 15 * s.ra, s.dist), e);
	...
	function U0(t, n) {
		...
		if (e) return e.dist;
		const r = x(n);
		return t in q ? v0(q[t][t1], r.tt / W0, !1) : c0(t, r).Length();
	}
	function Oe(t, n) {
		let e = n, r = 0;
		for (let o = 0; o < 10; ++o) {
			const s = t(e), i = s.Length() / Ft;
	...
	function Le(t, n, e, r) {
		...
		if (Jt(e)) {
			const c = c0(e, o);
			if (r) {
				const u = s1(n, o), l = new v(c.x - u.x, c.y - u.y, c.z - u.z, o), h = Ft / l.Length();
	...
	function ps(t, n) {
		...
		switch (t) {
			...
			case f.Moon:
			case f.EMB:
				const o = At(q[f.Earth], e.tt), s = t === f.Moon ? _t(e) : q1(e);
	...
	function s1(t, n) {
		...
		switch (t) {
			...
			case f.Pluto: return on(e, !0);
			case f.Moon:
			case f.EMB:
				const o = At(q.Earth, e.tt), s = t == f.Moon ? _t(e) : q1(e);
	...
	function $(t, n, e, r) {
		const o = D(r && r.dt_tolerance_seconds || 1), s = Math.abs(o / Ht);
	...
	function i1(t, n) {
		...
		let o, s, i, c;
		t === f.Sun ? (i = new v(-r.x, -r.y, -r.z, e), s = new v(0, 0, 0, e), o = 0) : (t === f.Moon ? (i = a0(e), s = new v(r.x + i.x, r.y + i.y, r.z + i.z, e)) : (s = c0(t, n), i = new v(s.x - r.x, s.y - r.y, s.z - r.z, e)), o = Vt(i, s));
	...
	function ot(t, n, e) {
		...
		for (let u = 0; u < 100; ++u) {
			let l = -a / 360 * i;
			if (c = c.AddDays(l), Math.abs(l) * Ht < 1) return c;
			let h = a;
	...
	function pt(t, n, e) {
		...
		let i = r(s), c, a, u;
		if (e < 0) {
			if (i < 0 && (i += 360), c = -(Kt * i) / 360, u = c + o, u < e) return null;
			a = Math.max(e, c - o);
		} else {
			if (i > 0 && (i -= 360), c = -(Kt * i) / 360, a = c - o, a > e) return null;
			u = Math.min(e, c + o);
		}
	...
	function He(t) {
		let n = an(t), e = (Math.floor(n / 90) + 1) % 4, r = pt(90 * e, t, 10);
		if (!r) throw "Cannot find moon quarter";
		return new Ke(e, r);
	}
	function ws(t) {
		return He(new Date(t.time.date.getTime() + 6 * Oo));
	}
	var Be = class {
		constructor(t, n, e) {
	...
	function Ts(t, n, e, r, o, s = 0) {
		...
		const i = Ns(t), c = Ge(n.height - s);
		return Ve(t, n, e, r, o, i, Ps(n, s) - Go * c.density);
	}
	function Ds(t, n, e, r, o, s) {
		if (!Number.isFinite(s) || s < -90 || s > 90) throw `Invalid altitude angle: ${s}`;
		return Ve(t, n, e, r, o, 0, s);
	}
	var Cs = class {
		constructor(t, n, e, r) {
	...
	function vs(t, n) {
		...
		const o = y * n;
		return Math.abs((360 / he - e) * Math.cos(o)) + Math.abs(r * Math.sin(o));
	}
	function Ve(t, n, e, r, o, s, i) {
		if (Q0(n), D(o), D(s), D(i), i < -90 || i > 90) throw `Invalid target altitude angle: ${i}`;
		const c = vs(t, n.latitude);
		function a(M) {
			const m = Mt(t, M, n, !0, !0);
			return e * (Wt(M, n, m.ra, m.dec).altitude + H * Math.asin(s / m.dist) - i);
		}
		const u = x(r);
		let l = u, h = u, d = a(l), _ = d;
		for (;;) {
			o < 0 ? (l = h.AddDays(-.42), d = a(l)) : (h = l.AddDays(.42), _ = a(h));
			const M = cn(0, a, c, l, h, d, _);
			if (M) {
				const m = $(a, M.tx, M.ty, {
	...
	function bs(t, n, e) {
		const r = x(n), o = xe(r), s = Mt(t, r, e, !0, !0);
	...
	function Us(t) {
		function n(i, c, a) {
			let u = new Date(Date.UTC(t, c - 1, a)), l = ke(i, u, 20);
		...
		return new Ye(n(0, 3, 10), n(90, 6, 10), n(180, 9, 10), n(270, 12, 10));
	}
	var $e = class {
		constructor(t, n, e, r) {
	...
	function je(t, n) {
		let e = x(n), r = sn(t, f.Sun, e), o;
		r > 180 ? (o = "morning", r = 360 - r) : o = "evening";
		let s = rt(t, e);
		return new $e(e, o, s, r);
	}
	function Os(t, n) {
		function r(c) {
	...
	function Qe(t, n) {
		if (n = x(n), t === f.Neptune || t === f.Pluto) return Fs(t, n);
		function e(a) {
	...
	function Gs(t, n) {
		const e = ln(t);
		return e.lon = We(e.lon), e.lat += gt(n, e.lat), e;
	}
	function Vs(t, n, e) {
		n = x(n);
		const r = We(t.lon);
		return a1(new q0(t.lat + Ze(e, t.lat), r, t.dist), n);
	}
	function gt(t, n) {
		let e;
	...
	function u1(t) {
		return t = x(t), P0($t(t, L.From2000), qt(t, L.From2000));
	}
	function Ys(t) {
		const n = x(t);
		return P0(u1(n), a2(n));
	...
	function l1(t) {
		return t = x(t), P0(qt(t, L.Into2000), $t(t, L.Into2000));
	}
	function hn(t, n) {
		t = x(t);
		const e = Math.sin(n.latitude * y), r = Math.cos(n.latitude * y), o = Math.sin(n.longitude * y), s = Math.cos(n.longitude * y), i = [
			...
			-s,
			0
		], u = -15 * _0(t), l = X0(u, i), h = X0(u, c), d = X0(u, a);
		return new Y([
			[
				h[0],
				d[0],
	...
	function t3(t, n) {
		...
		t %= 24, t < 0 && (t += 24), fn || (fn = u1(new g0(-45655.74141261017)), c2 = new g0(0));
		const e = a1(new q0(n, 15 * t, 1), c2), r = un(xt(fn, e)), o = 10 / 240, s = o / 15;
		for (let i of Zs) {
			const c = i[3] * o, a = i[1] * s, u = i[2] * s;
			if (c <= r.dec && a <= r.ra && r.ra < u) {
				const l = Ws[i[0]];
				return new u2(l[0], l[1], r.ra, r.dec);
			}
		}
	...
	function dn(t, n) {
		const e = we(t, n), r = r0(f.Sun, t, !0), o = a0(t), s = new v(e[0] - o.x, e[1] - o.y, e[2] - o.z, t);
	...
	function r3(t) {
		const n = $((e) => Mn(h1, e), t.AddDays(-.03), t.AddDays(.03));
		if (!n) throw "Failed to find peak Earth shadow time.";
		return h1(n);
	}
	function o3(t) {
		const n = $((e) => Mn(h2, e), t.AddDays(-.03), t.AddDays(.03));
		if (!n) throw "Failed to find peak Moon shadow time.";
		return h2(n);
	}
	function s3(t, n, e) {
		const r = $((o) => e3(t, n, o), e.AddDays(-1), e.AddDays(1));
	...
	function i3(t, n) {
		...
		const s = $((i) => Mn(o, i), e, r);
		if (!s) throw `PeakLocalMoonShadow: search failure for search_center_time = ${t}`;
		return dn(s, n);
	}
	function Sn(t, n, e) {
		const r = e / 1440, o = t.AddDays(-r), s = t.AddDays(+r), i = $((a) => -(h1(a).r - n), o, t), c = $((a) => +(h1(a).r - n), t, s);
	...
	function d2(t, n) {
		const e = new v(t.x + n.x, t.y + n.y, t.z + n.z, t.t), r = f2(Math.asin(fe / e.Length()), Math.asin(Bo / n.Length()), Vt(n, e) * y);
	...
	function M2(t) {
		...
		for (let r = 0; r < 12; ++r) {
			...
			if (Math.abs(s) < 1.8) {
				const i = r3(o);
				if (i.r < i.p + W) {
					let c = t0.Penumbral, a = 0, u = 0, l = 0, h = Sn(i.time, i.p + W, 200);
					return i.r < i.k + W && (c = t0.Partial, l = Sn(i.time, i.k + W, h), i.r + W < i.k ? (c = t0.Total, a = 1, u = Sn(i.time, i.k - W, l)) : a = f2(W, i.k, i.r)), new l2(c, a, i.time, h, l, u);
				}
			}
			e = o.AddDays(10);
		}
	...
	function a3(t) {
		...
		const i = u1(t.time), c = xt(i, t.dir), a = xt(i, t.target);
		...
		let M;
		if (_ > 0) {
			const m = (-h - Math.sqrt(_)) / (2 * l), g = m * c.x - a.x, R = m * c.y - a.y, E = (m * c.z - a.z) * s0, P = Math.hypot(g, R) * Bt;
			...
			const T = It(de, t.time, I, t.dir);
			if (T.r > 1e-9 || T.r < 0) throw `Unexpected shadow distance from geoid intersection = ${T.r}`;
			n = _2(T.k), M = n === t0.Total ? 1 : d2(t.dir, I);
		} else M = void 0;
		return new S2(n, M, e, r, o, s);
	}
	function c3(t) {
		return t = x(t), M2(t.AddDays(10));
	}
	function A2(t) {
		t = x(t);
		const n = 1.8;
	...
	function l3(t, n) {
		...
		const s = d1(n, 1, y2, r, t.time), i = d1(n, -1, y2, t.time, o);
		let c, a, u;
		t.r < Math.abs(t.k) ? (r = t.time.AddDays(-.01), o = t.time.AddDays(.01), c = d1(n, 1, p2, r, t.time), a = d1(n, -1, p2, t.time, o), u = _2(t.k)) : u = t0.Partial;
		const l = u === t0.Total ? 1 : d2(t.dir, t.target);
		return new E2(u, l, s, c, e, a, i);
	}
	...
	function f3(t, n) {
		return t = x(t), g2(t.AddDays(10), n);
	}
	var x2 = class {
		constructor(t, n, e, r) {
	...
	function I2(t, n, e, r, o) {
		const s = $((i) => d3(i, t, n, o), e, r);
	...
	function w2(t, n) {
		...
		for (;;) {
			...
			if (rt(t, s) < e) {
				const i = s3(t, r, s);
				if (i.r < i.p) {
					const c = i.time.AddDays(-1), a = I2(t, r, c, i.time, -1), u = i.time.AddDays(1), l = I2(t, r, i.time, u, 1), h = 60 * rt(t, i.time);
					return new x2(a, i.time, l, h);
				}
			}
			o = s.AddDays(10);
		}
	...
	function M3(t, n) {
		return n = x(n), w2(t, n.AddDays(100));
	}
	var A0;
	(function(t) {
	...
	function T2(t) {
		let n = x(t), e = Zt(n);
		for (;;) {
			const r = n.AddDays(N2), o = Zt(r);
			if (e.lat * o.lat <= 0) {
				const s = o.lat > e.lat ? A0.Ascending : A0.Descending, i = $((c) => s * Zt(c).lat, n, r);
	...
	function _3(t) {
		const n = $0(j0([
			...
			0,
			1
		], t, L.Into2000), t, L.Into2000), e = new v(n[0], n[1], n[2], t), r = un(e), o = 190.41375788700253 + 360.9856122880876 * t.ut;
		return new An(r.ra, r.dec, o, e);
	}
	function A3(t, n) {
		...
		switch (t) {
			...
			case f.Mars:
				...
				break;
			case f.Jupiter:
				const C = y * (99.360714 + 4850.4046 * o), k = y * (175.895369 + 1191.9605 * o), G = y * (300.323162 + 262.5475 * o), Q = y * (114.012305 + 6070.2476 * o), u0 = y * (49.511251 + 64.3 * o);
				s = 268.056595 - .006499 * o + 117e-6 * Math.sin(C) + 938e-6 * Math.sin(k) + .001432 * Math.sin(G) + 3e-5 * Math.sin(Q) + .00215 * Math.sin(u0), i = 64.495303 + .002413 * o + 5e-5 * Math.cos(C) + 404e-6 * Math.cos(k) + 617e-6 * Math.cos(G) - 13e-6 * Math.cos(Q) + 926e-6 * Math.cos(u0), c = 284.95 + 870.536 * r;
				break;
			case f.Saturn:
		...
		return new An(s / 15, i, c, h);
	}
	function m3(t, n, e, r) {
		const o = x(n), s = G1(e), i = G1(r);
		let c, a;
		return e === f.Earth && r === f.Moon ? (c = new V(0, 0, 0, 0, 0, 0, o), a = _t(o)) : (c = s1(e, o), a = s1(r, o)), D2(t, c, s, a, i);
	}
	function D2(t, n, e, r, o) {
		...
		if (t === 4 || t === 5) {
			...
			const I = t == 4 ? .8660254037844386 : -.8660254037844386, T = .5 * i + I * E, w = .5 * c + I * P, C = .5 * a + I * N, k = .5 * E - I * i, G = .5 * P - I * c, Q = .5 * N - I * a, u0 = l * T, o0 = l * w, m0 = l * C, l0 = h * i + d * c + _ * a, h0 = h * E + d * P + _ * N;
			M = new V(u0, o0, m0, l0 * T + h0 * k, l0 * w + h0 * G, l0 * C + h0 * Q, n.t);
		} else {
	...
	var E3 = class n0 {
		constructor(n, e, r) {
			...
			this.curr = new C2(o, i, s);
			const c = this.InternalBodyState(n);
			for (let a of r) {
				const u = new X(a.x + c.r.x, a.y + c.r.y, a.z + c.r.z), l = new X(a.vx + c.v.x, a.vy + c.v.y, a.vz + c.v.z), h = X.zero();
				s.push(new tn(o.tt, u, l, h));
			}
		...
		Update(n) {
			...
			if (r === 0) this.prev = this.Duplicate();
				...
				for (let i = 0; i < this.curr.bodies.length; ++i) {
					...
					a.tt = e.tt, a.r = tt(r, c.r, c.v, u), a.v = nn(r, c.v, u);
				}
				this.CalcBodyAccelerations();
			}
		...
		InternalBodyState(n) {
			if (n === f.SSB) return new b0(this.curr.time.tt, X.zero(), X.zero());
			const e = this.curr.gravitators[n];
			if (e) return e;
			throw `Invalid body: ${n}`;
		}
		static CalcSolarSystem(n) {
			const e = {}, r = new b0(n.tt, X.zero(), X.zero());
			e[f.Mercury] = Z(r, n.tt, f.Mercury, K1), e[f.Venus] = Z(r, n.tt, f.Venus, H1), e[f.Earth] = Z(r, n.tt, f.Earth, 8997011346712498e-25), e[f.Mars] = Z(r, n.tt, f.Mars, B1), e[f.Jupiter] = Z(r, n.tt, f.Jupiter, B0), e[f.Saturn] = Z(r, n.tt, f.Saturn, G0), e[f.Uranus] = Z(r, n.tt, f.Uranus, V0), e[f.Neptune] = Z(r, n.tt, f.Neptune, J0);
			for (let o in e) e[o].r.decr(r.r), e[o].v.decr(r.v);
			return e[f.Sun] = new b0(n.tt, r.r.neg(), r.v.neg()), e;
		}
		CalcBodyAccelerations() {
			for (let n of this.curr.bodies) n.a = X.zero(), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Sun].r, dt), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mercury].r, K1), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Venus].r, H1), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Earth].r, 8997011346712498e-25), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Mars].r, B1), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Jupiter].r, B0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Saturn].r, G0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Uranus].r, V0), n0.AddAcceleration(n.a, n.r, this.curr.gravitators[f.Neptune].r, J0);
		}
		...
		Duplicate() {
			...
			return new C2(this.curr.time, n, e);
		}
	}, C2 = class {
		constructor(t, n, e) {
	...
	const N3 = 5e4, T3 = 2e3, D3 = N3 / v2, C3 = T3 / v2;
	wt(D3);
	wt(C3);
	[...w1];
	function it(t, n, e) {
	...
	function z3(t, n) {
		...
		return b2(Math.sqrt(e));
	}
	function b3(t, n) {
		const e = t.solrad;
		return e != null && Number.isFinite(e) && e > 0 ? b2(e) * C1 : (z3(t.mag, n) ?? 1) * C1;
	}
	function U2(t) {
		if (En(t)) return 0;
		const { x: n, y: e, z: r, plx: o } = t;
	...
	function U3(t) {
		if (En(t)) return it(M0.x, M0.y, M0.z);
		const n = U2(t) * bt, { x: e, y: r, z: o, ra: s, dec: i } = t, c = e != null && r != null && o != null && Number.isFinite(e) && Number.isFinite(r) && Number.isFinite(o);
		let a = null;
		return c && (a = Hn({
			x: e,
			y: r,
			z: o
		})), a == null && (a = v1(s, i)), a == null ? it(0, 0, n) : it(a.x * n, a.y * n, a.z * n);
	}
	function O3(t, n) {
		const e = En(t), r = b3(t, U2(t)), s = mn(U3(t), "icrf-j2000-equatorial");
		let i;
		return t.proper && t.proper.length > 0 ? i = t.proper : e ? i = v3 : i = String(t.id), z2({
			source: "hyg",
			key: String(t.id),
			name: i,
			type: "star",
	...
	function K3(t, n, e) {
		const r = yn(t.semiMajorAxisAU) ? t.semiMajorAxisAU * F : 0;
		if (r === 0) return n;
		const o = t.orbitalPeriodDays;
		if (!yn(o) || !Number.isFinite(e)) return it(n.x + r, n.y, n.z);
		const s = L3(t.eccentricity), i = k3(Pt * (e - Ln) / o, s), c = r * (Math.cos(i) - s), a = r * Math.sqrt(1 - s * s) * Math.sin(i), u = (t.argPeriapsisDeg ?? 0) * O2, l = (t.inclinationDeg ?? 0) * O2, h = F3(t.hostStarId), d = Math.cos(u), _ = Math.sin(u), M = Math.cos(l), m = Math.sin(l), g = Math.cos(h), R = Math.sin(h), E = c * (g * d - R * _ * M) - a * (g * _ + R * d * M), P = c * (R * d + g * _ * M) - a * (R * _ - g * d * M), N = c * (_ * m) + a * (d * m);
		return it(n.x + E, n.y + P, n.z + N);
	}
	function H3(t, n, e, r) {
		const o = (yn(t.radiusEarth) ? t.radiusEarth : 1) * jn, s = Nr({
		...
		return z2({
			...
			appearance: s,
			getHeliocentricKm(i) {
				return mn(K3(t, n, i ?? e), "icrf-j2000-equatorial");
			},
			renderOverrides: r
		});
	}
	...
	let K2 = null, _1 = null;
	function X3(t) {
		return _1 != null && t === K2 || (K2 = t, _1 = q3(t)), _1;
	}
	const Q3 = 500 * F0;
	function W3(t, n) {
	...
	const n4 = Math.PI / 180, e4 = Math.PI / 180 / 60, r4 = wt(1e4);
	wt(500);
	...
	function l4(t) {
		...
		if (!Number.isFinite(e) || e <= 0) return null;
		const r = wt(e), o = Math.min(r, r4), s = t4(t.hubbleType), i = c4[t.catalogId] ?? s.kind, c = Number.isFinite(t.minAxArcmin) && (t.minAxArcmin ?? 0) > 0, a = t.majAxArcmin;
		return {
			discRadiusSu: o,
			discRadiusPc: e,
			morphScale: [
	...
	function f4(t, n) {
		const { stars: e, exoplanetRows: r, deepSkyObjects: o, ephemerisByBodyId: s } = t, { jd: i, viewpointScenePosition: c, viewForward: a, halfAngleRad: u, nearbyScanMaxLy: l, activeSystem: h } = n, d = R3(i, s), _ = l / I1, M = Qn(c), m = r && r.length > 0 ? X3(e) : /* @__PURE__ */ new Map(), g = r != null ? G3(i, r, m, {
			viewerPc: M,
			maxDistancePc: _,
			padPc: oe(O1(i))
		}) : [], R = J3(r), N = go({
		...
		const I = (w) => {
			...
			const C = p.get(w);
			return C != null ? h4(C) : 0;
		}, T = (w, C) => {
			if (w === "hor") {
				const k = D1[C]?.radiusKm;
