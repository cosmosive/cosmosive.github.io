(function() {
	...
	function _t(t, e = globalThis, n = ["*"]) {
		e.addEventListener("message", function o(r) {
			...
			Promise.resolve(u).catch((h) => ({
				...
				const [l, M] = P1(h);
				e.postMessage(Object.assign(Object.assign({}, l), { id: s }), M), i === "RELEASE" && (e.removeEventListener("message", o), S2(e), St in t && typeof t[St] == "function" && t[St]());
			}).catch((h) => {
				const [l, M] = P1({
	...
	function pt(t, e, n = [], o = function() {}) {
		...
		const s = new Proxy(o, {
			...
			apply(i, a, c) {
				...
				if (u === Hn) return L0(t, e, { type: "ENDPOINT" }).then(D0);
				if (u === "bind") return pt(t, e, n.slice(0, -1));
				const [h, l] = p2(c);
				return L0(t, e, {
					type: "APPLY",
	...
	function I3(t) {
		...
		let a = Math.sin(i), c = Math.cos(i), u = (-172064161 - 174666 * n) * a + 33386 * c, h = (92052331 + 9086 * n) * c + 15377 * a, l = 2 * (r - s + i);
		return a = Math.sin(l), c = Math.cos(l), u += (-13170906 - 1675 * n) * a - 13696 * c, h += (5730336 - 3015 * n) * c - 4587 * a, l = 2 * (r + i), a = Math.sin(l), c = Math.cos(l), u += (-2276413 - 234 * n) * a + 2796 * c, h += (978459 - 485 * n) * c + 1374 * a, l = 2 * i, a = Math.sin(l), c = Math.cos(l), u += (2074554 + 207 * n) * a - 698 * c, h += (-897492 + 470 * n) * c - 291 * a, a = Math.sin(o), c = Math.cos(o), u += (1475877 - 3633 * n) * a + 11817 * c, h += (73871 - 184 * n) * c - 1924 * a, {
			dpsi: -135e-6 + u * 1e-7,
			deps: 388e-6 + h * 1e-7
		};
	}
	...
	function g0(t) {
		...
		let i, a, c, u, h, l, M, p, A, E, m, I, g, R, x, w, U, N, k, O, j, F, q, u0 = o(-6, 6, 1, 4), r0 = o(-6, 6, 1, 4);
		function p0(v, b) {
			return r(u0, v, b);
		}
		function T0(v, b) {
		...
		for (N = .84 * d0 + .31 * n2 + 14.27 * o2 + 7.26 * r2 + .28 * s2 + .24 * dt, k = 2.94 * d0 + .31 * n2 + 14.27 * o2 + 9.34 * r2 + 1.12 * s2 + .83 * dt, O = -6.4 * d0 - 1.89 * dt, j = .21 * d0 + .31 * n2 + 14.27 * o2 - 88.7 * r2 - 15.3 * s2 + .24 * dt - 1.86 * Z4, F = N - O, p = -3332e-9 * P(.59734 - 5.37261 * e) - 539e-9 * P(.35498 - 5.37899 * e) - 64e-9 * P(.39943 - 5.37511 * e), g = e0 * V0(.60643382 + 1336.85522467 * e - 313e-8 * M) + N / m0, R = e0 * V0(.37489701 + 1325.55240982 * e + 2565e-8 * M) + k / m0, x = e0 * V0(.99312619 + 99.99735956 * e - 44e-8 * M) + O / m0, w = e0 * V0(.25909118 + 1342.2278298 * e - 892e-8 * M) + j / m0, U = e0 * V0(.82736186 + 1236.85308708 * e - 397e-8 * M) + F / m0, h = 1; h <= 4; ++h) {
			switch (h) {
				case 1:
					...
					break;
				case 2:
					c = x, a = 3, u = .997504612 - .002495388 * e;
					break;
				case 3:
			...
			for (l0(0, h, 1), l0(1, h, Math.cos(c) * u), h0(0, h, 0), h0(1, h, Math.sin(c) * u), l = 2; l <= a; ++l) ht(p0(l - 1, h), T0(l - 1, h), p0(1, h), T0(1, h), (v, b) => (l0(l, h, v), h0(l, h, b)));
			for (l = 1; l <= a; ++l) l0(-l, h, p0(l, h)), h0(-l, h, -T0(l, h));
		}
		function Qe(v, b, K, J) {
			for (var G = {
				x: 1,
	...
	function J1(t, e) {
		...
		o *= C0, r *= C0, s *= C0, i *= C0;
		const a = Math.sin(o), c = Math.cos(o), u = Math.sin(-r), h = Math.cos(-r), l = Math.sin(-s), M = Math.cos(-s), p = Math.sin(i), A = Math.cos(i), E = A * h - u * p * M, m = A * u * c + p * M * h * c - a * p * l, I = A * u * a + p * M * h * a + c * p * l, g = -p * h - u * A * M, R = -p * u * c + A * M * h * c - a * A * l, x = -p * u * a + A * M * h * a + c * A * l, w = u * l, U = -l * h * c - a * M, N = -l * h * a + M * c;
	...
	function T3(t, e) {
		...
		if (s < 1e-6) i = 0, a = r > 0 ? 90 : -90, c = Math.abs(r) - l3;
			...
			for (;;) {
				...
				const E = (k0 - 1) * M0, m = h * h, I = l * l, g = m + k0 * I;
				M = Math.sqrt(g);
				const R = E * l * h / M - r * h + s * l;
				if (Math.abs(R) < 1e-8) break;
				const x = E * ((m - I) / M - I * m * (k0 - 1) / (E * g)) + r * l + s * h;
				u -= R / x;
			}
			a = B * u;
			const A = M0 / M;
			Math.abs(l) > Math.abs(h) ? c = r / l - k0 * A : c = s / h - A;
		}
		return new Dt(a, i, 1e3 * c);
	}
	function Tt(t, e) {
		const n = t.latitude * _, o = Math.sin(n), r = Math.cos(n), s = 1 / Math.hypot(r, s0 * o), i = k0 * s, a = t.height / 1e3, c = M0 * s + a, u = M0 * i + a, h = (15 * e + t.longitude) * _, l = Math.sin(h), M = Math.cos(h);
		return {
			pos: [
				c * r * M / z,
				c * r * l / z,
	...
	function $1(t, e) {
		const n = I0(t), o = n.mobl * _, r = n.tobl * _, s = n.dpsi * C0, i = Math.cos(o), a = Math.sin(o), c = Math.cos(r), u = Math.sin(r), h = Math.cos(s), l = Math.sin(s), M = h, p = -l * i, A = -l * a, E = l * c, m = h * i * c + a * u, I = h * a * c - i * u, g = l * u, R = h * i * u - a * c, x = h * a * u + i * c;
		if (e === L.From2000) return new V([
			[
				M,
				E,
	...
	function j1(t, e, n, o, r) {
		let s = y(t);
		Y0(e), T(n), T(o);
		const i = Math.sin(e.latitude * _), a = Math.cos(e.latitude * _), c = Math.sin(e.longitude * _), u = Math.cos(e.longitude * _), h = Math.sin(o * _), l = Math.cos(o * _), M = Math.sin(n * Et), p = Math.cos(n * Et);
		let A = [
			a * u,
			a * c,
			i
	...
	function U0(t, e, n) {
		...
		for (let s of t) {
			let i = 0;
			for (let [c, u, h] of s) i += c * Math.cos(u + e * h);
			let a = o * i;
			n && (a %= e0), r += a, o *= e;
		}
		return r;
	...
	function S1(t, e) {
		const n = e / Q0, o = U0(t[Pt], n, !0), r = U0(t[zt], n, !1), s = U0(t[Q1], n, !1), i = Ot(t[Pt], n), a = Ot(t[zt], n), c = Ot(t[Q1], n), u = Math.cos(o), h = Math.sin(o), l = Math.cos(r), M = Math.sin(r), p = +(c * l * u) - s * M * u * a - s * l * h * i, A = +(c * l * h) - s * M * h * a + s * l * u * i, E = +(c * M) + s * l * a, m = F2(o, r, s), I = [
			p / Q0,
			A / Q0,
			E / Q0
		];
	...
	function Ht(t, e) {
		...
		const s = B3(k3, t.tt);
		if (s) {
			const i = j2((t.tt - s[0].tt) / X0, S0 - 1), a = s[i], c = s[i + 1], u = a.a.mean(c.a), h = W0(t.tt - a.tt, a.r, a.v, u), l = kt(t.tt - a.tt, a.v, u), M = W0(t.tt - c.tt, c.r, c.v, u), p = kt(t.tt - c.tt, c.v, u), A = (t.tt - a.tt) / X0;
	...
	function H3(t, e, n) {
		const o = n[0], r = n[1], s = n[2], i = n[3], a = n[4], c = n[5], u = Math.sqrt(e / (o * o * o));
		...
		const A = i * h - s * l, E = -s * h - i * l, m = 1 / (1 + E), I = 1 / (1 + Math.sqrt(1 - s * s - i * i)), g = o * (h - s - I * i * A), R = o * (l - i + I * s * A), x = u * m * o * (-l - I * i * E), w = u * m * o * (+h + I * s * E), U = 2 * Math.sqrt(1 - a * a - c * c), N = 1 - 2 * c * c, k = 1 - 2 * a * a, O = 2 * c * a;
	...
	function X2(t, e, n, o) {
		...
		if (K1(n)) {
			const a = c0(n, r);
			if (o) {
				const u = Z1(e, r), h = new D(a.x - u.x, a.y - u.y, a.z - u.z, r), l = z1 / h.Length();
	...
	function $(t, e, n, o) {
		...
		let i = o && o.init_f1 || t(e), a = o && o.init_f2 || t(n), c = NaN, u = 0, h = o && o.iter_limit || 20, l = !0;
		for (;;) {
	...
	function tt(t, e) {
		...
		else if (t === d.Moon) a = Y3(r, u, c);
		else if (t === d.Saturn) {
			const l = j3(r, u, c, i, n);
			a = l.mag, h = l.ring_tilt;
		} else a = G3(t, r, u, c);
		return new Z2(n, a, r, u, c, i, s, h);
	}
	function E1(t) {
	...
	function y1(t, e, n) {
		...
		let i = o(s), a, c, u;
		if (n < 0) {
			if (i < 0 && (i += 360), a = -(b1 * i) / 360, u = a + r, u < n) return null;
			c = Math.max(n, a - r);
		} else {
			if (i > 0 && (i -= 360), a = -(b1 * i) / 360, c = a - r, c > n) return null;
			u = Math.min(n, a + r);
		}
		return $(o, s.AddDays(c), s.AddDays(u), { dt_tolerance_seconds: .1 });
	}
	var te = class {
		constructor(t, e) {
	...
	function q3(t, e) {
		const n = t.latitude * _, o = Math.sin(n), r = Math.cos(n), s = 1 / Math.hypot(r, o * s0), i = s * (s0 * s0), a = (t.height - e) / 1e3, c = M0 * s + a, u = M0 * i + a, h = 1e3 * Math.hypot(c * r, u * o), l = .175 * Math.pow(1 - .0065 / 283.15 * (t.height - 2 / 3 * e), 3.256);
		return B * -(Math.sqrt(2 * (1 - l) * e / h) / (1 - l));
	}
	function X3(t) {
		switch (t) {
	...
	function $t(t, e, n, o, r, s, i) {
		...
		const c = new R0((o.ut + r.ut) / 2), u = e(c);
		return $t(1 + t, e, n, o, c, s, u) || $t(1 + t, e, n, c, r, u, i);
	}
	function e4(t, e) {
	...
	function n4(t, e, n, o, r = 1) {
		...
		for (;;) {
			++i;
			let a = A0(s), c = f1(t, s, e, !0, !0), u = (n + c.ra - e.longitude / 15 - a) % 24;
			if (i === 1 ? r > 0 ? u < 0 && (u += 24) : u > 0 && (u -= 24) : u < -12 ? u += 24 : u > 12 && (u -= 24), Math.abs(u) * 3600 < .1) {
				const l = j1(s, e, c.ra, c.dec, "normal");
				return new se(s, l);
			}
			let h = u / 24 * w2;
			s = s.AddDays(h);
	...
	function s4(t, e) {
		...
		let i = 0;
		for (; ++i <= 2;) {
			let a = Z0(b0(t, r) - b0(d.Earth, r)), c, u, h;
			a >= -s.s1 && a < +s.s1 ? (h = 0, c = +s.s1, u = +s.s2) : a >= +s.s2 || a < -s.s2 ? (h = 0, c = -s.s2, u = -s.s1) : a >= 0 ? (h = -E1(t) / 4, c = +s.s1, u = +s.s2) : (h = -E1(t) / 4, c = -s.s2, u = -s.s1);
			let l = r.AddDays(h), M = n1(t, c, l), p = n1(t, u, M), A = o(M);
	...
	function i4(t, e) {
		...
		const n = .01;
		function o(c) {
			const u = c.AddDays(-n / 2), h = c.AddDays(+n / 2), l = tt(t, u).mag;
			return (tt(t, h).mag - l) / n;
		}
		let r = y(e);
		const s = 10, i = 30;
		let a = 0;
		for (; ++a <= 2;) {
			let c = Z0(b0(t, r) - b0(d.Earth, r)), u, h, l;
			c >= -s && c < +s ? (l = 0, u = +s, h = +i) : c >= +i || c < -i ? (l = 0, u = -i, h = -s) : c >= 0 ? (l = -E1(t) / 4, u = +s, h = +i) : (l = -E1(t) / 4, u = -i, h = -s);
			let M = r.AddDays(l), p = n1(t, u, M), A = n1(t, h, p), E = o(p);
	...
	function ue(t) {
		function n(c) {
			let u = c.AddDays(-5e-4), h = c.AddDays(.001 / 2), l = g0(u).distance_au;
	...
	function le(t, e, n, o) {
		...
		for (;;) {
			...
			if (i < 1 / 1440) {
				...
				return new m1(u, e, P0(t, u));
			}
			let a = -1, c = 0;
			for (let u = 0; u < s; ++u) {
				const h = r * P0(t, n.AddDays(u * i));
				(u == 0 || h > c) && (a = u, c = h);
			}
	...
	function c4(t, e) {
		...
		for (let M = 0; M < 100; ++M) {
			const p = o.AddDays(M * u), A = P0(t, p);
			M === 0 ? c = a = A : (A > c && (c = A, i = p), A < a && (a = A, s = p));
		}
		const h = le(t, 0, s.AddDays(-2 * u), 4 * u), l = le(t, 1, i.AddDays(-2 * u), 4 * u);
	...
	function he(t, e) {
		...
		function n(c) {
			let h = c.AddDays(-5e-4), l = c.AddDays(.001 / 2), M = P0(t, h);
			return (P0(t, l) - M) / .001;
		}
		function o(c) {
			return -n(c);
		...
		for (let c = 0; c * s < 2 * r; ++c) {
			const u = i.AddDays(s), h = n(u);
			if (a * h <= 0) {
				let l, M;
				if (a < 0 || h > 0) l = n, M = w0.Pericenter;
				else if (a > 0 || h < 0) l = o, M = w0.Apocenter;
	...
	function Ne(t) {
		...
		for (let o = 0; o < 12; ++o) {
			...
			if (Math.abs(s) < 1.8) {
				const i = x4(r);
				if (i.r < i.p + X) {
					let a = Z.Penumbral, c = 0, u = 0, h = 0, l = Xt(i.time, i.p + X, 200);
					return i.r < i.k + X && (a = Z.Partial, h = Xt(i.time, i.k + X, l), i.r + X < i.k ? (a = Z.Total, c = 1, u = Xt(i.time, i.k - X, h)) : c = xe(X, i.k, i.r)), new ge(a, c, i.time, l, h, u);
				}
			}
			n = r.AddDays(10);
		}
	...
	function U4(t) {
		...
		const u = M0, h = a.x * a.x + a.y * a.y + a.z * a.z, l = -2 * (a.x * c.x + a.y * c.y + a.z * c.z), M = c.x * c.x + c.y * c.y + c.z * c.z - u * u, p = l * l - 4 * h * M;
		let A;
		if (p > 0) {
			const E = (-l - Math.sqrt(p)) / (2 * h), m = E * a.x - c.x, I = E * a.y - c.y, g = (E * a.z - c.z) * s0, R = Math.hypot(m, I) * k0;
	...
	function P4(t, e) {
		...
		let u, h, l;
		t.r < Math.abs(t.k) ? (s = t.time.AddDays(-.01), i = t.time.AddDays(.01), u = at(e, 1, be, s, t.time), h = at(e, -1, be, t.time, i), l = Ue(t.k)) : l = Z.Partial;
		const M = l === Z.Total ? 1 : Te(t.dir, t.target);
		return new Pe(l, M, a, u, r, h, c);
	}
	...
	function Ke(t, e) {
		...
		for (;;) {
			...
			if (e1(t, i) < n) {
				const a = N4(t, r, i);
				if (a.r < a.p) {
					const c = a.time.AddDays(-o), u = Be(t, r, c, a.time, -1), h = a.time.AddDays(+o), l = Be(t, r, a.time, h, 1), M = 60 * e1(t, a.time);
					return new ke(u, a.time, l, M);
				}
			}
			s = i.AddDays(10);
		}
	...
	function K4(t, e) {
		...
		const c = i * _, u = s * _, h = Math.cos(c), l = new D(h * Math.cos(u), h * Math.sin(u), Math.sin(c), n);
		return new Zt(s / 15, i, a, l);
	}
	function H4(t, e, n, o) {
	...
	function $e(t, e, n, o, r) {
		...
		let a = o.x - e.x, c = o.y - e.y, u = o.z - e.z;
		const h = a * a + c * c + u * u, l = Math.sqrt(h), M = o.vx - e.vx, p = o.vy - e.vy, A = o.vz - e.vz;
		let E;
		if (t === 4 || t === 5) {
			...
			const N = t == 4 ? +i : -i, k = .5 * a + N * R, O = .5 * c + N * x, j = .5 * u + N * w, F = .5 * R - N * a, q = .5 * x - N * c, u0 = .5 * w - N * u, r0 = l * k, p0 = l * O, T0 = l * j, l0 = M * a + p * c + A * u, h0 = M * R + p * x + A * w;
			E = new H(r0, p0, T0, l0 * k + h0 * F, l0 * O + h0 * q, l0 * j + h0 * u0, e.t);
		} else {
			const m = -l * (r / (n + r)), I = +l * (n / (n + r)), g = (n + r) / (h * l);
			let R, x, w;
			...
			R = (U - m) / l, E = new H(R * a, R * c, R * u, R * M, R * p, R * A, e.t);
		}
		return E;
	}
	var J4 = class t0 {
		...
		Update(e) {
			...
			if (o === 0) this.prev = this.Duplicate();
				...
				for (let i = 0; i < this.curr.bodies.length; ++i) {
					...
					c.tt = n.tt, c.r = W0(o, a.r, a.v, u), c.v = kt(o, a.v, u);
				}
				this.CalcBodyAccelerations();
			}
		...
		static AddAcceleration(e, n, o, r) {
			const s = o.x - n.x, i = o.y - n.y, a = o.z - n.z, c = s * s + i * i + a * a, u = r / (c * Math.sqrt(c));
			e.x += s * u, e.y += i * u, e.z += a * u;
		}
		Duplicate() {
			const e = {};
	...
	function $4(t, e, n) {
		...
		const a = (e - s.time.jd) / (i.time.jd - s.time.jd), c = s.position.x + a * (i.position.x - s.position.x), u = s.position.y + a * (i.position.y - s.position.y), h = s.position.z + a * (i.position.z - s.position.z);
		return n ? (n.x = c, n.y = u, n.z = h, n) : {
			x: c,
			y: u,
			z: h
		};
